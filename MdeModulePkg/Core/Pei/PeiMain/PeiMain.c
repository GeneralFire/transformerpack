...
// APTIOV_SERVER_OVERRIDE_START : Added FSP Support
#include <Token.h>
// APTIOV_SERVER_OVERRIDE_END : Added FSP Support
//*** AMI PORTING END ***//
...
//APTIOV_SERVER_OVERRIDE_START : Changes to Launch PeiCore from FSP
  // Pei Core of FSP is not published, hardcode the FV location of it
  // It is same as FV_FSP_M_BASE in BIOS
#if BUILD_FSP_BINARY != 0 || FSP_SUPPORT_IN_BIOS != 0
Status = EFI_SUCCESS;
#if BUILD_FSP_BINARY  
  PeiCoreFvLocationPpi->PeiCoreFvLocation = (VOID *)FSP_M_FLASH_ADDRESS;
#else 
  PeiCoreFvLocationPpi->PeiCoreFvLocation = (VOID *)FV_FSP_M_BASE;
#endif
#endif
//APTIOV_SERVER_OVERRIDE_END : Changes to Launch PeiCore from FSP
