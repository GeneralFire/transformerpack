  ...
  for (Index1 = NotifyStartIndex; Index1 < NotifyStopIndex; Index1++) {
    ...
    for (Index2 = InstallStartIndex; Index2 < InstallStopIndex; Index2++) {
      ...
      if ((((INT32 *)SearchGuid)[0] == ((INT32 *)CheckGuid)[0]) &&
        ...
        //*** AMI PORTING END *****//
// APTIOV_SERVER_OVERRIDE_START : Added to resolve boot issue while booting with FSP support enabled
        if(NotifyDescriptor->Notify != NULL) {
// APTIOV_SERVER_OVERRIDE_END :	Added to resolve boot issue while booting with FSP support enabled
        NotifyDescriptor->Notify (
