...
// Threshold value for EfiBootServicesData descriptors to decide when
// to set the Special field in mMemoryTypeStatistics in order to merge 
// the descriptors falling in BootServicesData bin as a single entry.
#define   BOOT_SERVICES_DATA_DESCRIPTORS_THRESHOLD    64
//*** AMI PORTING END *****//
  ...
  )
{
  ...
  //*** AMI PORTING BEGIN ***//
  UINT32                            BootServicesDataDescriptorCount = 0;
  //*** AMI PORTING END ***//
  ...
  // Calculate the total number of EfiBootServicesData descriptors that falls
  // into or adjacent to predefined memory range,if the count reaches threshold 
  // value set the Special flag in mMemoryTypeStatistics table in order to merge
  // the descriptors in the predefined range as a single entry.
  if (!mMemoryTypeStatistics[EfiBootServicesData].Special          &&
      mMemoryTypeStatistics[EfiBootServicesData].NumberOfPages > 0) {
    for (Link = gMemoryMap.ForwardLink; Link != &gMemoryMap; Link = Link->ForwardLink) {
      Entry = CR (Link, MEMORY_MAP, Link, MEMORY_MAP_SIGNATURE);
      if (Entry->Type == EfiBootServicesData                                             &&
          Entry->Start <= mMemoryTypeStatistics[EfiBootServicesData].MaximumAddress+1    &&
          Entry->End+1 >= mMemoryTypeStatistics[EfiBootServicesData].BaseAddress)         {
        if (++BootServicesDataDescriptorCount >= BOOT_SERVICES_DATA_DESCRIPTORS_THRESHOLD) {
          mMemoryTypeStatistics[EfiBootServicesData].Special = TRUE;
          break;
        }
      }
    }
  }
  // Add descriptors for a predefined memory type ranges with Special
  // flag set.
  for (Type = (EFI_MEMORY_TYPE) 0; Type < EfiMaxMemoryType; Type++) {
  ...
  for (Link = gMemoryMap.ForwardLink; Link != &gMemoryMap; Link = Link->ForwardLink) {
    ...
    for (Type = (EFI_MEMORY_TYPE) 0; Type < EfiMaxMemoryType; Type++) {
      if (mMemoryTypeStatistics[Type].Special           &&
          mMemoryTypeStatistics[Type].NumberOfPages > 0) {
        if (Entry->Type == Type) {
          if (Entry->Start <= mMemoryTypeStatistics[Type].MaximumAddress+1    &&
              Entry->End+1 >= mMemoryTypeStatistics[Type].BaseAddress)         {
            // This memory descriptor will not be part of the external memory map
            // because it falls into or adjacent to one of the predefined memory 
            // type ranges.
            break;
          }
        } else if (Entry->Start >= mMemoryTypeStatistics[Type].BaseAddress    &&
          ...
          if (Entry->Type == EfiConventionalMemory) {
            ...
            // This memory descriptor falls into predefined range of a different memory type.
                      
            //<--------- Predefined range for TypeX --------->
            //  ______________________________________________
            // |   TypeX   |   TypeY   |     TypeX            |
            // |___________|___________|______________________|
                      
            // As descriptor of TypeY falls in the TypeX's predefined range, this entry can't
            // be merged with other descriptors.So adding two descriptors size,one for TypeY 
            // and other for merging the consecutive descriptors of TypeX.
            BufferSize += 2*Size;
            break;
          }
        }
    ...
    if (Type != EfiMaxMemoryType) {
      // This descriptor falls into one of the predefined memory type ranges.
      continue;
    }
    BufferSize += Size;
  }
