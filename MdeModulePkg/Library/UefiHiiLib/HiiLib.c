  ...
  )
{
  ...
  //AMI PORTING
  //GCC compiler treats typecasts as getting addresses
  //For this particular case it generates warning "Function may return address of local variable
  //We're getting rid of typecasting to prevent this warning
  ...
  )
{
  ...
  OpCodePointer = InternalHiiCreateOpCodeExtended (
                                    OpCodeHandle,
                                    &OpCode,
                                    EFI_IFR_GUID_OP,
                                    sizeof (OpCode),
  ...
  if (OpCodePointer != NULL && GuidOpCode != NULL) {
    ...
    CopyMem ((EFI_IFR_GUID *)OpCodePointer + 1, (EFI_IFR_GUID *)GuidOpCode + 1, OpCodeSize - sizeof (OpCode));
  }
  //AMI PORTING
