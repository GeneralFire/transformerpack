...
//*** AMI PORTING BEGIN ***//
//Add support for getting string in any language
//when string in requested or platform language is not found
/**
  Updates list of supported languages by removing head of the list.

  @param[in]  Language   The list of supported languages.

  @retval NULL   There are no more languages to process.
  @retval Other  Updated list of supported languages.
...
{
  //For each subsequent call we trim one language from the string
  while (*Languages != '\0' && *Languages != ';')
    Languages++;
  
  return (*Languages == ';') ? Languages + 1 : NULL;
}
//*** AMI PORTING END ***//
...
/**
  ...
  the string package in the first available language the string package supports.
  ...
  )
{
  ...
  //Add support for getting string in any language
  //when string in requested or platform language is not found
  RemainingLanguages = SupportedLanguages;
  do {
    //
    // Get the best matching language from SupportedLanguages
    //
    BestLanguage = GetBestLanguage (
                     SupportedLanguages,
                     FALSE,                                             // RFC 4646 mode
                     Language != NULL ? Language : "",                  // Highest priority
                     PlatformLanguage != NULL ? PlatformLanguage : "",  // Next highest priority
                     RemainingLanguages,                                // Lowest priority
                     NULL
                     );
    if (BestLanguage == NULL) {
      goto Error;
    ...
    if (Status == EFI_BUFFER_TOO_SMALL) {
      break;
    } else if (Status == EFI_INVALID_LANGUAGE) { //try different languages
      if (Language != NULL) {
        Language = NULL;
        continue;
      }
      ...
      RemainingLanguages = GetNextLanguage (RemainingLanguages);
      if (RemainingLanguages != NULL) {
        continue;
      }
    }
    goto Error;
  } while (Status == EFI_INVALID_LANGUAGE);
#endif
  //*** AMI PORTING END ***//
