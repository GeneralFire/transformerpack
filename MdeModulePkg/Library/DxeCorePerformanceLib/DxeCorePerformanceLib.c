  ...
  )
{
  ...
  UINTN                         ConvertedUnicodeCharacters = 0;
...
//*** AMI PORTING BEGIN ***//
// Changed library function to account for the Controller name and device path being longer than FPDT_MAX_PERF_RECORD_SIZE
//  UnicodeStrToAsciiStrS would ASSERT if the string is too long. UnicodeStrnToAsciiStrS allows us to copy only the 
//  characters that fit in the string buffer and ignore the rest of the string.
//      UnicodeStrToAsciiStrS(StringPtr, AsciiStringPtr, DevicePathStringSize);
      // using DevicePathStringSize - 1 to remove count of NULL character on end.
      UnicodeStrnToAsciiStrS(StringPtr, DevicePathStringSize - 1, AsciiStringPtr, DevicePathStringSize, &ConvertedUnicodeCharacters);
      // the number of converted characters does not include the NULL character
