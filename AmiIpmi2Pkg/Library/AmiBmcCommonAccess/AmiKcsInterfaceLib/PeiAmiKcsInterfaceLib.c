...
/** @file PeiAmiKcsInterfaceLib.c
    KCS Transport Pei phase implementation library functions.
  ...
  IN OUT AMI_SYSTEM_INTERFACE   *Interface )
{
    if (Interface->Kcs == NULL) {
        Interface->Kcs = (AMI_KCS_SYSTEM_INTERFACE*)AllocateZeroPool (sizeof (AMI_KCS_SYSTEM_INTERFACE));
        if (Interface->Kcs == NULL) {
