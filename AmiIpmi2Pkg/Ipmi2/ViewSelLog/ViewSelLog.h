...
/** @file ViewSelLog.h
    Header file for ViewSelLog module.
...
/** @internal
    Flag definitions for ViewSelLogFlag.
*/
typedef enum {
    PrintColumnTitleString = 1,   ///< PrintColumnTitleString
    PrintNoProtocolFoundError,    ///< PrintNoProtocolFoundError
    PrintNoRecordsFoundString,    ///< PrintNoRecordsFoundString
    PrintViewMoreSelLogString     ///< PrintViewMoreSelLogString
} VIEW_SEL_LOG_FLAG;

#pragma pack(1)

/** @internal
    Sel sensor details and token lookup table structure.
*/
typedef struct {
    /// Sensor type.
    UINT8           SensorType;
    UINT32          TimeStamp;    ///< Time stamp.
    STRING_REF      Token;        ///< Token.
} SEL_SENSOR_DETAILS_TOKEN_LOOKUP_TABLE;

/** @internal
    Sel record descriptor and token lookup table.
*/
typedef struct {
    /// Sel record data.
...
/** @internal
    Value and its descriptor string.
*/
typedef struct {
    /// Value.
    UINT8         Value;
    EFI_STRING    DescString;     ///< Descriptor string.
} VALUE_TO_DESC_STRING;

/** @internal
    Sensor offset string table.
*/
typedef struct {
    /// Sensor spec offset.
    UINT8   SensorSpecOffset;
    UINT8   Mask2;              ///< Mask for Event data 2.
    UINT8   Or2;                ///< To print Hex or string of Event data 2.
    UINT8   Value2;             ///< Value of Event data 2.
    UINT8   Mask3;              ///< Mask for Event data 3.
    UINT8   Or3;                ///< To print Hex or string of Event data 3.
    UINT8   Value3;             ///< Value of Event data 3.
    CHAR16* DescString;         ///< Descriptor string.
} SENSOR_OFFSET_STRING_TABLE;

/**
    Sensor type value to descriptor string.
*/
typedef struct {
    /// Sensor type.
    UINT8                         SensorType;
    UINTN                         SizeOfTable;              ///< Size of the table.
    SENSOR_OFFSET_STRING_TABLE    *SensorOffsetStringTable; ///< Sensor offset string table.
