...
/** @internal
    ...
    @param[in] Value        Value to be searched in Lookup table.
    @param[in] Table[]      Lookup table.
    @param[in] TableSize    Size of the Lookup table.

    @retval RESERVED_UNKNOWN_STRING If the value is not found in lookup table.
...
GetDescStringFromValue (
  ...
  IN UINT8                  TableSize )
{
    UINT8   Counter = MAX_UINT8;

    DEBUG ((DEBUG_INFO, "ViewSelLog: Inside %a\n", __FUNCTION__));
    ...
    DEBUG ((DEBUG_INFO, "ViewSelLog: Exit %a\n", __FUNCTION__));
...
/** @internal
    ...
    @param[in] TimeStamp    Time stamp in Sec

    @return EFI_TIME    EFI time.
  ...
  IN UINT32     TimeStamp )
{
    ...
    DaysInYear = (UINT8)(TimeInSec / (24 * 60 * 60)) + 1;
    Month = 1;
    ...
    DEBUG ((DEBUG_INFO, "ViewSelLog: Exiting %a\n", __FUNCTION__));
...
/** @internal
    ...
    @param[in] RecordType  Record Type to validate.
  ...
  IN UINT8  RecordType )
{
    ...
    if (RecordType >= AMI_IPMI_SEL_OEM_TIME_STAMP_RECORD_START) {
        IsValid = TRUE;
...
/** @internal
    ...
    @param[in] HiiHandle        A handle that was previously registered in
                                the HII Database.
    @param[in] SelRecordData    SEL Record Data.

    @return STRING_REF      String reference token.
  ...
  IN  IPMI_SEL_EVENT_RECORD_DATA    SelRecordData )
{
    EFI_TIME        EfiTime;
    EFI_STRING      String;
    CHAR16          UnicodeSPrintBuffer[STRING_BUFFER_LENGTH];
    ...
    if (!(SelRecordData.RecordType >= AMI_IPMI_SEL_OEM_NO_TIME_STAMP_RECORD_START)) {
    ...
    if (SelRecordData.RecordType != IPMI_SEL_SYSTEM_RECORD) {
        CharPtrString = HiiGetString (
                            HiiHandle,
                            STRING_TOKEN(STR_OEM_RECORD_TYPE),
                            NULL );
        // Build SEL Sensor Details for OEM specific SEL record.
        if (CharPtrString != NULL) {
            UnicodeSPrint (
               UnicodeSPrintBuffer + StrnLenS (UnicodeSPrintBuffer, STRING_BUFFER_LENGTH),
               STRING_BUFFER_LENGTH,
               CharPtrString,
    ...
    if ((SelRecordData.SensorType >= RESERVED_SENSOR_TYPE_START) && \
        ...
        if (StrnCmp (
                String,
                RESERVED_UNKNOWN_STRING,
                STRING_BUFFER_LENGTH ) == 0x00) {
  ...
  IN IPMI_SEL_EVENT_RECORD_DATA    SelRecordData )
{
    ...
    AMI_SEL_OEM_TIMESTAMP_RECORD    SelOemRecord;
    ...
    for (Index = 0; Index < gSelRecordDescSavedTokenCount; Index++) {
        if (CompareMem (
                &(gSelRecordDescSavedToken[Index].SelRecordData), \
                &SelRecordData,
                sizeof(SelRecordData) ) == 0) {
    ...
    CharPtrString = HiiGetString (
                        HiiHandle,
                        STRING_TOKEN(STR_SEL_RECORDS_DATA),
                        NULL );
    ...
    if (SelRecordData.RecordType != IPMI_SEL_SYSTEM_RECORD) {
        if ((SelRecordData.RecordType >= AMI_IPMI_SEL_OEM_TIME_STAMP_RECORD_START) &&
            (SelRecordData.RecordType <= AMI_IPMI_SEL_OEM_TIME_STAMP_RECORD_END)) {
            ...
            CharPtrString = HiiGetString (
                                HiiHandle,
                                STRING_TOKEN(STR_MANUFACTURER_ID),
                                NULL );
            ...
            if (CharPtrString != NULL) {
                UnicodeSPrint (
                     ...
                     STRING_BUFFER_LENGTH,
                     CharPtrString,
                     SelOemRecord.ManufacturerId[0] + \
                     (SelOemRecord.ManufacturerId[1] << 8) + \
                     (SelOemRecord.ManufacturerId[2] << 16));
                gBS->FreePool (CharPtrString);
            }
        }
    ...
    if ((SelRecordData.GeneratorId & IPMI_SOFTWARE_ID) \
            == IPMI_SOFTWARE_ID) {  // Check if generator id has software id.
        UINT8   SoftwareId = AMI_IPMI_SOFTWARE_ID_FROM_GENERATOR_ID(SelRecordData.GeneratorId);

        // Software Type String.
        if ((SoftwareId >= AMI_IPMI_BIOS_ID_RANGE_START) && \
                (SoftwareId <= AMI_IPMI_BIOS_ID_RANGE_END)) {
            String = AMI_IPMI_BIOS_SOFTWARE_TYPE_STRING;
        } else if ((SoftwareId >= AMI_IPMI_SMI_HANDLER_ID_RANGE_START) && \
                    (SoftwareId <= AMI_IPMI_SMI_HANDLER_ID_RANGE_END)) {
            String = AMI_IPMI_SMI_HANDLER_SOFTWARE_TYPE_STRING;
        } else if ((SoftwareId >= AMI_IPMI_SMS_ID_RANGE_START) && \
                    (SoftwareId <= AMI_IPMI_SMS_ID_RANGE_END)) {
            String = AMI_IPMI_SMS_SOFTWARE_TYPE_STRING;
        } else if ((SoftwareId >= AMI_IPMI_OEM_ID_RANGE_START) && \
                    (SoftwareId <= AMI_IPMI_OEM_ID_RANGE_END)) {
            String = AMI_IPMI_OEM_SOFTWARE_TYPE_STRING;
        } else if ((SoftwareId >= AMI_IPMI_REMOTE_CONSOLE_ID_RANGE_START) && \
                    (SoftwareId <= AMI_IPMI_REMOTE_CONSOLE_ID_RANGE_END)) {
            String = AMI_IPMI_REMOTE_CONSOLE_SOFTWARE_TYPE_STRING;
        } else if (SoftwareId == AMI_IPMI_TERMINAL_CONSOLE_ID) {
            String = AMI_IPMI_TERMINAL_CONSOLE_SOFTWARE_TYPE_STRING;
        } else {
            String = AMI_IPMI_RESERVED_SOFTWARE_TYPE_STRING;
        }

        // Add Channel number along with Generator string
        CharPtrString = HiiGetString (
                            HiiHandle,
                            STRING_TOKEN(STR_GENERATORID_CHANNELNO),
                            NULL );

        ChannelString = HiiGetString (
                            HiiHandle,
                            STRING_TOKEN(STR_CHANNEL_NUMBER),
                            NULL);
        ...
        if ((CharPtrString != NULL) && (ChannelString != NULL)) {
            UnicodeSPrint (
                ...
                String,
                ChannelString,
                AMI_IPMI_CHANNEL_NUMBER_FROM_GENERATOR_ID(SelRecordData.GeneratorId) );
            gBS->FreePool (CharPtrString);
        ...
        CharPtrString = HiiGetString (
                            HiiHandle,
                            STRING_TOKEN(STR_BMCLUN_GENERATORID_CHANNELNO),
                            NULL );

        BmcLunString = HiiGetString (
                            HiiHandle,
                            STRING_TOKEN(STR_BMC_LUN),
                            NULL );
        ...
        if ((CharPtrString != NULL) && \
            ...
            UnicodeSPrint (
                ...
                CharPtrString,
                BmcLunString,
                AMI_IPMI_LUN_FROM_GENERATOR_ID (SelRecordData.GeneratorId),
                ChannelString,
                AMI_IPMI_CHANNEL_NUMBER_FROM_GENERATOR_ID (SelRecordData.GeneratorId) );
            gBS->FreePool (CharPtrString);
            gBS->FreePool (ChannelString);
    ...
    CharPtrString = HiiGetString (
                        HiiHandle,
                        STRING_TOKEN(STR_SENSOR_NUMBER),
                        NULL );
    ...
    CharPtrString = HiiGetString (
                        HiiHandle,
                        STRING_TOKEN(STR_EVENT_DESCRIPTION),
                        NULL );
    ...
    if (AMI_IPMI_SEL_EVENT_TYPE(SelRecordData.EventDirType) == AMI_IPMI_SEL_EVENT_TYPE_SENSOR_SPECIFIC) {
            ...
            for (Index = 0; Index < DescTable.SizeOfTable; Index++) {
                ...
                if (SensorSpecOffsetStringUpdated == FALSE) {
                    ...
                    if (UpdateSensorSpecOffsetString (OffsetTable, SelRecordData)) {
                        ...
                        MatchOEMEvData2 = IsOEMEvData2SelRecordDataValid (
                                            SelRecordData.OEMEvData1 );
                        MatchOEMEvData3 = IsOEMEvData3SelRecordDataValid (
                                            SelRecordData.OEMEvData1 );
                    ...
                    /* If MatchOEMEvData2 is TRUE then check for OEMEvData2 
                       ...
                       Continue the loop.*/
                    if (MatchOEMEvData2 && UpdateOEMEvData2String (
                                                OffsetTable,
                                                SelRecordData )) {
                    ...
                    /* If MatchOEMEvData3 is TRUE then check for OEMEvData3 
                       match in table entry. If matched, Update the string and 
                       Continue the loop.*/
                    if (MatchOEMEvData3 && UpdateOEMEvData3String (
                                                OffsetTable,
                                                SelRecordData )) {
    ...
    if (AMI_IPMI_SEL_EVENT_DIR(SelRecordData.EventDirType) == AMI_IPMI_SEL_EVENT_DIR_ASSERTION_EVENT) {
        CharPtrString = HiiGetString (
                            HiiHandle,
                            STRING_TOKEN(STR_ASSERTION_EVENT),
                            NULL );
    } else {
        CharPtrString = HiiGetString (
                            HiiHandle,
                            STRING_TOKEN(STR_DEASSERTION_EVENT),
                            NULL );
    }
    if (CharPtrString != NULL) {
        UnicodeSPrint (
  ...
  IN IPMI_SEL_EVENT_RECORD_DATA    SelRecordData )
{
    ...
    DEBUG ((DEBUG_INFO, "ViewSelLog: SensorDetailsToken: %X \n", SensorDetailsToken));
    ...
    DEBUG ((DEBUG_INFO, "ViewSelLog: SelRecordDescriptionToken: %X \n", SelRecordDescriptionToken));
...
/** @internal
    ...
    @param[in] ViewSelLogFlag   ViewSelLogFlag variable
  ...
  IN VIEW_SEL_LOG_FLAG  ViewSelLogFlag )
{
    ...
    if (!BrowserDataStatus) {
        DEBUG ((DEBUG_ERROR, "ViewSelLog: Status of HiiGetBrowserData(): %d\n",BrowserDataStatus));
        return;
    }
    ...
    if (!BrowserDataStatus) {
        DEBUG ((DEBUG_ERROR, "ViewSelLog: Status of HiiSetBrowserData(): %d\n",BrowserDataStatus));
        return;
    }
    DEBUG ((DEBUG_INFO, "ViewSelLog: Exiting %a\n", __FUNCTION__));
}
  ...
  IN  UINT16            Key )
{
    ...
    if ((Key != VIEW_SEL_LOG_FORM_KEY) && \
        ...
        DEBUG ((DEBUG_ERROR, "ViewSelLog: Callback function is called with \
    ...
    if (CallbackParameters->Action != EFI_BROWSER_ACTION_CHANGING) {
        DEBUG ((DEBUG_INFO, "ViewSelLog: CallbackParameters->Action != \
                EFI_BROWSER_ACTION_CHANGING... so return EFI_SUCCESS\n"));
        return EFI_SUCCESS;
    }

    DEBUG ((DEBUG_INFO, "ViewSelLog: Key: %x \n", Key));

    /* Locate Ami Post Manager Protocol and if Status is not an error
       display the Message box else continue without Message box.*/
    Status = gBS->LocateProtocol (
                    &gAmiPostManagerProtocolGuid,
                    NULL,
                    (VOID **) &AmiPostMgr );
    
    DEBUG ((DEBUG_INFO, "ViewSelLog: LocateProtocol for gAmiPostManagerProtocolGuid Status: %r.\n", Status));

    if (!EFI_ERROR (Status)) {
        /* Get the Message box strings and display Message box if returned
        ...
        if (MsgBoxTitleString != NULL) {
            ...
            if (MessageString != NULL) {
                Status = AmiPostMgr->DisplayMsgBox (
                                         ...
                                         MSGBOX_TYPE_YESNO,
                                         &MsgBoxSel );
                DEBUG ((DEBUG_INFO, "ViewSelLog: DisplayMsgBox Status: %r.\n", Status));
                DEBUG ((DEBUG_INFO, "ViewSelLog: MsgBoxSel value: %x.\n", MsgBoxSel));
                ...
                if ((MsgBoxSel == MSGBOX_NO)|| (EFI_ERROR (Status))) {
                    return EFI_ABORTED;
                }
            }
        }
    ...
    // Locate the IPMI Transport Protocol and return if error status.
    Status = gBS->LocateProtocol (
    ...
    // Send GET_SEL_INFO command to get the SEL count.
    ...
    DEBUG ((DEBUG_INFO, "ViewSelLog: IPMI_STORAGE_GET_SEL_INFO Status: %r\n", Status));
    ...
    DEBUG ((DEBUG_INFO, "ViewSelLog: SelInfo.NoOfEntries: %x \n", SelInfo.NoOfEntries));

    // Update No Of Log Entries in STR_SYSTEM_ELOG_COUNT_STRING.
    CharPtrString = HiiGetString (
                        HiiHandle,
                        STRING_TOKEN(STR_LOG_ENTRY_IN_SEL),
                        NULL );
    if(CharPtrString != NULL) {
    ...
    DEBUG ((DEBUG_INFO, "ViewSelLog: IsFirstCall: %x\n", IsFirstCall));
    ...
    if (IsFirstCall == TRUE) {
        ...
        if (gSensorDetailsSavedToken == NULL) {
            DEBUG((DEBUG_ERROR, "Failed to allocate memory for gSensorDetailsSavedToken! \n"));
            return EFI_OUT_OF_RESOURCES;
        }

        gSelRecordDescSavedToken = AllocateZeroPool (sizeof(SEL_RECORD_DESC_TOKEN_LOOKUP_TABLE) * MAX_TOKENS_TO_SAVE);
        if (gSelRecordDescSavedToken == NULL) {
            DEBUG((DEBUG_ERROR, "Failed to allocate memory for gSelRecordDescSavedToken! \n"));
            return EFI_OUT_OF_RESOURCES;
        }
    ...
    if (gErrorManager == NULL) {
        DEBUG((DEBUG_ERROR, "Failed to allocate memory for gErrorManager! \n"));
        return EFI_OUT_OF_RESOURCES;
    }
    ...
    DEBUG ((DEBUG_INFO, "ViewSelLog: MaxSelPerForm: %x\n", MaxSelPerForm));
    ...
    SelResponseData.NextSelRecordId = 0x00;
    ...
    while (SelResponseData.NextSelRecordId != 0xFFFF) {
        ...
        SelResponseSize = sizeof (SelResponseData);

        DEBUG ((DEBUG_INFO, "ViewSelLog: RecordId of the event to be read: 0x%02X%02X \n", SelEntry.SelRecID[1], SelEntry.SelRecID[0] ));
        ...
        DEBUG ((DEBUG_INFO, "ViewSelLog: IPMI_STORAGE_GET_SEL_ENTRY Status: %r CompletionCode: %x Next RecordId: %x \n", 
                Status, SelResponseData.CompletionCode, SelResponseData.NextSelRecordId ));
        
        // For Error case, goto DONE.
        if (EFI_ERROR (Status) || (SelResponseData.CompletionCode)) {
            goto DONE;
        }
        ...
        /* If ErrorCount reaches MaxSelPerForm then
           ...
           2. Set PrintViewMoreSelLogString flag and goto DONE.*/
...
DONE:
    DEBUG ((DEBUG_INFO, "ViewSelLog: ViewSelLogFlag: %x \n", ViewSelLogFlag));
    ...
    DEBUG ((DEBUG_INFO, "ViewSelLog: GetVariable Status of ServerSetup Status: %r \n", Status));
    ...
    DEBUG ((DEBUG_INFO, "ViewSelLog: gRT->SetVariable Status: %r \n", Status));
    // Free the memory allocated for Error Manager.
    gBS->FreePool (gErrorManager);

    DEBUG ((DEBUG_INFO, "ViewSelLog: ViewSelLogCallbackFunction() Exit\n"));
