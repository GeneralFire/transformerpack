...
/** @file SmmBmcElog.c
    Server Management SMM BMC ELOG Driver. This driver is able to talk to
    the IPMI aware controller and logs the data into a SEL repository.
...
/**
    ...
    2. Install the SMM version of Redir Elog protocol for EfiElogRedirSmIPMI
       data type.
    ...
    @retval EFI_UNSUPPORTED         Sel is not supported or invalid spec
                                    version.
    @retval EFI_OUT_OF_RESOURCES    Failed to allocate memory.
    @retval Others                  Error status returned from functions
                                    used.
  ...
  IN EFI_SYSTEM_TABLE   *SystemTable )
{
    ...
    DEBUG ((DEBUG_INFO, " gEfiSmmIpmiTransportProtocolGuid Status: %r \n", Status));
    ...
    DEBUG ((DEBUG_INFO, "GetDeviceIdResponse.SelDeviceSupport %X\n", GetDeviceIdResponse.DeviceSupport.Bits.SelDeviceSupport));
