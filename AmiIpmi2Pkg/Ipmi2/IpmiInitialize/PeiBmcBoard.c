...
/** @file PeiBmcBoard.c
    This file contains code for Initialize LPC BMC base address and SIO KCS
    in the PEI stage.
...
// LPC Bridge Bus, Device and Function macros.
#define LPC_BUS                 0
  ...
  IN CONST EFI_PEI_SERVICES     **PeiServices )
{
    ...
    /*{
        ...
        // Point to logical device selection register.
        IoWrite8 (SIO_CONFIG_INDEX, PILOT3_LD_SEL_REGISTER);
        IoWrite8 (SIO_CONFIG_DATA, PILOT3_LDN_KCS4); //Set logical device KCS4.

        // Point to Base Address LSB register.
        IoWrite8 (SIO_CONFIG_INDEX, PILOT3_BASE1_LO_REGISTER);
        // Write Base Address LSB register.
        IoWrite8 (SIO_CONFIG_DATA, (UINT8)((UINT16)SIO_KCS4_DATA_BASE_ADDRESS & 0x00FF));

        //Point to Base Address MSB register.
        IoWrite8 (SIO_CONFIG_INDEX, PILOT3_BASE1_HI_REGISTER);

        // Write Base Address MSB register.
        IoWrite8 (SIO_CONFIG_DATA, (UINT8)((UINT16)SIO_KCS4_DATA_BASE_ADDRESS >> 8));

        // Point to Activate Register.
        IoWrite8 (SIO_CONFIG_INDEX, PILOT3_BASE2_LO_REGISTER);
        // Enable serial port with Activation bit.
        IoWrite8 (SIO_CONFIG_DATA, (UINT8)((UINT16)SIO_KCS4_CMD_BASE_ADDRESS & 0x00FF));

        // Point to Base Address LSB register.
        IoWrite8 (SIO_CONFIG_INDEX, PILOT3_BASE2_HI_REGISTER);
        // Write Base Address LSB register.
        IoWrite8 (SIO_CONFIG_DATA, (UINT8)((UINT16)SIO_KCS4_CMD_BASE_ADDRESS >> 8));

        // Point to Activate Register.
        IoWrite8 (SIO_CONFIG_INDEX, PILOT3_ACTIVATE_REGISTER);
        // Enable serial port with Activation bit.
        IoWrite8 (SIO_CONFIG_DATA, PILOT3_ACTIVATE_VALUE);

        // Point to logical device selection register.
        IoWrite8 (SIO_CONFIG_INDEX, PILOT3_LD_SEL_REGISTER);
        // Set logical device KCS4.
        IoWrite8 (SIO_CONFIG_DATA, PILOT3_LDN_KCS4);
...
//PORTING////PORTING////PORTING////PORTING////PORTING////PORTING////PORTING////PORTING//
    ...
    PciWrite32 (
        ...
        ((IPMI_KCS_BASE_ADDRESS & KCS_BASE_ADDRESS_MASK) | \
...
/*
    ...
    /* There are five KCS interfaces implemented on the Pilot. The KCS1 and
       KCS2 interfaces are disabled and will not be supported by the Pilot
       firmware. The remaining three KCS (KCS3, 4, & 5) interfaces can be
       used for the communication between the host and the BMC.*/

    /* The BMC and the host processor communicate through KCS3, KCS4, KCS5,
       BT, SMIC or Mailboxes. Enable KCS4 in Pilot Sio to communicate with
       BMC. By default base address is 0xCA2 and 0xCA3. just we need to
       enable in Activate Reg.*/
/*
    // Enable decoding of SIO index ports 0x4E, 0x4F.
    PciOr16 (
        (UINTN) PCI_LIB_ADDRESS(LPC_BUS,LPC_DEVICE,LPC_FUNC,REG_LPC_ENABLE),
        CNF2_LPC_EN) );
...
BmcInitDelay (
  IN CONST EFI_PEI_SERVICES     **PeiServices )
{
    DEBUG ((DEBUG_INFO, "Check for Power Failure PWR_FLR bit: %x \n", \
    PciRead8 (
        (UINTN) PCI_LIB_ADDRESS(LPC_BUS, LPC_DEVICE, LPC_FUNC, LPC_GEN_PMCON_3) ) & \
        PWR_FLR_BIT));
    ...
    if (PciRead8 (
            (UINTN) PCI_LIB_ADDRESS(LPC_BUS, LPC_DEVICE, LPC_FUNC, LPC_GEN_PMCON_3) ) & \
            PWR_FLR_BIT) {
        DEBUG ((DEBUG_INFO, "Power Failure PWR_FLR bit for AC power loss. \n"));
        return TRUE;
    }
    return FALSE;
}
