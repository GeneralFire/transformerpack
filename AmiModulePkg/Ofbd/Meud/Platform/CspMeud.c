...
)
{
    ...
    D22F0FunctionDisable = PchPcrRead32(
                            PID_PSF1,
                            CSP_MEUD_PSF1_PCR_T0_SHDW_HECI1_REG_BASE + R_PCH_PSFX_PCR_T0_SHDW_PCIEN );

    return ((D22F0FunctionDisable & B_PCH_PSFX_PCR_T0_SHDW_PCIEN_FUNDIS) == B_PCH_PSFX_PCR_T0_SHDW_PCIEN_FUNDIS);
}
/**
...
)
{
    ...
    Hfs = MeHeci1PciRead32( HECI_R_HFS );
    if( Hfs == 0xFFFFFFFF )
        Hfs = 0;
...
)
{
    ...
    if(((StructPtr->bBlockType) == PFR_BLK) && NewRegion)
    {
        if(gPfrSupport)
        {
            //If the image buffer size is less than (SPI size - BIOS region size), we assume the input file is PFR only.
            if(gImageBufferSize < (GetFlashCapacity() - FLASH_SIZE))
            {
                //Return the staging area offset which is corresponding to the map file.
                Offset = gStagingAreaOffset;
            }
        }
    }
