...
#if defined(MeEsrtUpdate_SUPPORT) && (MeEsrtUpdate_SUPPORT == 1)
...
)
{
    ...
    else
    {
        ...
        do
        {
            ...
            {
                // InitProgressBar() is invoked from OsUpdateCapsuleWrap() based on Windows screen image
                // Skip 0% to avoid InitProgressBar() call from AmiShowProgress().
                if(CurrentPercent != 0) 
                {
                    AmiShowProgress (CurrentPercent);
                }
            }
#endif
