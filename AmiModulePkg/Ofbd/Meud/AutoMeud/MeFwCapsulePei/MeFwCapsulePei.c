...
#endif 
        {
            UINT8   *FfsHeadPoint = (UINT8*)FwCapHdr;
            UINT32  i;
            ...
            for( i = FwCapHdr->HeaderSize; i < FwCapHdr->CapsuleImageSize; i += 16 )
            {
                ...
                if( ((EFI_FIRMWARE_VOLUME_HEADER*)(FfsHeadPoint + \
                                i))->Signature != EFI_FVH_SIGNATURE ) continue;
                if( !guidcmp( (UINT8*)FfsHeadPoint + i + ((EFI_FIRMWARE_VOLUME_HEADER*)\
                        (FfsHeadPoint + i))->ExtHeaderOffset, &mMeFwCapsuleFvGuid ) ) break;
                i += (UINT32)(((EFI_FIRMWARE_VOLUME_HEADER*)(FfsHeadPoint + i))->FvLength - 16);
            } 
