...
#include <Protocol/SpsRecoveryProtocol.h>
...
#if (SPS_FW_RECOVERY_UPDATE_SUPPORT == 1)
UINT32                              gSpsRegionUpdated;
EFI_GUID                            gSpsRecoveryRegionVariableGuid = SPS_RECOVERY_REGION_VARIABLE_GUID;
#endif
...
#if (SPS_FW_RECOVERY_UPDATE_SUPPORT == 1)
/**
    Find SPS FFS file and decompress from recovery buffer 

    @param Buffer  Recovery image buffer
    @param AmiPostMgr Pointer to AMI_POST_MANAGER_PROTOCOL

    @retval EFI_STATUS
**/
EFI_STATUS 
SpsRecoveryUpdateFunc(
    IN  UINT8                       *Buffer,
    IN  AMI_POST_MANAGER_PROTOCOL   *AmiPostMgr
)
{
    EFI_STATUS                         Status;
    EFI_GUID                           SpsFwRecoveryUpdateProtocolGuid = SPS_FW_RECOVERY_UPDATE_PROTOCOL_GUID;
    SPS_FW_RECOVERY_UPDATE_PROTOCOL    *SpsFwRecoveryUpdateProtocol    = NULL;
    UINTN                              *SpsRegionFile;
    UINTN                              *UcSpsBuffer; 
    UINTN                              UcSpsBufferSize;
    EFI_FFS_FILE_HEADER                *Hdr;
    EFI_GUID                           MeRegionFfsGuid = {0xDE90FFA8, 0xB985, 0x4575, 0xAB, 0x8D, 0xAD, 0xE5, 0x2C, 0x36, 0x2C, 0xA3};

    // Locate SpsFwRecoveryUpdateProtocol.
    Status = pBS->LocateProtocol( &SpsFwRecoveryUpdateProtocolGuid, \
                                        NULL, (VOID**)&SpsFwRecoveryUpdateProtocol );
    if( EFI_ERROR(Status) || (Buffer == NULL) ) return Status;

    SpsFwRecoveryUpdateProtocol->AmiPostMgr = NULL;
    if( AmiPostMgr != NULL ) SpsFwRecoveryUpdateProtocol->AmiPostMgr = AmiPostMgr;

    if( (SpsRegionFile = (UINTN*)FvFindFfsFileByGuid(\
            Buffer, &MeRegionFfsGuid )) == NULL ) return EFI_NOT_FOUND;

    Hdr = (EFI_FFS_FILE_HEADER*)SpsRegionFile;
    Status = DecompressBinary(
             SpsRegionFile,
             (UINTN)(*(UINT32*)Hdr->Size & 0xffffff),
             &UcSpsBuffer,
             &UcSpsBufferSize);
    if( EFI_ERROR(Status) ) return Status;    

    Status = SpsFwRecoveryUpdateProtocol->SpsRecovery( SpsFwRecoveryUpdateProtocol, (UINT8*)UcSpsBuffer, gSpsRegionUpdated);
...
)
{
    ...
    Status = SpsRecoveryUpdateFunc( RecoveryBuffer, gAmiPostMgr );
...
#endif
#endif //SPS_FW_RECOVERY_UPDATE_SUPPORT    
    if( EFI_ERROR(Status) )
    {    
        pRS->SetVariable(
                L"MeAutoUpdateReq",
...
)
{
    ...
    // Save "SpsRecoveyRegion" variable here for avoiding NVRAM could be updated later.
#if (SPS_FW_RECOVERY_UPDATE_SUPPORT == 1)
    Length = sizeof(UINT32);
    Status = pRS->GetVariable(
                    L"SpsRegionUpdated",
                    &gSpsRecoveryRegionVariableGuid,
                    NULL,
                    &Length,
                    &gSpsRegionUpdated );
    if( EFI_ERROR(Status) )
        MemSet( &gSpsRegionUpdated, sizeof(UINT32), 0 );     
#endif    
