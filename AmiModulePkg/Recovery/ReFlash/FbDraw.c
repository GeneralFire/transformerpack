...
/**
 * @brief Returns max accessible framebuffer size
 * 
 * It is guaranteed, that all drawing routines won't
 * write past Info->FrameBuf + returned value
 *
 * @param Info Pointer to framebuffer info
 * 
 * @return Maximum accessible size in bytes
 */
UINT32 GetMaxSize (AMI_REFLASH_FRAMEBUFFER_INFO *Info)
{
    UINT16 start_x;      //left-top X coordinate
    UINT16 start_y;      //left-top Y coordinate
    UINT32 start_offset; //offset in bytes from start of video buffer to left-top pixel
    UINT16 num_pixels;   //total pixels to draw
    
    UINT16 minHorRes;
    UINT16 minVerRes;
    UINT32 minSizeInBytes;  //minimal possible size of drawable buffer in pixels
    
    minHorRes = LOGO_WIDTH;
    minVerRes = LOGO_HEIGHT + PROGRESS_BAR_THICKNESS * 3;
    num_pixels = minHorRes * minVerRes;
    minSizeInBytes = num_pixels << 2;     //we need 4 bytes for each pixel
    
    //sanity check
    if (Info->HorRes < minHorRes ||
        Info->VerRes < minVerRes ||
        Info->PxScanLine < Info->HorRes)
        return 0;
    
    start_x = (Info->HorRes - minHorRes) / 2;      //center horizontally
    start_y = (Info->VerRes - minVerRes) / 2;      //center vertically
    start_offset = (start_x + start_y * Info->PxScanLine) << 2;

    if (start_offset > 0xffffffff - minSizeInBytes)     //check for possible overflow
        return 0;
    else
        return start_offset + minSizeInBytes;
}
