 ...
 */
EFI_STATUS CheckRegularBootFlashUpdate (VOID)
{
	return (SETUP_FIRMWARE_UPDATE ||
                FMP_FIRMWARE_UPDATE) ? EFI_SUCCESS : EFI_UNSUPPORTED;
}
...
EFI_STATUS UpdateImage (VOID *Image)
{
    ...
    if (Image != NULL) {
        ...
        if (Buffer == NULL || Size != FLASH_SIZE) {
            DEBUG ((DEBUG_ERROR, "can't find file or file is of wrong size\n"));
