...
)
{
    ...
    if((CodeType & EFI_STATUS_CODE_TYPE_MASK) == EFI_PROGRESS_CODE ) {
        ...
        TextOutPpi->WriteString (TextOutPpi, PANEL_DISPLAY, (UINT8 *)TextString);
        ...
        if(ErrorData != NULL) { 
            ...
            TextOutPpi->WriteString(TextOutPpi, PANEL_DISPLAY, (UINT8 *)ErrorString);
        }
  ...
  IN CONST EFI_PEI_SERVICES  **PeiServices )
{
    ...
    if (!EFI_ERROR (Status)) {
        RscHandlerPpi->Register (PeiReportStatusConOutWorker);
    }
