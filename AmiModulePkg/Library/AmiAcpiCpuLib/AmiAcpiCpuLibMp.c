   ...
   for (Index = 0; Index < NumProtEntries; ++Index) {
    ...
    //AmiCpuInfo2.h file.
        if (AcpiProcInfo[Index].Type == ACPI_PROCESSOR_INFO_TYPE
            ...
            Entry->LocalApicId = (UINT8)AcpiProcInfo[Index].ApicId;
            Entry->LocalApicVersion = (UINT8)AcpiProcInfo[Index].ApicVer;
            Entry->CpuFlags.CpuEnable = (UINT8)AcpiProcInfo[Index].Enable;
            Entry->CpuFlags.CpuBootStrap = (UINT8)AcpiProcInfo[Index].Bsp;
