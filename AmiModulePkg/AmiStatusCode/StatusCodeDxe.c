...
UINT8 CheckpointInt;
EFI_EVENT CheckpointEvent = NULL;
...
{ 
    ...
    if (CheckpointEvent != NULL){
        CheckpointInt = Checkpoint;
        Status = pBS->SignalEvent(CheckpointEvent);
        
    }
    return;
}

VOID EFIAPI ConOutCheckpointInt   (
        IN  EFI_EVENT Event, 
        IN  VOID *Context
)
{
...
{
    ConsoleControl=NULL;
    pBS->CloseEvent(CheckpointEvent);
