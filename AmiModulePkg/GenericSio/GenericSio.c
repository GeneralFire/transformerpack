...
**/ 
EFI_STATUS EFIAPI LaunchCfgModeRoutine(GSIO2 *Sio, UINTN Function)
{
...
//-----------------------------------------
	...
	//		gspio->SioCompProtocol=gSioCompProtocol;
            ...
            //If SIO_CHIP[i] do not have any logical device then the very first AMI_SDL_LOGICAL_DEV_INFO would be
			//a start address of the next AMI_SDL_SIO_CHIP_INFO.
            if(gspio->LdCount == 0)
                SioChipSdlInfo = (AMI_SDL_SIO_CHIP_INFO*)p8;
            else
	...
	for (i=0; i<sl->InstrCount; i++){
		switch (sl->OpType){
			...
			case cfRoutine:
			{
				...
				LaunchCfgModeRoutine(Sio,*FunctionNo);
				break;
			}
...
//------------------------	
	...
	SIO_TRACE((TRACE_SIO, "Updating ISA_DMA_MASK = 0x%X", gAvailableDmaChnl));
...
//--------------------------
	...
	if((Dev->DeviceInfo->Flags&SIO_SHR_IO2) && ResNo ) a=TRUE;
		...
		else a=AssignIo(&Base, rd->_LEN, 0);
	}
	if(rd->_BAS != Base)
	    rd->_BAS = Base;
	if(a){
...
VOID EFIAPI FreeIrq(SIO_DEV2 *Dev, ASLR_IRQ *Dsc)
{
	...
	EFI_STATUS Status = EFI_SUCCESS;
//-------------------	
	if ((gAvailableIsaIrq & Dsc->_INT)) {
		gAvailableIsaIrq &= ~(Dsc->_INT);
		gUsedSioIrq &= ~(Dsc->_INT);
		
		Dev->VlData.DevIrq1 = 0;
		Dev->VlData.DevIrq2 = 0;
		
		if(Dev->Owner->SpioSdlInfo->StdSioDevice){
		    SioRegister(Dev, TRUE, Dev->Owner->SpioSdlInfo->Irq1Reg, &Value);
		    SioRegister(Dev, TRUE, Dev->Owner->SpioSdlInfo->Irq2Reg, &Value);
        } else {
		    SioLibLaunchResourceRoutine(Dev, srIrqReg1);
		    SioLibLaunchResourceRoutine(Dev, srIrqReg2);
		}
		Status = AmiIsaIrqMask(&gAvailableIsaIrq, FALSE);
		SIO_TRACE((TRACE_SIO, "Updating ISA_IRQ_MASK = 0x%X...%r", gAvailableIsaIrq, Status));
	}
}
...
//-------------------	
	if ((gAvailableDmaChnl & Dsc->_DMA)) {
        ...
        if(Dev->Owner->SpioSdlInfo->StdSioDevice){
            SioRegister(Dev, TRUE, Dev->Owner->SpioSdlInfo->Dma1Reg, &Value);
		    SioRegister(Dev, TRUE, Dev->Owner->SpioSdlInfo->Dma2Reg, &Value);
        } else {
		    SioLibLaunchResourceRoutine(Dev, srDmaReg1);
		    SioLibLaunchResourceRoutine(Dev, srDmaReg2);
		}
        Status = AmiIsaDmaMask(&gAvailableDmaChnl, FALSE);
        SIO_TRACE((TRACE_SIO, "Updating ISA_DMA_MASK = 0x%X...%r", gAvailableDmaChnl, Status));
	}
}
...
//--------------------------------
    ...
    if(Status == EFI_NOT_FOUND) {
        Status = pBS->AllocatePool(EfiBootServicesData, DataSize, (VOID**)&SioData);
		if(!EFI_ERROR(Status)) {
        	VarAttrib = EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_NON_VOLATILE;
        	SioData->LockLegacyRes = 0;
        	pRS->SetVariable(L"SioSetupData", &SioSetupGuid, VarAttrib, DataSize, (VOID*)SioData);
		}
    }
