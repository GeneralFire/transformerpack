...
UINT8 confirmUser( )
{
    ...
    if(!EFI_ERROR(Status))
    {
        return(ConfirmOverride->ConfirmUser(ppi_request, 0));
    }
    ...
    OUT TCG_DIGEST          *NewPCRValue )
{
    ...
    ExtendCmd.hdr.Tag = SwapBytes16( TPM_TAG_RQU_COMMAND );
    ExtendCmd.hdr.ParamSize = SwapBytes32(sizeof(TPM_1_2_CMD_HEADER) + sizeof(TPM_PCRINDEX) + 20);
    ExtendCmd.hdr.Ordinal = SwapBytes32( TPM_ORD_Extend );
    ExtendCmd.PcrIndex    = SwapBytes32( PCRIndex );
    ...
    IN EFI_TCG_PROTOCOL *tcg )
{
    ...
    {
        cmdGetCap.CommandCode = SwapBytes32( TCM_ORD_GetCapability );
        cmdGetCap.caparea     = SwapBytes32( TCM_CAP_PROPERTY );
        cmdGetCap.subCap      = SwapBytes32( TCM_CAP_PROP_OWNER );
    }
    else
    {
        cmdGetCap.CommandCode = SwapBytes32( TPM_ORD_GetCapability );
        cmdGetCap.caparea     = SwapBytes32( TPM_CAP_PROPERTY );
        cmdGetCap.subCap      = SwapBytes32( TPM_CAP_PROP_OWNER );
    }
    ...
    IN EFI_TCG_PROTOCOL *tcg )
{
    ...
    SetCapFwUpgrade.tag             = SwapBytes16( TPM_TAG_RQU_COMMAND );
    SetCapFwUpgrade.paramSize       = SwapBytes32( sizeof(SetCapFwUpgrade) );
    SetCapFwUpgrade.ordinal         = SwapBytes32( TPM_ORD_SetCapability );
    SetCapFwUpgrade.capArea         = SwapBytes32( TPM_SET_STCLEAR_DATA );
    SetCapFwUpgrade.subCapSize      = SwapBytes32( 4 );
    SetCapFwUpgrade.subCap          = SwapBytes32( TPM_SD_DEFERREDPHYSICALPRESENCE );
    SetCapFwUpgrade.setValueSize    = SwapBytes32( 4 );
    SetCapFwUpgrade.setValue        = SwapBytes32( 0x01 ); //bit 0: unownedFieldUpgrade
    ...
    DEBUG((DEBUG_INFO, "SetCapFwUpgrade: %r; size: %x; retCode:%x; tag:%x\n", Status,
            SwapBytes32(pRetHeader->ParamSize), SwapBytes32(pRetHeader->RetCode),
           (UINT32) SwapBytes16(pRetHeader->Tag) ));
    ...
    if ( mBootAttempts == 0 )  //do this once
    {
        ...
        if(!EFI_ERROR(Status))
        {
            if ( cap.physicalPresenceLifetimeLock == 0)
            {
                if(IsTcmSupportType())
                {
                    physical_presence = SwapBytes16(TPM_PHYSICAL_PRESENCE_CMD_ENABLE );
                    ...
                    physical_presence = SwapBytes16( TPM_PHYSICAL_PRESENCE_LIFETIME_LOCK );
                    SendTpmCommand( tcg, TCM_TSC_ORD_PhysicalPresence,
                                    sizeof(physical_presence), &physical_presence );
                }
                else
        ...
        else
        {
            ...
            if( SkipPpLock == FALSE)
            {
                ...
                physical_presence = SwapBytes16( TPM_PHYSICAL_PRESENCE_LOCK );
                SendTpmCommand( tcg, TSC_ORD_PhysicalPresence,
                                sizeof(physical_presence), &physical_presence );
            }
#else
    ...
    if(!IsTcmSupportType())
    {
        ...
        switch( rqst)
        {
            ...
            case 10:
                ...
                tpmStatus = SendTpmCommand( tcg, TPM_ORD_SetOwnerInstall,1, &one );
                if((SwapBytes32( tpmStatus ) & TCG_DEACTIVED_ERROR) == TCG_DEACTIVED_ERROR )
                {
                    ...
                    WritePpiResult( rqst, (UINT16)( SwapBytes32( tpmStatus )));
    ...
    IN VOID      *ctx)
{
    ...
    PERSISTENT_BIOS_TPM_MANAGEMENT_FLAGS_PROTOCOL *OemTpmBiosPolicy = NULL;
...
EFIAPI ProcessTcgPpiRequest()
{
    ...
    if (Temp != 0)
    {
        ...
        if(!IsTcmSupportType())
        {
            ...
            {
                tpmStatus = SendTpmCommand( tcg, TPM_ORD_SetOwnerInstall, 1, &one );
                WritePpiResult( ppi_request >> 04, (UINT16)( SwapBytes32( tpmStatus )));
                if(!IsTcmSupportType())
                {
                    TpmDevice->Close( TpmDevice );
                }
