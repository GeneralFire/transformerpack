...
BOOLEAN  EFIAPI IsTcmSupportType ();
...
EFI_PHYSICAL_ADDRESS         NvsMemoryAddress=0;
EFI_SMM_VARIABLE_PROTOCOL    *SmmVarProtocol = NULL;
AMI_ASL_PPI_NV_VAR           *TpmAcpiNvsFlags = NULL;
...
VOID NVOSWrite_PPI_request( )
{
    ...
    AMI_ASL_PPI_NV_VAR   *TpmAcpiWriteNvsFlags = ((AMI_ASL_PPI_NV_VAR *)(UINTN)NvsMemoryAddress);

    Read_value = TpmAcpiWriteNvsFlags->RQST;
    ...
    if((Read_value > TCPA_PPIOP_DISABLE_ENDORSEMENT_ENABLE_STORAGE_HIERARCHY &&
       ...
       (Read_value > TCPA_PPIOP_SET_PPREQUIRED_FOR_DISABLE_BLOCK_SID_FUNC_FALSE &&
        ...
        TpmAcpiWriteNvsFlags->RequestFuncResponse = TREE_PP_SUBMIT_REQUEST_TO_PREOS_NOT_IMPLEMENTED;
        return;
    }
    ...
    if(Read_value < TCPA_PPIOP_VENDOR)
    {
        ...
        Temp.Flag  = 0;
        Temp.AmiMisc = 0;
        Temp.OptionalData = TpmAcpiWriteNvsFlags->OptionalRqstData;
        ...
        if(Status == EFI_INVALID_PARAMETER)
        {
            ...
            if(EFI_ERROR(Status))
            {
                TpmAcpiWriteNvsFlags->RequestFuncResponse = TREE_PP_SUBMIT_REQUEST_TO_PREOS_GENERAL_FAILURE;
                return;
            }
...
VOID Read_User_Confirmation_Status( )
{
    ...
    EFI_STATUS                  Status;
    UINT32                      attrib =     EFI_VARIABLE_NON_VOLATILE | EFI_VARIABLE_BOOTSERVICE_ACCESS;
    AMI_ASL_PPI_NV_VAR              *TpmAcpiConfirmNvsFlags = ((AMI_ASL_PPI_NV_VAR *)(UINTN)NvsMemoryAddress);


    Read_value = TpmAcpiConfirmNvsFlags->RQST;
    ...
    if(EFI_ERROR(Status))
    {
        TpmAcpiConfirmNvsFlags->RequestFuncResponse = TREE_PP_GET_USER_CONFIRMATION_BLOCKED_BY_BIOS_CONFIGURATION;
        return;
    }
    ...
    if(Read_value < TCPA_PPIOP_VENDOR)
    {
        if(Read_value == 0 )
        {
            ...
            TpmAcpiConfirmNvsFlags->RequestFuncResponse = TREE_PP_GET_USER_CONFIRMATION_ALLOWED_AND_PPUSER_NOT_REQUIRED;
            return;
        }
        ...
        if((Read_value > TCPA_PPIOP_DISABLE_ENDORSEMENT_ENABLE_STORAGE_HIERARCHY &&
           ...
           (Read_value > TCPA_PPIOP_SET_PPREQUIRED_FOR_DISABLE_BLOCK_SID_FUNC_FALSE &&
            ...
            TpmAcpiConfirmNvsFlags->RequestFuncResponse = TREE_PP_GET_USER_CONFIRMATION_NOT_IMPLEMENTED;
            return;
        }
        ...
        else if(Read_value == TCPA_PPIOP_CLEAR || Read_value == TCPA_PPIOP_ENABLE_ACTV_CLEAR )
        {
            ...
            {
                TpmAcpiConfirmNvsFlags->RequestFuncResponse = TREE_PP_GET_USER_CONFIRMATION_ALLOWED_AND_PPUSER_REQUIRED;
            }
            return;
        }
...
VOID NVOSWrite_MOR_request( )
{
    ...
    UINT32          Read_value  = 0;
    UINT32          attrib =     EFI_VARIABLE_NON_VOLATILE | EFI_VARIABLE_BOOTSERVICE_ACCESS;
    AMI_ASL_PPI_NV_VAR   *TpmAcpiMorNvsFlags = ((AMI_ASL_PPI_NV_VAR *)(UINTN)NvsMemoryAddress);

    Read_value = TpmAcpiMorNvsFlags->RQST;
    ...
    if(Status)
    {
        TpmAcpiMorNvsFlags->RequestFuncResponse = MOR_REQUEST_GENERAL_FAILURE;
        return;
    }

    TpmAcpiMorNvsFlags->RequestFuncResponse = MOR_REQUEST_SUCCESS;
}
