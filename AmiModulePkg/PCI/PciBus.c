    ...
    for(ChildIndex=0; ChildIndex < ext->ChildCount; ChildIndex++){
      ...
      if (TempEndPointDev->Address.Addr.Bus == TempDev->Address.Addr.Bus &&
          ...
          if (tmp & BIT2) { // BME
              ...
              {
                  Brg=(PCI_BRG_INFO*)TempDev;
                  CheckBmeStatus(&Brg->Bridge,TempEndPointDev,ErrorOccured,DontclearAttr);
...
//------------------------------
    ...
    //--------------
        ...
        {
                DXE_SERVICES            *dxe;    
                EFI_PHYSICAL_ADDRESS    Phyaddr=0x3C0;
            //-----------------------------
                ...
                if(Set) Status=dxe->AllocateIoSpace(EfiGcdAllocateAddress,
                            ...
                            gPciBusDriverBinding.DriverBindingHandle);
                else Status=dxe->FreeIoSpace(Phyaddr,0x20);
                if(EFI_ERROR(Status)){
    ...
    )
{
    ...
    PCI_DEV_INFO                                                                      *dev=(PCI_DEV_INFO*)This;
    EFI_PCI_ROOT_BRIDGE_IO_PROTOCOL_OPERATION           RbIoOperation ;


    if ((Operation < EfiPciIoOperationBusMasterRead) || (Operation >= EfiPciIoOperationMaximum) ) {
        return EFI_INVALID_PARAMETER;
    }
    ...
    RbIoOperation = (EFI_PCI_ROOT_BRIDGE_IO_PROTOCOL_OPERATION)Operation;
    
    if (dev->Attrib & EFI_PCI_IO_ATTRIBUTE_DUAL_ADDRESS_CYCLE) 
        RbIoOperation = ((EFI_PCI_ROOT_BRIDGE_IO_PROTOCOL_OPERATION) (Operation + EfiPciOperationBusMasterRead64));
...
//-----------------------------
    ...
    //APTIOV_SERVER_OVERRIDE_START: REMOVE Below ASSERT when PCI_ENUMERATION_LIGHT Token Mode is disabled
#if defined PCI_ENUMERATION_LIGHT && PCI_ENUMERATION_LIGHT == 1
    ASSERT_EFI_ERROR(Status);
#endif
    //APTIOV_SERVER_OVERRIDE_END: REMOVE Below ASSERT when PCI_ENUMERATION_LIGHT Token Mode is disabled
...
//-------------------------------------------
    ...
    if(!cnt) cnt = MAX_UINTN;
    if(!len) len = MAX_UINTN;
...
//#if PCI_X_SUPPORT
        if(cp.CapabilityID==PCI_CAP_ID_PCIX){
            ...
            devaddr.Addr.Register = (UINT8)Device->PciX->PcixOffs+PCIX_DEV_STA_OFFSET;
            Status=PciCfg32(Device->RbIo, devaddr, FALSE, &Device->PciX->PcixDevStatus.DEV_STA);
            ASSERT_EFI_ERROR(Status);
            if(EFI_ERROR(Status)) return Status;

            devaddr.Addr.Register = (UINT8)Device->PciX->PcixOffs+PCIX_SEC_STA_OFFSET;
    ...
    for(i=0; i<maxbar; i++){
        ...
        switch (Device->Bar[i].Type){
            ...
            case tBarMmio64        :
                ...
                // SpecialBarCheck :  It could be the BAR that claims - Im a 64bit BAR but not actually.
                specialbarcheck = buff >> 32;
                
                if (specialbarcheck < 0xFFFFFFFF) {
                    buff = buff & 0xFFFFFFFF; // use only the lower bits of the BAR
                    if (specialbarcheck == 0) {
                        specialbarcheck = (UINT32) -1;
                        Device->Bar[i].Type-=2; //reduce tBarType to 32 bit BAR
                    }
                    else {
                        specialbarcheck |= ((UINT32)(-1) << HighBitSet32 ((UINT32)specialbarcheck));
                    }
                    
                    
                   // specialbarcheck = specialbarcheck << 32;
                    specialbarcheck =  LShiftU64 (specialbarcheck, 32);
                    buff = buff | specialbarcheck;
                }
                ...
                if(buff){
                    ...
                    if(buff<=0xFFFFFFFF) {
                        ...
                        // Check : If specialbarcheck == 0, no upper32 bits are set. Treat this bar as 32 bit.
                        // If it has non-zero, few upper 32bits are set but not fully.
                        // We can consider this bar still as 64 bit bar as like EDK PciBus.
                        if (specialbarcheck == 0) {
                            Device->Bar[i].Type-=2; //reduce tBarType to 32 bit BAR
                        }
                    }
...
//--------------------
    ...
    for (i=0; i<sResMaxResType; i++) {
        if (((MRES_TYPE)sdldata->FixedResources[i].BarType == ResType ) && (sdldata->FixedResources[i].BarOffset==bbar->Offset)) {
                    ...
                    if (((MixedModeConfiguration == TRUE)  && (Brg->ParentBrg->AmiSdlPciDevData->PciDevFlags.Bits.FixedResources == 0)) ||
                        ...
                        case tBarIo32:                
                            ...
                            if(!EFI_ERROR(Status)) {
                                ...
                                gAmiFixedBrigeRemovalLen[j].ResType[bbar->Type - 1] = (UINT8)(bbar->Type - 1);
...
//-------------------------------
    ...
    if(dev) {
        ...
        if(dev->PciExpress){
            ...
            if(Pcie5CheckPcie5Compatible(dev)) pBS->FreePool(dev->PciExpress->Pcie5);
        ...
        for(pcidev.Addr.Function=0;  pcidev.Addr.Function<=PCI_MAX_FUNC; pcidev.Addr.Function++){
            ...
            switch (ht){
                case HDR_TYPE_DEVICE    : 
                    ...
                    //APTIOV_SERVER_OVERRIDE_START (PCI_ENUMERATION_LIGHT_OVERRIDE): Skip SMBUS and SPI Devices from enumeration. 
                    //  This needs to be removed once we disabled PCI_ENUMERATION_LIGHT and 
                    //  PciPortSkipDeviceInit routine should skip these devices from enumeration.
                    {
...
//-------------------------------------------------------------------------
                    ...
                    if ( (mMaxBusReport > mMaxBusFound) || (mMaxBusScan < mMaxBusFound) ) {
                        ...
                        if (mMaxBusFound > omb){
                            ...
                            mPadBusFound = mPadBusFound + ext->Pad[rtBus].Length;
                        }
                        return EFI_OUT_OF_RESOURCES;
                    } else if (BusReScan) {
                        mMaxReScanBusFound += mMaxBusFound-mMaxBusReport;
    ...
    //EIP 26787 -
        ...
        //--------------------
            if(bar->DiscoveredType == tBarMmio64 || bar->DiscoveredType == tBarMmio64pf) {
                if(bar->Type == tBarMmio32 || bar->Type == tBarMmio32pf) {
                    buffer = buffer & 0xFFFFFFFF; // Clear the Upper Bits to be in safe side
                }
...
//APTIOV_SERVER_OVERRIDE_START (PCI_ENUMERATION_LIGHT_OVERRIDE): Boot in NO SDL Data Mode. Override can be removed once PCIDEVICE Porting is available for all sockets from RC.
