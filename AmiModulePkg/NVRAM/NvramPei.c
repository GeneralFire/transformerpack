...
)
{
    ...
    if ((VarPpi->PeiVarListNumber != 0) && (VarPpi->PeiVarListNumber != 0xFFFF))
    {
        ...
        {
            if (!StrCmp (VariableName, &ListEnt->Name) && !guidcmp(VendorGuid, &ListEnt->Guid))
            {
    ...
    if (VarPpi->PeiVarListNumber == 0)
    {
        ...
        while (!EFI_ERROR(Status1))
        {
            ...
            {
                if (!StrCmp (&(VarHob->Entry.Name), VariableName) && !guidcmp(&(VarHob->Entry.Guid), VendorGuid))
                {
                    return Status;
                }
            }   
        ...
        if (!EFI_ERROR(Status1))
        {
            ...
            MemCpy(&(VarHob->Entry.Name), VariableName, NameSize);
            MemCpy(&(VarHob->Entry.Guid), VendorGuid, sizeof(EFI_GUID));
        }
   }
...
)
{
    ...
    if (NameSize == 0){
            ...
            VarPpi->PeiVarListSize = DataSize;
