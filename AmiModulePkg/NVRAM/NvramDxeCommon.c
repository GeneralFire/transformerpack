...
/**
    This function searches for Var with specific GUID and Name and updates
    it with provided Data

    @param VariableName pointer to Var Name in Unicode
    @param VendorGuid pointer to Var GUID
    @param DataSize size of Var - if different than actual EFI_INVALID_PARAMETER
        will be returned
    @param Data Pointer to memory where Data is located

    @retval EFI_SUCCESS if was found
    @retval EFI_ERROR if not
...
)
{
    ...
    if (Data==NULL || DataSize==0) return EFI_INVALID_PARAMETER;
    Found = FindVariableGetRecords(VariableName,VendorGuid,&VariableNameSize,&Nvar,&LastNvar, &Info);
    if (!Found) return EFI_NOT_FOUND;
    
    if (Info->Flags & NVRAM_STORE_FLAG_NON_VALATILE) return EFI_INVALID_PARAMETER;
    
    DataPtr = NvGetData(LastNvar, VariableNameSize, &RealDataSize, Info);
    if (!DataPtr) return EFI_NOT_FOUND;
    if (RealDataSize != DataSize) return EFI_INVALID_PARAMETER;
    MemCpy(DataPtr, Data, DataSize);
    if (NvramChecksumSupport)
    {
        if ( (LastNvar->flags & NVRAM_FLAG_EXT_HEDER) && ((*NvGetExtFlags (LastNvar) & NVRAM_EXT_FLAG_CHECKSUM)) )
        {
        
            UINT8 Dummy, *p;
        
            p = (UINT8*)((UINTN)DataPtr + DataSize + NvExtSize(LastNvar) - 3);
            Dummy = 0;
            MemCpy((VOID*)p, (VOID*)&Dummy, 1);
            Dummy = NvCalculateNvarChecksum(LastNvar);
...
)
{
    ...
    Var.flags = NVRAM_FLAG_VALID&FLASH_EMPTY_BYTE;
...
)
{
    ...
    Var.size = 0;
    Var.next = FLASH_EMPTY_NEXT;
    Var.flags = (NVRAM_FLAG_VALID&FLASH_EMPTY_BYTE)|NVRAM_FLAG_DATA_ONLY;
