...
VOID EFIAPI UpdateSystemTableConIn(IN EFI_EVENT Event, IN VOID *Context)
{
    ...
    // all console devices are virtual devices (without device path).
...
)
{
    ...
    while ( ListPtr != NULL)
    {
        ...
        if ( SimpleOut->Handle == ControllerHandle)
        {
            ...
            // Adding code to uninstall the ConOutDeviceGuid if the device that it is attached to is being uninstalled
            pBS->UninstallMultipleProtocolInterfaces (ControllerHandle, &gEfiConsoleOutDeviceGuid,NULL,NULL);
            break;
        }
...
)
{
    ...
    if( SupportedModes[MasterMode.Mode].AllDevices != TRUE)
    {
        ...
        // install the ConOutDeviceGuid on any handle controlled by the ConSplitter.
...
)
{
    ...
    // This is used to check if the mode resolutions changed in the SupportedMode list when it was recreated above 
