...
)
{
    ...
    UINT8   Byte2;
    UINT8   FcrValue;
    UINT16  Counter = 100; //100 ms timeout
    ...
    do {
        ...
        if(Byte & DR) {
            SerialReadPort( BaseAddress, MmioDevice, SERIAL_REGISTER_RBR );
        }
        Counter --;
    } while (((Byte & DR) == TRUE) && (Counter != 0));
...
)
{
    ...
    for (i = 0; i < TOTAL_PCI_SERIAL_PORTS; i++) {
        if ((gTerminalSerialSupport.PciSerialPortsLocationVar.Bus[i] == Bus) && 
                        ...
                        for(i=0; i<ResourcesList->ItemCount; i++){
                            ...
                            switch(Header->Name) {
                                ...
                                case ASLV_RT_IO: 
                                    ...
                                    break;
                                case ASLV_RT_IRQ:
                                    IrqMask = ((ASLR_IRQNoFlags*)Header)->_INT;
                                    ...
                                    if((AmiSioVersion >= 12 )|| IsVirtualSerialInstance) {
                                        for(IrqNo = 0; IrqNo < 16; IrqNo++){
                                            if(IrqMask & (1 << IrqNo)) {
                                               gAmiComParameters.SerialIRQ = IrqNo;
                                               break;
                                            }
                                        }
                                    }else{
                                       gAmiComParameters.SerialIRQ = (UINT8)IrqMask;
                                    }
                                    break; 
                            }
                        }
