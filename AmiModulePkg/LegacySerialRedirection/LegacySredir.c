...
)
{
    ...
    UINT16                          *Addr;
    UINT32                          Addr16;
    UINT32                          RpSBCSig = SIGNATURE_32('$','S','B','C');
    UINT32                          RpSBFSig = SIGNATURE_32('$','S','B','F');
    ...
    {
        if (!MemCmp((UINT8*)(UINTN)Addr16, (VOID*)&RpSBCSig, 4))
        {
    ...
    while(Addr16 < 0xf0000)
    {
        if (!MemCmp((UINT8*)(UINTN)Addr16, (VOID*)&RpSBFSig, 4))
        {
...
)
{
    ...
    // If VGA is not Present or InstallLegacyOSthroughRemote is enabled or Recorder mode is enabled,
    // Increase 4KB to maintain Temp Video buffer
    if(!IsVgaPresent() || gAmiComParameters.InstallLegacyOSthroughRemote || gAmiComParameters.RecorderMode) {
        BinSize += 4;
    }
    BinSize = BinSize << 10;
...
)
{
    ...
    if(AmiComParameters->MMIOBaseAddress != 0) {
        ...
        *(UINT32*)(AmiComParameters->MMIOBaseAddress+ \
        ...
        IoWrite8(AmiComParameters->BaseAddress + DIV_MSB, ((Divisor >> 8) & 0xFF));
...
/**
    ...
    @retval EFI_EVENT_NOTIFY function, returns nothing.
