...
)
{
    ...
    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "Install SCard on %lx status = %x SCardDev->ChildHandle %x \n", CCIDHandle, Status, SCardDev->ChildHandle);
...
)
{
    ...
    if (fpICCDevice && !fpICCDevice->ChildHandle) {
        ...
        USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "InstallUSBCCID ....\n" );
        ...
        USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "ICCDev->ChildHandle = %x\n", ICCDev->ChildHandle);
        ...
        USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "Install CCID on %x status = %r\n", CCIDHandle, Status);
        ASSERT_EFI_ERROR(Status);
...
)
{
    ...
    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "UnInstallSCardProtocol ....ChildHandle : %lx", ScardHandle);
    ...
    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, " Status : %r\n", Status);
    return Status;
}
...
)
{
    ...
    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "UnInstallUSBCCID ....ChildHandle : %lx", ChildHandle );
...
)
{
    ...
    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "USBSCardReaderAPIConnect AccessMode : %x CardAction : %x  PreferredProtocols : %x \n",\
    ...
    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "USBSCardReaderAPIConnect Status : %r ActiveProtocol : %x\n", \
...
{
    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "USBSCardReaderAPIDisconnect CardAction : %x \n", CardAction);
    ...
    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "USBSCardReaderAPIDisconnect Status : %r \n", \
...
)
{
    ...
    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "USBSCardReaderAPIStatus ReaderName : %lx ReaderNameLength : %lx Atr : %lx AtrLength : %lx \n",\
    ...
    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "USBSCardReaderAPIStatus ReaderNameLength : %lx AtrLength : %lx  Status : %r\n",\
...
)
{
    ...
    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "USBSCardReaderAPITransmit CAPDU : %lx CAPDULength : %lx RAPDU : %lx RAPDULength : %lx \n",\
    ...
    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "USBSCardReaderAPITransmit RAPDULength : %lx Status : %r\n",\
...
)
{
    ...
    if( Lock ){
        USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "ICCOnTimer::  is locked; return\n" );
        return;
    }
    ...
    do {
        ...
        USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "Controller %lx\n", CcidHandle);
        USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "Insert/Removal: FpDevInfo %lx IccDevice %lx IccDevice->ChildHandle %lx IccDevice->ConfiguredStatus %x\n", \
      ...
      if ((gUsbData->UsbFeature & USB_CCID_USE_INT_INS_REMOVAL) == USB_CCID_USE_INT_INS_REMOVAL){
        ...
        if (IccDevice->ConfiguredStatus & CARDREMOVED) {
            USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "UnInstallUSBCCID ....\n" );
            //
...
)
{
...

    {
        ...
        ASSERT(CCIDDev);
        HcData = gHcTable[CCIDDev->DevInfo->HcNumber - 1];
        UsbStatus = UsbSmiReConfigDevice(HcData, CCIDDev->DevInfo);
        ...
        if (gEvICCEnumTimer == 0) {
            USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "USBBUS: Start: setup timer callback %x\n", &IccOnTimer );
...
)
{
    ...
    if (!NumberOfChildren) {
        ...
        if(--gCounterCCIDEnumTimer==0){
            ...
            USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "USB: Timer Stopped\n");
        }
        ...
        USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "USB: StopUSBCCID: done : %r gCounterCCIDEnumTimer : %x\n", \
...
)
{
    ...
    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "USB CCID binding:\n\t");
    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "SupportedUSBCCID:%x\n", &SupportedUsbCcid );
    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "\tInstallUSBCCID:%x\n", &StartUsbCcid);
    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "\tUninstallUSBCCID:%x\n", &StopUsbCcid );
