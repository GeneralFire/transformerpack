...
InitInt13RuntimeImage()
{
    ...
    //
    {
        UINT8 MjCsmVer = *(UINT8*)(UINTN)0xF0018;
        UINT8 MnCsmVer = *(UINT8*)(UINTN)0xF0019;

        if (MjCsmVer<8 && MnCsmVer<0x55) return EFI_UNSUPPORTED;
    }

    gLegacy16Data = (LEGACY16_TO_EFI_DATA_TABLE_STRUC*)(UINTN)(0xF0000 + *(UINT16*)(UINTN)0xFFF4C);
...
)
{
    ...
    BBS_STATUS_FLAGS    StatusFlags;		//(EIP73024)
    ...
    SetMem(&StatusFlags, sizeof(BBS_STATUS_FLAGS), 0);
