...
UINTN EFIAPI UsbSmiCore( UINT8 Func, ...  )
{
    ...
    gParameters->ApiData.CoreProc.ParamSize = gCoreprocStackSize[Func];
...
)
{
    ...
    ASSERT(Func < sizeof(gHcprocStackSize)/sizeof(int));
    ...
    gParameters->SubFunc = Func;
    gParameters->RetValue = USB_ERROR;       //(EIP91840+)
    gParameters->ApiData.HcProc.ParamBuffer = gCoreStackBuffer;
    gParameters->ApiData.HcProc.ParamSize = gHcprocStackSize[Func];
...
)
{
    ...
    if (IsNewTransfer){
        ...
        if( DevInfo == NULL  || DevInfo->ExtraData == NULL ){
            USB_DEBUG(DEBUG_DEV_INIT, "Canceling bad AsyncInterrupt request\n");
            return EFI_INVALID_PARAMETER;
        }
...
)
{
    ...
    if ((!(gUsbData->UsbStateFlag & USB_FLAG_AFTER_END_OF_DXE)) &&
        ...
        if (EFI_ERROR (Status)) {
            ...
            USB_DEBUG(DEBUG_DEV_INIT, "DcbaaPtr %x \n", Usb3Hc->DcbaaPtr);
            USB_DEBUG(DEBUG_DEV_INIT, "MappedBaseAddr %x \n", MappedBaseAddr);
