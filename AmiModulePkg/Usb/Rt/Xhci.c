...
//    ASSERT(Usb3Hc->OpRegs->UsbSts.Field.Cnr == 0);
    ...
    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_HC, "CMD Ring is at %x\n", (UINTN)&Usb3Hc->CmdRing);
    ...
    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_HC, "EVT Ring is at %x\n", (UINTN)&Usb3Hc->EvtRing);
...
)
{
    ...
    if (HcReadOpReg(HcStruc, XHCI_USBSTS_OFFSET) & XHCI_STS_HALTED) {
        USB_DEBUG(DEBUG_DEV_INIT|DEBUG_HC, "XHCI: XHCI Controller halted. XHCI_ProcessInterrupt Exit_1");
        return USB_ERROR;
    }
    if (!(Cmd & BIT1)) {
