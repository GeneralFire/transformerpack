...
)
{
    ...
    if (gCheckUsbApiParameter) {
        ...
        if ((EFI_ERROR(Status))&&(Status != EFI_ABORTED)){
            USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "UsbCcid Invalid Pointer, Buffer is in SMRAM.\n");
...
)
{
    ...
    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "PCToRDRXfrBlock ....");
    ...
    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "\n");
    for (Index = 0; Index < sizeof(PC_TO_RDR_XFRBLOCK_STRUC) + CmdLength; Index++) {
        USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "%02X ", ((UINT8 *)CcidCmdBuffer)[Index]);
    }
    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "\n");
    ...
    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "%r ....", Status);
...
)
{
    ...
    USB_DEBUG (DEBUG_DEV_INIT, "PCtoRDRIccPowerOn .... PowerLevel : %x...", PowerLevel);
...
)
{
    ...
    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "PCtoRDRIccPowerOff ....");
...
)
{
    ...
    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "PCToRDRGetSlotStatus ....");
...
)
{
    ...
    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "PCToRDRGetParameters ....");
...
)
{
    ...
    USB_DEBUG (DEBUG_DEV_INIT, "PCToRDRSetParameters .... ProtocolNum : %x ", ProtocolNum);
...
)
{
    ...
    USB_DEBUG (DEBUG_DEV_INIT, "PCToRDRSetDataRateAndClockFrequency ....");
...
{    
    ...
    do {
        ...
        for (Index =0; Index < sizeof(RDR_TO_PC_DATABLOCK_STRUC) + FpReceiveBuffer->dwLength; Index++) {
            USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "%02X ", ((UINT8 *)FpReceiveBuffer)[Index]);
        }
...
exit_RDRToPCDataBlock:
    ...
    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, " Status : %r bStatus : %02X bError : %02X\n", Status, FpICCDevice->bStatus, FpICCDevice->bError);
...
exit_RDRToPCDataRateAndClockFrequency:
    ...
    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, " dwClockFrequency :  %4x dwDataRate : %4x\n", 
...
)
{
    ...
    if (((CCIDDescriptor->dwFeatures & (AUTO_PPS_NEGOTIATION_CCID | AUTO_PPS_NEGOTIATION_ACTIVE)) == 0 &&
        ...
        USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "IssuePPSCmd ...");
        ...
        USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "%r\n", Status);
    ...
    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "CCIDDescriptor.... dwFeatures: %x \n",CCIDDescriptor->dwFeatures);
    ...
    if (CCIDDescriptor->dwFeatures & AUTO_PARAMETER_CONFIG) {   
        ...
        if (!FpICCDevice->bProtocolNum && !FpICCDevice->bmFindIndex && !FpICCDevice->bmTCCKST && !FpICCDevice->bGuardTime \
            ...
            USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "Get Parameters is all zero..\n");
    ...
    if (
        ...
        USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "Set Parameters required..%x\n", FpICCDevice->bProtocolNum);
        ...
        if (FpICCDevice->bProtocolNum != FirstOfferredProtocol) {
            USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "bProtocolNum %x doesn't match with  FirstOfferredProtocol%x\n", FpICCDevice->bProtocolNum, FirstOfferredProtocol);
            FpICCDevice->bProtocolNum =  FirstOfferredProtocol;
        }
    }
        ...
        switch(CCIDDescriptor->dwFeatures & 0x70000) { 
            ...
            case TDPU_LEVEL_EXCHANGE:
              ResponseLength = 1;
                ...
                USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "SetIFS[0] %x\n", SetIFS[0]);
...
)
{
    ...
    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "bProtocolNum     : %02X\n", Data[0]); 
    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "bmFindexDIndex   : %02X\n", Data[1]); 
    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "bmTCCKST0        : %02X\n", Data[2]); 
    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "bGaurdTime       : %02X\n", Data[3]); 
    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "bWaitingInterger : %02X\n", Data[4]); 
    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "bClockStop       : %02X\n", Data[5]); 
    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "bIFSC            : %02X\n", Data[6]); // Valid only for T1    
    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "bNadValue        : %02X\n", Data[7]); // Valid only for T1
...
)
{
    ...
    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "etu              : %02X  \n", FpICCDevice->etu); 
    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "GlobalFi         : %04x  \n", FpICCDevice->GlobalFi); 
    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "GlobalFmax       : %02X  \n", FpICCDevice->GlobalFmax); 
    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "GlobalDi         : %02X  \n", FpICCDevice->GlobalDi); 

    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "SpecificMode     : %02X  \n", FpICCDevice->SpecificMode); 

    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "ClassABC         : %02X  \n", FpICCDevice->ClassABC); 
    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "StopClockSupport : %02X  \n", FpICCDevice->StopClockSupport); 

    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "ExtraGuardTime   : %02X  \n", FpICCDevice->ExtraGuardTime); 
    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "WTwaittime       : %08x  \n", FpICCDevice->WTwaittime); 

    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "BWI              : %02X  \n", FpICCDevice->BWI); 
    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "CWI              : %02X  \n", FpICCDevice->CWI); 
    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "IFSC             : %02X  \n", FpICCDevice->IFSC); 
    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "NAD              : %02X  \n", FpICCDevice->NAD); 
    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "EpilogueFields   : %02X  \n", FpICCDevice->EpilogueFields); 
    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "BWT              : %02X  \n", FpICCDevice->BWT); 
...
)
{
    ...
    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "    ATR Data \n");

    for (Index=0; Index < 32; Index++) {
        USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "%02X ", ATRData[Index]);
    }
    ...
    TDx = ATRData[Index];
    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "T0  : %02X  \n", ATRData[Index++]); 
    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "TA1 : %02X  \n", (TDx & 0x10) ? ATRData[Index++] : 0); 
    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "TB1 : %02X  \n", (TDx & 0x20) ? ATRData[Index++] : 0);
    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "TC1 : %02X  \n", (TDx & 0x40) ? ATRData[Index++] : 0);
    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "TD1 : %02X  \n", (TDx & 0x80) ? ATRData[Index++] : 0);

    if (!(TDx & 0x80)) return;
    TDx = ATRData[Index-1];

 
    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "TA2 : %02X \n", (TDx & 0x10) ? ATRData[Index++] : 0);
    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "TB2 : %02X \n", (TDx & 0x20) ? ATRData[Index++] : 0);
    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "TC2 : %02X \n", (TDx & 0x40) ? ATRData[Index++] : 0);
    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "TD2 : %02X \n", (TDx & 0x80) ? ATRData[Index++] : 0);

    if (!(TDx & 0x80)) return;
    TDx = ATRData[Index-1];

    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "TA3 : %02X \n", (TDx & 0x10) ? ATRData[Index++] : 0);
    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "TB3 : %02X \n", (TDx & 0x20) ? ATRData[Index++] : 0);
    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "TC3 : %02X \n", (TDx & 0x40) ? ATRData[Index++] : 0);
    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "TD3 : %02X \n", (TDx & 0x80) ? ATRData[Index++] : 0);

    if (!(TDx & 0x80)) return;
    TDx = ATRData[Index-1];

    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "TA4 : %02X \n", (TDx & 0x10) ? ATRData[Index++] : 0);
    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "TB4 : %02X \n", (TDx & 0x20) ? ATRData[Index++] : 0);
    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "TC4 : %02X \n", (TDx & 0x40) ? ATRData[Index++] : 0);
    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "TD4 : %02X \n", (TDx & 0x80) ? ATRData[Index++] : 0);

    if (!(TDx & 0x80)) return;
    TDx = ATRData[Index-1];

    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "TA5 : %02X \n", (TDx & 0x10) ? ATRData[Index++] : 0);
    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "TB5 : %02X \n", (TDx & 0x20) ? ATRData[Index++] : 0);
    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "TC5 : %02X \n", (TDx & 0x40) ? ATRData[Index++] : 0);
    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "TD5 : %02X \n", (TDx & 0x80) ? ATRData[Index++] : 0);

    if (!(TDx & 0x80)) return;
    TDx = ATRData[Index-1];

    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "TA6 : %02X \n", (TDx & 0x10) ? ATRData[Index++] : 0);
    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "TB6 : %02X \n", (TDx & 0x20) ? ATRData[Index++] : 0);
    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "TC6 : %02X \n", (TDx & 0x40) ? ATRData[Index++] : 0);
    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "TD6 : %02X \n", (TDx & 0x80) ? ATRData[Index++] : 0);

    if (!(TDx & 0x80)) return;
    TDx = ATRData[Index-1];

    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "TA7 : %02X \n", (TDx & 0x10) ? ATRData[Index++] : 0);
    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "TB7 : %02X \n", (TDx & 0x20) ? ATRData[Index++] : 0);
    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "TC7 : %02X \n", (TDx & 0x40) ? ATRData[Index++] : 0);
    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "TD7 : %02X \n", (TDx & 0x80) ? ATRData[Index++] : 0);
...
)
{
    ...
    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "Sizeof SMART Class Descriptor :  %X\n", sizeof (SMARTCLASS_DESC));
    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "DescLength            :  %04X\n", FpCCIDDesc->DescLength);
    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "DescType              :  %04X\n", FpCCIDDesc->DescType);
    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "bcdCCID                :  %04X\n", FpCCIDDesc->bcdCCID);
    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "bMaxSlotIndex          :  %04X\n", FpCCIDDesc->bMaxSlotIndex);
    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "bVoltageSupport        :  %04X\n", FpCCIDDesc->bVoltageSupport);
    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "dwProtocols            :  %04X\n", FpCCIDDesc->dwProtocols);
    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "dwDefaultClock         :  %04X\n", FpCCIDDesc->dwDefaultClock);
    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "dwMaximumClock         :  %04X\n", FpCCIDDesc->dwMaximumClock);
    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "bNumClockSupported     :  %04X\n", FpCCIDDesc->bNumClockSupported);
    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "dwDataRate             :  %04X\n", FpCCIDDesc->dwDataRate);
    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "dwMaxDataRate          :  %04X\n", FpCCIDDesc->dwMaxDataRate);
    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "bNumDataRatesSupported :  %04X\n", FpCCIDDesc->bNumDataRatesSupported);
    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "dwMaxIFSD              :  %04X\n", FpCCIDDesc->dwMaxIFSD);
    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "dwSynchProtocols       :  %04X\n", FpCCIDDesc->dwSynchProtocols);
    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "dwMechanical           :  %04X\n", FpCCIDDesc->dwMechanical);
    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "dwFeatures             :  %04X\n", FpCCIDDesc->dwFeatures);
    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "bClassGetResponse      :  %04X\n", FpCCIDDesc->dwMaxCCIDMessageLength);
    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "bClassGetResponse      :  %04X\n", FpCCIDDesc->bClassGetResponse);
    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "bClassEnvelope         :  %04X\n", FpCCIDDesc->bClassEnvelope);
    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "wLcdLayout             :  %04X\n", FpCCIDDesc->wLcdLayout);
    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "bPINSupport            :  %04X\n", FpCCIDDesc->bPINSupport);
    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "bMaxCCIDBusySlots      :  %04X\n", FpCCIDDesc->bMaxCCIDBusySlots);

    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "*************************************\n"); 
    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, " Device is in:"); 
    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "%a Exchange mode\n", Strings[Exchange]); 
    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "*************************************\n"); 
...
)
{
    ...
    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "USBCCID_ProcessInterruptData.... %X %X %X %X\n", 
    ...
    switch (*Buffer) {
        ...
        case RDR_TO_PC_HARDWAREERROR:        
            ...
            USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "RDR To PC Hardware Error Slot %X Sequence %X Error Code %X \n", 
...
//      MemFill((UINT8 *)FpICCDevice, sizeof(ICC_DEVICE), 0);
        ...
        USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "ICC device removed - Slot : %X\n", Slot);
    ...
    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "Removal: FpDevInfo %X FpICCDevice %X\n", FpDevInfo, FpICCDevice);
...
)
{
    ...
    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "ICCInsertEvent Slot %X \n", Slot);
    //
    ...
    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "FpICCDevice->ConfiguredStatus %X \n", FpICCDevice->ConfiguredStatus);
    ...
    if(EFI_ERROR(Status)){
        ...
        USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "ICC device added - Slot : %X\n", Slot);
    ...
    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "Insert : FpDevInfo %X FpICCDevice %X\n", FpDevInfo, FpICCDevice);
...
)
{
    ...
    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "((SMARTCLASS_DESC*)(CcidDevData->CcidDescriptor))->bMaxSlotIndex %X \n", ((SMARTCLASS_DESC*)(CcidDevData->CcidDescriptor))->bMaxSlotIndex );
...
)
{
    ...
    USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "USBCCIDConfigureDevice ....\n");
    ...
    if (EFI_ERROR(Status)) {
        USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID,"ICCInsertEvent Status = %r\n", Status);
    }
    }
...
)
{
    ...
    if (CcidFuncIndex >= NumberOfCcidFunctions) {
        //UsbUrp->RetValue = USBAPI_INVALID_FUNCTION;
        USB_DEBUG(DEBUG_DEV_INIT|DEBUG_CCID, "UsbApi CCIDRequest Invalid function#%x\n", CcidFuncIndex);
        return;
    }
    //
