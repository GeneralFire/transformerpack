...
)
{
    ...
    if ((gUsbData->UsbDevSupport & USB_KB_DEV_SUPPORT) == USB_KB_DEV_SUPPORT) {
        if (DevInfo) {
            Status = UsbDevInfoValidation(DevInfo);
            if (EFI_ERROR(Status)) {
                fpURP->RetValue = USB_PARAMETER_ERROR;
                return;
            }
        }
        ...
        fpURP->RetValue = USB_SUCCESS;

    } else {
        fpURP->RetValue = USB_NOT_SUPPORTED;
    }
...
USBAPI_Core_Proc(URP_STRUC *Urp)
{
    ...
    ASSERT( Urp->SubFunc < COUNTOF(CORE_PROC_TABLE));

    if (Urp->SubFunc >= COUNTOF(CORE_PROC_TABLE)) {
        USB_DEBUG(DEBUG_DEV_INIT, "USBAPI_Core_Proc: Invalid SubFunc#%x\n", Urp->SubFunc);
        return;
    }

    Urp->ApiData.CoreProc.RetVal = UsbApiInvokeInFrame(
        CORE_PROC_TABLE[Urp->SubFunc],buffer,ParamSize);
