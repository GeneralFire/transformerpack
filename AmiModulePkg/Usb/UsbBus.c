...
)
{
    ...
    Status = gBS->AllocatePool (EfiBootServicesData,
                   Dev->ConfigCount*sizeof(EFI_USB_CONFIG_DESCRIPTOR*), 
                   (VOID**)&Dev->ConfigDesc);
    ASSERT_EFI_ERROR(Status);
    SetMem(Dev->ConfigDesc, Dev->ConfigCount*sizeof(EFI_USB_CONFIG_DESCRIPTOR*), 0);
...
)
{
    ...
    FirstEp = (UINT8)Dev->FirstEndpoint[Dev->DevInfo->AltSettingNum];
    LastEp = (UINT8)Dev->EndEndpoint[Dev->DevInfo->AltSettingNum];
...
)
{
    ...
    if (Dev->AsyncEndpoint != 0) {
        Status = Dev->Io.UsbAsyncInterruptTransfer(&Dev->Io, (UINT8)Dev->AsyncEndpoint, FALSE, 
                        0, 0, NULL, NULL);
        Dev->AsyncEndpoint = 0;
...
)
{
    ...
    if (Lock) {
        USB_DEBUG(DEBUG_DEV_INIT|DEBUG_BUS, "UsbHcOnTimer::  is locked; return\n");
        return;
    }
