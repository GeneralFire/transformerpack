    ...
    for (ControllerHandleIndex = 0; ControllerHandleIndex < gUsbDataList->Usb3HcCount; ControllerHandleIndex++) {
        ...
        if ((gUsbData->UsbRuntimeDriverInSmm != USB_RUNTIME_DRV_MODE_0) && 
            ...
            if (Usb3Hc->DcbaaPages > 0) {
                ...
                USB_DEBUG(DEBUG_DEV_INIT, "Usb3Hc->DcbaaPtr %x\n", Usb3Hc->DcbaaPtr);
...
)
{
    ...
    for(LayerofPortLink = 0x00;LayerofPortLink <= TotalHub; LayerofPortLink++){
        if(ParentAddress == RootPortLink[LayerofPortLink].AddressOfLayer){
            TestResult = CheckHubOnRootPort(RootPortLink, RootPortLink[LayerofPortLink].ParentAddress,TotalHub);
    ...
    for (Index = 0; Index < HandleCount; Index++) {
      ...
      if (!EFI_ERROR (Status)) {
        ...
        //
          ...
          if (((UINT8)Device == (UINT8)PciDpNode->Device) && ((UINT8)Func == (UINT8)PciDpNode->Function)) {
...
)
{
    ...
    MaxDevCount = MaxHidCount + MaxHubCount + MaxMassCount + MaxCcidCount;
