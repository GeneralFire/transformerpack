    ...
    IN EFI_PEI_SERVICES    **PeiServices )
{
    ...
    for (ControllerIndex = 0; ControllerIndex < gUsbPeiData->XhciHcCount;
        ...
        (*PeiServices)->PciCfg->Read(
                          ...
                          XHCI_PCI_ADDRESS(ControllerIndex, 
        ...
        ZeroMem((VOID*)(UINTN)TempPtr, MemPages * 4096); // Clear allocated memory
        ...
        if (((PciAttributes & (BIT1 |BIT2)) == BIT2) && (sizeof(UINTN) != sizeof(UINT32))) {            
            (*PeiServices)->PciCfg->Read(
                              (CONST EFI_PEI_SERVICES**)PeiServices,
                              (*PeiServices)->PciCfg,
        ...
        do {
            Status = PeiServicesLocatePpi (&gLoadXhcFirmwarePpiGuid, Instance++, NULL, (VOID**)&LoadXhcFwPpi);
            if (!EFI_ERROR(Status)) {
        ...
        Usb3Hc->MaxIntrs = (UINT16)Usb3Hc->CapRegs.HcsParams1.MaxIntrs;     // Get maximum number of interrupters
        ...
        Status = PeiServicesInstallPpi (&Usb3Hc->PpiDescriptor); 
...
{
    EFI_STATUS              Status;
    UINT16                  NumBufs = (UINT16)(((Usb3Hc->CapRegs.HcsParams2.MaxScratchPadBufsHi) << 5) + 
    ...
    ZeroMem((VOID*)(UINTN)TempPtr, (sizeof(UINT64) * NumBufs));
    ...
    ZeroMem((VOID*)(UINTN)TempPtr, Usb3Hc->PageSize4K * 4096 * (NumBufs + 1));
...
)
{
    ...
    if (Usb3Hc->Usb2Protocol.ThirdDword.PortCount) {
		for (Count = 0; Count < Usb3Hc->Usb2Protocol.ThirdDword.PortCount; Count++) {
			PortNumber = (UINT8)(Count + Usb3Hc->Usb2Protocol.ThirdDword.PortOffset);
            PortStsOffset = XHCI_PORTSC_OFFSET + + (0x10 * (PortNumber - 1));
...
)
{
    ...
    if (Status == EFI_SUCCESS) {
        ...
        PeiUsbDev->TransactionTranslator = (UINT16)(Port << 7) + PeiUsbDev->DeviceAddress;
    }
    return Status;
}
...
)
{
    ...
    if (DevAddr == 0) {
        ...
        for (Index = 1; Index < PEI_XHCI_MAX_SLOTS + 1; Index++, Map++) {
            if (Map->SlotId == 0) {
                Map->SlotId = SlotId;
                Map->DevAddr = DevAddr;
                Map->PortNumber  = PortNumber;
...
)
{
    ...
    *PortNumber = (UINT8)Usb3Hc->CapRegs.HcsParams1.MaxPorts;
    return EFI_SUCCESS;
}
...
)
{
    ...
    for (Index = 0; Index < PEI_XHCI_PORT_RESET_TIMEOUT_MS; Index++) {
        if (PortSts->XhciPortscUnion.Field.Pr == 0) {
            break;
        }
	...
	switch (PortSts->XhciPortscUnion.Field.Pls) {
		case XHCI_PORT_LINK_U0:
		case XHCI_PORT_LINK_RXDETECT:
			break;
		case XHCI_PORT_LINK_RECOVERY:
             for (Index = 0; Index < PEI_XHCI_PORT_LINK_RECOVERY_TIMEOUT_MS; Index++) {
				XHCI_FIXED_DELAY_MS(Usb3Hc, 1);  
                XhciPortSts = XhciReadOpReg(Usb3Hc, PortStsOffset);
				if (PortSts->XhciPortscUnion.Field.Pls != XHCI_PORT_LINK_RECOVERY) {
					break;
				}
			}
			break;
		case XHCI_PORT_LINK_POLLING:
			if (!XhciIsUsb3Port(Usb3Hc, PortNumber)) {
             ...
             for (Index = 0; Index < PEI_XHCI_PORT_LINK_POLLING_TIMEOUT_MS; Index++) {
				XHCI_FIXED_DELAY_MS(Usb3Hc, 1);  
                XhciPortSts = XhciReadOpReg(Usb3Hc, PortStsOffset);
				if (PortSts->XhciPortscUnion.Field.Pls != XHCI_PORT_LINK_POLLING) {
					break;
				}
			}
			if (PortSts->XhciPortscUnion.Field.Pls == XHCI_PORT_LINK_U0 || 
				PortSts->XhciPortscUnion.Field.Pls == XHCI_PORT_LINK_RXDETECT) {
				break;
			}
			break;
		case XHCI_PORT_LINK_INACTIVE:
             for (Index = 0; Index < PEI_XHCI_PORT_LINK_INACTIVE_TIMEOUT_MS; Index++) {
				XHCI_FIXED_DELAY_MS(Usb3Hc, 1);  
                XhciPortSts = XhciReadOpReg(Usb3Hc, PortStsOffset);
				if (PortSts->XhciPortscUnion.Field.Pls != XHCI_PORT_LINK_INACTIVE) {
					break;
				}
			}
            if (PortSts->XhciPortscUnion.Field.Pls == XHCI_PORT_LINK_RXDETECT) {
                break;
			}
    ...
    *(UINT32*)PortStatus = 0;
    ...
    if (PortSts->XhciPortscUnion.Field.Ccs != 0) {
        PortStatus->PortStatus |= USB_PORT_STAT_CONNECTION;
    }
    if (PortSts->XhciPortscUnion.Field.Ped != 0) {
        PortStatus->PortStatus |= USB_PORT_STAT_ENABLE;
    }
    if (PortSts->XhciPortscUnion.Field.Oca != 0) {
        PortStatus->PortStatus |= USB_PORT_STAT_OVERCURRENT;
    }
    if (PortSts->XhciPortscUnion.Field.Pr != 0) {
        PortStatus->PortStatus |= USB_PORT_STAT_RESET;
    }
    if (PortSts->XhciPortscUnion.Field.Pp != 0) {
        PortStatus->PortStatus |= USB_PORT_STAT_POWER;
    }
    if (PortSts->XhciPortscUnion.Field.Csc != 0) {
        PortStatus->PortChangeStatus |= USB_PORT_STAT_C_CONNECTION;
    }
    if (PortSts->XhciPortscUnion.Field.Pec != 0) {
        PortStatus->PortChangeStatus |= USB_PORT_STAT_C_ENABLE;
    }
    if (PortSts->XhciPortscUnion.Field.Occ != 0) {
        PortStatus->PortChangeStatus |= USB_PORT_STAT_C_OVERCURRENT;
    }
    if ((PortSts->XhciPortscUnion.Field.Prc != 0) ||(PortSts->XhciPortscUnion.Field.Wrc != 0))  {
        PortStatus->PortChangeStatus |= USB_PORT_STAT_C_RESET;
        PortStatus->PortChangeStatus |= USB_PORT_STAT_C_CONNECTION;
    }

    UpdatePortStatusSpeed(PeiServices, (UINT8)PortSts->XhciPortscUnion.Field.PortSpeed, &PortStatus->PortStatus);
        ...
        case EFI_USB_HC_RESET_GLOBAL:
            ...
            for (Index = 0; Index <= PEI_XHCI_MAX_SLOTS; Index++) {
                if (Usb3Hc->DcbaaPtr->DevCntxtAddr[Index]) {
                    SlotId = (UINT8)(Index + 1);
                    SlotCtx = (XHCI_SLOT_CONTEXT*)XhciGetContextEntry(Usb3Hc, (VOID*)(UINTN)Usb3Hc->DcbaaPtr->DevCntxtAddr[Index], 0);
                    for (Dci = 1; Dci <= SlotCtx->ContextEntries; Dci++) {
                            EpCtx = (XHCI_EP_CONTEXT*)XhciGetContextEntry(Usb3Hc, (VOID*)(UINTN)Usb3Hc->DcbaaPtr->DevCntxtAddr[Index], Dci);
            ...
            if (Usb3Hc->FreeResource == TRUE) {
                ...
                (*PeiServices)->PciCfg->Write(
                                  ...
                                  (*PeiServices)->PciCfg,
                                  EfiPeiPciCfgWidthUint32,
                                  XHCI_PCI_ADDRESS(Usb3Hc->ControllerIndex, 
                                  PCI_BASE_ADDRESSREG_OFFSET),
                                  &Val64
                                  );
...
)
{
    ...
    {
        UINT8   *Ptr = (UINT8*)RingBase;
        UINTN   Index;
        for (Index = 0; Index < RingSize*sizeof(XHCI_TRB); Index++, Ptr++) *Ptr = 0;
    }
...
/**
    ...
    @retval  TRB that can be used for command, transfer, etc.
...
)
{
    ...
    // Clear the TRB
    {
        UINT32 *Ptr = (UINT32*)Trb;
        UINT8 Index = 0;
        for (Index=0; Index<(sizeof(XHCI_TRB)/sizeof(UINT32)); Index++) {
            *Ptr++ = 0;
        }
    }
...
)
{
    ...
    XHCI_TRB *Trb = (XHCI_TRB*)(UINTN)TrbPtr;
...

    {
        ...
        for (Index = 0; Index < 8; Index++) {
            DEBUG((DEBUG_INFO, "%x", Usb3Hc->HidData[Index]));
        }
        DEBUG((DEBUG_INFO, "\n"));
    }
        ...
        switch (Trb->TrbType) {
            case XhciTTransferEvt:
                Status = XhciProcessXferEvt(
                    ...
                    (UINT8)EvTrb->TransferEvt.SlotId,
                    (UINT8)EvTrb->TransferEvt.EndpointId);
                break;
...
)
{
    ...
    if (IsHub) {
        ...
        Status = XhciHcControlTransfer(
                   PeiServices, 
                   This,
                   0,  // Current address
                   Speed,
                   (UINT8)EpCtx->MaxPacketSize,
...
)
{
    ...
    if ((Usb3Hc->Usb3Protocol.ThirdDword.PortCount != 0) && (Port >= Usb3Hc->Usb3Protocol.ThirdDword.PortOffset) &&
        (Port < Usb3Hc->Usb3Protocol.ThirdDword.PortOffset + Usb3Hc->Usb3Protocol.ThirdDword.PortCount)) {
        return TRUE;
    }
    ...
    if (Usb3Hc->Usb3xProtocolCount) {
        for (Usb3xProtocolCount = 0; Usb3xProtocolCount < Usb3Hc->Usb3xProtocolCount; Usb3xProtocolCount++) {
            ...
            if ((Usb3xProtocol->ThirdDword.PortCount != 0) && (Port >= Usb3xProtocol->ThirdDword.PortOffset) &&
                (Port < Usb3xProtocol->ThirdDword.PortOffset + Usb3xProtocol->ThirdDword.PortCount)) {
                return TRUE;
            }
        }
    }
...
)
{
    ...
    if (Usb3Hc->Usb2Protocol.ThirdDword.PortCount) {
		for(Count = 0; Count < Usb3Hc->Usb2Protocol.ThirdDword.PortCount; Count++) {
            ...
            PortStsOffset = XHCI_PORTSC_OFFSET + (0x10 * (PortNumber - 1));
            XhciPortSts = XhciReadOpReg(Usb3Hc, PortStsOffset);
            if (PortSts->XhciPortscUnion.Field.Ccs) {
                if (!(PortSts->XhciPortscUnion.Field.Ped)) {
                    XhciWriteOpReg(Usb3Hc, PortStsOffset, XHCI_PCS_PR | XHCI_PCS_PP);
                    XhciPortSts = XhciReadOpReg(Usb3Hc, PortStsOffset);
                    for (Index = 0; Index < PEI_XHCI_RESET_USB2_PORT_TIMEOUT_MS * 10; Index++) {
                        if (PortSts->XhciPortscUnion.Field.Prc) {
                            break;
                        }
                        XHCI_FIXED_DELAY_US(Usb3Hc, 100);
                        XhciPortSts = XhciReadOpReg(Usb3Hc, PortStsOffset);
...
)
{
    ...
    for (;;) {
        ...
        switch (XhciExtCap->CapId) {
            case XHCI_EXT_CAP_SUPPORTED_PROTOCOL:
                if (((XHCI_EXT_PROTOCOL*)XhciExtCap)->FirstDword.MajorRev == XHCI_EXT_PROTOCOL_MAJOR_REV_03) {
                    if (IsUsb3xSupportProtocol(((XHCI_EXT_PROTOCOL*)XhciExtCap)->FirstDword.MinorRev)) {
                        Usb3xProtocolCount++;
                    }
                }
                break;
    ...
    if (Usb3xProtocolCount > 0) {
        ...
        ZeroMem ((VOID*)(UINTN)TempPtr, MemPages * 4096);
        Usb3Hc->Usb3xProtocolCount = Usb3xProtocolCount;
        Usb3Hc->Usb3xProtocol = (VOID*)(UINTN)TempPtr;
    }
    return EFI_SUCCESS;
}
    ...
    for(;;) {
        ...
        switch (XhciExtCap->CapId) {
            case XHCI_EXT_CAP_USB_LEGACY:
                break;
            case XHCI_EXT_CAP_SUPPORTED_PROTOCOL:
                DEBUG((DEBUG_INFO, "XHCI: CAP supported : MajorRev %x MinorRev %x\n", ((XHCI_EXT_PROTOCOL*)XhciExtCap)->FirstDword.MajorRev, ((XHCI_EXT_PROTOCOL*)XhciExtCap)->FirstDword.MinorRev));
                if (((XHCI_EXT_PROTOCOL*)XhciExtCap)->FirstDword.MajorRev == XHCI_EXT_PROTOCOL_MAJOR_REV_02) {
                    ...
                    DEBUG((DEBUG_INFO, "XHCI: USB2 Support Protocol %x, PortOffset %x PortCount %x\n", 
                        Usb3Hc->BaseAddress + CurPtrOffset, Usb3Hc->Usb2Protocol.ThirdDword.PortOffset, Usb3Hc->Usb2Protocol.ThirdDword.PortCount));
                } else if (((XHCI_EXT_PROTOCOL*)XhciExtCap)->FirstDword.MajorRev == XHCI_EXT_PROTOCOL_MAJOR_REV_03) {
                    if (((XHCI_EXT_PROTOCOL*)XhciExtCap)->FirstDword.MinorRev == XHCI_EXT_PROTOCOL_MINOR_REV_00) {
                        ...
                        DEBUG((DEBUG_INFO, "XHCI: USB3.0 Support Protocol %x, PortOffset %x PortCount %x\n", 
                            Usb3Hc->BaseAddress + CurPtrOffset, Usb3Hc->Usb3Protocol.ThirdDword.PortOffset, Usb3Hc->Usb3Protocol.ThirdDword.PortCount));
                    } else if (IsUsb3xSupportProtocol(((XHCI_EXT_PROTOCOL*)XhciExtCap)->FirstDword.MinorRev)) {
                        if ((Usb3Hc->Usb3xProtocolCount) && (Usb3xProtocolCount < Usb3Hc->Usb3xProtocolCount)) {
                            ...
                            DEBUG((DEBUG_INFO, "XHCI: USB3.x Support Protocol %x, PortOffset %x PortCount %x\n", 
                                Usb3Hc->BaseAddress + CurPtrOffset, Usb3xProtocol->ThirdDword.PortOffset, Usb3xProtocol->ThirdDword.PortCount));
                            Usb3xProtocolCount++;
                        }
                    }
                }
