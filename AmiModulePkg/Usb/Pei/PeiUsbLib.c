...
)
{
    ...
    for (KeyIndex = 2; (KeyIndex < 8) && (UsbKeys[KeyIndex] == 0); KeyIndex++) {}
        if (KeyIndex == 8) {
            return;
        }
    ...
    for (KeyIndex = 0, Modifier = UsbKeys[0]; KeyIndex < 8; KeyIndex++) {
        if ((1 << KeyIndex) & Modifier) {
            KeyData.KeyState.KeyShiftState |= ModifierBits[KeyIndex];
        }
    }

    // Process up to six USB keys pressed at the same time
    for (KeyIndex = 2; KeyIndex < 8; KeyIndex++) {
        if (UsbKeys[KeyIndex] == 0) continue;
        ...
        // Process Keypad
        if (UsbKeys[KeyIndex] >= 0x59 && UsbKeys[KeyIndex] <=0x63 && KeyData.Key.ScanCode != EFI_SCAN_NULL) {
            if (gNumLockOn == 1
                && (KeyData.KeyState.KeyShiftState & (RIGHT_SHIFT_PRESSED | LEFT_SHIFT_PRESSED)) == 0) {
    ...
    for (KeyIndex = 0; KeyIndex < 8; KeyIndex++) {
        DEBUG((DEBUG_INFO, "%x ", UsbKeys[KeyIndex]));
    }
    DEBUG((DEBUG_INFO, "\n"));
}
