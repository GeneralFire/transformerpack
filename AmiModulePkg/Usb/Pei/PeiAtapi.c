    ...
    IN PEI_BOT_DEVICE   *PeiBotDevice )
{
    ...
    if (PeiBotDevice->DeviceType == USBFLOPPY) {
        ...
        PeiUsbValidatePartitionTable( 
          PeiServices, 
          Buffer,
          PeiBotDevice->Media.LastBlock, 
          &(PeiBotDevice->FdEmulOffset) 
          );
    }
    return EFI_SUCCESS;
}
...
{
    UINT8 Index;
    UINT8 *TableStart = Buffer + 0x1be;   // Start offset of partition
                                      // table
    UINT8 *ActivePartitionAddr = NULL;
    UINT8 PartitionFound = 0;     // Total number of valid
                                  // partitions
    UINT8 *PartitionTableEntries[4];

    for (Index = 0; Index < 4; Index++) {
        PartitionTableEntries[Index] = 0;
        ...
        if (*(UINT32 *) (TableStart + 8) > LastBlock) {
            return;
        }
        ...
        /** This check has to be refined
                ...
                if (*(UINT32*)(TableStart + 0xc) > LastBlock)
                    return;
         #endif
         **/
        PartitionFound++;
        PartitionTableEntries[Index] = TableStart;
        
        // Update active entry offset
        if (*TableStart & 0x80) {
            ActivePartitionAddr = TableStart;
        }

        TableStart += 0x10; //Get next Partition
    }
    ...
    if (ActivePartitionAddr == NULL) {
        for (Index = 0; (Index < 4) && !PartitionTableEntries[Index]; Index++) {
            ;
        }
        if (Index == 4) return;
        ActivePartitionAddr = PartitionTableEntries[Index];
    }

    *FdEmulOffset = *((UINT32 *)(ActivePartitionAddr + 8));
    
    DEBUG((DEBUG_INFO,"USBPEIM: PeiUsbValidatePartitionTable() FdEmulOffset %x\n", *FdEmulOffset));
}
