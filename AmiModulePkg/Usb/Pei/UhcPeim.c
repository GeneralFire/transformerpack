    ...
    IN EFI_PEI_SERVICES    **PeiServices )
{
    ...
    Status = PeiServicesLocatePpi (&gEfiPeiCpuIoPpiInstalledGuid, 0, NULL, (VOID **) &CpuIoPpi);
    if (EFI_ERROR(Status)) return EFI_UNSUPPORTED;

    // Locate the PEI_STALL_PPI interface
    if (gUsbPeiStall==NULL) return EFI_UNSUPPORTED;

    // Locate the EFI_PEI_USB_CONTROLLER_PPI interface
    Status = PeiServicesLocatePpi (&gPeiUsbControllerPpiGuid, 0, NULL, (VOID **) &ChipSetUsbControllerPpi);
    if (EFI_ERROR(Status)) return EFI_UNSUPPORTED;
...
)
{
    ...
    QH_STRUCT   *PtrQh;
    TD_STRUCT   *PtrFirstTd=NULL;
    TD_STRUCT   *PtrTd;
    TD_STRUCT   *PtrPreTd=NULL;

    static UINT8 UhciPollingStarted = FALSE;
...
)
{
    ...
    //
    // Index is used to calculate the total number of TDs.
    //
    Index = 0;
    ...
    *TransferResult = EFI_USB_NOERROR;
    ...
    do {
        ...
        if (gUsbPeiStall!=NULL)
            gUsbPeiStall->Stall( 
                            (CONST EFI_PEI_SERVICES**)UhcDev->PeiServices, 
                            gUsbPeiStall, 
                            50
                            );
...
)
{
    ...
    switch (Attributes) {
        case EFI_USB_HC_RESET_GLOBAL:
            ...
            PciCfgPpi->Write((CONST EFI_PEI_SERVICES**)PeiServices, PciCfgPpi, EfiPeiPciCfgWidthUint32,
                gUhciPciReg[UhcDev->ControllerIndex] |
                PCI_REG_UHCI_BASE_ADDRESS_REGISTER, &Val64);
                        
                        // Clear UHCI Command Register
            PciCfgPpi->Write((CONST EFI_PEI_SERVICES**)PeiServices, PciCfgPpi, EfiPeiPciCfgWidthUint16,
                gUhciPciReg[UhcDev->ControllerIndex] |
    ...
    BOOLEAN     SearchOther )
{
    ...
    for (Index = BeginFrame; Index < EndFrame; Index++) {
        ...
        pCurFrame = UhcDev->FrameListEntry + Index;
    ...
    UINTN       AllocSize )
{
    ...
    for (TempHeaderPtr = MemoryHeader; TempHeaderPtr != NULL;
        ...
        if ( (Pool >= TempHeaderPtr->MemoryBlockPtr) &&
            ...
            //
            for (Index = StartBytePos, IndexJ = StartBitPos, Count = 0;
                 Count < (RealAllocSize / 32); Count++) {

                TempHeaderPtr->BitArrayPtr[Index] ^= (UINT8) ( bit( IndexJ ) );
                IndexJ++;
                if (IndexJ == 8) {
                    Index += 1;
                    IndexJ = 0;
