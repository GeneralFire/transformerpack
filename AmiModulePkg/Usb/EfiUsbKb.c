...
)
{
    ...
    // APTIOV_SERVER_OVERRIDE_START: Changes to fix UEFI SCT tool fail under Hard Disk. 
    if (EFI_ERROR(Status)){
        if (Status == EFI_DEVICE_ERROR) return Status;
    ...
    // APTIOV_SERVER_OVERRIDE_END: Changes to fix UEFI SCT tool fail under Hard Disk. 
