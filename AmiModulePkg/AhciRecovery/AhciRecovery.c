...
)
{
    ...
    (*PeiServices)->LocatePpi((CONST EFI_PEI_SERVICES**)PeiServices,
                              &gEdkiiIoMmuPpiGuid,
                              0,
                              NULL,
                              (VOID **)&Edk2IoMmuPpi
    ...
    for(Index=0; Index < AHCI_MAXIMUM_PCI_ROOT_BRIDGE ;Index++) {
        if( gAhciPciBridgeInfo[Index] ) {
            if(PrimaryBusNo == gAhciPciBridgeInfo[Index]->SecBusNumber) {
                gAhciPciBridgeInfo[Index]->SubBusNumber = SubordinateBusNo;
                gPciCfg->Write ( gPeiServices, \
                                 gPciCfg, \
                                 EfiPeiPciCfgWidthUint8, \
            ...
            switch(HeaderType) {
                ...
                case HEADER_TYPE_DEVICE :   
                    ...
                    if((RevisionId[3]== AHCI_CLASS_CODE) &&         // Check for MassStorage Device
                        ...
                        AhciBarMask = ~DeviceDecodeRange;
                        DeviceDecodeRange = AhciBarMask + 1;

                        // Ahci base address (31-13)                                                               
                        if((AhciMmioDeviceAddress & AhciBarMask) != 0 ) {
                             AhciMmioDeviceAddress = (AhciMmioDeviceAddress & (~AhciBarMask)) + DeviceDecodeRange;
                        }                        
...
InstallPeiRecoveryBlkIoPpi ()
{
    ...
    (**gPeiServices).SetMem ((VOID*)Ahci_BlkIoDev,
                             sizeof(AHCI_RECOVERY_BLK_IO_DEV),
                             0 );
    ...
    Status = (**gPeiServices).InstallPpi((const EFI_PEI_SERVICES**)gPeiServices, &Ahci_PpiDescriptor );
    ASSERT_EFI_ERROR (Status );
    ...
    Status = (**gPeiServices).InstallPpi((const EFI_PEI_SERVICES**)gPeiServices, &Ahci_PpiDescriptor2 );
    ASSERT_EFI_ERROR (Status );
