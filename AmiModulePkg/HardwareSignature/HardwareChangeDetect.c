...
STATIC EFI_CPU_IO2_PROTOCOL  *gCpuIo2Protocol = NULL;
#endif
/**
 * 
...
#if HARDWARE_SIGNATURE_DETECT_PS2_KEYBOARD || HARDWARE_SIGNATURE_DETECT_PS2_MOUSE
///-----------------------------------------------------------------------------
/// The following functions is used for detecting PS2 mouse and keyboard.
/// And the original source code, please refer to Ps2Ctl module.  
///------------------------------------------------------------------------------

/**
 * Reads registers in the EFI CPU I/O space.
 * 
 * Reads 1 byte from the I/O port specified by Port. The read value is returned. 
 * If such operations are not supported, then ASSERT(). This function must guarantee 
 * that all I/O read and write operations are serialized.
 * 
  ...
  IN      UINTN                      Port )
{
  ...
  Status = gCpuIo2Protocol->Io.Read (gCpuIo2Protocol, EfiCpuIoWidthUint8, Port, 1, &Data);
  
  if (EFI_ERROR (Status)) {
  
      DEBUG((DEBUG_ERROR, "IoReadByte status:%r\n", Status));
  }
...
/**
  ...
  Writes 1 byte to the I/O port specified by Port.Data is returned. If such operations 
  are not supported, then ASSERT(). This function must guarantee that all I/O read and 
  ...
  IN      UINT64                     Data )
{
  ...
  Status = gCpuIo2Protocol->Io.Write (gCpuIo2Protocol, EfiCpuIoWidthUint8, Port, 1, &Data);
  if (EFI_ERROR (Status)) {
  
      DEBUG ((DEBUG_ERROR, "IoWriteByte status %r\n", Status));
  }
 ...
 * Refer to IbFreeTimeout function in Ps2Ctl module.
 * 
 * @param[in]   TimeoutValue   Specify the timeout value 
 * 
 * @retval      EFI_STATUS     
 * @retval      EFI_TIMEOUT
**/
...
EFI_STATUS
Ps2InputBufferTimout (
    IN UINT32 TimeoutValue )
{
    UINTN   Count; 

    for (Count = 0; Count < TimeoutValue; Count++) {
        if (!(IoReadByte(PS2_COMMAND_STATUS_PORT) & PS2_STATUS_INPUT_BUFFER)) {
            return EFI_SUCCESS;
        }
        gBS->Stall (ONE_MILLISECOND);   // 1 ms
    }
    return EFI_TIMEOUT;
}
 ...
 * Refer to IbFree function in Ps2Ctl module.
 * 
 * @param    None
 * 
 * @retval   VOID
...
Ps2InputBufferFree ()
{
    // If attempt to write data to IO port 0x60 or IO port 0x64,
    // must wait for the bit of Input buffer status be set to 0 
    // by ps2 controller.
    for (;;) {  
        if (!(IoReadByte(PS2_COMMAND_STATUS_PORT) & PS2_STATUS_INPUT_BUFFER)) {
            break;
 ...
 * Refer to ObFullRead function in Ps2Ctl module.
 * 
 * @param    VOID
 * 
...
Ps2OutputBufferRead ()
{
    // If attempt to read data from IO port 0x60, 
    // must wait for the bit of Output buffer status be set to 1 
    // by ps2 controller.
    for (;;) {
        if (IoReadByte(PS2_COMMAND_STATUS_PORT) & PS2_STATUS_OUTPUT_BUFFER) {
            return IoReadByte(PS2_DATA_PORT);
        }
    }
}
 ...
 * Refer to WriteKeyboardCommand function in Ps2Ctl module.
 * 
 * @param[in]   Command - Command to send to KBC
 * 
 * @retval      VOID
 * 
**/
...
VOID
Ps2WriteKeyboardCommand (
    IN  UINT8   Command )
{
    EFI_STATUS              Status;
    
    Status = Ps2InputBufferTimout (MAXIMUM_TIMEOUT_FOR_INPUT_BUFFER);
    if (EFI_ERROR(Status)) {
        return;
    }
    IoWriteByte (PS2_COMMAND_STATUS_PORT, Command);
    Ps2InputBufferFree ();
    return;
}
...
/**
 * Writes CCB to KBC
 * Refer to Write8042CommandByte function in Ps2Ctl module.
 * 
 * @param[in]   Command - Command byte to send to KBC 
 * 
 * @retval      VOID
 * 
**/
    ...
    IN  UINT8   Command )
{
    ...
    Ps2WriteKeyboardCommand (PS2_KEYBOARD_WRITE_MODE_COMMAND);     // CMD to send command byte
    IoWriteByte (PS2_DATA_PORT, Command);  // Write command byte into KBC
    Ps2InputBufferFree ();              // Wait until input buffer is free
    return;
}
 ...
 * output buffer.
 * Refer to ObFullReadTimeout function in Ps2Ctl module.
 * 
 * @param[in]   Data                   - Pointer to the byte to be updated
 * @param[in]   Timeout                - Milliseconds timeout 
 * @param[in]   OnlyOutputBufferFull   - Only waits for OBF if true 
 * 
 * @retval      FALSE     If data is successfully updated(no timeout)
 * @retval      TRUE      If time-out
**/

BOOLEAN
Ps2OutputBufferReadTimeout (
    IN UINT8*      Data, 
    IN UINT32      Timeout,
    IN BOOLEAN     OnlyOutputBufferFull )
{
    
    UINT8       StatusRegister;
    UINT32      LoopCount = Timeout << 1; 

    for (; LoopCount; LoopCount--) {

        StatusRegister = IoReadByte (PS2_COMMAND_STATUS_PORT);

        if (OnlyOutputBufferFull && (StatusRegister & PS2_STATUS_OUTPUT_BUFFER)) {
            *Data = IoReadByte (PS2_DATA_PORT);
            return FALSE;
        }

        if ((StatusRegister & (PS2_STATUS_OUTPUT_BUFFER | PS2_STATUS_TRANSMIT_TIME_OUT)) == PS2_STATUS_OUTPUT_BUFFER) {
            *Data = IoReadByte (PS2_DATA_PORT);
            if (StatusRegister & PS2_STATUS_RECEIVE_TIME_OUT) {
                return TRUE;
            }
            else 
        ...
        if ((StatusRegister & (PS2_STATUS_OUTPUT_BUFFER|PS2_STATUS_TRANSMIT_TIME_OUT)) == (PS2_STATUS_OUTPUT_BUFFER | PS2_STATUS_TRANSMIT_TIME_OUT)){
            return TRUE;
        }
        gBS->Stall (ONE_MILLISECOND / 2);        // 0.5msec
    }
    return TRUE;    // Timeout
}
...
/**
 *  Writes data to KBC.
 *  Refer to WriteKeyboardData function in Ps2Ctl module.
 * 
 * @param[in]   Command - Data to send to KBC 
 * @retval      VOID
 * 
**/

VOID
Ps2WriteKeyboardData (
    UINT8   Command )
{
    EFI_STATUS              Status;
    ...
    IoWriteByte (PS2_DATA_PORT, Command);
    Ps2InputBufferFree ();
    return;
}
 ...
 *  Refer to ReadDevice function in Ps2Ctl module.
 * 
 * @param[in]   Command    - Command to send to KBC
 * @param[in]   *Data      - Pointer to data buffer
 * @param[in]   Response   - Response expected
 * 
...
EFI_STATUS
Ps2ReadDeviceKeyboard (
    UINT8   Command,
    UINT8   *Data,
    UINT8   Response )
{

    Ps2WriteKeyboardData (Command);
    if (Ps2OutputBufferReadTimeout (Data, PS2_OUTPUT_BUFFER_READ_TIMEOUT, FALSE)) return EFI_DEVICE_ERROR;
...
/**
 * Disables KBD interface and reads the data from KBC data port.
 * Refer to DisableKeyboard function in Ps2Ctl module.
 * 
 * @param    None
 * 
 * @retval   VOID
...
Ps2DisableKeyboard ()
{
    ...
    IoWriteByte (PS2_COMMAND_STATUS_PORT, PS2_DISABLE_KEYBOARD_INTERFACE);
    Ps2InputBufferFree ();
 ...
 * @param[in]   Timeout - Milliseconds timeout
 * 
 * @retval      FALSE   If data is successfully updated 
 * @retval      TRUE    If time-out
**/

BOOLEAN
Ps2MouseOutputBufferTimout (
    IN  UINT8*  Data,
    IN  UINT32  Timeout )
{
    ...
    for (; LoopCount; LoopCount--) {
        ...
        gBS->Stall (ONE_MILLISECOND / 2);  // 0.5 msec
    }
 ...
 * Acknowledgment byte returned from KBD.
 * 
 * The original source code, please refer to  ReadDeviceM function in Ps2Ctl Mouse.c 
 * 
 * @param[in]   Command  - Command to send to KBC
 * @param[in]   *Data    - Pointer to data buffer
 * @param[in]   Response - Response expected
    ...
    IN  UINT8   Response )
{
    ...
    IoWriteByte (PS2_COMMAND_STATUS_PORT, PS2_WRITE_MOUSE_COMMAND);
    Ps2InputBufferFree ();
    
    Ps2WriteKeyboardData (Command);

    if (Ps2MouseOutputBufferTimout (Data, ONE_MILLISECOND)) { 
        return EFI_DEVICE_ERROR;
    }
    if (*Data == Response) { 
        return EFI_SUCCESS;
 ...
 * 
 * The original source code, please refer to  DisableAuxDevice function in Ps2Ctl Mouse.c 
 * 
 * @param     None
 * @retval    VOID
 * 
...
Ps2DisableAuxDevice ()
{
    ...
    IoWriteByte (PS2_COMMAND_STATUS_PORT, PS2_DISABLE_PS2_PORT);
    Ps2InputBufferFree ();
 ...
 * 
 * The original source code, please refer to EnableKeyboard function in Ps2Ctl Mouse.c 
 * 
 * @param     None
 * @retval    Status
 * 
...
Ps2EnableKeyboard ()
{
    ...
    IoWriteByte (PS2_COMMAND_STATUS_PORT, PS2_ENABLE_KEYBOARD_INTERFACE);
    Status = Ps2InputBufferTimout (MAXIMUM_TIMEOUT_FOR_INPUT_BUFFER);
...
/**
 * The following routine is used for detecting PS2 Mouse.
 * The original source code, please refer to  DetectPs2Mouse function in Ps2Ctl Mouse.c 
 * 
 * @param[in]        ConfigType      Current Type is PS2 KB or PS2 Mouse
 * @param[in,out]    Present         Current Type is present or not.
 * 
 * @retval   EFI_SUCCESS        If the device was reset
 * @retval   EFI_DEVICE_ERROR   Device is not functioning correctly and could not be reset.
**/
    ...
    IN OUT BOOLEAN *Present )
{
    ...
    UINT8       Data = 0;
    UINT8       loop;
    UINT8       Counter = PS2_MOUSE_CLEAR_OBF_COUNTDOWN;
    UINT8       BasicAssuranceTestResponse;
    UINT8       DeviceId;
    
    Status = gBS->LocateProtocol (&gEfiCpuIo2ProtocolGuid, NULL, (VOID **)&gCpuIo2Protocol);
    if (EFI_ERROR(Status))
        return Status;
    
    Ps2DisableKeyboard ();
       
    for (loop = MAX_RETRY_TIMES_OF_DETECTING_PS2_MOUSE; loop; loop--) {        
        // Clear OBF
        while (IoReadByte(PS2_COMMAND_STATUS_PORT) & PS2_STATUS_OUTPUT_BUFFER) {
            IoReadByte (PS2_DATA_PORT);
            // Wait 10 ms for KBC OBF output.
            gBS->Stall (10 * ONE_MILLISECOND);
            Counter--;
        ...
        // Resetting Mouse
        Status = Ps2ReadDeviceMouse (PS2_RESET_COMMAND, &Data, PS2_RESPONSE_ACK);
        // ACK received
        if (Status == EFI_SUCCESS) {    
            if (Ps2MouseOutputBufferTimout (&BasicAssuranceTestResponse, ONE_MILLISECOND)) { 
                continue;
            }
            if (Ps2MouseOutputBufferTimout (&DeviceId, ONE_MILLISECOND)) {
                continue;
            }
            if (BasicAssuranceTestResponse == PS2_RESPONSE_RESET_SUCCESS && DeviceId == PS2_MOUSE_ID) {
                // Enable wheel report format
                if (EFI_ERROR(Ps2ReadDeviceMouse (PS2_SET_SAMPLE_RATE, &Data, PS2_RESPONSE_ACK))) {
                    continue;       // Set Sample Rate
                }
                if (EFI_ERROR(Ps2ReadDeviceMouse (PS2_MOUSE_SAMPLE_RATE_200, &Data, PS2_RESPONSE_ACK))) {
                    continue;       // 200 samples/sec
                }
                if (EFI_ERROR(Ps2ReadDeviceMouse (PS2_SET_SAMPLE_RATE, &Data, PS2_RESPONSE_ACK))) {
                    continue;       // Set Sample Rate
                }
                if (EFI_ERROR(Ps2ReadDeviceMouse (PS2_MOUSE_SAMPLE_RATE_100, &Data, PS2_RESPONSE_ACK))) {
                    continue;       // 100 samples/sec
                }
                if (EFI_ERROR(Ps2ReadDeviceMouse (PS2_SET_SAMPLE_RATE, &Data, PS2_RESPONSE_ACK))) {
                    continue;       // Set Sample Rate
                }
                if (EFI_ERROR(Ps2ReadDeviceMouse (PS2_MOUSE_SAMPLE_RATE_80, &Data, PS2_RESPONSE_ACK))) {
                    continue;       // 80 samples/sec
                }
                    
                Ps2ReadDeviceMouse (PS2_GET_DEVICE_ID_COMMAND, &Data, PS2_RESPONSE_ACK); // Read Mouse ID
                Ps2MouseOutputBufferTimout (&Data, ONE_MILLISECOND);
                // If scroll wheel is supported Mouse Response = 3 
                // else Mouse Response = 0
                if (Data == PS2_SCROLLING_MOUSE) {
                    *Present = TRUE;
                }
                if (EFI_ERROR(Ps2ReadDeviceMouse (PS2_SET_SAMPLE_RATE, &Data, PS2_RESPONSE_ACK))) {
                    continue;       // Set Sample Rate
                }    
                if (EFI_ERROR(Ps2ReadDeviceMouse (PS2_MOUSE_SAMPLE_RATE_20, &Data, PS2_RESPONSE_ACK))) { 
                    continue;       // 20 samples/sec
                }
                if (EFI_ERROR(Ps2ReadDeviceMouse (PS2_SET_RESOLUTION_COMMAND, &Data, PS2_RESPONSE_ACK))) {
                    continue;       // Set resolution
                }
                if (EFI_ERROR(Ps2ReadDeviceMouse (PS2_MOUSE_4_COUNT_MM, &Data, PS2_RESPONSE_ACK))) { 
                    continue;        // 4 Count/mm
                }
                // Enable streaming
                Ps2ReadDeviceMouse (PS2_ENABLE_SCANNING_COMMAND, &Data, PS2_RESPONSE_ACK);
                Ps2DisableAuxDevice ();
...
/**
 * The following routine is used for detecting PS2 Keyboard.
 * 
 * @param[in]        ConfigType      Current Type is PS2 KB or PS2 Mouse
 * @param[in,out]    Present         Current Type is present or not.
 * 
  ...
  IN OUT BOOLEAN *Present )
{
    ...
    Ps2Write8042CommandByte (PS2_SCANCODE_TRANSLATION_BIT |
                             PS2_DISABLE_AUX_DEVICE_BIT | 
                             PS2_OVERRIDE_KEYBOARD_INHIBIT_BIT | 
                             PS2_SET_STATUS_REGISTER_SYSTEM_BIT |
                             PS2_ENABLE_OUTPUT_BUFFER_INTERRUPT_BIT);
    IoReadByte (PS2_DATA_PORT);     // Dummy read

    for (Count = 0; Count < PS2_KEYBOARD_COMMEND_RETRY_TIMEOUT; Count++) {
        // Disable Scanning
        if (!Ps2ReadDeviceKeyboard(PS2_DISABLE_SCANNING_COMMAND, &Data, PS2_RESPONSE_ACK)) break;
        if (IoReadByte (PS2_COMMAND_STATUS_PORT) & PS2_STATUS_RECEIVE_TIME_OUT) { // Time out error
            gBS->Stall (6 * ONE_MILLISECOND);  // 6 msec
    ...
    for (Count = 0; Count < PS2_KEYBOARD_COMMEND_RETRY_TIMEOUT; Count++) {
        if (!Ps2ReadDeviceKeyboard(PS2_RESET_COMMAND, &Data, PS2_RESPONSE_ACK)) {   // ACK received
            IsAck = TRUE;
            break;
        } else {        
            IoReadByte(PS2_DATA_PORT);     // Dummy read
    ...
    if (IsAck) {    //If not not Keyboard
        if (Ps2OutputBufferRead() == PS2_RESPONSE_RESET_SUCCESS) {    // Reset successful
            *Present = TRUE;
            return EFI_SUCCESS;
        } 
    }
    ...
    // Check for lock key
    if (!(IoReadByte (PS2_COMMAND_STATUS_PORT) & PS2_STATUS_INHIBIT_SWITCH)) {
