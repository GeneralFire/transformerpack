...
)
{
    ...
    *NvmeControllerReturnAddress = NvmeController;
    ...
    if ( NvmeController->MaxQueueEntrySupported >= ADMIN_QUEUE_SIZE ) {
        ...
        NvmeController->AdminSubmissionQueueSize =  NvmeController->AdminCompletionQueueSize = (UINT16)NvmeController->MaxQueueEntrySupported;
    }
...
)
{
    ...
    // ASSERT_EFI_ERROR(Status);
    DEBUG((DEBUG_VERBOSE, "[ASTDBG]ASSERT_EFI_ERROR(Status);\n"));
    DEBUG((DEBUG_VERBOSE, "[ASTDBG]NvmeSendSecurityFreezeLock : %r\n", Status));
