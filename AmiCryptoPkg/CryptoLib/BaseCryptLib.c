    ...
    IN  UINTN        SigLength)
{
    INTN        err = -1;
    size_t     *sig_len;
    UINT8      *DecriptedSig = NULL;

    if( RsaContext == NULL || 
       (MessageHash == NULL || HashLength < SHA1_DIGEST_SIZE || HashLength > SHA512_DIGEST_SIZE) ||
       (Signature == NULL || SigLength < DEFAULT_RSA_SIG_LEN) // <RSA2048
       )
        return FALSE;

    sig_len=(size_t*)&SigLength;
    DecriptedSig = os_malloc((unsigned int)(SigLength));
    if(DecriptedSig) {
