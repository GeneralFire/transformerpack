...
IsFvMeasured (
  UINT32 Attributes
    ...
    return ( (Attributes & AMI_ROM_AREA_MEASURED) == AMI_ROM_AREA_MEASURED
           ||
           // TBD. Remove when AREA_MEASURED attribute usage becomes mainstream
           (
...
InstallFvDxePublishPpi(
    ...
    DEBUG((DEBUG_ERROR, "Install PublishFvDxePpi %X: Area %X(%8X)\n", pPpiDesc->Ppi, &((AMI_PUBLISH_FV_DXE_PPI*)(pPpiDesc->Ppi))->Area, (UINTN)((AMI_PUBLISH_FV_DXE_PPI*)(pPpiDesc->Ppi))->Area.Address));
    
//    mPublishFvDxePpiDesc.Ppi = (VOID*)PublishFvDxePpi;
//    Status = (*PeiServices)->InstallPpi (PeiServices, &mPublishFvDxePpiDesc);
//    DEBUG((DEBUG_ERROR, "Install Ppi.PublishFvDxePpi %X: Area %X(%lX) : %r\n", mPublishFvDxePpiDesc.Ppi, ((AMI_PUBLISH_FV_DXE_PPI*)(mPublishFvDxePpiDesc.Ppi))->Area, ((AMI_PUBLISH_FV_DXE_PPI*)(mPublishFvDxePpiDesc.Ppi))->Area.Address, Status));
        ...
        if(!EFI_ERROR(Status)) {
            ...
            if(RecoveryHob != NULL) {
                if(RecoveryHob->Status == EFI_SUCCESS && RecoveryHob->Address != 0) {
                    DEBUG ((DEBUG_ERROR, "Recovery FV Addr %8X\n", RecoveryHob->Address + Area->Offset));
