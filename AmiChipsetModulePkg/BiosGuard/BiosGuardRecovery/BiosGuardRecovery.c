...
RecoveryImageSize = RecoveryHob->ImageSize;
    DEBUG ((EFI_D_INFO, "BiosGuardRecovery.c : ImageSize =%x\n", RecoveryHob->ImageSize));
    ...
    if (!EFI_ERROR(Status)) {
        ...
        if ( !EFI_ERROR(Status) ) {
            ...
            if (RecoveryImageSize == BIOSGUARD_MEUD_BIOS_SIZE + BIOSGUARD_ME_ALIGNMENT_SIZE) {
...
#if defined (FWCAPSULE_FILE_FORMAT) && (FWCAPSULE_FILE_FORMAT == 1)
    FWCapsuleHdr = (UINT8*)(RecoveryRomAddress + RecoveryImageSize - FWCAPSULE_IMAGE_ALLIGN);
#if defined (BIOS_GUARD_MEUD_SUPPORT) && (BIOS_GUARD_MEUD_SUPPORT == 1)        
    if (RecoveryImageSize != BIOSGUARD_RECOVERY_IMAGE_SIZE) {
      FWCapsuleHdr = (UINT8*)(RecoveryRomAddress + BIOSGUARD_MEUD_BIOS_SIZE + BIOSGUARD_ME_ALIGNMENT_SIZE - FWCAPSULE_IMAGE_ALLIGN);
    }
#endif    
    ...
    (*PeiServices)->CopyMem ((UINT8*)(RecoveryHob->Address + FWCAPSULE_IMAGE_ALLIGN), (UINT8*)Buffer + (CurrentBGUPAddress - FLASH_SIZE), FLASH_SIZE );
#else
