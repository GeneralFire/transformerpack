...
:BuildAmiBiosGuardHeader
    ...
    if %BEFORE_END_IMAGE_SIZE% NEQ %RECOVERY_IMAGE_SIZE% (
        %CUTROM% AMI_BIOS_GUARD_%OUTPUT_BIOS_IMAGE% BeforeEndRom.bin 0 %BEFORE_END_IMAGE_SIZE%
        %CUTROM% %ORIGINAL_ROM_PATH% TempCert.bin 0 %FWCAPSULE_IMAGE_ALIGN%
        copy /b BeforeEndRom.bin+TempCert.bin AMI_BIOS_GUARD_%OUTPUT_BIOS_IMAGE%
...
:BuildBiosGuardMeRom
    %WORKING_PATH%\BuildAmiBiosGuardHeader.exe %AMI_BIOS_GUARD_FLASH_CONFIGURATIONS% %AMI_BIOS_GUARD_HEADER%
    copy /b %AMI_BIOS_GUARD_HEADER%+%BIOS_GUARD_INTREGRATION_ROM% Temp.bin
    %WORKING_PATH%\RomAlignment.exe Temp.bin TEMP.CAP %MEUD_BIOS_SIZE%
    copy /b TEMP.CAP + MEFWSIGN.CAP TEMP.CAP
    SET /a BIOSGUARDME_SIZE=%MEUD_BIOS_SIZE%+%ME_ALIGNMENT_SIZE%
    %WORKING_PATH%\RomAlignment.exe TEMP.CAP BIOSGUARDME.CAP %BIOSGUARDME_SIZE%
    Set BIN_SELECTION=BIOSGUARDME.CAP
    ...
    Set /a BEFORE_END_IMAGE_SIZE=%BIOSGUARDME_SIZE%-%FWCAPSULE_IMAGE_ALIGN%
    %CUTROM% %BIN_SELECTION% DIVIDE_END_OF_4K.CAP 0 %BEFORE_END_IMAGE_SIZE%
    copy /b DIVIDE_END_OF_4K.CAP+%CERTIFICATE_HEADER_PATH% %BIN_SELECTION%
