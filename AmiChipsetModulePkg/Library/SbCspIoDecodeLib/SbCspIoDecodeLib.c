    ...
    )
{
    ...
    if(LpcPciIo != NULL) {
        if (Enable) {
            ...
            return PchLpcGenIoRangeSet (Base, Length);
        } else {
    ...
    if (LpcPciIo==NULL) D31F0Addr = (UINT32)(PCI_CF8_LIB_ADDRESS(DEFAULT_PCI_BUS_NUMBER_PCH,PCI_DEVICE_NUMBER_PCH_LPC, PCI_FUNCTION_NUMBER_PCH_LPC, 0));
        ...
        MaxIoGenDecIndex = 1;
        BaseIoGenDecReg = R_LPC_CFG_PCCS1GIR1;
        DmiBaseIoGenericDecReg = R_PCH_PCR_DMI_SEGIR;
    #else
        MaxIoGenDecIndex = 4;
        BaseIoGenDecReg = R_LPC_CFG_GEN1_DEC;
        DmiBaseIoGenericDecReg = R_PCH_DMI_PCR_LPCLGIR1;
    #endif
    ...
    )
{
    ...
    //    RW_PCI16_SB(R_LPC_CFG_IOD, IoRangeSet16, ~IoRangeMask16);
    if (LpcPciIo==NULL){
        tmp16=PciCf8Read16((UINTN)D31F0Addr+R_LPC_CFG_IOD);
	} else {
        Status=LpcPciIoProtocol->Pci.Read(LpcPciIoProtocol, EfiPciIoWidthUint16,R_LPC_CFG_IOD,1,&tmp16);
        if (EFI_ERROR(Status)) return Status;
    }
	...
	if (LpcPciIo==NULL){    
        PciCf8Write16((UINTN)D31F0Addr+R_LPC_CFG_IOD,tmp16);
        MmioWrite16 (PCH_PCR_ADDRESS(PID_DMI, R_PCH_DMI_PCR_LPCIOD), (UINT16)tmp16);
    } else {
	...
	#if defined(PILOT4_LPC_ESPI_SLAVE_CONNECTION) && PILOT4_LPC_ESPI_SLAVE_CONNECTION
      IoRoutingEnablesReg = R_LPC_CFG_PCCS1IORE;
	#else
      IoRoutingEnablesReg = R_LPC_CFG_IOE;
	#endif
	...
	if (LpcPciIo==NULL){    
        PciCf8Write16((UINTN)D31F0Addr+IoRoutingEnablesReg,tmp16);
            ...
            //
            PciCf8Or16((UINTN)D31F0Addr+R_LPC_CFG_IOE, tmp16);
        #endif

        MmioWrite16 (PCH_PCR_ADDRESS(PID_DMI, R_PCH_DMI_PCR_LPCIOE), (UINT16)tmp16);
    } else {
        #if defined(PILOT4_LPC_ESPI_SLAVE_CONNECTION) && PILOT4_LPC_ESPI_SLAVE_CONNECTION
        ...
        Status = PchLpcIoEnableDecodingSet (tmp16);
        if (EFI_ERROR(Status)) return Status;
    }
