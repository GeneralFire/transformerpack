/** @file
  Implementation of SystemInfoLib in SMM phase
...
{
  //
  // Posted accesses currently not enabled in SMM
  //
...
EFI_STATUS
EFIAPI
SmmSystemInfoLibConstructor (
  IN EFI_HANDLE              ImageHandle,
  IN EFI_SYSTEM_TABLE        *SystemTable
  )
{
  EFI_STATUS                 Status;
  SYSTEM_INFO_VAR_PROTOCOL   *SmmSystemInfoVarProtocol = NULL;

  if (mSystemInfoVar == NULL) {
    if (gSmst == NULL) {
      ASSERT (FALSE);
      return EFI_UNSUPPORTED;
    }
  ...
  mSystemInfoVar =  (SYSTEM_INFO_VAR *) SmmSystemInfoVarProtocol;
