  ...
  )
{
  ...
  if (((ThermalThrottlingOptions & OFFPKG_MEM_TO_THERMTRIP_EN) != 0) && (((ThermalThrottlingOptions & (OLTT_EN | CLTT_EN | CLTT_PECI_EN)) != 0))) {
    //
    // Select the non-AEP Hi Temp
    //
    ThrottleValue.TempThersholdHigh = PcdGet8 (PcdTempHiMemTripEnabled);
  } else {
    //
    // Select the default Hi Temp
    //
    ThrottleValue.TempThersholdHigh = PcdGet8 (PcdTempHiMemTripDisabled);
  }
  if ((*DimmNvList)[Dimm].keyByte == SPD_TYPE_AEP) {
    //
    // AEP DIMM has a different Hi Temp and its actually lower than PcdGet8 (PcdTempHiMemTripEnabled);.
    //
    ThrottleValue.TempThersholdHigh = TEMP_HI_AEP;
  }
  ...
  if (ThermalThrottlingOptions & (CLTT_EN | CLTT_PECI_EN)) {
    ...
    ThrottleValue.TempThrottleMid = CLTT_THRT_MID_DEFAULT;

    if (DimmInThermalThrottlingModeTable) {
      ThrottleValue.TempThrottleHigh = ThersholdHigh; //HSD 4987559
    } else {
    ...
    if ((*DimmNvList)[Dimm].keyByte == SPD_TYPE_AEP) {
      ThrottleValue.TempThersholdLow = PcdGet8 (PcdClttTempLoSingleRefreshDefault);
      ThrottleValue.TempThersholdMid = PcdGet8 (PcdClttTempMidSignleRefreshAepDefault);
    }
  } else if ((ThermalThrottlingOptions & OLTT_EN) != 0) {
    ThrottleValue.TempThrottleMid = OLTT_THRT_MID_DEFAULT;
    ...
    ThrottleValue.TempThersholdLow = OLTT_TEMP_LO_DEFAULT;
    ThrottleValue.TempThersholdMid = OLTT_TEMP_MID_DEFAULT;
  }
  ...
  )
{
  ...
  for (ch = 0; ch < MaxChDdr; ch++) {
    ...
    if ((*channelNvList)[ch].enabled != 0) {
      ...
      if ((*channelNvList)[ch].ddrtEnabled != 0) { //if DDRT present on Ch set bit
        bwlimittfdata = BW_LIMIT_TF_DDRT_DEFAULT;
      }
      ...
      if ((varThermalThrottlingOptions & (OLTT_EN | CLTT_EN | CLTT_PECI_EN)) != 0)  {
        if (CheckDimmInThrottleModeTable.TableEntryPresent)  {
          ThrottleValue.ThrottleExternal = ptrDimmTTMODE->TTTHRT_HI; //HSD 4987488
        } else {
          if ((varThermalThrottlingOptions & OLTT_EN) != 0) {
        ...
        if ((*channelNvList)[ch].ddrtEnabled != 0) { //if DDRT present on Ch set bit
          ThrottleValue.ThrottleExternal = CLTT_THRT_HI_DDRT_DEFAULT;
        }
      ...
      for (dimm = 0; dimm < MAX_DIMM; dimm++) {
        ...
        if ((*dimmNvList)[dimm].keyByte == SPD_TYPE_DDR4) {
          if (WTcurrentDimm.WTDimmType == SPD_LRDIMM_DDR4) {
        ...
        if (CheckDimmInWeightTable.CfgValid) {
          ...
          if (((*dimmNvList)[dimm].keyByte == SPD_TYPE_DDR5)) {
            ...
            RcDebugPrint (SDBG_MAX,
              "\n::MEM DDR5:: DDR5 Memory is detected in system. DDR5 Power table not supported...\n");
          ...
          //
            ...
            for (WTRow = 0; WTRow < DimmWtTableSize; WTRow++) {
              ...
              if ((ptrDimmWT->WTDimmType == WTcurrentDimm.WTDimmType) &&
                ...
                RcDebugPrint (SDBG_MAX,
                  "\n::MEM DDR4:: DDR4 Table is available to use. CheckDimmInWeightTable.TableEntryPresent = %x\n\n", CheckDimmInWeightTable.TableEntryPresent ? 1 : 0);
                RcDebugPrint (SDBG_MAX,
                  "::MEM DDR4:: (%d)        %x, %x, %x, %x, %x, %x, %x, ",
                  WTRow,
        ...
        if (TTcurrentDimm.TTDimmType == SPD_LRDIMM_DDR4) {
        ...
        if ((((TTcurrentDimm.TTRankNumIoWidth & 0x7) > SPD_DRAM_IO_WIDTH_X8) ||
          ((TTcurrentDimm.TTRankNumIoWidth & 0x38) > (SPD_NUM_RANKS_8 << 3))) && (TTcurrentDimm.TTDimmType == SPD_LRDIMM_DDR4)) {
        ...
        if (TTcurrentDimm.TTFrequency > DDR_2933) {
          TTcurrentDimm.TTFrequency = DDR_2933;
        }
        ...
        if ((varThermalThrottlingOptions & CLTT_EN) != 0) {
          ...
          if (CheckDimmInThrottleTable.TableEntryPresent) {
            ...
            ThrottleValue.ThrottlingDimmTempOffsetValue = TT_MAX_WORST_CASE_DIMM_TEMP_OFFSET;
          }
