...
{
  UINT32 Data;
  Data = UsraCsrRead (SocId, CtrId, R2PGNCTRL_SB_M2PCIE_REG);
  if (Data == 0) {
    DEBUG ((DEBUG_ERROR, "[WARNING]: Read of side band register R2PGNCTRL has returned 0\n"));
  }
  return Data;
}
...
{
  UsraCsrWrite (SocId, CtrId, R2PGNCTRL_SB_M2PCIE_REG, R2GenCtrl);
}