...
// Legacy Decode
//
#define CHA_MAX_SNOOP_VALUE  31
#define CHA_SNOOP_SHIFT      0x3
#define CHA_SNOOP_MASK       0xFF07

STATIC UINT8 ChaThrottleThreshold [CHA_SNOOP_THRESHOLD_MAX] = {
                                                                31,
                                                                25,
                                                                23,
                                                                21
  ...
  )
{
  ...
  if ((IsCpuAndRevisionOrLater (CPU_SNR, REV_B0)) ||
      (IsCpuAndRevision (CPU_TNR, REV_ALL)) ||
      (IsCpuAndRevisionOrLater (CPU_ICXSP, REV_L0)) ||
      (IsCpuAndRevisionOrLater (CPU_ICXD, REV_B0))
     ) {
    TOR_THRESHOLDS_N0_CHABC_SAD1_STRUCT           TorThresholds;

    TorThresholds.Data = UsraCsrRead(SocId, BoxInst, TOR_THRESHOLDS_N0_CHABC_SAD1_REG);
    TorThresholds.Bits.loc2rem_thresh_norm = Loc2rem_thresh_norm;
    TorThresholds.Bits.loc2rem_thresh_empty = Loc2rem_thresh_empty;
    TorThresholds.Bits.rrq_count_threshold = Tor_threshold;
    UsraCsrWrite(SocId, BoxInst, TOR_THRESHOLDS_N0_CHABC_SAD1_REG, TorThresholds.Data);
  } else {
#endif // B0_HOST
  ...
  )
{
  ...
  if (DdrtQosMode == DDRT_QOS_MODE_2) {
    ...
    LowBwMemLocTorThresholds.Bits.loc_wr_hi_th = 0x08;
    LowBwMemLocTorThresholds.Bits.loc_wr_maxbw_th = 0x06;
...
{
  KTI_CONFIG_CHABC_SAD1_STRUCT  KtiConfigChabc;

  KtiConfigChabc.Data = (UINT16)UsraCsrRead (SocId, 0, KTI_CONFIG_CHABC_SAD1_REG);

  if (!IsSiliconWorkaroundEnabled ("S1706838675")) {

    if ((Enable && (KtiConfigChabc.Bits.use_bcst_route_table == 1)) ||
     ((!Enable) && (KtiConfigChabc.Bits.use_bcst_route_table == 0))) {
     return EFI_SUCCESS;
    }
  }

  if (Enable) {
    KtiConfigChabc.Bits.use_bcst_route_table = 1;
  } else {
    KtiConfigChabc.Bits.use_bcst_route_table = 0;
  }
  UsraCsrWrite (SocId, 0, KTI_CONFIG_CHABC_SAD1_REG, KtiConfigChabc.Data);
...
ChaConfigureSnc (
  UINT8    SocId,
  UINT8    BoxInst,
  UINT32   FullSncEnable,
  UINT32   SncIndEnable,
  UINT32   NumSncClusters,
  UINT32   SncMidPointId
  )
{
...
/**
  CHA: Configure CHA to M2M credit

  @param SocId                    - CPU Socket Node number (Socket ID)
  @param BoxInst                  - Box Instance, 0 based
  @param HaAdCredit               - AD Credit to be programmed
  @param HaBlCredit               - BL Credit to be programmed
  @param HaAdCreditMask           - AD Credit Mask
  @param HaBlCreditMask           - BL Credit Mask
...
ChaConfigureChaToM2MCredit (
  ...
  UINT32   AdCreditMask,
  UINT32   BlCreditMask,
  BOOLEAN  DisableResCreditForIsoch
  )
{
  return EFI_UNSUPPORTED;
}
...
{
  HA_COH_CFG_2_CHABC_SAD1_STRUCT HaCoh;
  UINT16 ChaSnoopValue;

  if (IsCpuAndRevisionOrLater (CPU_ICXSP, REV_L0)) {
    if (SnoopThrottleLevel > CHA_SNOOP_THRESHOLD_HIGH) {
      return EFI_INVALID_PARAMETER;
    }
    //
    // CHABC Registers are only valid on CHA 0
    //
    HaCoh.Data = UsraCsrRead (SocId, 0, HA_COH_CFG_2_CHABC_SAD1_REG);

    ChaSnoopValue = (CHA_MAX_SNOOP_VALUE - ChaThrottleThreshold[SnoopThrottleLevel]) << CHA_SNOOP_SHIFT;
    ChaSnoopValue = (ChaSnoopValue & ~CHA_SNOOP_MASK) | (HaCoh.Bits.sparehacsrunnnh & CHA_SNOOP_MASK);
    HaCoh.Bits.sparehacsrunnnh = ChaSnoopValue;

    UsraCsrWrite (SocId, 0, HA_COH_CFG_2_CHABC_SAD1_REG, HaCoh.Data);
  }
  return EFI_SUCCESS;
}
  ...
  )
{
  HA_THRESHOLDS_CHABC_SAD1_STRUCT   HaThresholds;
  HaThresholds.Data = UsraCsrRead (SocId, BoxInst, HA_THRESHOLDS_CHABC_SAD1_REG);
  HaThresholds.Bits.snoop_credits = 1;
  UsraCsrWrite (SocId, BoxInst, HA_THRESHOLDS_CHABC_SAD1_REG, HaThresholds.Data);
  return EFI_SUCCESS;
}
