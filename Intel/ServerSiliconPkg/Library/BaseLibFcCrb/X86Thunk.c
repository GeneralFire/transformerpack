  ...
  )
{
  ...
  if ((ExtraStackSize == NULL) || (RealModeBufferSize == NULL)) {
    return;
  }
  ...
  )
{
  ...
  if (((UINTN)ThunkContext->RealModeBuffer >= 0x100000) ||
      (ThunkContext->RealModeBufferSize < m16Size) ||
  ...
  )
{
  ...
  if (((ThunkContext->ThunkAttributes & (THUNK_ATTRIBUTE_DISABLE_A20_MASK_INT_15 | THUNK_ATTRIBUTE_DISABLE_A20_MASK_KBD_CTRL)) == \
           (THUNK_ATTRIBUTE_DISABLE_A20_MASK_INT_15 | THUNK_ATTRIBUTE_DISABLE_A20_MASK_KBD_CTRL))) {
    return;
  }
