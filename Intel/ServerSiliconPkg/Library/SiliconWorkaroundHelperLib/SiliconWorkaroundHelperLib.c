/** @file
  ...
  This library provides common function(s) that are used in all implementation of
  ...
  )
{
  ...
  while (WorkaroundTable[Index] != NULL) {
    if (AsciiStrCmp (WorkaroundTable[Index], WorkaroundName) == 0) {
      return TRUE;
    }
    Index++;
  }
