...
{
  ZeroMem (pTestCfg, sizeof(*pTestCfg));
  ...
  IN CONST MEM_SHMOO_CONFIG *pTestCfg)
{
  ...
  NORMAL_LOG(SsaServicesHandle, SSA_D_ERROR,
    "  IoLevel=%u (%a)\n", pTestCfg->IoLevel, GetLevelStr (pTestCfg->IoLevel));
  if (EFI_ERROR (GetGroupString (pTestCfg->MarginGroup, &GroupString))) {
    GroupString = "Unknown group";
  }
  NORMAL_LOG(SsaServicesHandle, SSA_D_ERROR,
    "  MarginGroup=%u (%a)\n", pTestCfg->MarginGroup, GroupString);
  NORMAL_LOG(SsaServicesHandle, SSA_D_ERROR,
  ...
  OUT TEST_STATUS *pTestStat)
{
  ...
  ZeroMem (pMemShmooData->MinOffsetByMemoryTech, sizeof (pMemShmooData->MinOffsetByMemoryTech));
  ZeroMem (pMemShmooData->MaxOffsetByMemoryTech, sizeof (pMemShmooData->MaxOffsetByMemoryTech));
#else
  #if SUPPORT_FOR_EXTENDED_RANGE
    ...
    if (pMemShmooCfg->EnableExtendedRange) {
      ...
      // possible values
      SetMem (pMemShmooData->MinOffsets, sizeof(pMemShmooData->MinOffsets), 0xFF);
      SetMem (pMemShmooData->MaxOffsets, sizeof(pMemShmooData->MaxOffsets), 0xFF);
    }
  ...
  #endif // SUPPORT_FOR_EXTENDED_RANGE
    {
      ...
      pMemShmooData->MinOffsetByMemoryTech[0] = -30000;
      pMemShmooData->MinOffsetByMemoryTech[1] = -30000;

      pMemShmooData->MaxOffsetByMemoryTech[0] = 30000;
      pMemShmooData->MaxOffsetByMemoryTech[1] = 30000;
    }
#endif // !SUPPORT_FOR_SERIALIZATION

  pMemShmooData->SettleDelay = 0; // initialize to smallest positive value
  ZeroMem (pMemShmooData->SettleDelayByMemoryTech, sizeof(pMemShmooData->SettleDelayByMemoryTech));
  ...
  if (*pCurrentOffset != NewOffset) {
    ...
    if (*pCurrentOffset != NewOffset) {
      if (pMemShmooData->TestStatusLogLevel >= 4) {
        if (EFI_ERROR (GetGroupString (pMemShmooCfg->MarginGroup, &GroupString))) {
          GroupString = "Unknown group";
        }
        NORMAL_LOG(SsaServicesHandle, SSA_D_ERROR,
          "\nMoving IoLevel=%u (%a) MarginGroup=%u (%a) from %d to %d for socket=%d, controller=%d and channel=%d\n",
          pMemShmooCfg->IoLevel, GroupString,
          pMemShmooCfg->MarginGroup, GroupString,
  ...
  OUT TEST_STATUS *pTestStat)
{
  ...
  if (IsCmdMarginGroup (pMemShmooCfg->MarginGroup) ||
    IsCtlMarginGroup (pMemShmooCfg->MarginGroup)) {
    // FOR each socket:
    for (Socket = 0; Socket < MAX_SOCKET_CNT; Socket++) {
      // IF the socket is not enabled for testing THEN skip it
      if (!IsSocketTestingEnabled(SsaServicesHandle, pSystemInfo,
  ...
  OUT TEST_STATUS *pTestStat)
{
  ...
  *pTestingOffset =  BSInnerLimitOffset;
  ...
  ZeroMem (pMemShmooData->pOffsetTestStat, pMemShmooData->OffsetTestStatBfrSize);
  ...
  OUT TEST_STATUS *pTestStat)
{
  ...
  *ppVoidMemShmooData = SsaServicesHandle->SsaCommonConfig->Malloc(
  ...
  ZeroMem (pMemShmooData, sizeof (*pMemShmooData));
  ...
  NORMAL_LOG(SsaServicesHandle, SSA_D_ERROR, "    Specificity=0x%X (",
    pMemShmooData->Specificity);
  LogSpecificity (pMemShmooData->Specificity);
  NORMAL_LOG(SsaServicesHandle, SSA_D_ERROR, ")\n");
#endif
  ...
  if (pMemShmooCfg->IsDdrT) {
    ...
    for (Socket = 0; Socket < MAX_SOCKET_CNT; Socket++) {
      ...
      if (HasDdrtDevicesValEnabled (SsaServicesHandle, pSystemInfo, pMemCfg, pMemPointTestCfg)) {
        TrainingMode = FnvTrainModeDisableTrainingStep; // General training mode setup:tm_en=1 and disable_rid_feedback = 1
  ...
  OUT TEST_STATUS *pTestStat)
{
  ...
  if (pMemShmooCfg->IsDdrT) {
    ...
    for (Socket = 0; Socket < MAX_SOCKET_CNT; Socket++) {
      ...
      // Place the FNV training mode back to general training setting
      if (IsCmdMarginGroup (pMemShmooCfg->MarginGroup)||
        (pMemShmooCfg->MarginGroup == EridDelay) ||
        (pMemShmooCfg->MarginGroup == EridVref)) {
#if ENBL_BIOS_SSA_API_DEBUG_MSGS || ENBL_DDRT_DEBUG_LOG_MSGS
        NORMAL_LOG(SsaServicesHandle, SSA_D_ERROR,
          "  SetAepDimmTrainingMode(Socket=%u Mode=%u)\n",
          Socket, FnvTrainModeDisableTrainingStep);
#endif
        SsaServicesHandle->SsaMemoryDdrtConfig->SetAepDimmTrainingMode(
          SsaServicesHandle, Socket, FnvTrainModeDisableTrainingStep);
      }
