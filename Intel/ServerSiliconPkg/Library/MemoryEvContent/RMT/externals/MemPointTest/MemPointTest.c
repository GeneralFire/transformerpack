  ...
  OUT DIMM_RANK_PAIR *pDimmRankB)
{
  ...
  ZeroMem (pDimmRankA, sizeof(*pDimmRankA));
  ZeroMem (pDimmRankB, sizeof(*pDimmRankB));
  ...
  OUT DIMM_RANK_PAIR *pDimmRankB)
{
  ...
  ZeroMem (&Temp_DimmRankA, sizeof(Temp_DimmRankA));
  ZeroMem (&Temp_DimmRankB, sizeof(Temp_DimmRankB));
  ...
  NORMAL_LOG(SsaServicesHandle, SSA_D_ERROR,
    ...
    "IsGetDimmRankPair=%u ",
    Socket, Controller, Channel, IsGetDimmRankPair);
  } else {
    NORMAL_LOG(SsaServicesHandle, SSA_D_ERROR,
  ...
  for (i = 0; i < MAX_DIMM_CNT * MAX_RANK_CNT; i++){
    if (NormalizedDimmRankBitmask_A & (0x1 << i)){
      ...
      if (pMemPointTestCfg->TurnaroundsTestMode == OnlyIntraDimmTurnarounds) {
        ...
        NormalizedDimmRankBitmask_B_filtered = NormalizedDimmRankBitmask_B & (((0x1 << MAX_RANK_CNT) - 1) << ((i / MAX_RANK_CNT) * MAX_RANK_CNT));
      ...
      for (j = 0; j < MAX_DIMM_CNT * MAX_RANK_CNT; j++){
        ...
        if (pMemCfg->MemoryTech[Socket][Controller][Channel][i / MAX_RANK_CNT] != pMemCfg->MemoryTech[Socket][Controller][Channel][j / MAX_RANK_CNT]) {
          continue;
        }

      // If the TurnaroundsPairMode is OrderIndependent, we just need to test A->B. Otherwise both A->B and B->A
      // if just A->B, we need to start the "bit" for rank B after the bit for rank A 
      if (pMemPointTestCfg->TurnaroundsPairMode == OrderIndependent){
        j_start = i + 1;
      } else { 
        // TurnaroundsPairMode == OrderDependent
        j_start = 0;
      }

      for (j = j_start; j < MAX_DIMM_CNT * MAX_RANK_CNT; j++){
        ...
        } // if B is available (NormalizedDimmRankBitmask_B_filtered & (0x1 << j))
      } // j
    } // if A is available (NormalizedDimmRankBitmask_A & (0x1 << i))
  }// i
  ...
  IN UINT16 *pBacksideCmdBitmasks)
{
  ...
  //
  // Unlike DDR4 which only the Gen2 RCD supports backside CMD/CTL tests,
  // all DDR5 RDIMM/LRDIMM support backside CMD/CTL tests.
  //
#ifndef DDR5_SUPPORT
  UINT8 Socket;
...
} // end function RemoveNonDdrtDimms
  ...
  OUT TEST_STATUS *pTestStat)
{
  ...
  if (pRankCombinationCnts) {
    ZeroMem (pRankCombinationCnts, sizeof (*pRankCombinationCnts));
  }
