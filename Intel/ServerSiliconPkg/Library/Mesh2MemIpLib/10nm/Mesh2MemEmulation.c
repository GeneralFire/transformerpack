...
//
#define CHANNEL0_CREDIT_CONFIG_DONE_BIT_MASK BIT3
//
// Channel 1 credit config done bit mask for cmi misc config register
//
#define CHANNEL1_CREDIT_CONFIG_DONE_BIT_MASK BIT4
//
// Channel 2 credit config done bit mask for cmi misc config register
//
#define CHANNEL2_CREDIT_CONFIG_DONE_BIT_MASK BIT5
...
const UINT32 PollBitLocation[] = { CHANNEL0_CREDIT_CONFIG_DONE_BIT_MASK,
                                   ...
                                   CHANNEL2_CREDIT_CONFIG_DONE_BIT_MASK };
...
STATIC
VOID
PollRegisterOffsetForEmulation1 (
  IN PSYSHOST Host,
  IN UINT8    Socket,
  IN UINT8    BoxInst,
  IN UINT32   Offset,
  ...
  //Set Initialized bit and wait for done
    ...
    for (ChId = 0; ChId < MAX_MC_CH; ChId++) {
      ...
      if ((*ChannelNvList)[Ch].enabled == 1) {
        PollRegisterOffsetForEmulation1 (Host, Socket, McId, Offset, PollBitLocation[ChId], 1);
      }
    }
  }
}
