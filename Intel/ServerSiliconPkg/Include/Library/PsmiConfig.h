...
@copyright
  INTEL CONFIDENTIAL
  Copyright 2018 - 2018 Intel Corporation.
...
typedef union {
  ...
  struct {
    ...
    UINT32 TraceRegion4CacheType:2; /// 31:30 - Trace Region 4 Cache Type
  } Bits;
...
typedef struct{
  UINT64  Size;
  UINT8   AllocationError;
  UINT64  BaseAddress;
} TRACE_REGION_INFO;

typedef struct{
  UINT8   CurrentSocketNum; //socket number
  UINT8   PsmiHandlerSize; //PSMI handler size:
  TRACE_REGION_INFO Trace[MAX_TRACE_CACHE_TYPE]; //trace buffer information per cache type: Index: Any (0) UC(1) WC (2)
} SOCKET_PSMI_INFO;

typedef struct{
  UINT8 Version;
  UINT8 TotalSocketNum; //total number of sockets that has PSMI enabled
  SOCKET_PSMI_INFO SocketBuffer[MAX_SOCKET];
} PSMI_TRACE_INFO_HOB;
