/** @file
  Data structures used by SPR SGX init flows
...
// SGX DEBUG print macro along with parameters corresponding to EFI_ERROR
...
#define EPC_SHIFT_LOW  12
#define EPC_SHIFT_HIGH 32

// EPOCH
#define SGX_EPOCH_NO_CHANGE        0
#define SGX_EPOCH_CHANGE_TO_RANDOM 1
#define SGX_EPOCH_USER_MANUAL      2
#define SGX_EPOCH_SIZE             32

// MCHECK
#define MCHECK_MAX_P2P_LINKS     7
#define MCHECK_MAX_PACKAGES      8
#define MCHECK_MAX_KEYS          32
#define MCHECK_DEFAULT_ALIGNMENT 64

// PRMRR
#define SGX_MAX_PRMRR_COUNT 8
...
#define B_MKTME_INTEGRITY_ENABLE BIT49
...
// to be moved (to SgxSetupDefinitions.h)...
...
//
// Internal NVRAM interfaces used in registration flows
//
...
#define SGX_UEFIFW_KEY_BLOB_INTEGRITY_VARIABLE_NAME              L"SgxKeyBlobsIntegrity"
...
extern EFI_GUID gSgxUefiFwKeyBlobsWithIntegrityVariableGuid;
extern EFI_GUID gSgxUefiFwPlatformManifestVariableGuid;

//
// External SGX NVRAM interfaces consumed by Multi Package Registration Agent (OS Agent)
//

// Registration Protocol Variables names
...
#define SGX_REGISTRATION_PACKAGE_INFO_INTEGRITY_VARIABLE_NAME  L"SgxRegistrationPackageIntegrityInfo"
...
// Backup Registration Protocol Variables names in flash
...
extern EFI_GUID gSgxRegistrationPackageInfoWithIntegrityVariableGuid;
...
//
// Following definitions are based on MCHECK 2.5 Core SAS Master 2.2
//
...
typedef union
{
  UINT8               Uuid[16];
  UINT64              Uuid64[2];
} MCHECK_UUID; // Total: 16 bytes
#define MCHECK_UUID_SIZE (sizeof (MCHECK_UUID))

typedef struct {
  MCHECK_UUID         Uuid;         // MMSB (big-endian)
  UINT16              Size;
  UINT16              Version;
  UINT8               Reserved[12];
} MCHECK_HEADER; // Total: 32 bytes
#define MCHECK_HEADER_SIZE            (sizeof (MCHECK_HEADER))
#define MCHECK_HEADER_DEFAULT_VERSION 1 // Default MCHECK header version

typedef struct {
  VOID   *Address;
  UINT64 Size;
} MCHECK_BUFFER; // Total: 16 bytes
#define MCHECK_BUFFER_SIZE (sizeof (MCHECK_BUFFER))

typedef struct {
  UINT8         NumSockets;                        // Number of currently populated sockets
  UINT8         Master;                            // Node ID of master socket
  UINT8         Rfu[6];                            // RFU, must be 0
  UINT64        PlatformConfig[2];                 // RFU, must be 0
  UINT8         NodeIds[MCHECK_MAX_PACKAGES];      // Node IDs, valid = 0x00, invalid = 0xFF
  UINT64        NodeTopology[MCHECK_MAX_PACKAGES]; // Node-specific topology information
  MCHECK_BUFFER WorkingArea;                       // WorkingArea for MCHECK, 4096 * NUM_SOCKETS * NUM_SOCKETS
} MCHECK_TOPOLOGY_INFO; // Total: 112 bytes
#define MCHECK_TOPOLOGY_INFO_SIZE (sizeof (MCHECK_TOPOLOGY_INFO))

// NodeTopology is an array of per-node 32-bit fields
// providing additional information about configuration of each node
#define NODE_TOPOLOGY_MEM_TYPE_1LM             0x00
#define NODE_TOPOLOGY_MEM_TYPE_1LM_2LM_IN_TILE 0x01
#define NODE_TOPOLOGY_MEM_TYPE_1LM_2LM_X_TILE  0x02
#define NODE_TOPOLOGY_MEM_TYPE_2LM_IN_TILE     0x03
#define NODE_TOPOLOGY_MEM_TYPE_2LM_X_TILE      0x04

typedef struct {
  MCHECK_HEADER Header;
  UINT64        Features;     // BIT0: SGX, BIT1: SEAM, BIT2: C6DRAM, BIT3: Key Locker, BIT4: FuSa SAF, other rsrvd
  MCHECK_BUFFER SgxInfo;      // Pointer to MCHECK_SGX_INFO, NULL if SGX not required or Client Attestation used
  MCHECK_BUFFER SeamInfo;     // Pointer to CMR array, NULL if SEAM not required
  MCHECK_BUFFER TopologyInfo; // Pointer to SoC-specific MCHECK_TOPOLOGY_INFO structure
} MCHECK_BIOS_PARAM_INFO; // Total: 88 bytes
#define MCHECK_BIOS_PARAM_INFO_SIZE (sizeof (MCHECK_BIOS_PARAM_INFO))
#define MCHECK_BIOS_PARAM_INFO_UUID \
{ 0x13, 0x43, 0xF1, 0x5C, 0xAA, 0x00, 0x42, 0x07, 0xA2, 0x14, 0xA8, 0x4F, 0x35, 0x58, 0x22, 0xA2 }
#define MCHECK_BIOS_PARAM_INFO_HEADER_VERSION 3

// Following bits define the bitmap
// of features supported by MCHECK
#define FEATURES_SGX       BIT0
#define FEATURES_SEAM      BIT1
#define FEATURES_C6DRAM    BIT2
#define FEATURES_KEYLOCKER BIT3
#define FEATURES_FUSASAF   BIT4
...
#define MCHECK_CMR_SIZE (sizeof (MCHECK_CMR))

//
// Following definitions are based on
// MCHECK 2.5 Platform Attestation SAS Master 2.2
//

typedef struct {
  UINT8  Modulus[SHA3072_DIGEST_SIZE];
  UINT32 PubExp;
} MCHECK_SGX_PUBKEY; // Total: 388 bytes
#define MCHECK_SGX_PUBKEY_SIZE (sizeof (MCHECK_SGX_PUBKEY))

typedef struct {
  MCHECK_HEADER       Header;
  UINT8               CpuSvn;
  MCHECK_UUID         Prid;
  MCHECK_UUID         PlatformInstanceId;
  UINT8               PlatformEpoch[SGX_EPOCH_SIZE];
  MCHECK_SGX_PUBKEY   Prsk;
  MCHECK_SGX_PUBKEY   Rsak;
  UINT8               Signature[SHA3072_DIGEST_SIZE];
} MCHECK_SGX_PLATFORM_MEMBERSHIP_CERTIFICATE; // Total: 1257 bytes
#define MCHECK_SGX_PLATFORM_MEMBERSHIP_CERTIFICATE_SIZE (sizeof (MCHECK_SGX_PLATFORM_MEMBERSHIP_CERTIFICATE))
#define MCHECK_SGX_PLATFORM_MEMBERSHIP_CERTIFICATE_UUID \
{ 0xEB, 0xBD, 0x00, 0xC0, 0xEF, 0x90, 0x49, 0x10, 0xA4, 0x80, 0xF7, 0x72, 0xE7, 0x35, 0x5D, 0xB4 }

typedef struct {
  MCHECK_HEADER       Header;
  UINT8               RsName[SHA256_DIGEST_SIZE];
  MCHECK_SGX_PUBKEY   Rsak;
  MCHECK_SGX_PUBKEY   Rsek;
  UINT8               Signature[SHA3072_DIGEST_SIZE];
} MCHECK_SGX_REGISTRATION_SERVER_ID; // Total: 1224
#define MCHECK_SGX_REGISTRATION_SERVER_ID_SIZE (sizeof (MCHECK_SGX_REGISTRATION_SERVER_ID))
#define MCHECK_SGX_REGISTRATION_SERVER_UUID \
{ 0x31, 0xA1, 0x2A, 0xFE, 0x07, 0x20, 0x4E, 0xBC, 0xB6, 0x4E, 0xC4, 0xB3, 0xC7, 0xF8, 0xBC, 0x0F }

typedef struct {
  UINT16 McheckProperties; // Properties implicitly set by MCHECK, currently reserved
  UINT16 BiosProperties;   // Properties explicitly set by BIOS
} MCHECK_SGX_SECURITY_PROPERTIES; // Total: 4 bytes
#define MCHECK_SGX_SECURITY_PROPERTIES_SIZE (sizeof (MCHECK_SGX_SECURITY_PROPERTIES))

#define SECURITY_PROPERTIES_SGX_TEM      BIT0
#define SECURITY_PROPERTIES_MEM_INGERITY BIT1
...
typedef struct {
  ...
  MCHECK_SGX_PUBKEY              Rsak;
  UINT8                          PlatformEpoch[SGX_EPOCH_SIZE];
  MCHECK_SGX_SECURITY_PROPERTIES SecurityProperties;
  UINT32                         PackageCount;
} MCHECK_SGX_PLATFORM_INFO; // Total: 476 bytes
#define MCHECK_SGX_PLATFORM_INFO_SIZE (sizeof (MCHECK_SGX_PLATFORM_INFO))
#define MCHECK_SGX_PLATFORM_INFO_UUID \
{ 0xC6, 0x7A, 0x94, 0x84, 0x40, 0x84, 0x89, 0x41, 0x90 ,0x2A, 0x7E, 0x76, 0xCD, 0x65, 0x89, 0x26 }
typedef struct {
  MCHECK_HEADER Header;
  UINT32        KeySlotCount;
  UINT32        KeyCount;
  ...
  UINT8         PprResetKey[MCHECK_MAX_KEYS][AES128_KEY_SIZE];
  UINT8         Signature[SHA3072_DIGEST_SIZE];
} MCHECK_SGX_ENCRYPTED_PLATFORM_KEYS; // Total: 1369 bytes
#define MCHECK_SGX_ENCRYPTED_PLATFORM_KEYS_SIZE (sizeof (MCHECK_SGX_ENCRYPTED_PLATFORM_KEYS))
#define MCHECK_SGX_ENCRYPTED_PLATFORM_KEYS_UUID \
{ 0x41, 0x5C, 0x8F, 0xFD, 0x61, 0x1B, 0x97, 0x4B, 0xA7, 0x47, 0x96, 0xF0, 0x89, 0x26, 0x75, 0x7B }
...
typedef struct {
  ...
  MCHECK_UUID         Prid;
  UINT8               PlatformInfoHash[SHA256_DIGEST_SIZE];
  MCHECK_UUID         TargetPrid[MCHECK_MAX_P2P_LINKS];
  MCHECK_SGX_PUBKEY   TargetPrsk[MCHECK_MAX_P2P_LINKS];
  UINT8               Signature[SHA3072_DIGEST_SIZE];
} MCHECK_SGX_PAIRING_RECEIPT; // Total: 3293 bytes
#define MCHECK_SGX_PAIRING_RECEIPT_SIZE (sizeof (MCHECK_SGX_PAIRING_RECEIPT))
#define MCHECK_SGX_PAIRING_RECEIPT_UUID \
{ 0xB4, 0x0B, 0xC4, 0x67, 0x1A, 0xB5, 0x40, 0x66, 0xB7, 0xF9, 0x60, 0xB6, 0x50, 0x4B, 0xC1, 0x8B }

typedef struct {
  MCHECK_HEADER                      Header;
  MCHECK_SGX_PLATFORM_INFO           PlatformInfo;
  MCHECK_SGX_ENCRYPTED_PLATFORM_KEYS EncryptedKeys;
  MCHECK_SGX_PAIRING_RECEIPT         Receipt[MCHECK_MAX_PACKAGES];
} MCHECK_SGX_PLATFORM_MANIFEST; // Total: 28221 bytes
#define MCHECK_SGX_PLATFORM_MANIFEST_SIZE (sizeof (MCHECK_SGX_PLATFORM_MANIFEST))
#define MCHECK_SGX_PLATFORM_MANIFEST_UUID \
{ 0x17, 0x8E, 0x87, 0x4B, 0x49, 0xE4, 0x4A, 0xA5, 0x99, 0xBB, 0x30, 0x57, 0x17, 0x09, 0x25, 0xB4 }
...
typedef struct {
  ...
  UINT8                          Pfk1[AES128_KEY_SIZE];
  UINT8                          PprResetKey[MCHECK_MAX_KEYS][AES128_KEY_SIZE];
  UINT8                          LastUsedSvn;
  UINT8                          Reserved2[15];
  MCHECK_UUID                    TargetPrid[MCHECK_MAX_P2P_LINKS];
  MCHECK_UUID                    MasterComms[MCHECK_MAX_P2P_LINKS];
  UINT8                          Iv[MCHECK_MAX_KEYS][12];  // IV for different components of the structure
  UINT8                          Mac[MCHECK_MAX_KEYS][16]; // MAC for different components of the structure
  UINT8                          IvComm[12];               // IV for MasterCommsKeys
  UINT8                          MacComm[16];              // MAC for MasterCommsKeys
} MCHECK_SGX_KEY_BLOB; // Total: 1802 bytes
#define MCHECK_SGX_KEY_BLOB_SIZE (sizeof (MCHECK_SGX_KEY_BLOB))
#define MCHECK_SGX_KEY_BLOB_UUID \
{ 0x2E, 0xCF, 0x43, 0xFD, 0x61, 0x4E, 0x4F, 0x94, 0x98, 0x2C, 0xDF, 0x36, 0x10, 0xF4, 0x3A, 0x9D }

typedef struct {
  UINT64        Flags;        // BIT0: has input blob, BIT1: MCHECK modified key blob
  MCHECK_BUFFER KeyBlob;      // SGX_MCHECK_KEY_BLOB
  MCHECK_BUFFER Certificate;  // SGX_MCHECK_PLATFORM_MEMBERSHIP_CERTIFICATE
  UINT8         Reserved[24];
} MCHECK_SGX_SOCKET_DATA; // Total: 64 bytes
#define MCHECK_SGX_SOCKET_DATA_SIZE (sizeof (MCHEC_SGX_SOCKET_DATA))

// Flags is a variable defining the direction of
// MCHECK_SGX_KEY_BLOB inside MCHECK_SGX_SOCKET_DATA structure usage.
#define FLAGS_BLOB_IN_BIT  BIT0
#define FLAGS_BLOB_OUT_BIT BIT1

typedef enum {
  McheckRegularBoot          = 0x00,
  McheckFirstPlatformBinding = 0x01,
  McheckTcbRecovery          = 0x02,
  McheckAddingNewPackages    = 0x03,
  McheckInvalid              = 0xFF
} MCHECK_SGX_SCENARIO; // Total: 1 byte
#define MCHECK_SGX_SCENARIO_SIZE (sizeof (MCHECK_SGX_SCENARIO))

typedef struct {
  UINT8                          Scenario;
  UINT8                          CpuSvn;             // CPUSVN to be used
  UINT16                         Reserved;
  MCHECK_SGX_SECURITY_PROPERTIES SecurityProperties; // Platform Security Properties
  MCHECK_BUFFER                  RegistrationServer; // MCHECK_SGX_REGISTRATION_SERVER_INFO
  MCHECK_BUFFER                  PlatformManifest;   // MCHECK_SGX_PLATFORM_MANIFEST
  MCHECK_UUID                    Prids[MCHECK_MAX_PACKAGES];      // Array of 8 PRID values
  MCHECK_BUFFER                  SocketData[MCHECK_MAX_PACKAGES]; // Array of pointers to SOCKET_DATA structures
} MCHECK_SGX_INFO; // Total: 296 bytes
#define MCHECK_SGX_INFO_SIZE (sizeof (MCHECK_SGX_INFO))
//
// SGX Multi-Package Registration UEFI Interface
//

//
// Following definitions are used across SGX UEFI FW initialization flows
//
...
typedef struct {
  ...
  MCHECK_SGX_REGISTRATION_SERVER_ID RegServerId;
...
typedef struct {
  SGX_REGISTRATION_HEADER            Header;
  MCHECK_SGX_PLATFORM_MANIFEST       McheckPlatformManifestIpe;
  MCHECK_SGX_PLATFORM_MANIFEST       McheckPlatformManifestTr;
...
typedef struct {
  ...
  UINT16        NumPackages;
  MCHECK_UUID   PlatformInstanceId;
  MCHECK_UUID   Prid[MCHECK_MAX_PACKAGES];
  MCHECK_UUID   Reserved;
} SGX_REGISTRATION_ADD_REQUEST;
#define SGX_REGISTRATION_ADD_REQUEST_UUID \
{ 0x69, 0x65, 0x19, 0xCA, 0x73, 0xC1, 0x47, 0x85, 0xA0, 0xF6, 0x4D, 0x28, 0x9D, 0x37, 0xE9, 0x95 }
...
typedef struct {
  SGX_REGISTRATION_HEADER            Header;
  MCHECK_SGX_KEY_BLOB                KeyBlobs[MCHECK_MAX_PACKAGES];
} SGX_REGISTRATION_PACKAGE_INFO;

typedef struct {
  SGX_REGISTRATION_HEADER                      Header;
  MCHECK_SGX_PLATFORM_MEMBERSHIP_CERTIFICATE   PlatformMembershipCertificate[MCHECK_MAX_PACKAGES];
...
#define MCHECK_SGX_PRX_SERVER_URL "https://pre.api.trustedservices.intel.com:443"
#define MCHECK_SGX_SBX_SERVER_URL "https://sbx.api.trustedservices.intel.com:443"
#define MCHECK_SGX_SERVER_RSNAME \
      { \
         0x5E306161EA7257AF\
        ,0x337B416B89177A10\
        ,0x5D3ECB09435253C5\
...
#define MCHECK_SGX_SERVER_RSAK_EXP 0x10001

#define MCHECK_SGX_SERVER_RSEK \
      {\
         0x12CF4B066EF611A9\
        ,0x56462E8BF6912264\
        ,0x37218434859C086A\
...
#define MCHECK_SGX_SERVER_RSEK_EXP 0x10001

#define MCHECK_SGX_SERVER_SIGNATURE \
     { \
         0xE0FD8D87C3EB078\
        ,0x6C8767CC8A7D0DCF\
        ,0x9A3AE4B9B1436597\
...
//
// SGX_UEFIFW_SOCKET structure contains fields used in registration flow
//
typedef struct {
  UINT64                    Prids_0;
  UINT64                    Prids_1;
  BOOLEAN                   AddPackageNew; // Socket is new in AddPackage flow
} SGX_UEFIFW_SOCKET;

//
// SGX_UEFIFW_KEY_BLOB is a wrapper for MCHECK_SGX_KEY_BLOB
// used for internal SGX UEFI FW purposes
//
typedef struct {
  MCHECK_SGX_KEY_BLOB KeyBlob;
  EFI_TIME            TimeStamp;
  BOOLEAN             Present;
  BOOLEAN             Integrity;
} SGX_UEFIFW_KEY_BLOB;
...
//
// SGX_UEFIFW_PLATFORM_MEMBERSHIP_CERTIFICATE is a wrapper for MCHECK_SGX_PLATFORM_MEMBERSHIP_CERTIFICATE
// used for internal SGX UEFI FW purposes
//
typedef struct {
  MCHECK_SGX_PLATFORM_MEMBERSHIP_CERTIFICATE PlatformMembershipCertificate;
  BOOLEAN                                    Present;
} SGX_UEFIFW_PLATFORM_MEMBERSHIP_CERTIFICATE;

//
// SGX UEFI FW Internal HOB used for communication between SgxPreMemInit and SgxEarlyInit phases
//
extern EFI_GUID gSgxPreMemInitHobGuid;
#define SGX_INIT_PREMEM_DATA_HOB_GUID \
{ 0x9A9E2225, 0xE026, 0x4BFF, { 0x99, 0x17, 0xF1, 0x23, 0xE1, 0xD4, 0x3A, 0xD5 } }
typedef struct {
  ...
  UINT8               McBitMap[MAX_SOCKET][MAX_PRMRR_COUNT];
  SGX_ERROR_CODE_ENUM ErrorCode;
} SGX_PREMEM_INIT_HOB;

//
// SGX UEFI FW Internal HOB used for communication between SgxEarlyInit and SgxLateInit phases
//
extern EFI_GUID gSgxInitDataHobGuid;
#define SGX_INIT_DATA_HOB_GUID \
{ 0x4EF0BEDC, 0xC5C1, 0x4E69, { 0x82, 0x18, 0x7D, 0xBE, 0x8A, 0x60, 0x68, 0x8C } }
typedef struct {
  SGX_PREMEM_INIT_HOB           SgxPreMemInitHob;
  SGX_UEFIFW_SOCKET             Socket[MCHECK_MAX_PACKAGES];
  UINT64                        Prids_QualifiedForAddPkg[MCHECK_MAX_PACKAGES];
  UINT8                         PrResetSvn[MCHECK_MAX_PACKAGES];
  UINT8                         BootingSvn;
  BOOLEAN                       SgxFirstBoot;
  SGX_UEFI_BOOT_SCENARIO        SgxBootScenario;
  UINT32                        ErrorCode;
  SGX_UEFIFW_KEY_BLOB           UefiFwKeyBlobs[MCHECK_MAX_PACKAGES];
  SGX_UEFIFW_PLATFORM_MEMBERSHIP_CERTIFICATE UefiFwPlatMemCert[MCHECK_MAX_PACKAGES];
  BOOLEAN                       KeyBlobsExistInNvram;
  BOOLEAN                       KeyBlobsCreatedOrModifiedByMcheck;
  BOOLEAN                       KeyBlobsRestoredFromPackageInfo;
  UINT8                         SgxLeWr;
  BOOLEAN                       MktmeIntegrityEnabled;
...
*/
#define CMR_TABLE_ENTRIES        32
#define EFI_MEMORY_CPU_CRYPTO    0x0000000000080000ULL

typedef struct {
  MCHECK_CMR          CmrTable[CMR_TABLE_ENTRIES];
} MCHECK_SEAM_INFO;
