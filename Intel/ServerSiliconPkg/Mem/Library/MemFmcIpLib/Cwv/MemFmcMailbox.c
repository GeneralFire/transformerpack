  ...
  )
{
  ...
  RcDebugPrintWithDevice (SDBG_MAX, Socket, Ch, Dimm, NO_SUBCH, NO_RANK, NO_STROBE, NO_BIT, "FnvDisplayErrorLog Starts \n");
  ...
  RcDebugPrintWithDevice (SDBG_MAX, Socket, Ch, Dimm, NO_SUBCH, NO_RANK, NO_STROBE, NO_BIT, "FnvDisplayErrorLog Ends \n");
}
...
)
{
  ...
  if (IsChannelEnabled (Socket, Channel) == TRUE) {
    if (IsDcpmmPresentDimm (Socket, Channel, Dimm)) {
      RcDebugPrintWithDevice (SDBG_MEM_TRAIN, Socket, Channel, Dimm, NO_SUBCH, NO_RANK, NO_STROBE, NO_BIT,
                      "SaveDdrtTrainingFailureCode Starts\n");
      //
      // Byte 1 = DDRT Training Status
      //
      inputPayload[0] = (DDRT_TRAINING_STATUS_FAILURE << 8);
      //
      // Bye (3 -4) DDRT Training Failure Code
      //
      inputPayload[0] |= (MinorWarningCode << 24);
      inputPayload[1] = WarningCode;
      status = FnvMailboxInterface(Host, Socket, Channel, Dimm, inputPayload, outputPayload, DDRT_IO_INIT, &mbStatus);

      if (status != SUCCESS) {
        RcDebugPrintWithDevice (SDBG_MEM_TRAIN, Socket, Channel, Dimm, NO_SUBCH, NO_RANK, NO_STROBE, NO_BIT,
                       "SaveDdrtTrainingFailureCode Failed.\n");
        return FAILURE;
      }
  ...
  )
{
  ...
  if (AitDramFlag) {
    ...
    InputPayload[1] = (UINT16) FIS_CURR_VERSION_CWV;               // API revision Byte 5:4

    RcDebugPrintWithDevice (SDBG_MAX, Socket, Ch, Dimm, NO_SUBCH, NO_RANK, NO_STROBE, NO_BIT, FIS_VERSION_DBG_MSG (FIS_CURR_VERSION_CWV));

    Host->var.mem.BiosFisRevision = FIS_CURR_VERSION_CWV;
  }
...
static CHAR8 *GetBSRFieldStr(UINT32 bitmask, CHAR8 *StringBuffer) {
  ...
  switch (bitmask) {
    case FNV_MB_READY:
      ...
      break;
    case FNV_DT_DONE:
      AsciiStrCpyS (StringBuffer, BSR_STRING_LENGTH, "PMem IO Init Complete");
      break;
    case FNV_MEDIA_DONE:
...
)
{
  ...
  switch (Bitmask) {
    case FNV_MB_READY:
      ...
      EmulationReadFnvCfgSmbus (Host, Socket, Channel, Dimm, D_FW_STATUS_CWV_D_UNIT_0_REG, FNV_MB_READY, 1);
      break;
    case FNV_DT_DONE:
      EmulationReadFnvCfgSmbus (Host, Socket, Channel, Dimm, D_FW_STATUS_CWV_D_UNIT_0_REG, FNV_DT_DONE, 1);
      break;
    case FNV_CSR_UNLOCK:
      EmulationReadFnvCfgSmbus (Host, Socket, Channel, Dimm, D_FW_STATUS_CWV_D_UNIT_0_REG, FNV_CSR_UNLOCK, 0);
      break;
    case FNV_MEDIA_DONE:
      ...
      EmulationReadFnvCfgSmbus (Host, Socket, Channel, Dimm, D_FW_STATUS_CWV_D_UNIT_0_REG, FNV_MEDIA_READY, 1);
      break;
    case FNV_CREDIT_READY:
      EmulationReadFnvCfgSmbus (Host, Socket, Channel, Dimm, D_FW_STATUS_CWV_D_UNIT_0_REG, FNV_CREDIT_READY, 1);
      break;
    default:
  ...
  )
{
  ...
  for (ch = 0; ch < MaxChDdr; ch++) {
    ...
    for (dimm = 0; dimm < MAX_DIMM; dimm++) {
      ...
      if (!(UbiosGenerationOrHsleEnabled ())) {
        do {
          Bsr.Data = ReadFnvCfg (Host, socket, ch, dimm, D_FW_STATUS_CWV_D_UNIT_0_REG);
        ...
        } while (CheckforBSRError (bitmask, Bsr.Data, &error));
          ...
          switch (bitmask) {
            case FNV_DT_DONE:
              EwlOutputType11 (WARN_FNV_BSR, WARN_DT_ERROR,
                               socket, ch, dimm, 0xFF, EwlSeverityWarning, Bsr.Data);
            case FNV_MB_READY:
        ...
        (*dimmNvList)[dimm].mailboxReady = (UINT8) Bsr.Bits.MailboxInterfaceReady;
      } else {

        //
        // Set Mailbox Ready for ubios path
        //
        (*dimmNvList)[dimm].mailboxReady = 1;
...
//      Base address and offset can be found in the CWV C-Spec
...
const PROTECTED_DCPMM_REG mProtectedDcpmmRegisters[] = {
  ...
  //
  {IO_INIT_CTRL_CWV_DFX_MISC_0_REG,       0xD80003A8,           BIT31},  // io_init_ctrl.ddrtio_ckidle_mask[31] - pDOS bit in the same CSR = io_init_ctrl.pfail_pwon_mask [bit 29]
  {D_FUNC_DEFEATURE1_CWV_D_UNIT_0_REG,    0xC4000050,           BIT21}   // d_func_defeature1.wcrd_init[21]     - pDOS bit in the same CSR = d_func_defeature1.frc_wr_fifo_rtry[25]
};
  ...
  )
{
  ...
  if (Register == NULL) {
    RC_FATAL_ERROR (FALSE, ERR_MRC_POINTER, RC_FATAL_ERROR_MRC_POINTER_MINOR_CODE_005);
  ...
  )
{
  ...
  if (Value == NULL) {
    RC_FATAL_ERROR (FALSE, ERR_MRC_POINTER, RC_FATAL_ERROR_MRC_POINTER_MINOR_CODE_003);
    return MRC_STATUS_INVALID_PARAMETER;
  }
  ...
  if (GsmReadOnly (Mode)) {
    ...
    *Value = ReadFnvCfg (Host, Socket, Channel, Dimm, Register);
    ...
    if (RegisterEntry == NULL) {
      RC_FATAL_ERROR (FALSE, ERR_INVALID_REG_ACCESS, ERR_INVALID_WRITE_REG_OFFSET);
      return MRC_STATUS_INVALID_PARAMETER;
    }
    ...
    if (Host->var.mem.socket[Socket].channelList[Channel].FisVersion[Dimm] < FIS_300_VERSION) {
      //
      // Fall back on CSR access for writes if FIS < 3.0
      //
      ...
      //
      // FIS version is >= 3.0, use protected register mailbox command
      //
...
*/
  ...
  if (payload == PLL_LOCK){
    ...
    switch (Host->nvram.mem.socket[socket].ddrtFreq) {
      ...
      case DDRT_FREQUENCY_INDEX1:
        tempFreq = FMC_OPFREQ_3600MTS;
        break;
      case DDRT_FREQUENCY_INDEX2:
        tempFreq = FMC_OPFREQ_4000MTS;
        break;
      case DDRT_FREQUENCY_INDEX3:
        tempFreq = FMC_OPFREQ_4400MTS;
        break;
      case DDRT_FREQUENCY_INDEX4:
        tempFreq = FMC_OPFREQ_4800MTS;
        break;
      default:
        RcDebugPrintWithDevice (SDBG_MEM_TRAIN, socket, NO_CH, NO_DIMM, NO_SUBCH, NO_RANK, NO_STROBE, NO_BIT,
  ...
  for (ch = 0; ch < MaxChDdr; ch++) {
    ...
    for (dimm = 0; dimm < MAX_DIMM; dimm++) {
      ...
      if ((*channelNvList)[ch].enabled == 0) {
        inputPayload[0] = BIT9;
        (*channelNvList)[ch].mb3trainingfailure = 1;
      } else {
        (*channelNvList)[ch].mb3trainingfailure = 0;
      }
  ...
  //
  // Wait 100ms before polling for completion. Allow resync/reset to occur
  //
...
Smm mailbox will support SMBus or CPGC reads and SMbus, or CPGC writes
...
)
{
  ...
  MB_SMM_CMD_CWV_DPA_MAPPED_0_STRUCT               SmmCommandRegister;
  UINT64                                           StartCount;
  UINT8                                            opcodeIndex;
  UINT32                                           payloadCount;
  UINT8                                            i;
  ...
  if ((UbiosGenerationOrHsleEnabled ()) && (Setup->common.ddrtXactor == 0)){
    if (smm) {
      EmulationReadFnvCfg (Host, socket, ch, dimm, MB_SMM_CMD_CWV_DPA_MAPPED_0_REG, BIT16, 0);
    } else {
      if (GetUbiosOutputMode () == ASM_OUTPUT_ENABLE_ON_SMBUS) {
        EmulationReadFnvCfgSmbus (Host, socket, ch, dimm, MB_SMBUS_CMD_CWV_DS_UNIT_0_REG, BIT16, 0);
      }
    }
  } else {
    ...
    while (TRUE) {
      if (smm) {
        SmmCommandRegister.Data = ReadFnvCfg (Host, socket, ch, dimm, MB_SMM_CMD_CWV_DPA_MAPPED_0_REG);
        doorbell = SmmCommandRegister.Bits.doorbell;
      } else {
        SMBusCommandRegister.Data = ReadFnvCfg (Host, socket, ch, dimm, MB_SMBUS_CMD_CWV_DS_UNIT_0_REG);
  ...
  if (smm) {
    ...
    RegOffset.Data = MB_SMM_CMD_CWV_DPA_MAPPED_0_REG;
    ...
    WriteFnvCfg (Host, socket, ch, dimm, MB_SMBUS_CMD_CWV_DS_UNIT_0_REG, SMBusCommandRegister.Data);
  }
  ...
  for (i = 0; i < payloadCount; i++){
    if (smm) {
      RegOffset.Data = MB_SMM_INPUT_PAYLOAD_0_CWV_DPA_MAPPED_0_REG + (i * 4);
      RegOffset.Bits.size = 0;      //select quadword
      csrData64.lo = inputPayload[i++];
      csrData64.hi = inputPayload[i];
      WriteFnvCfgDpa (Host, socket, ch, dimm, RegOffset.Data, csrData64);
    } else {
      WriteFnvCfg (Host, socket, ch, dimm, MB_SMBUS_INPUT_PAYLOAD_0_CWV_DS_UNIT_0_REG + (i * 4) , inputPayload[i]);
    }
  }
...
)
{
  ...
  MB_SMM_STATUS_CWV_DPA_MAPPED_0_STRUCT            SmmStatusRegister;
  MB_SMM_CMD_CWV_DPA_MAPPED_0_STRUCT               SmmCommandRegister;
  MB_SMBUS_CMD_CWV_DS_UNIT_0_STRUCT                SMBusCommandRegister;
  UINT32                                           PayloadCount;
  UINT8                                            i;
  ...
  if ((UbiosGenerationOrHsleEnabled ()) && (Setup->common.ddrtXactor == 0)){
    if (Smm) {
      EmulationReadFnvCfg (Host, socket, ch, dimm, MB_SMM_STATUS_CWV_DPA_MAPPED_0_REG, BIT0, 1);
    } else {
      if (GetUbiosOutputMode () == ASM_OUTPUT_ENABLE_ON_SMBUS) {
        EmulationReadFnvCfgSmbus (Host, socket, ch, dimm, MB_SMBUS_STATUS_CWV_DS_UNIT_0_REG, BIT0, 1);
      }
    }
  } else {
    // Check/Wait for doorbell idle status
    ...
    if (Smm) {
      SmmStatusRegister.Data = ReadFnvCfg (Host, socket, ch, dimm, MB_SMM_STATUS_CWV_DPA_MAPPED_0_REG);
      Comp = SmmStatusRegister.Bits.comp;
      Stat = SmmStatusRegister.Bits.stat;
      *mbStatus = (UINT8) Stat;
    } else {
      SMBusStatusRegister.Data = ReadFnvCfg (Host, socket, ch, dimm, MB_SMBUS_STATUS_CWV_DS_UNIT_0_REG);
  ...
  for (i = 0; i < PayloadCount; i++){
    if (Smm) {
      RegOffset.Data = MB_SMM_OUTPUT_PAYLOAD_0_CWV_DPA_MAPPED_0_REG + (i * 4);
      RegOffset.Bits.size = 0;      //select quadword
      ReadFnvCfgDpa (Host, socket, ch, dimm, RegOffset.Data, &CsrData64);
      outputPayload [i++] = CsrData64.lo;
      outputPayload [i] = CsrData64.hi;
    } else {
      outputPayload [i] = ReadFnvCfg (Host, socket, ch, dimm, MB_SMBUS_OUTPUT_PAYLOAD_0_CWV_DS_UNIT_0_REG + (i * 4));
    }
  }
  ...
  )
{
  ...
  RcDebugPrintWithDevice (SDBG_MAX, Socket, Ch, Dimm, NO_SUBCH, NO_RANK, NO_STROBE, NO_BIT,
      "Pmem Sending command. Timestamp = %u Opcode = %x\n", TimeInSecond, Opcode);
#endif // DEBUG_CODE_BLOCK
    ...
    if ((Status == FAILURE) && (*MbStatus == MB_STAT_DEVICE_BUSY)) {
      ...
      if (CheckLongOperationStatus(Host, Socket, Ch, Dimm, &Command, &PercentComplete, &Etc, &StatusCode, &MbStatus2) == SUCCESS) {
        ...
        while ((PercentComplete < 0x100) && (StatusCode == MB_STAT_DEVICE_BUSY)) {
          ...
          RcDebugPrintWithDevice (SDBG_MINMAX, Socket, Ch, Dimm, NO_SUBCH, NO_RANK, NO_STROBE, NO_BIT,
              "Pmem Long Operation in progress. %x %% Complete\n", PercentComplete);
...
{
  BootStatusRegister->Data     = ReadFnvCfg (Host, Socket, Ch, Dimm, D_FW_STATUS_CWV_D_UNIT_0_REG);
  BootStatusHighRegister->Data = ReadFnvCfg (Host, Socket, Ch, Dimm, D_FW_STATUS_H_CWV_D_UNIT_0_REG);
}
  ...
  )
{
  ...
  FeatureStatusReg->Data.lo = ReadFnvCfg (Host, Socket, Ch, Dimm, FW_MISC_4_CWV_DPA_MAPPED_0_REG);
  FeatureStatusReg->Data.hi = ReadFnvCfg (Host, Socket, Ch, Dimm, FW_MISC_5_CWV_DPA_MAPPED_0_REG);
  return EFI_SUCCESS;
}
  ...
  )
{
...
#ifdef DEBUG_CODE_BLOCK
  RcDebugPrintWithDevice (SDBG_MINMAX, Socket, Ch, Dimm, NO_SUBCH, NO_RANK, NO_STROBE, NO_BIT, "FW_STATUS / Legacy Boot Status Register(LBSR):\n");
  RcDebugPrintWithDevice (SDBG_MINMAX, Socket, Ch, Dimm, NO_SUBCH, NO_RANK, NO_STROBE, NO_BIT, "============================================\n");
  ...
  RcDebugPrintWithDevice (SDBG_MINMAX, Socket, Ch, Dimm, NO_SUBCH, NO_RANK, NO_STROBE, NO_BIT, "  [31:31] SVNWC (SVN Opt-In Window Closed) ----------- = 0x%1X (0:Open, 1:Closed)\n", Bsr.Bits.SvnOptInWindowClosed);
  ...
  RcDebugPrintWithDevice (SDBG_MINMAX, Socket, Ch, Dimm, NO_SUBCH, NO_RANK, NO_STROBE, NO_BIT, "  [35:34] DTS (Pmem Training Status) = 0x%02X (0: Not Complete, 1: Complete, 2:Failure, 3:S3 Complete)\n", BsrHi.Bits.DdrtIoInitTrainingStatus);
  RcDebugPrintWithDevice (SDBG_MINMAX, Socket, Ch, Dimm, NO_SUBCH, NO_RANK, NO_STROBE, NO_BIT, "  [63:36] Reserved\n\n");
#endif //DEBUG_CODE_BLOCK
}
  ...
  for (Ch = 0; Ch < MaxChDdr; Ch++) {
    ...
    for (Dimm = 0; Dimm < MAX_DIMM; Dimm++) {
      ...
      if (Host->nvram.mem.socket[Socket].channelList[Ch].dimmList[Dimm].NvmDimmDisable == 1) {
        ...
        RcDebugPrintWithDevice (SDBG_MINMAX, Socket, Ch, Dimm, NO_SUBCH, NO_RANK, NO_STROBE, NO_BIT,
                   "Read Media Disable BSR for disabled Pmem, Bsr=0x%x\n", Bsr);
