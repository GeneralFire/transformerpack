  ...
  )
{
  return PCI_SEGMENT_LIB_ADDRESS (
           ...
           PCI_DEVICE_NUMBER_PCH_CDF_SCS_EMMC,
           PCI_FUNCTION_NUMBER_PCH_CDF_SCS_EMMC,
           0
           );
}

/**
  Get eMMC PCI cfg space address in the BootScript library
  supported format.

  @return UINT64  PCI base address for boot script library
**/
UINT64
ScsGetEmmcS3BootScriptPciBaseAddress (
  VOID
  )
{
