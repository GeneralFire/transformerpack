...
/**
  Checks if in Slave Acces flash mode
  If so then checks for NFE, UC, UCT and UA errors
  In case of any of these errors performs warm reset otherwise clears error
...
**/
VOID
CheckSafNonFatalErrorsAndReset (
  VOID
  )
{
  UINT32 Data32;
  ...
  if ((Data32 & B_SPI_MEM_HSFSC_SAF_MODE_ACTIVE) == 0) {
    return;
  }
  ...
  if ((Data32 & B_ESPI_PCR_FCERR_FCNFES) != 0) {
    Data32 >>= N_ESPI_PCR_FCERR_FCNFEC;
    if (Data32 == V_ESPI_PCR_FCERR_FCNFEC_SRC_NFE ||
      Data32 == V_ESPI_PCR_FCERR_FCNFEC_SCR_UC ||
      Data32 == V_ESPI_PCR_FCERR_FCNFEC_UCT ||
      Data32 == V_ESPI_PCR_FCERR_FCNFEC_UA) {

      DEBUG ((DEBUG_ERROR, "EspiRecoverFromFatalError: Performing Warm Reset\n"));
      IoWrite8 ((UINTN) 0xCF9, 0x06);
      ///
      ...
      Status = PchPcrAndThenOr32 (
                  ...
                  (UINT16) (R_ESPI_PCR_FCERR_SLV0),
                  (UINT32) ~0,
                  (UINT32) (B_ESPI_PCR_FCERR_FCNFES)
                  );
      ASSERT_EFI_ERROR (Status);
    }
  }
