  ...
  )
{
  ...
  for (RpIndex = 0; RpIndex < MaxPorts; RpIndex++) {
    ...
    if (PciSegmentRead16 (RpBase + PCI_VENDOR_ID_OFFSET) != 0xFFFF) {
      PciSegmentAnd8 (RpBase + R_PCH_PCIE_CFG_SCICSR, (UINT8)(~(B_PCH_PCIE_CFG_SCICSR_PMSCIE)));
      PciSegmentOr32 (RpBase + R_PCH_PCIE_CFG_SCICSR, B_PCH_PCIE_CFG_SCICSR_PMSCISTS);
    }
  }
