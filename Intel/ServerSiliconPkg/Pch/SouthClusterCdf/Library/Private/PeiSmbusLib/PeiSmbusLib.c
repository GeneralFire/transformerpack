  ...
  )
{
  ...
  PchPcrAndThenOr32 (
    ...
    ~(UINT32) (B_SMBUS_PCR_PCE_HAE |
               B_SMBUS_PCR_PCE_D3HE |
               B_SMBUS_PCR_PCE_I3E |
               B_SMBUS_PCR_PCE_PMCRE),
    0
    );
  ...
  ///
  /// Check to disable Smbus DMA controller
  ///
  if (SmbusPreMemConfig->DmaSmbusEnable == FALSE) {
    ///
    ...
    SmbusDmaBaseAddress = PCI_SEGMENT_LIB_ADDRESS (
                         DEFAULT_PCI_SEGMENT_NUMBER_PCH,
                         DEFAULT_PCI_BUS_NUMBER_PCH,
    ...
    ///
    InterruptPin = ItssGetDevIntPin (SiPolicyPpi, PCI_DEVICE_NUMBER_SMBUS_HOST, PCI_FUNCTION_NUMBER_SMBUS_HOST);

    PciSegmentWrite8 (SmbusDmaBaseAddress + PCI_INT_PIN_OFFSET, InterruptPin);
  }
