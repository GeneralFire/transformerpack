...
typedef struct {
  ...
  UINT32 Offset;
  UINT32 Value;
} MCTP_PROGRAMMING_STRUCT;

MCTP_PROGRAMMING_STRUCT mMctpTable [] = {
  {0,     R_PCH_PSF_0_PSF_MC_AGENT_MCAST1_RS0_TGT0_MCTP1,  0x00000000},
  {1,     R_PCH_PSF_1_PSF_MC_AGENT_MCAST0_RS0_TGT0_MCTP0,  0x00008000},
  {2,     R_CDF_PSF_2_PSF_MC_AGENT_MCAST0_RS0_TGT0_MCTP0,  0x00018000},
  {4,     R_CDF_PSF_4_PSF_MC_AGENT_MCAST0_RS0_TGT0_MCTP0,  0x00028000},
};
...
PsfSetPcieControllerGrantCount (
  ...
  IN PCIE_BIFURCATION_CONFIG  *PsfPcieCtrlConfigTable,
  ...
  )
{
  ...
  RootPsfId = PsfRootId ();
  //Begin MCTP with setting forward target
  for (Index = 0; Index < ARRAY_SIZE(mMctpTable); Index++) {
    PchPcrWrite32(PsfSbPortId (mMctpTable[Index].PsfNumber), mMctpTable[Index].Offset, mMctpTable[Index].Value);
    NumOfEnabledTargets[mMctpTable[Index].PsfNumber]++;
  }
  //
  ...
  for (RpIndex = 0; RpIndex < GetPchMaxPciePortNum (); RpIndex++) {
    ...
    //
    // In addition to checking FUNDIS check port availability
    //
    if (PsfIsPcieRootPortEnabled (RpIndex) && (PciSegmentRead16 (RpBase) != 0xFFFF)) {
