...
/**
  This function is to be used In GpioSetAllPadsTermination() to skip programming
  termination for eMMC.

  @param[in]  GpioPad             GPIO pad

  @retval EmmcPin                 TRUE:  This is eMMC pin
                                  FALSE: This is not eMMC pin
**/
BOOLEAN
IsEmmcPin (
  IN GPIO_PAD GpioPad
  ...
  )
{
  ...
  for (GroupIndex = 0; GroupIndex < NumberOfGroups; GroupIndex++ ){
    Group = GpioGetGroupFromGroupIndex (GroupIndex);
    for(PadNumber = 0; PadNumber < GpioGroupInfo[GroupIndex].PadPerGroup; PadNumber++){
      GpioPad = GpioGetGpioPadFromGroupIndexAndPadNumber (GroupIndex, PadNumber);
      ...
      GpioGetPadMode (GpioPad, &GpioMode);
      if (GpioMode == GpioPadModeGpio) {
        TermValue = 0x0;
      } else if (IsEmmcPin (GpioPad)) {
        continue;
      } else {
        TermValue = 0xF;
      }
      TermValue = TermValue << N_GPIO_PCR_TERM;
