  ...
  )
{
  ...
  LpcRegBase = PCI_SEGMENT_LIB_ADDRESS (
  ...
  ///
  /// 5. Clear RTC Register 0Bh.
  ///
  IoWrite8 (R_RTC_IO_INDEX_ALT, (UINT8) R_RTC_IO_REGB);
  IoAnd8 (R_RTC_IO_TARGET_ALT, 0);

  ///
  /// 6. Clear RTC Register 0Dh.
  ///
  IoWrite8 (R_RTC_IO_INDEX_ALT, (UINT8) R_RTC_IO_REGD);
  IoAnd8 (R_RTC_IO_TARGET_ALT, 0);
  ...
  ///
  PchPcrAndThenOr32 (PID_RTC_HOST, R_RTC_PCR_CONF, 0, B_RTC_PCR_CONF_BILD | B_RTC_PCR_CONF_UCMOS_EN);

  ///
  /// 9. Set DSO and depending on Top Spin strap value, TS in BUC register
  ///
  TsEnable = PciSegmentRead8 (((UINTN) (LpcRegBase + R_LPC_CFG_BC)) & B_LPC_CFG_BC_TS);
  if (TsEnable) {
    PchPcrAndThenOr8 (PID_RTC_HOST, R_RTC_PCR_BUC, 0, B_RTC_PCR_BUC_DSO | B_RTC_PCR_BUC_TS);
  } else {
    PchPcrAndThenOr8 (PID_RTC_HOST, R_RTC_PCR_BUC, 0, B_RTC_PCR_BUC_DSO);
  }
  ...
  )
{
  ...
  if (LockDownConfig->RtcMemoryLock == TRUE) {
