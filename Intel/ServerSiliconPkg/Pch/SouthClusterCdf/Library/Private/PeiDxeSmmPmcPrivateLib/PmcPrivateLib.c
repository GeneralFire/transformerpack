  ...
  )
{
  ...
  //When "V_PMC_PWRM_IPC_CMD_WBUF0_PROXY_NMI" command is detected all debug logging MUST be skipped to avoid dead loop here.
  ...
  )
{
  ...
  //
  // Enable IOSpace and BusMaster in PMC Device
  //
  PciSegmentOr16 (
...
{
  //
  // PM_CFG[29] = 1, Allow 24MHz Crystal Oscillator Shutdown (ALLOW_24_OSC_SD)
  // PM_CFG[25] = 1, Allow USB2 PHY Core Power Gating (ALLOW_USB2_CORE_PG)
  //
  return ((MmioRead32 (PmcGetPwrmBase () + R_PMC_PWRM_CFG) &
           (B_PMC_PWRM_CFG_ALLOW_USB2_CORE_PG | B_PMC_PWRM_CFG_ALLOW_24_OSC_SD)) == 0);
}
...
)
{
  ...
  Status = PmcSendCommand(V_PMC_PWRM_IPC_ME_DISABLE, DisableMe, 0, &Wbuf, NULL);
  ASSERT_EFI_ERROR(Status);
}
