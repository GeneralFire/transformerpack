...
/**
  This function checks if R-Link (DMI Mode) Secured Register Lock (SRL) is set
...
{
  return ((PchPcrRead32 (PID_DMI, R_PCH_RLINK_PCR_DMIC) & B_PCH_RLINK_PCR_DMIC_SRL) != 0);
}
...
/**
  Set PCH TCO base address decoding in R-Link (DMI Mode)
  ...
  )
{
  ...
  //
  // Program "TCO Base Address" PCR[DMI] + E78h[15:5, 1] to [SMBUS PCI offset 50h[15:5], 1].
  //
  PchPcrWrite16 (
    PID_DMI, R_PCH_RLINK_PCR_TCOBASE,
    (Address | BIT1)
    );
...
{
  //
  // Read "TCO Base Address" PCR[DMI] + E78h[15:5]
  //
  return (PchPcrRead16 (PID_DMI, R_PCH_RLINK_PCR_TCOBASE) & B_PCH_RLINK_PCR_TCOBASE_TCOBA);
}
...
/**
  Clear PCH LPC/eSPI generic IO range decoding in R-Link (DMI Mode)
...
/**
  Clear PCH eSPI CS1# generic IO range decoding in R-Link (DMI Mode)
  ...
  )
{
  ...
  //
  // Program LPC Generic IO Range #, PCR[DMI] + EBCh to the same value programmed in eSPI PCI Offset A4h.
  //
  PchPcrWrite32 (PID_DMI, R_PCH_RLINK_PCR_SEGIR, 0);
...
/**
  Set PCH IO port 80h cycle decoding to PCIE root port in R-Link (DMI Mode)
  ...
  )
{
  ...
  /// of General Control and Status register, located at PCR[DMI] + E4Ch[11] , allows software to
  ...
  /// The "RPR Destination ID", PCR[DMI] + E4Ch[31:16] need to be set accordingly to point
  ...
  //
  // Program "RPR Destination ID", PCR[DMI] + E4Ch[31:16] to the Dest ID of RP.
  //
  PchPcrWrite16 (PID_DMI, R_PCH_RLINK_PCR_GCS + 2, DmiRpDestinationId);
  //
  // Program "Reserved Page Route", PCR[DMI] + E4Ch[11] to '1'.
  // Use byte write on GCS+1 and leave the BILD bit which is RWO.
  //
  PchPcrAndThenOr8 (PID_DMI, R_PCH_RLINK_PCR_GCS + 1, 0xFF, (B_PCH_RLINK_PCR_RPR >> 8));
...
{
  PchPcrAndThenOr8 (
    PID_DMI, R_PCH_RLINK_PCR_GCS + 1,
    (UINT8) ~0,
    (UINT8) (B_PCH_RLINK_PCR_RPR >> 8)
    );
}
...
{
  ///
  /// Setup "uCode Patch Region Enable", PCR [DMI] + E48h[0] to '0b'
  ///
  PchPcrAndThenOr32 (PID_DMI, R_PCH_RLINK_PCR_UCPR, (UINT32) ~B_PCH_RLINK_PCR_UCPR_UPRE, 0);
}
