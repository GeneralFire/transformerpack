  ...
  )
{
  ...
  //
  Status = PchSbiRpPciRead32 (CtrlFirstRpNum, R_PCH_PCIE_MSG_PPBIFCTL_PRIV, &Data32);
  ASSERT_EFI_ERROR (Status);

  switch (Data32 & B_PCH_PCIE_MSG_PPBIFCTL_PRIV_BIFCTL0) {
    case V_PCH_PCIE_CFG_PPBIFCTL_PRIV_BIFCTL0_8:
      if (RootPortNum % 4 == 0) {
        LaneOccupied = (BIT7|BIT6|BIT5|BIT4|BIT3|BIT2|BIT1|BIT0);
      }
      break;
  ...
  )
{
  ...
  ///
  PciSegmentAndThenOr32 (SataRegBase + R_PCH_PCIE_CFG_DEVCTL2 + V_SATA_MEM_RST_PCIE_STORAGE_REMAP_RP_OFFSET,
  ...
  if ((mSavedConfigSpace[CrNumber].EndpointLctlData16 & B_PCIE_LCTL_CCC) == B_PCIE_LCTL_CCC) {
    ...
    PciSegmentOr16 (SataRegBase + R_PCH_PCIE_CFG_LINKCTL + V_SATA_MEM_RST_PCIE_STORAGE_REMAP_RP_OFFSET, (UINT16) (B_PCIE_LCTL_RL));

    RstPollPcieLinkStateUntil (TRUE, 30000, SataRegBase + V_SATA_MEM_RST_PCIE_STORAGE_REMAP_RP_OFFSET + R_PCH_PCIE_CFG_LINKSTS);
  }
  ...
  ///
  Data16 = PciSegmentRead16 (SataRegBase + R_PCH_PCIE_CFG_LINKSTS + V_SATA_MEM_RST_PCIE_STORAGE_REMAP_RP_OFFSET);
  ...
  )
{
  ...
  ///
  mSavedConfigSpace[CrNumber].RootPortDctl2Data16   = PciSegmentRead16 (PciRootPortRegBase + R_PCH_PCIE_CFG_DEVCTL2) & B_PCIE_DCTL2_LTREN;
  ...
  ///
  PciSegmentOr16 (PciRootPortRegBase + R_PCH_PCIE_CFG_LINKCTL, (UINT16) (B_PCIE_LCTL_LD));

  RstPollPcieLinkStateUntil (FALSE, 50000, PciRootPortRegBase + R_PCH_PCIE_CFG_LINKSTS);
  ...
  /// Re-enable PCIe link
  ///
  PciSegmentAnd32 (SataRegBase + R_PCH_PCIE_CFG_LINKCTL + V_SATA_MEM_RST_PCIE_STORAGE_REMAP_RP_OFFSET, (UINT32) ~(B_PCIE_LCTL_LD));
  ...
  )
{
  ...
  for (CrNumber = 0; CrNumber < PCH_MAX_RST_PCIE_STORAGE_CR; CrNumber++) {
    ...
    if (RstSetCycleRouter (SataRegBase, CrNumber)) {
      RstPollPcieLinkStateUntil (TRUE, 1000 * 1000, SataRegBase + V_SATA_MEM_RST_PCIE_STORAGE_REMAP_RP_OFFSET + R_PCH_PCIE_CFG_LINKSTS);
    }
  }
  ...
  )
{
  ...
  PciSataRegBase  = GetSataRegBase (SATA_1_CONTROLLER_INDEX);
