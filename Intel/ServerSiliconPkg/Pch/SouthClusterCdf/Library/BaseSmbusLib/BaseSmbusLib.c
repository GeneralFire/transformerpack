...
/**
  Detects incomplete SMBUS transaction.

  Software can detect the presence of an incomplete transaction by using the
  SMBus_PIN_CTL register to observe SMBCLK at a high state for > 50 µs while
  SMBDATA is low during that same time period. It is assumed that software attempting
  to detect or clear this transaction owns the INUSE_STS semaphore.

  @retval TRUE Incomplete SMBUS transaction is detected.
  @retval FALSE Incomplete SMBUS transaction is not detected.

**/

BOOLEAN
SmBusDetectIncompleteTransaction (
  VOID
  )
{
  UINT16    IoPortBaseAddress;
  ...
  while (IoRead8 (IoPortBaseAddress + R_SMBUS_IO_SMBC) & B_SMBUS_IO_SMBCLK_CUR_STS &&
         !(IoRead8 (IoPortBaseAddress + R_SMBUS_IO_SMBC) & B_SMBUS_IO_SMBDATA_CUR_STS) &&
         Timeout < 50) {
    MicroSecondDelay (1);
    Timeout ++;
  }
...
/**
  Clears SmBus from incomplete transaction.

  @retval     EFI_SUCCESS          SMBUS wac cleared successfully.
  @retval     EFI_TIMEOUT      Failed to clear SMBUS.

**/

EFI_STATUS
SmBusKillIncompleteTransaction (
  VOID
  )
{
  UINT16    IoPortBaseAddress;
  ...
  Data = IoRead8 (IoPortBaseAddress + R_SMBUS_IO_HCTL);
  Data |= B_SMBUS_IO_KILL;
  IoWrite8 (IoPortBaseAddress + R_SMBUS_IO_HCTL, Data);

  //7. Repeat step 2 to 6 30 times to complete the longest possible transaction.
  for (i = 0; i < 30; i++) {
    //2. Force the SMBus clock low by clearing the SMBCLK_CTL bit, SMBus + 0Fh[2].
    Data = IoRead8 (IoPortBaseAddress + R_SMBUS_IO_SMBC);
    Data &= ~B_SMBUS_IO_SMBCLK_CTL;
    IoWrite8 (IoPortBaseAddress + R_SMBUS_IO_SMBC, Data);
    ...
    //5. Read the SMBus clock state by reading the SMBCLK_CUR_STS bit, SMBus + 0Fh[0],
    //   and if it is not set, then another device is stretching the SMBus clock.
    //   Repeat step 5 until the SMBCLK_CUR_STS bit set or TIMEOUT, MAX (35 ms) has
    //   passed, at which point the incomplete transaction cannot be cleared and this
  ...
  if (IoRead8(IoPortBaseAddress + R_SMBUS_IO_HSTS) & B_SMBUS_IO_FAIL) {
    return EFI_SUCCESS;
  }
  DEBUG ((DEBUG_ERROR, "No FAILED bit set after Kill bit set!\n"));
  return EFI_TIMEOUT;
}
  ...
  )
{
  ...
  if (SmBusDetectIncompleteTransaction()) {
    SmBusKillIncompleteTransaction();
  }
