...
{
  UINT8 BclkS3;
  BclkS3 = MmioRead8 (((UINTN) (PmcGetPwrmBase () + R_PMC_PWRM_BIOS_SCRATCHPAD_1)));

  if (BclkS3 & B_PMC_PWRM_BIOS_SCRATCHPAD_1_BCLK_S3) {
    DEBUG ((DEBUG_INFO, "BCLK reset occured during S3 resume on the previous boot.\n"));
    return TRUE;
  }

  DEBUG ((DEBUG_INFO, "BCLK reset during S3 did not occur.\n"));
  return FALSE;
}
...
{
  ///
  /// Set BCLK S3 resume scratchpad to indicate BCLK OC reset during S3
  ///
...
{
  ///
  /// Clear B_PMC_PWRM_BIOS_SCRATCHPAD_1_BCLK_S3 so we do not detect another BCLK reset.
  ///
  ...
  )
{
  ...
  if ((PchPwrmBase != 0) &&
      (MmioRead32 (PchPwrmBase + R_PMC_PWRM_HPR_CAUSE0) & B_PMC_PWRM_HPR_CAUSE0_GBL_TO_HOST)) {
    return TRUE;
  } else {
    return FALSE;
  }
