...
)
{
  ...
  if (mIioUdsData->SystemStatus.OutKtiCpuSktHotPlugEn != 1) {
    DEBUG((DEBUG_ERROR, "Hotplug is not enabled, so Quiesce protocol won't be installed\n"));
    return EFI_SUCCESS;
  }
...
)
{
  ...
  DEBUG ((DEBUG_INFO, "Quiesce Base: %x\n", QuiesceBaseAddress));


  DEBUG ((DEBUG_INFO, "Quiesce Prot CodeBase: %x\n",
...
)
{
  ...
  if((ApicId & 1) == 0) { //only primary thread do core scope msr
    ...
    AsmWriteMsr64 (ICX_MSR_MISC_FEATURE_CONTROL, 0x000F);
  }
...
)
{
  ...
  for(i = 0; i < MAX_SOCKET; i++) {
    ...
    if(mIioUdsData->PlatformData.IIO_resource[i].Valid) {
      ...
      if ((mCpuFamilModel >> 04) == CPU_FAMILY_SKX) {
        Data32 = (UINT32)-1;
      } else {
