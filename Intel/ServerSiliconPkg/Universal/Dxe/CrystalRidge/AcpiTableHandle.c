...
AML_ENTRY   AmlTable[] = {
              ...
              {.Signature = SIGNATURE_32 ('F', 'I', 'X', 'Y'), .OpCode = AML_OPCODE_DEVICE, .ExpectedOccurrences = MAX_SOCKET * 16 },
              {.Signature = SIGNATURE_32 ('F', 'I', 'X', 'W'), .OpCode = AML_OPCODE_DEVICE, .ExpectedOccurrences = MAX_SOCKET * 16 },
              {.Signature = SIGNATURE_32 ('F', 'X', 'C', 'D'), .OpCode = AML_OPCODE_NAME,   .ExpectedOccurrences = MAX_SOCKET * 16 },
              {.Signature = SIGNATURE_32 ('F', 'X', 'S', 'T'), .OpCode = AML_OPCODE_NAME,   .ExpectedOccurrences = MAX_SOCKET * 16 },
              {.Signature = SIGNATURE_32 ('F', 'X', 'I', 'N'), .OpCode = AML_OPCODE_NAME,   .ExpectedOccurrences = MAX_SOCKET * 16 },
              {.Signature = SIGNATURE_32 ('F', 'X', 'O', 'U'), .OpCode = AML_OPCODE_NAME,   .ExpectedOccurrences = MAX_SOCKET * 16 },
              {.Signature = SIGNATURE_32 ('F', 'X', 'B', 'S'), .OpCode = AML_OPCODE_NAME,   .ExpectedOccurrences = MAX_SOCKET * 16 },
              {.Signature = SIGNATURE_32 ('F', 'X', 'F', 'H'), .OpCode = AML_OPCODE_NAME,   .ExpectedOccurrences = MAX_SOCKET * 16 },
              {.Signature = SIGNATURE_32 ('F', 'I', 'X', 'X'), .OpCode = AML_OPCODE_DEVICE, .ExpectedOccurrences = 9 },
              {.Signature = SIGNATURE_32 ('F', 'I', 'X', 'V'), .OpCode = AML_OPCODE_DEVICE, .ExpectedOccurrences = 9 },
            };
...
/**
  ...
  in AmlTable array. If AML structure is correct, then actual patching begins - proper functions
  from NvdimmAcpiConfig are called.
  ...
  while(EFI_SUCCESS == FindNextOccurrence (&CurrentPointer, TableEndPointer)) {
    ...
    switch (Signature)
    {
      ...
      case (SIGNATURE_32 ('F', 'I', 'X', 'X')):
        PopulateControlRegion (CurrentPointer - 1);
        break;
      case (SIGNATURE_32 ('F', 'I', 'X', 'V')):
        PopulateFlushRegion (CurrentPointer - 1);
        break;
      case (SIGNATURE_32 ('F', 'I', 'X', 'Y')):
        PopulateDimmEnableStatus (CurrentPointer - 1);
        break;
      case (SIGNATURE_32 ('F', 'I', 'X', 'W')):
        PopulateFisVersion (CurrentPointer - 1);
        break;
      case (SIGNATURE_32 ('F', 'X', 'C', 'D')):
        PopulateCommandRegister (CurrentPointer + sizeof(Signature) + 2);
        break;
      case (SIGNATURE_32 ('F', 'X', 'S', 'T')):
        PopulateStatusRegister (CurrentPointer + sizeof(Signature) + 2);
        break;
      case (SIGNATURE_32 ('F', 'X', 'I', 'N')):
        PopulateInputPayloadRegister (CurrentPointer + sizeof(Signature) + 2);
        break;
      case (SIGNATURE_32 ('F', 'X', 'O', 'U')):
        PopulateOutputPayloadRegister (CurrentPointer + sizeof(Signature) + 2);
        break;
      case (SIGNATURE_32 ('F', 'X', 'B', 'S')):
        PopulateBootStatusRegister (CurrentPointer + sizeof(Signature) + 2);
        break;
      case (SIGNATURE_32 ('F', 'X', 'F', 'H')):
        PopulateFlushAddress (CurrentPointer + sizeof(Signature) + 2);
        break;
      default:
        break;
    }
