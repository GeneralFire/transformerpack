  ...
  )
{
  ...
  //
  // There is no SNC/XPT(UPI)Prefetch/2LM requirement for Sub_Numa B2P command
  // Any one of SNC, XPT/UPI Prefetch or 2LM enabled will program the SNC_BASE_2_MS2IDI
  //
  ...
  MailboxData = IioGetSnc2Base (KtiInternalGlobal->SbspSoc, 0);
  KtiDebugPrintInfo1 ((KTI_DEBUG_INFO1, "SNC_BASE_2_IIO_VTD_REG=%x\n", MailboxData));
  ...
  )
{
  ...
  for (Socket = 0; Socket < MAX_SOCKET; Socket++) {
    ...
    if (SocketPresent (Socket)) {
      ...
      KtiDebugPrintInfo0 ((KTI_DEBUG_INFO0, "\nUMA_CLUSTER_MS2IDI          : 0x%08X", Ms2IdiUmaClusterCfgDefIcx(Socket)));
      //XPT
      ...
      for (Cluster = 0; Cluster <= MAX_CLUSTERS; Cluster++) {
        ...
        KtiDebugPrintInfo0 ((KTI_DEBUG_INFO0, "\nSNC_BASE_%x       : 0x%08X", (Cluster + 1), Data32));
      }
