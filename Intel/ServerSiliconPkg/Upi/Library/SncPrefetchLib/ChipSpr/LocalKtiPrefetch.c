...
/**
  ...
  W/A for kti prefetch, in  the routine, when find far memory controller in cross-tile 2LM, need to disable KTI Prefetch

  @param[in, out] KtiInternalGlobal - KTIRC Internal Global data
  ...
  )
{
  ...
  if (IsSiliconWorkaroundEnabled ("S14010455369")) {
    if ((KTIVAR->OutKtiPrefetch == KTI_DISABLE) || CheckSysConfig (SYS_CONFIG_1S)) {
      return ;
    }
    ...
    for (Socket = 0; Socket < MAX_SOCKET; Socket++) {
      ...
      for (McIdx = 0; McIdx < MaxIMC; McIdx++) {
        if (((MemInfo->imcBitmap[Socket][MC_TECH_DDR] >> McIdx) & 1) == 0) {
          continue;
        }
        if (GetM2mSysFeatures0FarMcMode (Socket, McIdx) == 1) {
          KTIVAR->OutKtiPrefetch = KTI_DISABLE;
          KtiDebugPrintInfo0 ((KTI_DEBUG_INFO0, "  Far Memory controller in cross-tile 2LM, need to disable Kti Prefetch!\n"));
