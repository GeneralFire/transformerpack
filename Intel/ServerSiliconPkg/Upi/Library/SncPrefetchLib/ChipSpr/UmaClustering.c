  ...
  )
{
  ...
  if (EFI_SUCCESS == UBoxGetUmaBasedClusteringDowngradeInScratchpad (\
    ...
    if (UmaBasedClustingDowngrade == 0) {
      //NO downgrade happening.
      return KTI_IGNORE;
    } else if (UmaBasedClustingDowngrade == QUAD2HEMI) {
      // UMA Quad->Hemi mode
      KtiDebugPrintInfo0 ((KTI_DEBUG_INFO0, "\n   UMA Quad->Hemi detected!\n"));
      KTIVAR->OutUmaClustering = UMABASEDCLUSTERING_HEMISPHERE;
      KTIVAR->OutSncEn = 0;
      KTIVAR->UmaClusteringDowngrade = TRUE;
    } else if (UmaBasedClustingDowngrade == QUAD2DISABLE) {
      // UMA Quad->Disabled mode
      KtiDebugPrintInfo0 ((KTI_DEBUG_INFO0, "\n   UMA Quad->Disabled detected!\n"));
      KTIVAR->OutUmaClustering = UMABASEDCLUSTERING_DISABLED;
      KTIVAR->OutSncEn = 0;
      KTIVAR->UmaClusteringDowngrade = TRUE;
    } else if (UmaBasedClustingDowngrade == HEMI2DISABLE) {
    ...
    KTIVAR->OutSncPrefetchEn = (KTIVAR->OutKtiPrefetch | KTIVAR->OutXptPrefetch | KTIVAR->OutXptRemotePrefetch | KTIVAR->OutUmaClustering | KTIVAR->OutSncEn);
  }
