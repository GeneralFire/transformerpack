  ...
  )
{
  ...
  BOOLEAN ProgramSncRegs;

  KtiDebugPrintInfo1 ((KTI_DEBUG_INFO1, "\n ProgramNumOfChaPerCluster\n"));

  for (SocId = 0; SocId < MAX_SOCKET; SocId++) {
    ProgramSncRegs = IsSncRegistersInHardwareDefaultSetting (KtiInternalGlobal->SbspSoc);

    //
    // If ProgramSncRegs = FALSE, it means PCode already programmed the SNC registers
    //
    if (!ProgramSncRegs) {
      continue;
    }
  ...
  if (Operation == NORMAL_OPERATION) {    // SNC normal cold reset setting in cold reset flow
    ...
    if (NumOfCluster == 2) {
      if (KTIVAR->OutSncEn != 0) {
        FullSncEnable = 1; // FullSncEnable is only for NUMA
      } else {
        FullSncEnable = 0; // FullSncEn doesn't set in UMA clustering mode
      }
      NumClusters   = 1;
  ...
  )
{
  ...
  UINT8   DefeatureXor;
  UINT8   LinkIndex;
  UINT32  SncConfigMs2Idi;
  UINT32  UncoreSncConfig;
  ...
  // Defeature_Xor doesn't support in SPR. Should clear it.
  DefeatureXor = 0;
  //
...
/**
  Program XPT when warm reset elemination is enabled.
...
/**
  Program SNC topology in M2Mesh for Mc or HBMIO

  @param Socket            - Socket id
  @param McIndex           - Mc Index or HbmIo index
  @param SncCluster        - SNC cluster
  @param BaseCluster1      - Base cluster1
  @param BaseCluster2      - Base cluster2
  @param McTechType        - MC tech type: DDR or HBM

**/
VOID
ProgramSncInM2MemPerMc (
  UINT8                      Socket,
  UINT8                      McIndex,
  UINT8                      SncCluster,
  UINT32                     BaseCluster1,
  UINT32                     BaseCluster2,
  MC_TECH_TYPE               McTechType
  )
{
  UINT8   Idx;

  if (McTechType == MC_TECH_DDR) {
    SetM2mTopologySnc (Socket, McIndex, SncCluster, BaseCluster1, BaseCluster2);
  } else if (McTechType == MC_TECH_HBM) {
    for (Idx = 0; Idx < 4; Idx++) {
      // each HBMIO has 4 Mesh2Mem for HBM, and M2Mem index for DDR is 0~3, M2Mem index for
      // HBM starts from 4
      SetM2mTopologySnc (Socket, 4 + McIndex * 4 + Idx, SncCluster, BaseCluster1, BaseCluster2);
    }
  }
}

/**
  Program SNC topology in M2Mesh
  ...
  )
{
  ...
  MaxInstance = 0;
  if (McTechType == MC_TECH_DDR) {
    MaxInstance = GetMaxImc ();
  } else if (McTechType == MC_TECH_HBM) {
    MaxInstance = GetMaxIoHbm ();
  }
  switch (MemInfo->SncInfo[Socket].NumOfCluster) {
    case 1:
    for (Ctr = KTI_MC0; Ctr < MaxInstance; Ctr++) {
      ProgramSncInM2MemPerMc (Socket, Ctr, SNC_CLUSTER_0, 0, MemInfo->SncInfo[Socket].NumOfChaPerCluster, McTechType);
    }
    break;

    case 2:
    for (Ctr = KTI_MC0; Ctr < MaxInstance / 2; Ctr++) {
      ProgramSncInM2MemPerMc (Socket, Ctr, SNC_CLUSTER_0, 0, (KTIVAR->CpuInfo[Socket].TotCha / 2) - 1, McTechType);
    }
    for (Ctr = MaxInstance / 2; Ctr < MaxInstance; Ctr++) {
      ProgramSncInM2MemPerMc (Socket, Ctr, SNC_CLUSTER_1, KTIVAR->CpuInfo[Socket].TotCha / 2, KTIVAR->CpuInfo[Socket].TotCha - 1, McTechType);
    }
    break;

    case 4:
    ProgramSncInM2MemPerMc (Socket, KTI_MC0, SNC_CLUSTER_0, 0, MemInfo->SncInfo[Socket].NumOfChaPerCluster - 1, McTechType);
    ProgramSncInM2MemPerMc (Socket, KTI_MC1, SNC_CLUSTER_1, MemInfo->SncInfo[Socket].NumOfChaPerCluster, MemInfo->SncInfo[Socket].NumOfChaPerCluster * 2 - 1, McTechType);
    ProgramSncInM2MemPerMc (Socket, KTI_MC2, SNC_CLUSTER_2, MemInfo->SncInfo[Socket].NumOfChaPerCluster * 2, MemInfo->SncInfo[Socket].NumOfChaPerCluster * 3 - 1, McTechType);
    ProgramSncInM2MemPerMc (Socket, KTI_MC3, SNC_CLUSTER_3, MemInfo->SncInfo[Socket].NumOfChaPerCluster * 3, KTIVAR->CpuInfo[Socket].TotCha - 1, McTechType);
    break;
  ...
  )
{
  ...
  for (Socket = 0; Socket < MAX_SOCKET; Socket++) {
    if (!SocketPresent (Socket) || (MemInfo->SncInfo[Socket].NumOfMcEnabled == 0)) {
      continue;
    }
  ...
  KtiPrefetchWa (KtiInternalGlobal, MemInfo);
  ...
  for (Socket = 0; Socket < MAX_SOCKET; Socket++) {
    ...
    if (SocketPresent (Socket)) {
      if (MemInfo->SncInfo[Socket].NumOfMcEnabled == 0) {
        continue;
      }
        ...
        if (IsPostResetPowerGood && GetSysResetRequired () != 0) {
          ...
          if (KTIVAR->OutSncPrefetchEn && (Status == KTI_SUCCESS)) {         // normal warm reset path, i.e. no snc failure detection during warm reset
            ...
            for (Cluster = 0; Cluster < MemInfo->SncInfo[Socket].NumOfCluster; Cluster++) {
              ...
              MemInfo->McInfo[Socket][Cluster].PrefLogic2PhyChl = H0ChRouteTable & FLD_MASK (MemInfo->Ways[Socket][Cluster] * 2, 0);
            ...
            if (MaxIMC > 0 && MaxIMC <= 4) {
              ProgramSncInM2Mem (KtiInternalGlobal, MemInfo, Socket, MC_TECH_DDR);
            } else {
              KtiDebugPrintInfo0 ((KTI_DEBUG_INFO0, "\nWARN: Cannot find proper MaxIMC configuration!!!!\n\n"));
            }
            if (KTIVAR->HbmSku) {
              MaxHbmio = GetMaxIoHbm ();
              if (MaxHbmio > 0 && MaxHbmio <= 4) {
                ProgramSncInM2Mem (KtiInternalGlobal, MemInfo, Socket, MC_TECH_HBM);
              } else {
                KtiDebugPrintInfo0 ((KTI_DEBUG_INFO0, "\nWARN: Cannot find proper HBM configuration!!!!\n\n"));
              }
            }
    ...
    if (SocketPresent (Socket)) {
      if ((!IsPostResetPowerGood) || (GetSysResetRequired () == 0)) {
        if (AllPrefetchToDisable) {
          if ((KTIVAR->OutKtiPrefetch == KTI_ENABLE) || (KTIVAR->OutXptPrefetch == KTI_ENABLE) || (KTIVAR->OutXptRemotePrefetch == KTI_ENABLE)) {
            //
