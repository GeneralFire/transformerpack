  ...
  )
{
  ...
  if (EFI_ERROR (Status)) {
    Status = gBS->LocateProtocol (&gHeciControlProtocolGuid, NULL, &pHeciControl);
    if (EFI_ERROR (Status)) {
      DEBUG ((DEBUG_ERROR, "[SPS] ERROR: Cannot locate HECI protocol (status: %r)\n", Status));
      return EFI_DEVICE_ERROR;
    }
  }
  Status = gBS->AllocatePool (EfiBootServicesData, sizeof (ME_READ_STORAGE_SERVICES), (VOID**)&HeciMsg);
  if (EFI_ERROR (Status)) {
  ...
  *pStatus = 0;
  gBS->SetMem (HeciMsg, sizeof (ME_READ_STORAGE_SERVICES), 0);
  ...
  if (pHeci != NULL) {
    ...
    if (pHeciControl != NULL) {
        Status = pHeciControl->HeciSendAndReceive (
                                 pHeciControl,
                                 HECI_DEFAULT_PCH_ID,
                                 NULL,
  ...
  )
{
  ...
  Status = gBS->AllocatePool (EfiBootServicesData, sizeof (ME_WRITE_STORAGE_SERVICES), (VOID**)&HeciMsg);
  if (EFI_ERROR (Status)) {
    ASSERT_EFI_ERROR (Status);
    DEBUG ((DEBUG_ERROR, "[SPS] ERROR: Cannot allocate memory (status: %r)\n", Status));
    return Status;
  ...
  *pStatus = 0;
  gBS->SetMem (HeciMsg, sizeof (ME_WRITE_STORAGE_SERVICES), 0);
