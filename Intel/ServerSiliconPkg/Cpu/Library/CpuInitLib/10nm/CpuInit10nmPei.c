  ...
  )
{
  ...
  if (resetNeeded != POST_RESET_NO_RESET) {
    if (PcdGetBool (PcdPmcStrapSetVdmSupported)) {
      ...
      //
      if (SmtCapable) {
        UpdatePmcSetStrapMsgSmtDisable ((UINT8) ((NvRamData->DesiredCoresCfg2Csr[socket] >> 30) & 0x1), NULL);
      }
      UpdatePmcSetStrapMsgCoreOffMask (DesiredCoresCsr.Data32.Low, DesiredCoresCsr.Data32.High, NULL);
    }
  }
  ...
  )
{
  ...
  UINT32                                MailboxData;
  UINT8                                 Socket;
  B2P_TSC_RESET_ENABLE_INTERFACE_STRUCT TscB2pCmd;

  if (!GetCpuPreMemPolicy ()->TscResetEnable)  {
    return;
  }
  RcDebugPrint (SDBG_DEFAULT, "Send B2p MAILBOX_BIOS_CMD_TSC_RESET_ENABLE!\n");

  //
  // Enable TSC reset for warm reboot
  //
  MailboxData = 0;
  TscB2pCmd.Data = 0;
  TscB2pCmd.Bits.Command = MAILBOX_BIOS_CMD_TSC_RESET_ENABLE;
  TscB2pCmd.Bits.ResetTscOnWarmReset = 1;
  ...
  for (Socket = 0; Socket < MAX_SOCKET; Socket++) {
    if (SocketPresent(Socket)) {
      ...
      if (PcodeMailBoxStatus != MAILBOX_BIOS_CC_NO_ERROR) {
        RcDebugPrint (SDBG_DEFAULT, "Send B2p MAILBOX_BIOS_CMD_TSC_RESET_ENABLE Failed!\n");
      }
    }
  }
}
