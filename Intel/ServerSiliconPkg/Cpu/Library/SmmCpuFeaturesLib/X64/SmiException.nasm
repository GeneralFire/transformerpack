...
extern   ASM_PFX(gSmmFeatureEnable5LevelPaging)
...
gSmiExceptionCr3 EQU SmiExceptionCr3Patch - 4
...
ASM_PFX(PMIdtHandlerMachineCheck):
  ...
  SmiExceptionCr3Patch:
    mov     cr3, rax

    mov     eax, 0x668                  ; as cr4.PGE is not set here, refresh cr3
    mov     rbx, ASM_PFX(gSmmFeatureEnable5LevelPaging)
    mov     cl, byte [rbx]
    cmp     cl, 0
...
LongMode:                               ; long mode starts here
    mov     rax, ASM_PFX(gSmmFeatureSmiHandlerIdtr) ; load long mode IDT table to handle the exception
