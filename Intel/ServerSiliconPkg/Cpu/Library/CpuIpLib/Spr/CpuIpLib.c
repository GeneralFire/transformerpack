  ...
  )
{
  ...
  B2P_READ_PM_CONFIG_OUTPUT_STRUCT             MailBoxData;

  if (((B2P_READ_PM_CONFIG_OUTPUT_STRUCT*)&SstCpSystemStatus)->Bits.SstCpCapability == 1) {
    if (SstCpSetting != ((B2P_READ_PM_CONFIG_OUTPUT_STRUCT*)&SstCpSystemStatus)->Bits.SstCpState) {
      WritePmCfgCmdB2p.Data = 0;
      WritePmCfgCmdB2p.Bits.command = MAILBOX_BIOS_CMD_WRITE_PM_CONFIG;
      WritePmCfgCmdB2p.Bits.SubCommand = MAILBOX_BIOS_CMD_RW_PM_CONFIG_INTERFACE_SUB_COMMAND_PM_FEATURE;

      MailBoxData.Data = 0;
      MailBoxData.Bits.SstCpState = SstCpSetting;
