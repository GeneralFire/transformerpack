...
{
  return PcdGetBool(PcdSerialPortEnable);
}
  ...
  )
{
  ...
  } else if ((SioExist & (ASPEED_EXIST | PC8374_EXIST)) == (ASPEED_EXIST | PC8374_EXIST)) {
    // Both Aspeed and PC8374, default use Aspeed
    SioEnable = ASPEED_EXIST;
  } else {
    SioEnable = SioExist;
  }
