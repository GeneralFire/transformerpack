...
**/
VOID DetectPcpsInfo(VOID)
{
  UINT8                   Socket;
  UINT32                  CpuPCPSInfo = 0;
  CAPID4_PCU_FUN3_STRUCT  Capid4Csr;
  UINT32                 SmtDisable = 0;
  CAPID0_PCU_FUN3_STRUCT  CapId0Csr;

  for (Socket = 0; Socket < MAX_SOCKET; Socket++) {
    if (mIioUds->IioUdsPtr->PlatformData.CpuQpiInfo[Socket].Valid) {
      Capid4Csr.Data = mCpuCsrAccess->ReadCpuCsr(Socket, 0, CAPID4_PCU_FUN3_REG);
      SmtDisable = PcuGetDesiredCoreSmtDis (Socket);

      CapId0Csr.Data = mCpuCsrAccess->ReadCpuCsr(Socket, 0, CAPID0_PCU_FUN3_REG);

      if (!SmtDisable && !CapId0Csr.Bits.ht_dis) {
  ...
  )
{
  ...
  if (((CPUID_THERMAL_POWER_MANAGEMENT_EAX*)&CpuidRegisters.RegEax)->Bits.TurboBoostTechnology != 0) {
    ...
    if (MiscEnable.Bits.TurboModeDisable == 0) {
      //
  ...
  )
{
  ...
  DEBUG ((EFI_D_VERBOSE, "SkipExternalSbOpcodes start\n"));
  ...
  DEBUG((EFI_D_ERROR, "ExternSbExpected: %d, ExternSbFound: %d\n"));

  ASSERT((ExternSbFound % ExternSbExpected) == 0);

  DEBUG((EFI_D_VERBOSE, "SkipExternalSbOpcodes end\n"));
  ...
  )
{
  ...
  DEBUG (( EFI_D_INFO, "Patching SSDT PatchCpuPmSsdtTable\n" ));
  ...
  )
{
  ...
  DEBUG (( EFI_D_INFO, "Patching SSDT PatchOem1SsdtTable\n" ));
  ...
  if( (EFI_ERROR (Status)) || (CpuMiscData == NULL ) ){
    DEBUG ((EFI_D_WARN, " PatchGv3SsdtTable - EIST info for BSP index not found \n"));
    return Status;
  }
  ...
  for (SsdtPointer = CurrPtr; SsdtPointer <= EndPtr; ++SsdtPointer) {
    ...
    switch (Signature) {
      case SIGNATURE_32 ('_', 'S', 'B', '_'):
        ...
        ++CpuFixes;
        CpuMiscData = NULL;
        //DEBUG((EFI_D_ERROR, "\n:: in PatchOem1SsdtTable, CpuIndex:ThreadIndex:CpuFixes = 0x%x 0x%x 0x%x  (%d)\n", CpuIndex, ThreadIndex, CpuFixes, CpuFixes));
        LocateCpuEistInfo (0, &CpuMiscData);  //use CPU0 for update NPSS and SPSS
        break;
      ...
      case SIGNATURE_32 ('G', 'P', 'S', 'S'):
        ...
        AdjustSize  = PssTable->NumEntries * sizeof (EFI_PSS_PACKAGE);
        AdjustSize -= (UINT32)(CpuMiscData->GreaterNumberOfPStates * sizeof (EFI_PSS_PACKAGE));
        PackageSize     = (PssTable->Size & 0xF) + ((PssTable->Size & 0xFF00) >> 4);
        NewPackageSize  = PackageSize - AdjustSize;
        PssTable->Size  = (UINT16) ((NewPackageSize & 0xF) + ((NewPackageSize & 0x0FF0) << 4));
  ...
  )
{
  ...
  DEBUG ((EFI_D_INFO, "Patching SSDT PatchOem3SsdtTable\n" ));
