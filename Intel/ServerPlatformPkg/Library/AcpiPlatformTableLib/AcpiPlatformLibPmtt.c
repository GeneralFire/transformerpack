  ...
  )
{
  ...
  MaxImc = GetMaxImc ();
  MaxChPerImc = GetNumChannelPerMc ();
  ...
  for (Socket = 0; Socket < MAX_SOCKET; Socket++) {
    ...
    PmttTable->Socket[Socket].Type      = EFI_ACPI_TOP_LEVEL_SOCKET;
    ...
    DieCnt =   GetAcpiDieCount(Socket);
    for (Die = 0; Die < DieCnt; Die++) {

      PmttTable->Socket[Socket].Die[Die].Type = EFI_ACPI_TOP_LEVEL_VENDOR_SPECIFIC_DEVICE;
      ...
      for (Imc = 0; Imc < MaxImc; Imc++) {
        ...
        PmttTable->Socket[Socket].Die[Die].Imc[Imc].Type = EFI_ACPI_TOP_LEVEL_IMC;
        ...
        for (Channel = 0; Channel < MaxChPerImc; Channel++) {
          ...
          PmttTable->Socket[Socket].Die[Die].Imc[Imc].Channel[Channel].Type = EFI_ACPI_TOP_LEVEL_VENDOR_SPECIFIC_DEVICE;
          ...
          for (Dimm = 0; Dimm < MAX_DIMM; Dimm++) {
            ...
            //
            PmttTable->Socket[Socket].Die[Die].Imc[Imc].Channel[Channel].Slot[Dimm].Type = EFI_ACPI_TOP_LEVEL_VENDOR_SPECIFIC_DEVICE;
