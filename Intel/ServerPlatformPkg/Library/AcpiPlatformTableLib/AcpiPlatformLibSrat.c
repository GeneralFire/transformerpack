...
VOID PrintThreadIdMsrValueTable ();
  ...
  )
{
  ...
  if (!IsMemTypeVolatile (MemType)) {
    //
    // Persistent memory nodes are just pending behinds volatile memories
    //
    ...
    if (SncEnabled == 0) {
      ...
      if (MaxEnabledImc <= SncNumOfCluster) {
        DomainId = (SocketId * SncNumOfCluster) + (UINT32)FirstImc;
      } else {
        if (IsCpuAndRevision (CPU_SPRSP, REV_ALL) && (GetChopType (0) == TypeHcc)) {
          //
          // SPR HCC SKU is north-south partition when system is in SNC2:
          // - MC0 & MC2 are in cluster 0
          // - MC1 & MC3 are in cluster 1
          //
          DomainId = (SocketId * SncNumOfCluster) + ((UINT32) FirstImc) % SncNumOfCluster;
        } else {
          DomainId = (SocketId * SncNumOfCluster) + ((UINT32) FirstImc) / SncNumOfCluster;
        }
      }
    ...
    if (MemMode == VOL_MEM_MODE_MIX_1LM2LM) {
      ...
      //
      DomainId += (UINT32)((IsMemType2lm(MemType)?1:0) * NumSockets * SncNumOfCluster);
    }
  }

  DEBUG((EFI_D_INFO, "SocketId = %d, SncEnabled = %d, SncNumOfCluster = %d, ImcInterBitmap = %d, MemType = %d, MemMode = %d, DomainId = %d\n",
           SocketId, SncEnabled, SncNumOfCluster,
           ImcInterBitmap, MemType, MemMode, DomainId));
  ...
  )
{
  ...
  DEBUG((EFI_D_INFO, "Idx  Base              Length           Proximity Flags\n"));
  //
  ...
  for (Pass = 1; Pass < 3; Pass++ ) {
    ...
    for (Index = 0; Index < mSystemMemoryMap->numberEntries; Index++ ) {
      ...
      //
      // Skip any memory region marked reserved or CXL memory
      //
      if (IsMemTypeReserved (mSystemMemoryMap->Element[Index].Type) || IsMemTypeCxl (mSystemMemoryMap->Element[Index].Type)) {
        continue;
      }
      ...
      if ((Pass == 2) || IsMemTypeVolatile (mSystemMemoryMap->Element[Index].Type)) {
        ...
        SratTable->Memory[TableIndex].ProximityDomain = ProximityDomainOf (
        ...
        if (IsMemTypeDdrt(mSystemMemoryMap->Element[Index].Type)) {
          SratTable->Memory[TableIndex].Flags |= EFI_ACPI_6_2_MEMORY_NONVOLATILE;
        }

        if (Pass == 2) {
          DEBUG((EFI_D_INFO, "%3d  %08x%08x, %08x%08x %2x         %x\n",
                 TableIndex,
  ...
  )
{
  ...
  if (mSystemMemoryMap != NULL) {
    ...
    if (mSocketMemoryConfiguration.SratMemoryHotPlug) {
      DEBUG(( EFI_D_INFO, "SRAT: Updating SRAT hotplug memory information!\n" ));
      for (NodeId = 0; NodeId < MC_MAX_NODE; NodeId++) {
        ...
        DEBUG ((EFI_D_INFO, "%3d  %08x%08x  %08x%08x %x %x\n", NewIndex,
  ...
  )
{
  ...
  if (SncEnabled) {
    ...
    if (!Is14nmCpu && !CollocatedChaIdPresent) {
      DEBUG ((EFI_D_WARN, "ERROR: Collocated CHA ID is not found! Correct SNC Proximity Domain programming not guaranteed.\n"));
    }
  }
  ...
  for(ThreadIndex = 0; ThreadIndex < MAX_CPU_NUM; ThreadIndex++) {
    ...
    if (!SncEnabled) {
      mCpuThreadIdMsrTable[ThreadIndex].SNCProximityDomain = SocketIndex;
    } else {
      NumOfChaPerCluster = mPackageChaTotal[SocketIndex]/SncNumOfCluster;
      if (CollocatedChaIdPresent) {
        mCpuThreadIdMsrTable[ThreadIndex].SNCProximityDomain = (UINT32)((SocketIndex * SncNumOfCluster) + (mCpuThreadIdMsrTable[ThreadIndex].CollocatedChaId / (NumOfChaPerCluster)));
      } else {
        mCpuThreadIdMsrTable[ThreadIndex].SNCProximityDomain = (UINT32)((SocketIndex * SncNumOfCluster) + (mCpuThreadIdMsrTable[ThreadIndex].ThreadIdValue / (NumOfChaPerCluster*NumberOfThreads)));
      }
      DEBUG ((EFI_D_INFO, "ThreadIndex=%x, SncNumOfCluster=%x, NumOfChaPerCluster=%x, SNCProximityDomain=%x\n", ThreadIndex, SncNumOfCluster, NumOfChaPerCluster, mCpuThreadIdMsrTable[ThreadIndex].SNCProximityDomain));
      DEBUG ((EFI_D_INFO, "ThreadIdValue=%x\n", mCpuThreadIdMsrTable[ThreadIndex].ThreadIdValue));
    }
  }
  PrintThreadIdMsrValueTable();
}
  ...
  )
{
  ...
  DEBUG((EFI_D_INFO, "Socket\t Thread#\t ApicId\t ThreadIdMsrId\t CollocatedChaId\t SNC2ProximityDomain\n"));
  for(ThreadIndex = 0; ThreadIndex < MAX_CPU_NUM; ThreadIndex++){
    if (mCpuThreadIdMsrTable[ThreadIndex].ThreadIdValue != 0xff) {
      SocketIndex = mCpuConfigLibConfigContextBuffer->CollectedDataBuffer[ThreadIndex].ProcessorLocation.Package;
      DEBUG((EFI_D_INFO, "%d\t 0x%x\t 0x%x\t 0x%x\t %d\t %d\n", SocketIndex, ThreadIndex, mCpuThreadIdMsrTable[ThreadIndex].ApicId,
        mCpuThreadIdMsrTable[ThreadIndex].ThreadIdValue, mCpuThreadIdMsrTable[ThreadIndex].CollocatedChaId, mCpuThreadIdMsrTable[ThreadIndex].SNCProximityDomain));
    }
  }
}
...
--*/
{
  ...
  DEBUG((EFI_D_ERROR, "APICID not found in CpuThreadIdMsrValueTable\n"));
  ASSERT(FALSE);
  ThreadIndex--;
  return mCpuThreadIdMsrTable[ThreadIndex].SNCProximityDomain;
}
  ...
  )
{
  ...
  if (Table->Header.Signature != EFI_ACPI_6_2_SYSTEM_RESOURCE_AFFINITY_TABLE_SIGNATURE) {
    DEBUG((EFI_D_ERROR, "[SRAT] Not SRAT table, skip!\n"));
    return;
  }

  TotalLength = Table->Header.Length;

  DEBUG((EFI_D_INFO, "----------------------------[SRAT Table] --------------------\n"));
  DEBUG((EFI_D_INFO, "----Header-----\n"));
  DEBUG((EFI_D_INFO, "Length          :  %d\n", Table->Header.Length));
  DEBUG((EFI_D_INFO, "Revision        :  %d\n", Table->Header.Revision));
  DEBUG((EFI_D_INFO, "Checksum        :  %2X\n", Table->Header.Checksum));
  // DEBUG((EFI_D_INFO, "OemId           :  %d\n", Table->Header.OemId));
  DEBUG((EFI_D_INFO, "OemTableId      :  %X\n", Table->Header.OemTableId));
  DEBUG((EFI_D_INFO, "OemRevision     :  %d\n", Table->Header.OemRevision));
  DEBUG((EFI_D_INFO, "CreatorId       :  %X\n", Table->Header.CreatorId));
  DEBUG((EFI_D_INFO, "CreatorRevision :  %X\n", Table->Header.CreatorRevision));
  DEBUG((EFI_D_INFO, "\n"));
  ...
  while (TempPtr < ((UINT8 *)Table + TotalLength)){
    ...
    if (SubType->Type == EFI_ACPI_6_2_PROCESSOR_LOCAL_APIC_SAPIC_AFFINITY) {
      ApicType = SubType;
      DEBUG((EFI_D_INFO, "APIC Type            :  %d\n", ApicType->Type));
      DEBUG((EFI_D_INFO, "Length               :  %d\n", ApicType->Length));
      DEBUG((EFI_D_INFO, "ProximityDomain7To0  :  %d\n", ApicType->ProximityDomain7To0));
      DEBUG((EFI_D_INFO, "ApicId               :  %X\n", ApicType->ApicId));
      DEBUG((EFI_D_INFO, "Flags                :  %X\n", ApicType->Flags));
      DEBUG((EFI_D_INFO, "LocalSapicEid        :  %d\n", ApicType->LocalSapicEid));
      DEBUG((EFI_D_INFO, "ProximityDomain31To8 :  %d\n", (*(UINT32 *)ApicType->ProximityDomain31To8) & 0xFFFFFF));
      DEBUG((EFI_D_INFO, "ClockDomain          :  %d\n", ApicType->ClockDomain));

    } else if (SubType->Type == EFI_ACPI_6_2_MEMORY_AFFINITY) {
      MemType = (EFI_ACPI_6_2_MEMORY_AFFINITY_STRUCTURE *)SubType;
      DEBUG((EFI_D_INFO, "Mem Type        :  %d\n", MemType->Type));
      DEBUG((EFI_D_INFO, "Length          :  %d\n", MemType->Length));
      DEBUG((EFI_D_INFO, "ProximityDomain :  %d\n", MemType->ProximityDomain));
      DEBUG((EFI_D_INFO, "AddressBaseLow  :  %X\n", MemType->AddressBaseLow));
      DEBUG((EFI_D_INFO, "AddressBaseHigh :  %X\n", MemType->AddressBaseHigh));
      DEBUG((EFI_D_INFO, "LengthLow       :  %X\n", MemType->LengthLow));
      DEBUG((EFI_D_INFO, "LengthHigh      :  %X\n", MemType->LengthHigh));
      DEBUG((EFI_D_INFO, "Flags           :  %X\n", MemType->Flags));
      ...
      DEBUG((EFI_D_INFO, "x2APIC Type     :  %d\n", x2ApicType->Type));
      DEBUG((EFI_D_INFO, "Length          :  %d\n", x2ApicType->Length));
      DEBUG((EFI_D_INFO, "ProximityDomain :  %d\n", x2ApicType->ProximityDomain));
      DEBUG((EFI_D_INFO, "X2ApicId        :  %X\n", x2ApicType->X2ApicId));
      DEBUG((EFI_D_INFO, "Flags           :  %X\n", x2ApicType->Flags));
      DEBUG((EFI_D_INFO, "ClockDomain     :  %d\n", x2ApicType->ClockDomain));

    } else {
      DEBUG((EFI_D_INFO, "Unknown Type : %d\n", SubType->Type));
      DEBUG((EFI_D_INFO, "Length : %d\n", SubType->Length));
    }
    if (SubType->Length < 10) {
      DEBUG((EFI_D_ERROR, "Error in Length %d, try previous length.\n", SubType->Length));
      TempPtr += TempLength;
    } else {
  ...
  )
{
  ...
  if (SratTable == NULL) {
    DEBUG ((EFI_D_ERROR, "InstallSratTable: allocate SRAT table failed \n"));
    return EFI_OUT_OF_RESOURCES;
  }
  ...
  if (SratTable->SratHeader == NULL) {
    DEBUG ((EFI_D_ERROR, "InstallSratTable: Create SratHeader has failed \n"));
    return EFI_OUT_OF_RESOURCES;
  }
