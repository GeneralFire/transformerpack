  ...
  )
{
  ...
  if (EFI_ERROR (Status)) {
    DEBUG ((EFI_D_ERROR, "[ACPI] ERROR: SortCpuLocalApicInTable failed: %r\n", Status));
    return Status;
  }
...
// APTIOV_SERVER_OVERRIDE_RC_END : To use PcdAcpiDefaultOemTableId
      ...
      if (!EFI_ERROR(Status)) {
        DEBUG((EFI_D_INFO, "[ACPI] Platform DSDT Fixup table found\n"));
        DEBUG((EFI_D_INFO, "[ACPI] Platform SRP: Using %a DSDT\n", OemSkuAcpiName));
        return EFI_SUCCESS;
      } else {
        DEBUG((EFI_D_ERROR, "[ACPI] Platform DSDT Fixup table not found.\n"));
  ...
  //
  // Hooks for checking additional platform ACPI Table is active or not. For example: MIGT will return EFI_NOT_FOUND in WhitleyPc.
  ...
  )
{
  ...
  *Version = EFI_ACPI_TABLE_VERSION_2_0;
  ...
  default:
    //
    // Hooks for Platform only table. If the ACPI Table is Platform only, add this table in below hook. then continu to update other Table.
    // Eg: SPMI Table is WhitleyPc only, it will invoke PatchSpmiAcpiTable function in the below hook.
    //
