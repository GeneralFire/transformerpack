  ...
  )
{
  ...
  DEBUG ((EFI_D_INFO, "UbaConfigDatabasedxeEntry: get first hob!\n"));
  ...
  )
{
  ...
  DEBUG ((EFI_D_INFO, "UbaConfigDatabasedxeEntry: after allocate Memory!\n"));
...
// APTIOV_SERVER_OVERRIDE_RC_START : Call InitSku before publishing protocol.
#if 0  
  DEBUG ((EFI_D_INFO, "UbaConfigDatabasedxeEntry: before get data from hob!\n"));
  // Init sku dxe and get configuration data from hob passed by PEIM.
  Status = InternalGetConfigDataFromHob (&UbaDxePrivate->UbaCfgDbProtocol);
  ASSERT_EFI_ERROR (Status);
#endif
