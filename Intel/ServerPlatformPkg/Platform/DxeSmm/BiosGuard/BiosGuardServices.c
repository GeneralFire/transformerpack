...
static EFI_HANDLE                   mPchIoTrapHandleForRegistering;
...
BOOLEAN                             PchBiosLockEnabled = FALSE;
/**
  ...
  )
{
  ...
  ASSERT (mPchIoTrapHandleForRegistering != NULL);
  if (mPchIoTrapHandleForRegistering == NULL) {
    DEBUG ((DEBUG_ERROR, "%a() was not called via trap handler!\n", __FUNCTION__));
    return;
  }

  //
  // Attempt to un register this i/o trap since we should only do this initialization
  // once before END_OF_DXE
  //
  Status = mPchIoTrap->UnRegister (mPchIoTrap, mPchIoTrapHandleForRegistering);
  ASSERT_EFI_ERROR (Status); // not much we can do if this fails, so just continue for release version of code
  if (Status == EFI_SUCCESS) {
    mPchIoTrapHandleForRegistering = NULL;
  }
...
/**
  ...
  @param  Protocol   (not used) Points to the protocol's unique
                     identifier.
  @param  Interface  (not used) Points to the interface
                     instance.
  @param  Handle     (not used) The handle on which the
  ...
  )
{
  ...
  ASSERT (mPchIoTrapHandleForRegistering == NULL);
  if (mPchIoTrapHandleForRegistering != NULL) {
    DEBUG ((DEBUG_ERROR, "%a() called multiple times!\n", __FUNCTION__));
  ...
  Status = mPchIoTrap->Register (
                        ...
                        &PchIoTrapContext,
                        &mPchIoTrapHandleForRegistering
                        );
  ASSERT_EFI_ERROR (Status);
...
// APTIOV_SERVER_OVERRIDE_RC_END : Changes for AMI BIOS GUARD
  ...
  if (PlatformInfoMsr.Bits.PfatEnable) {
    ...
    if (PlatFrmwCtrlMsr.Bits.PfatEnable) {
      ...
      if (mBiosGuardUpdatePackageInTseg == FALSE) {
        ...
        if(EFI_ERROR(Status)) {
          ...
          ASSERT_EFI_ERROR (Status);
          if (EFI_ERROR (Status)) {
            DEBUG ((DEBUG_ERROR, "SmmRegisterProtocolNotify() failed in %a, could not arrange for trap handler\n",
              __FUNCTION__));
            goto err_exit;
          }
        } else {
          //
          // Register IO Trap for the Tools interface init, leverage the notify function
          // so that we don't repeat/duplicate code.
          //
          Status = BiosGuardRegisterToolsIoTrap (NULL, NULL, NULL);
          ASSERT_EFI_ERROR (Status);
          if (EFI_ERROR (Status)) {
            goto err_exit;
          }
...
#ifdef CDF_SC_FLAG
  PCH_CONFIG_HOB                *mPchConfigHob = NULL;
#else
...
// APTIOV_SERVER_OVERRIDE_RC_END
  ...
  if (GuidHob != NULL) {
#ifdef CDF_SC_FLAG
    mPchConfigHob = GET_GUID_HOB_DATA (GuidHob);
    if (mPchConfigHob != NULL) {
      PchBiosLockEnabled = (BOOLEAN) mPchConfigHob->LockDown.BiosLock;
    }
#else
    mPchPolicyHob = GET_GUID_HOB_DATA (GuidHob);
    if (mPchPolicyHob != NULL) {
      PchBiosLockEnabled = (BOOLEAN)mPchPolicyHob->LockDownConfig.BiosLock;
    }
...
{
  BiosGuardInstance->TrigParam.Uint64 = AsmReadMsr64 (MSR_PLAT_FRMW_PROT_TRIG_PARAM);
  ...
  )
{
  ...
  BiosGuardInstance->TrigParam.Uint64 = ERR_LAUNCH_FAIL;
  ...
  if (!DisallowedUpdate) {
    ...
    //
    // APTIOV_SERVER_OVERRIDE_RC_START: Hob data will be lost after ExitBootService.
    //if (PchBiosLockEnabled == TRUE) {
      MmioOr16 (SpiBaseAddress + R_SPI_CFG_BC, B_SPI_CFG_BC_SYNC_SS);
    //  }
    // APTIOV_SERVER_OVERRIDE_RC_END: Hob data will be lost after ExitBootService.
    ...
    BiosGuardStatus = (UINT16)BiosGuardInstance->TrigParam.Bits.Status;
  ...
  if (EFI_ERROR (Status)) {
    BiosGuardAdditionalData = (UINT16)BiosGuardInstance->TrigParam.Bits.Data;
    BiosGuardTerminalLine = (UINT16)BiosGuardInstance->TrigParam.Bits.Terminal;
    BiosGuardSE = (UINT8)BiosGuardInstance->TrigParam.Bits.SE;

    DEBUG ((EFI_D_INFO, "BIOS Guard Status          = 0x%X\n", BiosGuardStatus));
    DEBUG ((EFI_D_INFO, "BIOS Guard Additional Data = 0x%X\n", BiosGuardAdditionalData));
    DEBUG ((EFI_D_INFO, "BIOS Guard Terminal Line   = 0x%X\n", BiosGuardTerminalLine));
    DEBUG ((EFI_D_INFO, "BIOS Guard SE              = 0x%X\n", BiosGuardSE));
    DEBUG ((EFI_D_INFO, "BIOS Guard Disallowed Upd  = %s\n", (DisallowedUpdate)?L"TRUE":L"FALSE"));
  }

  mBiosGuardFullStatus = BiosGuardInstance->TrigParam.Uint64;
  ...
  if (IsSimicsEnvironment()) {
    DEBUG ((DEBUG_INFO, "BiosGuardProtocolExecute Complete\n"));  // Exposing for Simics WA
  }
