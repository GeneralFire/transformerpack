...
//
{
  ...
  pSysHost = (PSYSHOST)mMemRas->pSysHost;
...
//
{
  ...
  if (mMemRas->MemoryNodeOnline == NULL || mMemRas->pSysHost == NULL) {
    DEBUG ((EFI_D_LOAD | EFI_D_ERROR, "\t\tERROR! Detect MemoryNodeOnline NULL pointer\n"));
    return ERROR_EXIT;
  }
    ...
    RequestedEvent = 0;
     ...
     for (NodeIndex = 0; NodeIndex < MC_MAX_NODE ; NodeIndex++) {
       ...
       DEBUG ((EFI_D_LOAD | EFI_D_INFO, "\tMultiple Socket/Riser Flow: N%d, enabled = %d,  systemMemInfo - RequiredMemorySize = %08x\n",
            ...
            pSysHost->var.mem.socket[socket].imc[mc].memSize));
     }
...
//
{
  ...
  // PM base address
  PmBase = MmioRead16 (MmPciBase (DEFAULT_PCI_BUS_NUMBER_PCH, PCI_DEVICE_NUMBER_PCH_PMC, PCI_FUNCTION_NUMBER_PCH_PMC) + PCH_PMC_ACPI_BASE);
    ...
    if (RetStatus != IN_PROGRESS) {
      ...
      if (RetStatus != STATUS_DONE) {
        ...
        if (Action & MEM_NODE_ON) {
          ...
          Action |= MEM_NODE_OFF;
          if (MemState[0] == EnumOnline) {
            MemRasError |= BIT0;
          }
          if (MemState[1] == EnumOnline) {
            MemRasError |= BIT1;
          }
          Branch = 0;
  ...
  if (mIsSoftSmiRequest) {
    ...
    // Check for Cpu Present
        if (!SocketPresent (*Socket)) {
          Status = NO_REQUEST;
          DEBUG ((EFI_D_LOAD | EFI_D_ERROR, "ERROR: Invalid socket for memory request S%dM%d\n", *Socket, *Branch));
        } else {
    ...
    if (mHpRequest[0] & 0xf) {
      ...
      if (Status == EFI_SUCCESS) {
        ...
        *Branch = 0;
        *Socket = i;
        SetSocketPresentBitMap (GetSocketPresentBitMap () | ((UINT32)1 << i));
...
//
{
  ...
  if(mRasMpLink->HostParmValid) {
    Socket = KTI_HOST_OUT_PTR->inComingSktId;
  }
  ...
  case EnumMemoryEntry:
    ...
    if (EFI_ERROR(CheckMrcSetupOption (EnumMemoryOnlineOfflineEntry))) {
      DEBUG ((EFI_D_LOAD | EFI_D_ERROR, "ERROR: Invalid MRC setup options\n"));
  ...
  case EnumMemoryExecuteRequest:
    ...
    if (Status != IN_PROGRESS) {
      ...
      if (Status != STATUS_DONE) {
        DEBUG ((EFI_D_LOAD | EFI_D_ERROR, "Error Abort!\n\n"));
        MemRasError = Status;
 ...
 if (mIsSoftSmiRequest) {
    ...
    if (Progress == NO_REQUEST) {
      ...
      if (i == MAX_SOCKET) {
        ...
        if (mHpRequest[0] == mHpRequest[3]) {
          // INVALID REQUEST!
          DEBUG ((EFI_D_LOAD | EFI_D_ERROR, "\tMemRasHandler::STATUS = NO_REQUEST\n"));
          return NO_REQUEST;
        }else {
          ...
          if (mHpRequest[2] != 0) { // attention present event occurs?
            for (i = 0; i < MC_MAX_NODE; i++) {
              ...
              if (i == MC_MAX_NODE) {
                DEBUG ((EFI_D_LOAD | EFI_D_WARN, "\tMemRasHandler::STATUS = NO_REQUEST\n"));
                return NO_REQUEST;
              }
            }
          }else {
