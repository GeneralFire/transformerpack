  ...
  )
{
  ...
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_ERROR, "Can not locate EfiIioUdsProtocol. Exiting without doing HpIox Initialization\n"));
    return Status;
  }

  IioUdsData = (IIO_UDS *)IioUds->IioUdsPtr;

  if (IioUdsData->SystemStatus.OutKtiCpuSktHotPlugEn != 1) {
...
)
{
  ...
  if((((ProcIohRasEventData >> 4 ) & 0x0F) ^ (UINT8)CpuCsrAccessVarPtr->socketPresentBitMap) != 0x00) {
    return TRUE;
  }
