...
//APTIOV_SERVER_OVERRIDE_START: PCI changes to solve PciTreeTraverse failure
//#include "../PciHostBridge/PciRootBridge.h"
//#include "../PciHostBridge/PciHostBridge.h"
#include <PciHostBridge.h>
//APTIOV_SERVER_OVERRIDE_END: PCI changes to solve PciTreeTraverse failure
...

{
  ...
  //
  XhciBar = MmioRead32 (MmPciBase (DEFAULT_PCI_BUS_NUMBER_PCH, PCI_DEVICE_NUMBER_PCH_XHCI, PCI_FUNCTION_NUMBER_PCH_XHCI) + R_XHCI_CFG_BAR0) & ~(0xF);
  ...
  if ((XhciBar & 0xFFFF0000) == 0) {
    ...
    //
    XhciBar = MmioRead32 (MmPciBase (DEFAULT_PCI_BUS_NUMBER_PCH, PCI_DEVICE_NUMBER_PCH_XHCI, PCI_FUNCTION_NUMBER_PCH_XHCI) + R_XHCI_CFG_BAR0 + 4);
    if (XhciBar == 0) {
      return;
    }
  }
...
// Read the entire config header
    ...
    if (!EFI_ERROR (Status) && IS_PCI_BRIDGE(&Pci0)) {
      ...
      if (!EFI_ERROR (Status)) {
        ...
        IioPciHookBeforeEnumeration ((UINT8)RootBridgeIo->SegmentNumber, PciAddress.Bus, PciAddress.Device, PciAddress.Function, DidVid);
...
/**
  ...
  To find the Resources of Root bridge.

  @param RootBrg          -  Pointer to the Root Bridge Data.
  @param ResDsc           -  Resource Descriptor.

  @retval EFI_SUCCESS     -  The function completed successfully.
  @retval EFI_NOT_FOUND -    Resource Descriptor not found for the device.
...
//APTIOV_SERVER_OVERRIDE_START: PCI changes to solve PciTreeTraverse failure
  UINTN                                             i;
  PCI_HOST_BRG_DATA                                 *HostBridgePtr;
  PCI_ROOT_BRG_DATA                                 *RootBridgePtr;
  ASLR_QWORD_ASD                                    *res;
...
//APTIOV_SERVER_OVERRIDE_START: PCI changes to solve PciTreeTraverse failure
    ...
    ASSERT_EFI_ERROR (Status);
    if(HostBridgePtr->Signature == AMI_HB_SIG){
// Find RootBridge pointer from obtained HostBridgePtr.
        for(i=0; i< HostBridgePtr->RootBridgeCount; i++){
            RootBridgePtr = HostBridgePtr->RootBridges[i];
// Find the Root Bridge data for the Rootbridge pointer obtained.
            Status = FindRbBusData(RootBridgePtr, &res);
            ...
            for (Socket = 0; Socket < NELEMENTS (mPciPrivateData.PciRootBridgeIo); Socket++) {
              ...
              for (StackBit = 1, Stack = 0;
                ...
                if ((CpuCsrAccessVarPtr->stackPresentBitmap[Socket] & StackBit) &&
                               CpuCsrAccessVarPtr->StackBus[Socket][Stack] == res->_MIN) {
                  //
                  // This is the stack handled by this instance of root bridge IO protocol. Store it for future use.
                  //
                               mPciPrivateData.PciRootBridgeIo[Socket][Stack] = &RootBridgePtr->RbIoProtocol;
  ...
  )
{
  ...
  for (i = 0 ; i < MaxIIO ; i++) {
    if (mIioUds->IioUdsPtr->PlatformData.IIO_resource[i].Valid) {
      ...
      MaxPortNumberPerSocket = IioGlobalData->IioVar.IioOutData.MaxPciePortNumberPerSocket[i];
      for(Dev = 0; Dev < MaxPortNumberPerSocket; Dev++) {
