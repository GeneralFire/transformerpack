...
/**
  ...
  @retval EFI_UNSUPPORTED  - The ME doesn't run client FW.
**/
EFI_STATUS
EFIAPI
AsfTableEntryPoint (
  ...
  )
{
  ...
  SetupSize = sizeof (ME_RC_CONFIGURATION);
  Status = gRT->GetVariable (ME_RC_CONFIGURATION_NAME, &gEfiMeRcVariableGuid, NULL, &SetupSize, (VOID*) &MeSetup);
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_WARN, "Cannot get \"MeRcConfiguration\" variable\n"));
    return FALSE;
  }
