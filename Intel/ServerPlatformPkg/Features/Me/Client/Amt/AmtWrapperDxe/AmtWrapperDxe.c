  ...
  )
{
  ...
  if (GuidHob == NULL) {
    DEBUG ((DEBUG_WARN, "Cannot find MEBX Hob, remote Assistance not supported\n"));
    return FALSE;
  }

  Status = gBS->LocateProtocol (&gDxeAmtPolicyGuid, NULL, (VOID **) &AmtPolicy);
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_WARN, "Locate AmtPolicy fail, remote Assistance not supported\n"));
    return FALSE;
  }
  ...
  )
{
  ...
  *String = NULL;
  ...
  if (!IsManageabilitySupported () || AsfIsSolEnabled () || AsfIsKvmEnabled () || !IsBiosHotkeyEnabled ()) {
    //
  ...
  )
{
  ...
  if (!IsManageabilitySupported () || !IsBiosHotkeyEnabled () || AsfIsSolEnabled () || AsfIsKvmEnabled ()) {
    //
  ...
  )
{
  ...
  *KeyData = NULL;
  ...
  if (IsManageabilitySupported () && IsBiosHotkeyEnabled ()) {
...
/**
  ...
  @retval EFI_UNSUPPORTED       The chipset is unsupported by this driver or the ME doesn't run client FW.
