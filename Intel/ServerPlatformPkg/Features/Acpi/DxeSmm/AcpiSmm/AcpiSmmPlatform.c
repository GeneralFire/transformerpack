...
#endif
PCH_SETUP  gPchSetup; // PCH RC configuration in order to get StateAfterG3 setup option.
  ...
  )
{
  ...
  //
  mAcpiSmm.PchPmBase = PmcGetAcpiBase ();
  ...
  DEBUG ((DEBUG_INFO, "TPM ACPI Parameter Block Address: 0x%X\n", mAcpiParameter));
...
// APTIOV_SERVER_OVERRIDE_RC_START : Locate Pch RC configuration variable in order to get StateAfterG3 setup option.
  Status = GetSpecificConfigGuid (&gPchSetupVariableGuid, &gPchSetup);
  ASSERT_EFI_ERROR (Status);
...
**/
{
  ...
  if (gPchSetup.StateAfterG3 == 0xFF) {
    UINT8                 DataUint8;
  ...
  if ((DispatchHandle == mAcpiSmm.S1SleepEntryHandle) && (((EFI_SMM_SX_REGISTER_CONTEXT *)DispatchContext)->Type == SxS1)) {
    if(mSystemConfiguration.XhciWakeOnUsbEnabled == TRUE){
      Status = gSmst->SmmIo.Io.Read( &gSmst->SmmIo, SMM_IO_UINT32, mAcpiSmm.PchPmBase + R_ACPI_IO_GPE0_EN_127_96, 1, &Data32 );
      Data32 |= (B_ACPI_IO_GPE0_EN_127_96_USB_CON_DSX_EN | B_ACPI_IO_GPE0_EN_127_96_PME_B0 | B_ACPI_IO_GPE0_EN_127_96_PME);
      Status = gSmst->SmmIo.Io.Write( &gSmst->SmmIo, SMM_IO_UINT32, mAcpiSmm.PchPmBase + R_ACPI_IO_GPE0_EN_127_96, 1, &Data32 );
    }
    ...
    if (mSystemConfiguration.WakeOnLanSupport == 1) {
      ...
      //
      Status = gSmst->SmmIo.Io.Read( &gSmst->SmmIo, SMM_IO_UINT32, mAcpiSmm.PchPmBase + R_ACPI_IO_GPE0_EN_127_96, 1, &Data32 );
      Data32 |= (B_ACPI_IO_GPE0_EN_127_96_PME_B0 | B_ACPI_IO_GPE0_EN_127_96_PCI_EXP | B_ACPI_IO_GPE0_EN_127_96_PME);
      Status = gSmst->SmmIo.Io.Write( &gSmst->SmmIo, SMM_IO_UINT32, mAcpiSmm.PchPmBase + R_ACPI_IO_GPE0_EN_127_96, 1, &Data32 );
  ...
  if (((DispatchHandle == mAcpiSmm.S4SleepEntryHandle) && (((EFI_SMM_SX_REGISTER_CONTEXT *)DispatchContext)->Type == SxS4)) ||
    ...
    if (mSystemConfiguration.WakeOnRTCS4S5 == 1) {
      ...
      gSmst->SmmIo.Io.Read (
                        &gSmst->SmmIo,
                        SMM_IO_UINT16,
                        mAcpiSmm.PchPmBase + R_ACPI_IO_PM1_STS,
      ...
      gSmst->SmmIo.Io.Read (
                        &gSmst->SmmIo,
                        SMM_IO_UINT16,
                        mAcpiSmm.PchPmBase + R_ACPI_IO_PM1_EN,
    ...
    if(FixedPcdGetBool(PcdCrystalRidgeEnable)){
    ...
    if (mSystemConfiguration.WakeOnLanSupport == 1) {
      ...
      //
      // LBG specific, don't disable WOL triggered by PCIE Wake signal
      //
      PcieWakeDisable(FALSE);

      return EFI_SUCCESS;
    } else {
      //
      // LBG specific, disable the WOL triggered by PCIE Wake signal
      //
      PcieWakeDisable(TRUE);
    }
  }
  ...
  )
{
  ...
  //
  Data32 = IoRead32 (mAcpiSmm.PchPmBase + R_ACPI_IO_SMI_EN);
  S3BootScriptSaveIoWrite(S3BootScriptWidthUint32, (mAcpiSmm.PchPmBase + R_ACPI_IO_SMI_EN), 1, &Data32);
  ...
  Data16 = IoRead16 (mAcpiSmm.PchPmBase + TcoBaseAddr + R_TCO_IO_TCO1_CNT);

  S3BootScriptSaveIoWrite(S3BootScriptWidthUint16, (mAcpiSmm.PchPmBase + TcoBaseAddr + R_TCO_IO_TCO1_CNT), 1, &Data16);
  ...
  )
{
  ...
  DEBUG ((DEBUG_INFO, "PowerButtonCallback Entry\n"));
  ...
  gSmst->SmmIo.Io.Read (
                    ...
                    mAcpiSmm.PchPmBase + R_ACPI_IO_PM1_CNT,

                    1,
                    &Data32
                    );
  Data32 &= ~(B_ACPI_IO_PM1_CNT_SLP_TYP);
