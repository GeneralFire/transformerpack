...
Scope(\_SB.PC00) {
  ...
  Device (UA00) {
    ...
    Name (_ADR, ACPI_PCI_ADDR (PCI_DEVICE_NUMBER_PCH_SERIAL_IO_UART0, PCI_FUNCTION_NUMBER_PCH_SERIAL_IO_UART0))
  ...
  Device (UA01) {
    ...
    Name (_ADR, ACPI_PCI_ADDR (PCI_DEVICE_NUMBER_PCH_SERIAL_IO_UART1, PCI_FUNCTION_NUMBER_PCH_SERIAL_IO_UART1))
...
Scope(\_SB) {
    ...
    Method (UARB, 2, Serialized) {
      If (LOr (LEqual (Arg0, SERIAL_IO_UART_HIDDEN), LEqual (Arg0, SERIAL_IO_UART_COM))) {
        ...
        Name(BUF0,ResourceTemplate(){
          Memory32Fixed (ReadWrite, 0, 0xFF8, BFR1) //4096 - 8 bytes, allocate remaining 4088 for BAR0 (MMIO BAR0)
          Memory32Fixed (ReadWrite, 0, 0x1000, BFR2) // allocate additional 4096 for BAR1 (PCI CFG SPACE)
        })

        CreateDWordField (BUF0, BFR1._BAS, ADR1)
        CreateDWordField (BUF0, BFR2._BAS, ADR2)
  ...
  Method (UPS3, 2, Serialized) {
    If (LEqual (Arg0, UART_PG_ON)) {
      ...
      Store (0x3, PMEC)
      And(PMEC, 0xFF, PMEC) // perform a read to avoid ordering and noncoherency problems
    }
