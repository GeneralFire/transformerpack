...
Scope(\)
{
  ...
  //
   Method(IPCS, 7, Serialized){
    ...
    ADBG (Concatenate("SubCmdId: ", ToHexString(Arg0)))
  ...
  Method (SPCO, 2, Serialized){
    ...
    IPCS(V_PMC_PWRM_IPC_SRC_CLK_CMD, 0, 0x8, Local0, Local1, 0, 0)
  }
  ...
  Method (TCBS, 0) {
    If (LEqual(TCBV, 0)) {
      // PCR[DMI] + 2778h[15:5]
      // APTIOV_SERVER_OVERRIDE_RC_START: Macros are not reflected in AMI ASL building method included required header file 
      Store(PCRR (PID_DMI, 0x2778),Local0)
      And(Local0,0xFFE0,TCBV)
      // APTIOV_SERVER_OVERRIDE_RC_END: Macros are not reflected in AMI ASL building method included required header file 
    }
    Return (TCBV)
  }
