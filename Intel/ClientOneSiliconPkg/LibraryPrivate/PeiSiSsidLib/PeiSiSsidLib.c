  ...
  )
{
  ...
  for (PciDevNum = 0; PciDevNum <= PCI_MAX_DEVICE; PciDevNum++) {
    ...
    for (PciFuncNum = 0; PciFuncNum <= PCI_MAX_FUNC; PciFuncNum++) {
      ...
      //
      // Skip if the device is not present Or if device is PCI bridge
      //
      PciCfgBase = PCI_SEGMENT_LIB_ADDRESS (0, 0, PciDevNum, PciFuncNum, 0);
      if ((PciSegmentRead32 (PciCfgBase + PCI_VENDOR_ID_OFFSET) == 0xFFFFFFFF) || (PciSegmentRead8 (PciCfgBase + PCI_HEADER_TYPE_OFFSET) == HEADER_TYPE_PCI_TO_PCI_BRIDGE)) {
        continue;
      }
