  ...
  )
{
  ...
  DEBUG ((DEBUG_INFO, "PchPcieRpConfigurationLock()\n"));
  for (RpIndex = 0; RpIndex < GetPchMaxPciePortNum (); RpIndex++) {
    RpBase = PchPcieRpPciCfgBase (RpIndex);
    PciSegmentOr16 (RpBase + R_PCH_PCIE_CFG_PLKCTL, (UINT16)( B_PCH_PCIE_CFG_PLKCTL_PLCL | B_PCH_PCIE_CFG_PLKCTL_LLCL | B_PCH_PCIE_CFG_PLKCTL_TLCL | B_PCH_PCIE_CFG_PLKCTL_CL ));
  }
}
