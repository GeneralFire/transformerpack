  ...
  )
{
  ...
  if (((Length & (Length - 1)) != 0)  ||
      ((Address & (UINT16) ~B_LPC_CFG_GENX_DEC_ADDR) != 0) ||
      (Length > 256)) {
    return FALSE;
  }
  if (Address < 0x100) {
  ...
  )
{
  ...
  for (Index = 0; Index < PCH_LPC_GEN_IO_RANGE_MAX; Index++) {
    ...
    GenIoRangeList->Range[Index].BaseAddr = (Data32 & B_LPC_CFG_GENX_DEC_ADDR);
    GenIoRangeList->Range[Index].Length   = ((Data32 & B_LPC_CFG_GENX_DEC_ADDR_MASK) >> 16) + 4;
