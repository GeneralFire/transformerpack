  ...
  )
{
  ...
  HsioPreMemConfig->ChipsetInitMessage = FALSE;
  ...
  )
{
  ...
  //
  // Turning off SRCCLKOUTs is not supported by this IP
  //
  for (RpIndex = 0; RpIndex < GetPchMaxPcieClockNum (); RpIndex++) {
    ...
    PcieRpPreMemConfig->PcieClock[RpIndex].ClkReq = PCH_PCIE_NO_SUCH_CLOCK;
  }
}
...
**/
VOID
LoadPcieRpLinkEqPreMemConfigDefault (
  IN VOID          *ConfigBlockPointer
  )
{
  ENTERPRISE_PCIE_LINK_PREMEM_CONFIG *PcieRpLinkPreMemConfig;

  PcieRpLinkPreMemConfig = ConfigBlockPointer;

  DEBUG ((DEBUG_INFO, "PcieRpLinkEqPreMemConfig->Header.GuidHob.Name = %g\n", &PcieRpLinkPreMemConfig->Header.GuidHob.Name));
  DEBUG ((DEBUG_INFO, "PcieRpLinkEqPreMemConfig->Header.GuidHob.Header.HobLength = 0x%x\n", PcieRpLinkPreMemConfig->Header.GuidHob.Header.HobLength));
}
...
GLOBAL_REMOVE_IF_UNREFERENCED COMPONENT_BLOCK_ENTRY  mPchIpBlocksPreMem [] = {
  ...
  {&gPcieLinkPrememConfigGuid,   sizeof (ENTERPRISE_PCIE_LINK_PREMEM_CONFIG), PCIE_LINK_PREMEM_CONFIG_REVISION,   LoadPcieRpLinkEqPreMemConfigDefault}
};
