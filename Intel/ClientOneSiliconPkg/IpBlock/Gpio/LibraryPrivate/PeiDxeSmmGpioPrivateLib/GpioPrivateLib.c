...
/**
  This procedure will set pad termination for native GPIO
  ...
  @param[in]  PinMux              GPIO Native pin mux platform config.
                                  ...
                                  on multiple pads
  @param[in]  PadTermination      GPIO Pad Termination

  @retval EFI_SUCCESS             The function completed successfully
  @retval EFI_INVALID_PARAMETER   Invalid group or pad number
**/
EFI_STATUS
GpioSetElectricalConfigByFunction (
  IN UINT32  PadFunction,
  IN UINT32  PinMux,
  IN UINT32  PadTermination
  )
{
  ...
  return GpioSetPadElectricalConfig (GpioNativePad, PadTermination);
}
  ...
  )
{
  ...
  if ((IoStandbyState == GpioIosStateDefault) && (IoStandbyTerm == GpioIosTermDefault)) {
    return EFI_SUCCESS;
  }
  ...
  PadCfgOrMask = 0;
  PadCfgAndMask = (UINT32)~0;

  if (IoStandbyState != GpioIosStateDefault) {
    PadCfgOrMask |= (((IoStandbyState >> 1) << N_GPIO_PCR_STANDBY_STATE) & B_GPIO_PCR_STANDBY_STATE );
    PadCfgAndMask &= (UINT32)~B_GPIO_PCR_STANDBY_STATE;
  }

  if (IoStandbyTerm != GpioIosTermDefault) {
    PadCfgOrMask |= (((IoStandbyTerm >> 1) << N_GPIO_PCR_STANDBY_TERM) & B_GPIO_PCR_STANDBY_TERM);
    PadCfgAndMask &= (UINT32)~B_GPIO_PCR_STANDBY_TERM;
  }
...
**/
EFI_STATUS
GpioConfigurePadIoStandbyByFunction (
  IN UINT32  PadFunction,
  IN UINT32  PinMux
  )
{
  GPIO_NATIVE_PAD       GpioNativePad;
  EFI_STATUS            Status;
  GPIO_IOSTANDBY_STATE  StandbyState;
  GPIO_IOSTANDBY_TERM   StandbyTerm;
  ...
  Status = GpioGetFunctionIoStandbyConfig (PadFunction, &StandbyState, &StandbyTerm);
  //
  ...
  return GpioConfigurePadIoStandby (GpioNativePad, StandbyState, StandbyTerm);
}
  ...
  )
{
  ...
  //
  Data32Or = (B_GPIO_PCR_MISCCFG_GPVNNREQEN     |
  ...
  )
{
  switch (PadTermination) {
    case GpioTermDefault:     return "GpioTermDefault";
    case GpioTermNone:        return "GpioTermNone";
    case GpioTermWpd5K:       return "GpioTermWpd5K";
    case GpioTermWpd20K:      return "GpioTermWpd20K";
    case GpioTermWpu1K:       return "GpioTermWpu1K";
    case GpioTermWpu2K:       return "GpioTermWpu2K";
    case GpioTermWpu5K:       return "GpioTermWpu5K";
    case GpioTermWpu20K:      return "GpioTermWpu20K";
    case GpioTermNative:      return "GpioTermNative";
    default:
      DEBUG (( DEBUG_ERROR, "Wrong pad termination set: 0x%X\n", PadTermination ));
      ASSERT (FALSE);
      return "GpioTermInvalid";
  }
}
