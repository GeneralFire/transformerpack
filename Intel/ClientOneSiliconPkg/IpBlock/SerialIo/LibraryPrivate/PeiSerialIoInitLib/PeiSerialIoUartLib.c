...
/**
  Set Serial IO UART Hidden Scratch Pad register
...
VOID
STATIC
SerialIoSetUartHiddenScratchPad (
  IN UINT8        UartNumber
  )
{
  PchPcrAndThenOr32 (PID_SERIALIO, R_SERIAL_IO_PCR_SPARE, MAX_UINT32, (BIT0 << UartNumber));
}
  ...
  switch (UartDeviceConfig->Mode) {
    ...
    case SerialIoUartCom:
      if (!IsSimicsEnvironment()) {
        SerialIoSetUartHiddenScratchPad (UartNumber);
      }
  ...
  )
{
  ...
  for (UartNumber = 0; UartNumber < GetPchMaxSerialIoUartControllersNum (); UartNumber++) {
    if (SerialIoConfig->UartDeviceConfig[UartNumber].Mode != SerialIoUartDisabled) {
      ItssGetDevIntConfig (SiPolicy, SerialIoUartDevNumber (UartNumber), SerialIoUartFuncNumber (UartNumber), &InterruptPin, &Irq);
    ...
    SerialIoUartConfiguration (UartNumber, &SerialIoConfig->UartDeviceConfig[UartNumber], TRUE);
  }
  ...
  )
{
  ...
  for (UartNumber = 0; UartNumber < GetPchMaxSerialIoUartControllersNum (); UartNumber++) {
    ...
    if ((SerialIoConfig->UartDeviceConfig[UartNumber].Mode == SerialIoUartDisabled) &&
      ...
      SerialIoUartConfiguration (UartNumber, &SerialIoConfig->UartDeviceConfig[UartNumber], PsfDisable);
    }
  }
}
