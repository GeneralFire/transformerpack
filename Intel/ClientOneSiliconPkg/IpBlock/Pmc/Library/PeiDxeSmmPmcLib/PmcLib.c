  ...
  )
{
  ...
  ZeroMem (&Wbuf, sizeof (PMC_IPC_COMMAND_BUFFER));
  return PmcSendCommand (V_PMC_PWRM_IPC_CMD_COMMAND_CRASHLOG, V_PMC_PWRM_IPC_CMD_CMD_ID_CRASHLOG_RE_ARM, 4, &Wbuf, NULL);
}
  ...
  )
{
  ...
  for (Offset = R_PMC_LPM0_REQ_0; Offset <= R_PMC_LPM7_REQ_5; Offset += 0x4) {
    Wbuf.Buf0 = Offset;
    Status = PmcSendCommand (V_PMC_PWRM_IPC_CMD_COMMAND_GEN_COMM,
    ...
    if (Index < BufLen) {
      ...
      DEBUG ((DEBUG_ERROR, "Overflow LpmReqData buffer\n"));
      return EFI_INVALID_PARAMETER;
    }
