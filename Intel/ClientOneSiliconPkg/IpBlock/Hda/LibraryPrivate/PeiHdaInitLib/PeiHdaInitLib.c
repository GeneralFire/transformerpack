  ...
  )
{
  ...
  for (Index = 0; Index < GetPchHdaMaxDmicLinkNum (); Index++) {
    if (HdaPreMemConfig->AudioLinkDmic[Index].Enable && IsAudioInterfaceSupported (HdaDmic, Index)) {
      switch (HdaPreMemConfig->AudioLinkDmic[Index].DmicClockSelect) {
  ...
  )
{
  ...
  ///
  /// 1. Set PCE.HAE bit, HDA PCI offset 92h [5] = 1b, HDA PCI offset 92h [2,1,0] = 0b,0b,0b
  ///
  PciSegmentAndThenOr8 (HdaPciBase + R_HDA_CFG_SPCE,
    (UINT8)~(B_HDA_CFG_SPCE_PMCRE | B_HDA_CFG_SPCE_I3E | B_HDA_CFG_SPCE_D3HE),
    (UINT8) (B_HDA_CFG_SPCE_HAE));
  ...
  ///
  /// 6. Arm audio IP function disable on the PMC side.
  ///
  PmcDisableHda ();
  ...
  )
{
  ...
  //
  // Gen9  - CML CPU: NodeID = 0x8
  // Gen10 - ICL CPU: NodeID = 0x2
  // Gen11 - LKF, TGL CPU: NodeID = 0x2
  if (IsGraphicsGen9 ()) {
...
/**
  ...
  @param[in]  SiPreMemPolicyPpi       Pre-mem Policy
  @param[out] XtalValue               Pointer to XtalFrequency value which is updated by this function for further ACPI use cases
**/
VOID
HdAudioInit (
  IN SI_PREMEM_POLICY_PPI      *SiPreMemPolicyPpi,
  ...
  )
{
  ...
  if (IsHdAudioEnabled ()) {
    ...
    ConfigureFncfgRegister (HdaPreMemConfig);
  } else {
    DisableHdAudio (HdaPciBase, SiPreMemPolicyPpi);
  }
  ...
  )
{
  ...
  ///
  InterruptPin = ItssGetDevIntPin (SiPolicy, HdaDevNumber (), HdaFuncNumber ());
