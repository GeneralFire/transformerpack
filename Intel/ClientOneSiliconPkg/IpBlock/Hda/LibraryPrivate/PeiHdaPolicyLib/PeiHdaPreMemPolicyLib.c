  ...
  )
{
  ...
  for (Index = 0; Index < GetPchHdaMaxSndwLinkNum (); Index++) {
    DEBUG ((DEBUG_INFO, " Audio Link: SoundWire#%d                       = %x\n", Index + 1, HdaPreMemConfig->AudioLinkSndw[Index].Enable));
  }
  ...
  DEBUG ((DEBUG_INFO, " Audio Fpga SoC Version                         = %d\n", HdaPreMemConfig->AudioFpgaSocVer));
...
#else
  HdaPreMemConfig->AudioLinkDmic[0].DmicClockSelect = HdaDmicClockSelectBoth;
  HdaPreMemConfig->AudioLinkDmic[1].DmicClockSelect = HdaDmicClockSelectBoth;
#endif
  /********************************
