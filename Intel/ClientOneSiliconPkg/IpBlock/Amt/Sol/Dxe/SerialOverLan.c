  ...
  )
{
  ...
  *Supports = 0x01;
  ...
  SerialDevice->SerialMode.ReceiveFifoDepth = PcdGet16 (PcdUartDefaultReceiveFifoDepth);
  ...
  )
{
  ...
  if (ReceiveFifoDepth == 0) {
    ReceiveFifoDepth = PcdGet16 (PcdUartDefaultReceiveFifoDepth);
  }
  ...
  if (DataBits == 0) {
    DataBits = (UINT32) PcdGet8 (PcdUartDefaultDataBits);
  }
  ...
  ///
  if (BaudRate < SERIAL_PORT_MIN_BAUD_RATE) {
    return EFI_INVALID_PARAMETER;
  }
  ///
  ...
  /// 1, 8, 16
  ///
  if (ReceiveFifoDepth < 8) {
    ReceiveFifoDepth = 1;
  } else if (ReceiveFifoDepth < 16) {
    ReceiveFifoDepth = 8;
  } else {
    ReceiveFifoDepth = 16;
  }
