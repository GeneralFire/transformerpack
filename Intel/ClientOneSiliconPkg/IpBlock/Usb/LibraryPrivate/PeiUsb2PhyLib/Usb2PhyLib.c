...
#define USB2PHY_ACTIVE_POLL_INTERVAL    100   // in microseconds
#define USB2PHY_ACTIVE_POLL_TIMEOUT  100000   // in microseconds
...
/**
  Confirm USB2 PHY is ungated before programming it.

  @retval EFI_SUCCESS USB2 PHY is available
  @retval EFI_TIMEOUT USB2 PHY error
**/
STATIC
EFI_STATUS
WaitForUsb2PhyAccessible (
  VOID
  )
{
  EFI_STATUS      Status;
  ...
  do {
    ...
    if (Status == EFI_SUCCESS) {
      DEBUG ((DEBUG_INFO, "USB2 PHY is accesible \n"));
      return EFI_SUCCESS;;
    }
    MicroSecondDelay (USB2PHY_ACTIVE_POLL_INTERVAL);
    WaitTime += USB2PHY_ACTIVE_POLL_INTERVAL;
  } while (WaitTime < USB2PHY_ACTIVE_POLL_TIMEOUT);

  DEBUG ((DEBUG_ERROR, "USB2 PHY ERROR Response = 0x%x\n", Response));
  return EFI_TIMEOUT;
}
...
/**
  This function shall perform USB2 PHY programming thats common between
  different PHY versions

  @param[in]  Usb2PhyConfig       The USB2_PHY_CONFIG policy instance
**/
VOID
CommonUsb2PhyProgramming (
...
/**
  This function shall perform USB2 PHY programming thats specific to
  USB2 PHY version 4.4
**/
VOID
Usb2Phy4p4SpecificProgramming (
  VOID
  )
{
  DEBUG ((DEBUG_VERBOSE, "[%a] Start\n", __FUNCTION__));
  ...
  Usb2PhyProgramRegister (
    R_USB2_PCR_GLOBAL_PORT,
    (UINT32)~0,
    (BIT29 | BIT25 | BIT24 | BIT13 | BIT12 | BIT11 | BIT3)
    );
  ...
  Usb2PhyProgramRegister (
    ...
    (UINT32)~(BIT14 | BIT13 | BIT11 | BIT8),
    (BIT15 | BIT12 | BIT10 | BIT9 | BIT7)
    );
}

/**
  This function shall perform USB2 PHY programming thats specific to
  USB2 PHY versions 4.3 and lower
**/
VOID
Usb2PhyPreviousVersionSpecificProgramming (
  VOID
  )
{
  DEBUG ((DEBUG_VERBOSE, "[%a] Start\n", __FUNCTION__));
  ...
  )
{
  ...
  //
  // Wait until USB2 PHY is ungated
  //
  Status = WaitForUsb2PhyAccessible ();
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_ERROR, "USB2 PHY is not available, skipping programming.\n"));
    return;
  }

  //
  // USB2 PHY version specific programming
  //
  Usb2Phy4p4SpecificProgramming ();

  //
  // USB2 PHY version independent programming
  //
  CommonUsb2PhyProgramming (Usb2PhyConfig);
}
