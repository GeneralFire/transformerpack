  ...
  )
{
  ...
  ReportMeUefiFwMbpHealthStatus (Status);
  if (EFI_ERROR (Status)) {
  ...
  )
{
  ...
  PostCodeWithDescription (0x0E02, "ME-BIOS: MBP Entry.");
  if (MeTypeIsClient ()) {
    ...
    Status = PrepareSpsMeBiosPayload (&MbpData, &MbpSensitiveData);
  }
  if (EFI_ERROR (Status)) {
  ...
  PostCodeWithDescription (0x0E22, "ME-BIOS: MBP Exit - Success.");
  return EFI_SUCCESS;
}
