  ...
  )
{
  ...
  BOOLEAN             HeciDumpsEnabled;

  HeciDumpsEnabled = GetHeciTraceDumpState ();
  if (HeciDumpsEnabled) {
    DebugLevel = DEBUG_ERROR;
    UsedPacketBodyLength = PacketBodyLength;
  } else {
    DebugLevel = DEBUG_INFO;
    UsedPacketBodyLength = MIN (PacketBodyLength, sizeof (UINT32));
  }
