  ...
  )
{
  ...
  if (!MeTypeIsClient ()) {
    DEBUG ((DEBUG_ERROR, "ME-BIOS: ME: %a Reset Request Exit - not supported.\n", ResetString));
    return EFI_UNSUPPORTED;
  }
  ...
  )
{
  ...
  Status = HeciFwCapsGetRuleData (FWCAPS_RULE_ID (KernelFeatureId, MeFwUpdateRuleTypeId), &FwUpdateState);
  if (!EFI_ERROR (Status)) {
    *RuleData = (UINT8) FwUpdateState;
  }
...
/**
  This message is sent by the BIOS on the boot where the host wants to get user capabilities.

  @param[out] *FwCapsSku            User capabilities
  ...
  )
{
  ...
  return HeciReadFileExMsg (MCA_PLAT_ID_SUPPORT_FILE_ID, 0, &DataSize, 0, PlatIdSupported);
}
  ...
  )
{
  ...
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_ERROR, "Send ARB SVN Commit Msg Fail, Status = %r\n", Status));
    return Status;
  }

  if ((ArbSvnCommit.Response.MkhiHeader.Fields.Command != MCA_ARB_SVN_COMMIT_CMD) ||
      (ArbSvnCommit.Response.MkhiHeader.Fields.IsResponse != 1) ||
      (ArbSvnCommit.Response.MkhiHeader.Fields.Result != MkhiStatusSuccess)) {
    Status = EFI_DEVICE_ERROR;
  }
  ...
  )
{
  ...
  Length                                        = sizeof (DNX_REQ_CLEAR);
  RecvLength                                    = sizeof (DNX_REQ_CLEAR_ACK);
  ...
  )
{
  ...
  Length                                         = sizeof (GEN_SEND_CPU_BRAND);
  RecvLength                                     = sizeof (GEN_SEND_CPU_BRAND_ACK);

  ///
  /// Send CPU brand class fuse Request to ME
  ///
  ...
  )
{
  ...
  Length                                              = sizeof (GET_IMAGE_FW_VER);
  RecvLength                                          = sizeof (GET_IMAGE_FW_VER_ACK);

  Status = Heci->SendwAck (
                   HECI1_DEVICE,
                   (UINT32 *) &GetImageFwVersion,
                   Length,
                   &RecvLength,
                   BIOS_FIXED_HOST_ADDR,
                   HECI_MKHI_MESSAGE_ADDR
  ...
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_ERROR, "GetImageFwVersion: Message failed! EFI_STATUS = %r\n", Status));
  }
  ...
  )
{
  ...
  PERF_START_EX (NULL, "EventRec", NULL, AsmReadTsc (), 0x80F0);
  Status = Heci->SendwAck (
                   HECI1_DEVICE,
  ...
  if (EFI_ERROR (Status)) {
    ...
    PERF_END_EX (NULL, "EventRec", NULL, AsmReadTsc (), 0x80F2);
    return Status;
  }
  ...
  if (NumProfiles != NULL) {
    *NumProfiles = GetClockProfile.Response.IccNumOfProfiles;
    ...
    if (IccSelectAllowed != NULL) {
      *IccSelectAllowed = GetClockProfile.Response.OemParams.Fields.IccRuntimeSelect;
    }
  ...
  PERF_END_EX (NULL, "EventRec", NULL, AsmReadTsc (), 0x80F1);
  ...
  )
{
  ...
  PERF_START_EX (NULL, "EventRec", NULL, AsmReadTsc (), 0x8100);
  Status = Heci->SendwAck (
                   HECI1_DEVICE,
  ...
  if (EFI_ERROR (Status)) {
    ...
    PERF_END_EX (NULL, "EventRec", NULL, AsmReadTsc (), 0x8102);
    return Status;
  }
  ...
  if (SetClockProfile.Response.Header.IccResponse != ICC_STATUS_SUCCESS) {
    ...
    PERF_END_EX (NULL, "EventRec", NULL, AsmReadTsc (), 0x8103);
    return EFI_DEVICE_ERROR;
  }
  PERF_END_EX (NULL, "EventRec", NULL, AsmReadTsc (), 0x8101);
  ...
  )
{
  ...
  Length                                            = sizeof (GET_FEATURE_STATE_REQUEST);
  RecvLength                                        = sizeof (GET_FEATURE_STATE_RESPONSE);

  Status = Heci->SendwAck (
                   HECI1_DEVICE,
                   (UINT32 *) &GetFeatureState,
                   Length,
                   &RecvLength,
                   BIOS_FIXED_HOST_ADDR,
  ...
  if (EFI_ERROR (Status)) {
    ...
    DEBUG ((DEBUG_ERROR, "UpidStatus Error, Status: %x\n", GetFeatureState.Response.Status));
    Status = EFI_DEVICE_ERROR;
  } else {
  ...
  )
{
  ...
  Length                                            = sizeof (SET_FEATURE_STATE_REQUEST);
  RecvLength                                        = sizeof (SET_FEATURE_STATE_RESPONSE);

  Status = Heci->SendwAck (
                   HECI1_DEVICE,
                   (UINT32 *) &SetFeatureState,
                   Length,
                   &RecvLength,
                   BIOS_FIXED_HOST_ADDR,
                   HECI_UPID_MESSAGE_ADDR
  ...
  if (EFI_ERROR (Status)) {
    ...
    DEBUG ((DEBUG_ERROR, "UpidStatus Error, Status: %x\n", SetFeatureState.Response.Status));
    Status = EFI_DEVICE_ERROR;
  }
  ...
  )
{
  ...
  Length                                                     = sizeof (GET_OS_CONTROL_FEATURE_STATE_REQUEST);
  RecvLength                                                 = sizeof (GET_OS_CONTROL_FEATURE_STATE_RESPONSE);

  Status = Heci->SendwAck (
                   HECI1_DEVICE,
                   (UINT32 *) &GetOsControlFeatureState,
                   Length,
                   &RecvLength,
                   BIOS_FIXED_HOST_ADDR,
                   HECI_UPID_MESSAGE_ADDR
  ...
  if (EFI_ERROR (Status)) {
    ...
    DEBUG ((DEBUG_ERROR, "UpidStatus Error, Status: %x\n", GetOsControlFeatureState.Response.Status));
    Status = EFI_DEVICE_ERROR;
  } else {
  ...
  )
{
  ...
  Length                                                         = sizeof (SET_OS_CONTROL_FEATURE_STATE_REQUEST);
  RecvLength                                                     = sizeof (SET_OS_CONTROL_FEATURE_STATE_RESPONSE);

  Status = Heci->SendwAck (
                   HECI1_DEVICE,
                   (UINT32 *) &SetOsControlFeatureState,
                   Length,
                   &RecvLength,
                   BIOS_FIXED_HOST_ADDR,
                   HECI_UPID_MESSAGE_ADDR
  ...
  if (EFI_ERROR (Status)) {
    ...
    DEBUG ((DEBUG_ERROR, "UpidStatus Error, Status: %x\n", SetOsControlFeatureState.Response.Status));
    Status = EFI_DEVICE_ERROR;
  }
  ...
  )
{
  ...
  Length                                          = sizeof (GET_PLATFORM_ID_REQUEST);
  ...
  )
{
  ...
  Length                                                = sizeof (GET_REFURBISH_COUNTER_REQUEST);
  RecvLength                                            = sizeof (GET_REFURBISH_COUNTER_RESPONSE);

  Status = Heci->SendwAck (
                   HECI1_DEVICE,
                   (UINT32 *) &GetRefurbishCounter,
                   Length,
                   &RecvLength,
                   BIOS_FIXED_HOST_ADDR,
                   HECI_UPID_MESSAGE_ADDR
  ...
  if (EFI_ERROR (Status)) {
    ...
    DEBUG ((DEBUG_ERROR, "UpidStatus Error, Status: %x\n", GetRefurbishCounter.Response.Status));
    Status = EFI_DEVICE_ERROR;
  } else {
...
/**
  ...
  @param[in] ManufacturingType     Update reason
  ...
  )
{
  ...
  Length                                                = sizeof (UPDATE_OEM_PLATFORM_ID_REQUEST);
  RecvLength                                            = sizeof (UPDATE_OEM_PLATFORM_ID_RESPONSE);

  Status = Heci->SendwAck (
                   HECI1_DEVICE,
                   (UINT32 *) &UpdateOemPlatformId,
                   Length,
                   &RecvLength,
                   BIOS_FIXED_HOST_ADDR,
                   HECI_UPID_MESSAGE_ADDR
  ...
  if (EFI_ERROR (Status)) {
    ...
    DEBUG ((DEBUG_ERROR, "UpidStatus Error, Status: %x\n", UpdateOemPlatformId.Response.Status));
    Status = EFI_DEVICE_ERROR;
  }
