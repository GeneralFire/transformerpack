  ...
  )
{
  ...
  for (PortIndex = 0; PortIndex < GetPchMaxPciePortNum (); PortIndex++) {
    ...
    RootPortFunction = PchPcieRpFuncNumber (PortIndex);
  ...
  )
{
  ...
  for (Index = 0; Index < TableSize; Index++) {
    PchPcrAndThenOr32 (
      PortRelaxedOrderingConfigTable[Index].PsfPid,
      PortRelaxedOrderingConfigTable[Index].RegisterAddress,
      (UINT32)~(B_PCH_PSF_PCR_PORT_CONFIG_EGRESS_FRO | B_PCH_PSF_PCR_PORT_CONFIG_INGRESS_FRO),
  ...
  )
{
  ...
  GetPsfPortRelaxedOrderingTables (&PsfPortRelaxedOrderingConfigRegs,
                                   &PsfPortRelaxedOrderingConfigRegsTableSize,
                                   &PsfPortRelaxedOrderingConfigRegsPchTypeSpecific,
                                   &PsfPortRelaxedOrderingConfigRegsPchTypeSpecificTableSize
                                  );
  PsfEnablePcieRelaxedOrderForTable (PsfPortRelaxedOrderingConfigRegs, PsfPortRelaxedOrderingConfigRegsTableSize);
  PsfEnablePcieRelaxedOrderForTable (PsfPortRelaxedOrderingConfigRegsPchTypeSpecific, PsfPortRelaxedOrderingConfigRegsPchTypeSpecificTableSize);
}
