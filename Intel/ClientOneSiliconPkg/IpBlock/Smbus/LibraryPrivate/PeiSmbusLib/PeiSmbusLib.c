  ...
  )
{
  ...
  if (SmbusPreMemConfig->SpdWriteDisable) {
    ...
    ///
    PciSegmentOr8 (SmbusBaseAddress + R_SMBUS_CFG_HOSTC, B_SMBUS_CFG_HOSTC_SPDWD);
  }
  ...
  ///
  InterruptPin = ItssGetDevIntPin (SiPolicyPpi, SmbusDevNumber (), SmbusFuncNumber ());
  ...
  Tco1Sts  = IoRead16 (PcdGet16 (PcdTcoBaseAddress) + R_TCO_IO_TCO1_STS);
  Tco1Sts |= B_TCO_IO_TCO1_STS_TIMEOUT;
  IoWrite16 ((PcdGet16 (PcdTcoBaseAddress) + R_TCO_IO_TCO1_STS), Tco1Sts);

  SmiSts   = IoRead32 (ABase + R_ACPI_IO_SMI_STS);
  SmiSts  |= B_ACPI_IO_SMI_STS_TCO;
  IoWrite32 ((ABase + R_ACPI_IO_SMI_STS), SmiSts);
}
