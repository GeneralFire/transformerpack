...
{
  return !!(PciSegmentRead32 (SpiPciCfgBase () + R_SPI_CFG_BC) & B_SPI_CFG_BC_EXT_BIOS_EN);
}
  ...
  )
{
  ...
  if (!SpiIsExtendedBiosRegionEnabled ()) {
    *RangeBase  = 0;
    *RangeLimit = 0;
    return;
  }

  Data32 = PchPcrRead32 (PID_DMI, R_PCH_DMI_PCR_GPMR1);
...
/**
  Checks lock of exnteded BIOS Region Limit address

  @retval TRUE    Extended BIOS Region limit address is locked
  @retval FALSE   Extended BIOS Region limit address is unlocked
**/
BOOLEAN
SpiIsExtendedBiosRegionLimitLocked (
  VOID
  )
{
  return !!(PchPcrRead32 (PID_DMI, R_PCH_DMI_PCR_GPMR1DID) & B_PCH_DMI_PCR_GPMRXDID_GPMR1DE);
}
...
{
  return !!(PciSegmentRead32 (SpiPciCfgBase () + R_SPI_CFG_BC) & B_SPI_CFG_BC_EXT_BIOS_LOCK);
}