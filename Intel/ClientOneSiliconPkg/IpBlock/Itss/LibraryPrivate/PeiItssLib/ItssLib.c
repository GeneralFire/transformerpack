  ...
  )
{
  ...
  //
  for (DevIntConfigIndex = 0; DevIntConfigIndex < DevIntConfigEntrySize; DevIntConfigIndex++) {
    if ((DevIntConfigEntry[DevIntConfigIndex].Device == PCI_DEVICE_NUMBER_PCH_PCIE_ROOT_PORT_1) ||
        (DevIntConfigEntry[DevIntConfigIndex].Device == PCI_DEVICE_NUMBER_PCH_PCIE_ROOT_PORT_9) ||
        (DevIntConfigEntry[DevIntConfigIndex].Device == PCI_DEVICE_NUMBER_PCH_PCIE_ROOT_PORT_17)) {
  ...
  )
{
  ...
  //
  DEBUG ((DEBUG_ERROR, "ITSS: Couldn't match interrupt policy for (%d:%d)\n", Device, Function));
  ASSERT (FALSE);
}
  ...
  )
{
  ...
  //
  // Additional PCH Interrupt configuration
  //
  ItssPchConfigure ();
