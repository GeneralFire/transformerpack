  ...
  )
{
  ...
  for (Index = 0; Index < PCH_MAX_DMI_LANES; Index++) {
    if (IsAdlPch () && IsPchS ()) {
      PchDmiPreMemConfig->Lane[Index].TestDmiTranCoOverEn      = FORCE_ENABLE;
      PchDmiPreMemConfig->Lane[Index].TestDmiUpPortTranPreset  = 0x0;
      PchDmiPreMemConfig->Lane[Index].TestDmiTranCoOverPostCur = 0x5;
      PchDmiPreMemConfig->Lane[Index].TestDmiTranCoOverPreCur  = 0x5;
      PchDmiPreMemConfig->TestDmiRtlepceb                      = FORCE_ENABLE;
    } else {
