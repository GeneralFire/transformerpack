...
STATIC BOOLEAN AssignBusNo = TRUE;
  ...
  )
{
  ...
  ///
  if (IsCpuPcie) {
    Ctrl1Register |= (0x28A << N_PCIE_EX_L1SCTL1_L12LTRTLV);
  ...
  if (IsCpuPcie) {
     Ctrl2Register |= B_PCIE_EX_CTRL2_PMETOFD;
  }
  //
  ...
  )
{
  ...
  UINT32       Data32And;
  UINT32       Data32Or;
  UINT16       Vc0CtlReg; // VC0 control register
  UINT16       Vc1CtlReg; // VC1 control register

  Vc0CtlReg = ExtVc.CapOffset + VC0_CTRL_REG;
  Vc1CtlReg = ExtVc.CapOffset + VC1_CTRL_REG;
  ...
  //
  // Program VC0CTL.TVM [7:1] to 0011111b
  //
  Data32And = (UINT32)~B_PCIE_CFG_VC0CTL_TVM_MASK;
  Data32Or = (UINT32)(V_PCIE_CFG_VC0CTL_TVM_EN_VC << B_PCIE_CFG_VC1CTL_TVM_OFFSET);
  PciSegmentAndThenOr32 (PcieBase + Vc0CtlReg, Data32And, Data32Or);

  //
  // Program VC1CTL.TVM [7:1] to 1100000b
  // Program VC1CTL.ID [27:24] to 0001b
  // Program VC1CTL.EN [31:31] t0 1b
  //
  Data32And = (UINT32)~(B_PCIE_CFG_VC1CTL_TVM_MASK | B_PCIE_CFG_VC1CTL_ID_MASK | B_PCIE_CFG_VC1CTL_EN_OFFSET);
  Data32Or = (UINT32)((V_PCIE_CFG_VC1CTL_TVM_EN_VC << B_PCIE_CFG_VC1CTL_TVM_OFFSET)
      | (V_PCIE_CFG_VC1CTL_ID_ONE << B_PCIE_CFG_VC1CTL_ID_OFFSET)
      | B_PCIE_CFG_VC1CTL_EN);
  PciSegmentAndThenOr32 (PcieBase + Vc1CtlReg, Data32And, Data32Or);
}
...
/**
  ...
  @return    Multi Virtual Channel Count Supported by Root Port.
**/
MULTI_VC_SUPPORT
  ...
  )
{
  ...
  if (AssignBusNo) {
    ...
    TempPciBusMax           = 10;
    BridgeCleanupList.Count = 0;
    RecursiveBusAssignment (Sbdf, TempPciBusMin, TempPciBusMax, &BridgeCleanupList);
    AssignBusNo             = FALSE;
  }

  DEBUG ((DEBUG_INFO, "RecursiveMultiVcConfiguration for %x:%x:%x\n", Sbdf.Bus, Sbdf.Dev, Sbdf.Func));
  ...
  if (MyExtVc.CapOffset) {
    MyExtVc.ExtVcCount = (PciSegmentRead8 (PcieBase + MyExtVc.CapOffset + VC_CAP_REG) & B_PCIE_CFG_PVCCR1_EVCC_MASK);
  } else if (Sbdf.Func == 0 && (PciSegmentRead8 (PcieBase + PCI_HEADER_TYPE_OFFSET) & HEADER_TYPE_MULTI_FUNCTION)) {
    //
    ...
    if (!MyExtVc.CapOffset) {
      DEBUG ((DEBUG_INFO, "Extended Multi VC Not Supported\n"));
      return MyExtVc;
    }
    MyExtVc.ExtVcCount = (PciSegmentRead8 (PcieBase + MyExtVc.CapOffset + VC_CAP_REG) & B_PCIE_CFG_PVCCR1_EVCC_MASK);
  } else {
  ...
  DEBUG ((DEBUG_INFO, "Extended Multi VC Count %x\n", MyExtVc.ExtVcCount));

  if (HasChildBus (Sbdf, &ChildSbdf)) {
    DevType = GetDeviceType (Sbdf);
    while (FindNextPcieChild (DevType, &ChildSbdf)) {
      ChildExtVc  = RecursiveMultiVcConfiguration (ChildSbdf);
      LinkVcCount = MyExtVc.ExtVcCount && ChildExtVc.ExtVcCount;
      if (LinkVcCount) {
        EnableMultiVc (Sbdf, MyExtVc);
        EnableMultiVc (ChildSbdf, ChildExtVc);
      }
    }
  }
  return MyExtVc;
