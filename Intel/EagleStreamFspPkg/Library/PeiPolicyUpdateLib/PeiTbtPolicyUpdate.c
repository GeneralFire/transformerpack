...
#include <ConfigBlock/PeiITbtConfig.h>
#include <SaTcssHob.h>
  ...
  )
{
  ...
  //
  SaTcssHob = (SA_TCSS_HOB *) GetFirstGuidHob (&gSaTcssHobGuid);
  ...
  for (Index = 0; Index < MAX_ITBT_PCIE_PORT; Index ++) {
    ...
    //
    if (SaTcssHob != NULL) {
      PeiITbtConfig->ITbtRootPortConfig[Index].ITbtPcieRootPortEn &= SaTcssHob->TcssData.ItbtPcieRpEn[Index];
    }
  }
  ...
  for (Index = 0; Index < SA_MAX_HOST_ITBT_DMA_NUMBER; Index ++) {
    PeiITbtConfig->IclAxITbtDmaUuid[Index] = FspsUpd->FspsConfig.IclAxITbtDmaUuid[Index];
