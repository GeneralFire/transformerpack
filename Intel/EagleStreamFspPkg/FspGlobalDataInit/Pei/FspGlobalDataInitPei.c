  ...
  )
{
  ...
  if ((UINTN)FspData == 0x00 || (UINTN)FspData == 0xFFFFFFFF) {
    ...
    if (FspData != NULL) {
      ...
      // FSP_IN_API_MODE = 0, means ApiMode enabled
      // FSP_IN_DISPATCH_MODE = 1, means DispatchMode enabled
      FspData->FspMode        = (PeiServicesLocatePpi (&gFspInApiModePpiGuid, 0, NULL, &PeiInstance) == EFI_SUCCESS)? FSP_IN_API_MODE:FSP_IN_DISPATCH_MODE;
  ...
  DEBUG ((DEBUG_INFO, " FspData: ApiMode       - 0x%x\n", FspData->FspMode));
