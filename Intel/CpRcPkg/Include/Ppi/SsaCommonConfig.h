...
typedef enum {
  ...
  RegWidthDelim = MAX_INT32               ///< This value ensures the enum size is consistent on both sides of the PPI.
} REG_WIDTH;
...
typedef enum {
  ...
  BaseAddrTypeDelim = MAX_INT32           ///< This value ensures the enum size is consistent on both sides of the PPI.
} BASE_ADDR_TYPE;
...
typedef enum {
  ...
  PrintLevelDelim = MAX_INT32             ///< This value ensures the enum size is consistent on both sides of the PPI.
} PRINT_LEVEL;
...
/**
  ...
  @param          [width] '*' get the width from a INT32 argument on the stack.
...
} SSA_COMMON_CONFIG, *PSSA_COMMON_CONFIG;
