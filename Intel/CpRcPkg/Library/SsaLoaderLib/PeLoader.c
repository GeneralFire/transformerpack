...
VOID fUpdateImageBase(UINT8 *baseAddr, UINT32 u32PeSignOffset, PEHDR *dsPeHdr)
{
  ...
  lOffset = u32PeSignOffset + 4 + sizeof(COFFHDR) + (INT32) (INTN) &dsPeHdr->u32ImgBase - (INT32) (INTN) &dsPeHdr->u16MagicNum;
  *(UINT32 *) (UINTN) (baseAddr + lOffset) =  (UINT32) (UINTN) baseAddr;
}
...
VOID fProcessRelocSection(UINT8 *baseAddr, PEHDR *dsPeHdr, UINT32 u32RelocSectionOffset, UINT32 u32RelocSectionVirtSize)
{
  ...
  u32FixupDelta = (UINT32) (UINTN) (baseAddr - dsPeHdr->u32ImgBase);
  ...
  while (u32Size >= 8) {
    //RcDebugPrint (SDBG_DEFAULT, "u32Size = %08x\n", u32Size);
    ...
    if (u32BlockSize == 0) {
      //RcDebugPrint (SDBG_DEFAULT, "u32BlockSize is 0, breaking out of while()...\n");
      break;
    }
    ...
    for (i = 0; i < ((u32BlockSize - 2 * sizeof(UINT32)) / sizeof(UINT16)); i++) {
      ...
      //RcDebugPrint (SDBG_DEFAULT, "%04x: PageRva: %08lx Offset: %04x Type: %x \n", i, u32PageRva, u16Offset, u8Type );
  ...
  ) 
{
  ...
  //RcDebugPrint (SDBG_DEFAULT, "u32PeSignOffset: %08x\n", u32PeSignOffset);
  ...
  for (i = 0; i < dsCoffHdr->u16NumSections; i++) {
    ...
    if (AsciiStrnCmp (dsSectionTbl->cName, ".reloc", 6) == 0) {
