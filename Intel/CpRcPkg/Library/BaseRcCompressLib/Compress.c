  ...
  )
{
  ...
  SetMem (Globals->mLevel + WNDSIZ, (MAX_UINT8 + 1) * sizeof (UINT8), 1);
  SetMem (Globals->mPosition + WNDSIZ, (MAX_UINT8 + 1) * sizeof (NODE), 0);
  ...
  )
{
  ...
  while (Globals->mRemainder > 0) {
    ...
    if (Globals->mMatchLen > LastMatchLen || LastMatchLen < THRESHOLD) {
      ...
      CompressOutput(Globals, LastMatchLen + (MAX_UINT8 + 1 - THRESHOLD),
...
/**
  ...
  @retval EFI_SUCCESS           The compression was sucessful.
  @retval EFI_BUFFER_TOO_SMALL  The buffer was too small.  DstSize is required.
  @retval EFI_OUT_OF_RESOURCES  There is not enough resources to compress.
**/
UINT32
CompressRc (
  IN       VOID     *SrcBuffer,
  ...
  )
{
  ...
  Globals = RcAllocatePool (sizeof (SCRATCH_DATA));
  if (Globals == NULL) {
    Status = EFI_OUT_OF_RESOURCES;
  } else {
    // Clear globals
    ZeroMem ((UINT8 *)Globals, sizeof (*Globals));


    Status = CompressMrc (SrcBuffer, SrcSize, DstBuffer, DstSize, 2, Globals);
    RcFreePool (Globals);
  }
  return (UINT32) Status;
}
