...
class VerifyFunction(SignFunction):
    ...
    # key embedded in binary. So dependning on which tag is placed in xml, proper _parse_key_tag() logic is triggered
    def _parse_key(self, xml_node):
        ...
        if custom_key_node is not None and key_node is not None:
            raise ComponentException(f"Cannot provide both '{self.customKeyTag}' and '{self.keyTag}' for verify function")
    ...
    def _build(self, buffer):
        ...
        for padding in [self.padding_scheme] if self.padding_scheme else SupportedPaddings.PaddingSchemeType:
            padding_args = SupportedPaddings.get_padding_args(self.salt_len, padding,
                                                              self.hash.sha_type, self.isLegacy)
