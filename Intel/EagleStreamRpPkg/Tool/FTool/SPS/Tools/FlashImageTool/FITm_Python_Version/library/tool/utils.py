...
from .exceptions import InvalidAttributeException, MissingAttributeException
...
def process_private_ec_key(pkey, hash_type: ss.SupportedSHAs.ShaType, is_legacy: bool):
    signing_key = ss.EcSigningKey()
    signing_key.ec_key = pkey
    signing_key.curve = pkey.curve.name
    public_key = pkey.public_key()
    public_numbers = public_key.public_numbers()
    bytes_length = (pkey.curve.key_size + 7) // 8
    signing_key.coordinate_size = bytes_length
    signing_key.qx = public_numbers.x.to_bytes(bytes_length, 'little')
    signing_key.qy = public_numbers.y.to_bytes(bytes_length, 'little')
    hash_signing_key(signing_key, hash_type, is_legacy)
    return signing_key
...
def process_public_key(data, hash_type, is_legacy):
    ...
    if isinstance(pkey, rsa.RSAPublicKey):
        return process_public_rsa_key(pkey, hash_type, is_legacy)
    if isinstance(pkey, ec.EllipticCurvePublicKey):
        return process_public_ec_key(data, hash_type, is_legacy)
...
def process_public_ec_key(_, __, ___):
...
def write_value_to_byte_array(component, buffer: bytearray):
    ...
    if len(buffer) < component.offset + len(component.get_bytes()):
        raise ComponentException(
            ...
            "Offset: {} Size: {}\n"
            "Maximum binary size: {}".format(component.offset, len(component.get_bytes()), len(buffer)),
            component.name)
...
def check_file_path(path, can_be_empty=True, throwing=True, restrict_types=None):
    ...
    is_proper_type = (can_be_empty and not path) or (not restrict_types or os.path.splitext(path)[1][1:] in restrict_types)
...
class XmlAttr:
    ...
    def value(self):
        ...
        if self.is_required:
            raise MissingAttributeException("Cannot find required attribute '{}' at node: {}"
                                            .format(self.name, self.node.tag))
        return self.default
