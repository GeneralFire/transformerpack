...
from ..LibException import ComponentException, LibException, DependencyException, ValueException
...
class IComponent:
    ...
    def __init__(self, xml_node, **kwargs):
        ...
        self._parse_children(xml_node, self.buffer)
    ...
    def _parse_gui_attributes(self, xml_node):
        self.params = ComponentParams(self._parse_attribute(xml_node, self.paramsTag, False), self.size, self.string_value_converter)
    ...
    def validate(self):
        ...
        if self.gui_tab is not None and self.gui_tab == '':
            raise ComponentException('gui_tab cannot be empty')

        if self.gui_tab and len(self.gui_tab) > 64:
    ...
    def _add_padding(self, buffer):
        if self.padding_formula is not None:
            ...
            self.padding = bytes(LibConfig.defaultPaddingValue * (new_size - self.size))
            self.size = new_size
            buffer.seek(self.offset + self.size)
    ...
    def to_xml_node(self, parent, create_groups: bool = False):
        ...
        if self.children_by_name:
            ...
            for child in self.children_by_name.values():
                child.to_xml_node(node_children_parent, create_groups)
    ...
    def parse_ui_params(self):
        ...
        ui_params_map = {self.readOnlyTag: (self.readOnlyTag, bool_func, False),
    ...
    def get_children_with_tag(self, tag) -> List["IComponent"]:
        if self.children:
    ...
    def _check_value_against_value_list(self, value):
        if not self.params.is_in_value_list(value):
            value_list = self.params.dict['value_list']
            possible_values = list(value_list.keys()) if LibConfig.isGui else list(value_list.values())
            raise ValueException(
                f"Specified value is not correct. Possible values are: {possible_values}", value,
                self.name)
