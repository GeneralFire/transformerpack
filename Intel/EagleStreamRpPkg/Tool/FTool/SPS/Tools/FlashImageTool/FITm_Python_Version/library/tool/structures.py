...
class SupportedSHAs:
    ...
    _supportedSHAClasses = {
        ShaType.Sha384: hashes.SHA384(),
        ShaType.Sha512: hashes.SHA512()
    }
    _legacySHAClasses = {
        ShaType.Sha256: hashes.SHA256()
    }

    _maskVersions = {
        ShaType.Sha256: 0,
        ShaType.Sha384: 0x1000,
        ShaType.Sha512: 0x2000,
    }
    ...
    def get_sha_class(cls, sha_type: ShaType, is_legacy=False):
        ...
        raise LibException(f"Given sha size is deprecated: {sha_type.value}.\n"
                           f"Accepted are: " +
                           ", ".join([sha.value for sha in cls._supportedSHAClasses.keys()]))
...
class SupportedPaddings:
    ...
    _maskVersions = {
        PaddingSchemeType.pkcs1_v1_5: 0x10000,
        PaddingSchemeType.pkcs1_pss: 0x20000
    }
    ...
    def get_padding_class(cls, padding_type: PaddingSchemeType, is_legacy=False):
        ...
        raise LibException(f"Given padding type is deprecated: {padding_type.value}.\n"
                           f"Accepted are: " +
                           ", ".join([sha.value for sha in cls._supportedPaddingClasses.keys()]))

    @classmethod
    def get_padding_args(cls, salt_len, padding_type, sha_type, is_legacy):
        padding_args = {cls.PaddingSchemeType.pkcs1_v1_5: [],
                        cls.PaddingSchemeType.pkcs1_pss: [
                            padding.MGF1(SupportedSHAs.get_sha_class(sha_type, is_legacy)),
                            salt_len]}
...
class RsaSigningKey:
    ...
    def get_key_len(cls, key_len: RsaKeyLength, is_legacy):
        if (key_len in cls._supportedRsaMapping) or \
                (is_legacy and key_len in cls._legacyRsaMapping):
...
class EcSigningKey:
    ...
    _maskCurveVersions = {
        EcCurve.Secp256: 0,
        EcCurve.Secp384: 0x100,
        EcCurve.Brainpool384: 0x200
    }
    ...
    def sign(self, computed_hash, length, hash_algorithm, reverse):
    ...
    class ErrorPadding(Padding):
        ...
        def get_encrypted_data_size(self, data_size):
            ...
            if self.padding_length:
                raise LibException(
                    "Padding is disabled for this encryption mode, data must be aligned to {}. Use 'padding' attribute to align data size"
                        .format(AesEncryption.aesBlockSizeBytes))
            return data_size
