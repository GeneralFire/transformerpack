...
class SignFunction(HashFunction):
    ...
    def _parse_children(self, xml_node, buffer=None):
        super()._parse_children(xml_node, buffer)
        ...
        if self.offline_signing:
            ...
            if self.load_external_data:
                if self.external_data_formula is None:
                    raise ComponentException(
                        "Cannot load external data - '{}' tag with '{}' attribute was not specified".
                ...
                if len(self.external_data) != self.size:
                    raise ComponentException("Invalid size of '{}', should be {} but is {}".
                                             format(self.externalDataTag, self.size, len(self.external_data)),
                                             self.name)

                # verify if loaded signature matches computed hash and given public key
                padding_args = SupportedPaddings.get_padding_args(self.salt_len, self.padding_scheme,
                                                                  self.sha_type, self.is_legacy)
