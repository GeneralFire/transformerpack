    ...
    def _parse_string_value(self, value):
        ...
        if self.params:
            if self.params.is_value_list():
                self._check_value_against_value_list(parsed_val)
            elif self.params.is_min_max_set():
                ...
                if parsed_val < value_min:
                    raise ValueException("Value cannot be lower than the specified limit: " + hex(value_min),
                                         value, self.name)
                if parsed_val > value_max:
                    raise ValueException("Value cannot be higher than the specified limit: " + hex(value_max),
                                         value, self.name)
