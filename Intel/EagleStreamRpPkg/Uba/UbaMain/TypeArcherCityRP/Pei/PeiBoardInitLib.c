...
EFI_STATUS
EFIAPI
TypeArcherCityRPPeiBoardInitLibConstructor (
  ...
  )
{
  ...
  if (PlatformInfo->BoardId == TypeArcherCityRP) {
    ...
    Status = TypeArcherCityRPInstallGpioData (UbaConfigPpi);
    if (EFI_ERROR(Status)) {
      return Status;
    }

    Status = TypeArcherCityRPInstallPcdData (UbaConfigPpi);
    if (EFI_ERROR(Status)) {
      return Status;
    }

    Status = TypeArcherCityRPPchEarlyUpdate (UbaConfigPpi);
    if (EFI_ERROR(Status)) {
      return Status;
    }

    Status = TypeArcherCityRPPlatformUpdateUsbOcMappings (UbaConfigPpi);
    if (EFI_ERROR(Status)) {
      return Status;
    }

    Status = TypeArcherCityRPInstallSlotTableData (UbaConfigPpi);
    if (EFI_ERROR(Status)) {
      return Status;
    }

    Status = TypeArcherCityRPInstallKtiEparamData (UbaConfigPpi);
    if (EFI_ERROR(Status)) {
      return Status;
    }
    ...
    //
    //  TypeArcherCityRPIioPortBifurcationInit will use PlatformInfo->InterposerType for PPO.
    //
    Status = TypeArcherCityRPIioPortBifurcationInit (UbaConfigPpi);
    if (EFI_ERROR(Status)) {
      return Status;
    }

    Status = TypeArcherCityRPPchPcieBifurcation (UbaConfigPpi);
    if (EFI_ERROR(Status)) {
      DEBUG ((DEBUG_INFO, "UBA PEI: %r\n", Status));
      return Status;
    }

    Status = TypeArcherCityRPInstallGpioPlatformData (UbaConfigPpi);
    if (EFI_ERROR(Status)) {
      return Status;
    }
