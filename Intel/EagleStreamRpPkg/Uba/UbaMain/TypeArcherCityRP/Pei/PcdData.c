...
/**
  Update ArcherCity IMON SVID Information

  retval N/A
**/
VOID
TypeArcherCityRPPlatformUpdateImonAddress (
  VOID
  )
{
...
EFI_STATUS
TypeArcherCityRPPlatformPcdUpdateCallback (
  VOID
  )
{
  ...
  CHAR16    PlatformName[]   = L"TypeArcherCityRP";
  UINTN     PlatformNameSize = 0;
  EFI_STATUS Status;

  //#Integer for BoardID, must match the SKU number and be unique.
  Status = PcdSet16S (PcdOemSkuBoardID                      , TypeArcherCityRP);
  ...
  Status = TypeArcherCityRPPlatformUpdateAcpiTablePcds ();
...
PLATFORM_PCD_UPDATE_TABLE    TypeArcherCityRPPcdUpdateTable =
{
...
EFI_STATUS
TypeArcherCityRPInstallPcdData (
  ...
  )
{
  ...
  Status = UbaConfigPpi->AddData (
                                 ...
                                 &TypeArcherCityRPPcdUpdateTable,
                                 sizeof(TypeArcherCityRPPcdUpdateTable)
                                 );
