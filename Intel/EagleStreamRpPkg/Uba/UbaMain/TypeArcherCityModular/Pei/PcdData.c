...
/**
  Update ArcherCity Modular IMON SVID Information

  retval N/A
**/
VOID
TypeArcherCityModularPlatformUpdateImonAddress (
  VOID
  )
{
...
EFI_STATUS
TypeArcherCityModularPlatformPcdUpdateCallback (
  VOID
  )
{
  CHAR8     FamilyName[]  = "EagleStream";

  CHAR8     BoardName[]   = "EXRP";
  UINT32    Data32;
  UINTN     Size;
  UINTN     PlatformFeatureFlag = 0;

  CHAR16    PlatformName[]   = L"TypeArcherCityModular";
  UINTN     PlatformNameSize = 0;
  EFI_STATUS Status;

  //#Integer for BoardID, must match the SKU number and be unique.
  Status = PcdSet16S (PcdOemSkuBoardID                      , TypeArcherCityModular);
...
#if MAX_SOCKET > 4
  Status = PcdSet32S (PcdOemSkuBoardSocketCount, 8);
#else
  ...
  Status = PcdSet32S (PcdOemSkuAssertPostGPIO, 0xFFFFFFFF);  //SPR Need to confirm!
  ASSERT_EFI_ERROR(Status);
  if (EFI_ERROR(Status)) {
    return Status;
  }
  ...
  Status = TypeArcherCityModularPlatformUpdateAcpiTablePcds ();
...
PLATFORM_PCD_UPDATE_TABLE    TypeArcherCityModularPcdUpdateTable =
{
...
)
{
  ...
  Status = UbaConfigPpi->AddData (
                                 ...
                                 &TypeArcherCityModularPcdUpdateTable,
                                 sizeof(TypeArcherCityModularPcdUpdateTable)
                                 );
