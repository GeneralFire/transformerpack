...
/**
    ...
    GpioConfigForInterposer: Configure the GPIO for detecting an interposer
...
**/
EFI_STATUS
TypeArcherCityModularGpioConfigForInterposer (
  IN GPIO_PAD   GpioId
  )
{
  EFI_STATUS           Status;
...
EFI_STATUS
EFIAPI
TypeArcherCityModularPeiBoardInitLibConstructor (
  ...
  )
{
  ...
  if (PlatformInfo->BoardId == TypeArcherCityModular) {

    PlatformInfo->MaxNumOfPchs = 4; // PCH Legacy + 3 x I/O Expander
    ...
    Status = TypeArcherCityModularInstallGpioData (UbaConfigPpi);
    if (EFI_ERROR(Status)) {
      return Status;
    }

    Status = TypeArcherCityModularInstallPcdData (UbaConfigPpi);
    if (EFI_ERROR(Status)) {
      return Status;
    }

    Status = TypeArcherCityModularPchEarlyUpdate (UbaConfigPpi);
    if (EFI_ERROR(Status)) {
      return Status;
    }

    Status = TypeArcherCityModularPlatformUpdateUsbOcMappings (UbaConfigPpi);
    if (EFI_ERROR(Status)) {
      return Status;
    }

    Status = TypeArcherCityModularInstallSlotTableData (UbaConfigPpi);
    if (EFI_ERROR(Status)) {
      return Status;
    }

    Status = TypeArcherCityModularInstallKtiEparamData (UbaConfigPpi);
    if (EFI_ERROR(Status)) {
      return Status;
    }
    ...
    //
    //  TypeArcherCityModularIioPortBifurcationInit will use PlatformInfo->InterposerType for PPO.
    //
    Status = TypeArcherCityModularIioPortBifurcationInit (UbaConfigPpi);
    if (EFI_ERROR(Status)) {
      return Status;
    }

    Status = TypeArcherCityModularPchPcieBifurcation (UbaConfigPpi);
    if (EFI_ERROR(Status)) {
      DEBUG ((DEBUG_INFO, "UBA PEI: %r\n", Status));
      return Status;
    }

    Status = TypeArcherCityModularInstallGpioPlatformData (UbaConfigPpi);
    if (EFI_ERROR(Status)) {
      return Status;
    }
