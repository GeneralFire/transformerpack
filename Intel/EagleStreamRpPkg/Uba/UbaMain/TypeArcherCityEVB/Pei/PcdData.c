...
/**
  Update EVB IMON SVID Information

  retval N/A
**/
VOID
TypeArcherCityEVBPlatformUpdateImonAddress (
  VOID
  )
{
...
EFI_STATUS
TypeArcherCityEVBPlatformPcdUpdateCallback (
  VOID
  )
{
  ...
  CHAR16    PlatformName[]   = L"TypeArcherCityEVB";
  UINTN     PlatformNameSize = 0;
  EFI_STATUS Status;

  //#Integer for BoardID, must match the SKU number and be unique.
  Status = PcdSet16S (PcdOemSkuBoardID                      , TypeArcherCityEVB);
  ...
  Status = TypeArcherCityEVBPlatformUpdateAcpiTablePcds ();
...
PLATFORM_PCD_UPDATE_TABLE    TypeArcherCityEVBPcdUpdateTable =
{
  ...
  )
{
  ...
  Status = UbaConfigPpi->AddData (
                                 ...
                                 &TypeArcherCityEVBPcdUpdateTable,
                                 sizeof(TypeArcherCityEVBPcdUpdateTable)
                                 );
