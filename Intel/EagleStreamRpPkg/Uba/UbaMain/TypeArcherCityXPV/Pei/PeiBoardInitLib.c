...
/**
    ...
    TypeArcherCityXPVGpioConfigForInterposer: Configure the GPIO for detecting an interposer
...
**/
EFI_STATUS
TypeArcherCityXPVGpioConfigForInterposer (
  IN GPIO_PAD   GpioId
  )
{
  EFI_STATUS           Status;
...
EFI_STATUS
EFIAPI
TypeArcherCityXPVPeiBoardInitLibConstructor (
  ...
  )
{
  ...
  if (PlatformInfo->BoardId == TypeArcherCityXPV) {
    ...
    Status = TypeArcherCityXPVInstallGpioData (UbaConfigPpi);
    if (EFI_ERROR(Status)) {
      return Status;
    }

    Status = TypeArcherCityXPVInstallPcdData (UbaConfigPpi);
    if (EFI_ERROR(Status)) {
      return Status;
    }

    Status = TypeArcherCityXPVPchEarlyUpdate (UbaConfigPpi);
    if (EFI_ERROR(Status)) {
      return Status;
    }

    Status = TypeArcherCityXPVPlatformUpdateUsbOcMappings (UbaConfigPpi);
    if (EFI_ERROR(Status)) {
      return Status;
    }

    Status = TypeArcherCityXPVInstallSlotTableData (UbaConfigPpi);
    if (EFI_ERROR(Status)) {
      return Status;
    }

    Status = TypeArcherCityXPVInstallKtiEparamData (UbaConfigPpi);
    if (EFI_ERROR(Status)) {
      return Status;
    }
    ...
    //
    //  TypeArcherCityXPVIioPortBifurcationInit will use PlatformInfo->InterposerType for PPO.
    //
    Status = TypeArcherCityXPVIioPortBifurcationInit (UbaConfigPpi);
    if (EFI_ERROR(Status)) {
      return Status;
    }

    Status = TypeArcherCityXPVPchPcieBifurcation (UbaConfigPpi);
    if (EFI_ERROR(Status)) {
      DEBUG ((DEBUG_INFO, "UBA PEI: %r\n", Status));
      return Status;
    }

    Status = TypeArcherCityXPVInstallGpioPlatformData (UbaConfigPpi);
    if (EFI_ERROR(Status)) {
      return Status;
    }
