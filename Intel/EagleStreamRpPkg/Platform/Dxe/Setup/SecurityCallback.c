...
STATIC EFI_PASSWORD_DATA *mSecurityData = NULL;
STATIC UINT8             mState;
...
{
// APTIOV_SERVER_OVERRIDE_RC_START : Password handled by AMI code
#if 0
  EFI_STATUS                      Status;
  ...
  if (CompareMem(mSecurityData->PasswordLocation, ZeroArray, SHA256_DIGEST_LENGTH) == 0) {
    //
  ...
  if (CompareMem (EncodedPassword, mSecurityData->PasswordLocation, SHA256_DIGEST_LENGTH) != 0) {
    //
...
#endif
  return EFI_SUCCESS;
// APTIOV_SERVER_OVERRIDE_RC_END : Password handled by AMI code
}

/**
  This function checks whether the input char is a special char.

  @param[in] Password            The unicode null-terminated password.

  @retval EFI_SUCCESS            The password is a special char.
  @retval EFI_NOT_FOUND          The password is not a special char.
...
{
  UINT16     SpecialCharNum;
  UINT16     Num;

  SpecialCharNum = sizeof(SpecialChars)/sizeof(CHAR16);

  for (Num=0; Num<SpecialCharNum; Num++){
    if (Password == SpecialChars[Num]){
      return EFI_SUCCESS;
    }
  }
...
/**
  This function checks whether the input password is a strong password.

  @param[in] Password            The unicode null-terminated password.

  @retval EFI_SUCCESS            The password is a strong password.
  @retval EFI_SECURITY_VIOLATION The password is a week password.
  @retval EFI_INVALID_PARAMETER  The password is not a valid password
...
{
  CHAR16      *TempNewPswd;
  UINT8       CharAlphaNumericSpl;

  TempNewPswd = Password;
  if(*TempNewPswd == L'\0') {                  //Null Password
    return EFI_SUCCESS;                        //If Null, Return Success.No need to verify strong password.
  }

  CharAlphaNumericSpl = 0;
  do {

    if (CharIsAlpha(*TempNewPswd)) {           //Char Lower/Upper
      CharAlphaNumericSpl |= CHAR_ALPHA;
    }else if (CharIsNumeric(*TempNewPswd)) {   //Char Number
      CharAlphaNumericSpl |= CHAR_NUMBER;
    }else if (CharIsSpecial(*TempNewPswd) == EFI_SUCCESS) {
      CharAlphaNumericSpl |= CHAR_SPECIAL;     //Char Special, Such as ! @ # $ % ^ & * ( ) _ - + = ?
    }else {
      return EFI_INVALID_PARAMETER;
    }
  ...
  if ((CharAlphaNumericSpl == CHAR_ALL) && (StrLen(Password) >= 8) && (StrLen(Password) <= 14)) {
    return EFI_SUCCESS;
  }
  ...
  )
{
  ...
  if (Status == EFI_SECURITY_VIOLATION) {
    do {
      CreatePopUp (
        ...
        &Key,
        L"Warning - a Strong Password should include at least one each",
        L"case sensitive alphabetic, numeric, and special character.",
        L"Length should be 8 to 14 characters.",
    ...
    do {
      CreatePopUp (
        EFI_LIGHTGRAY | EFI_BACKGROUND_BLUE,
        &Key,
        L"Password entered is not valid. Only case sensitive alphabetic,",
        L"numeric and special characters !@#$%^&*()-_+=? are allowed.",
        L"Press ENTER to continue ...",
        L"",
        NULL
        );
  ...
  if (StrLen (TempPasswordString) == 0) {
    ...
    //
    ZeroMem (mSecurityData->PasswordLocation, SHA256_DIGEST_LENGTH);
  } else {
    //
  ...
  )
{
  ...
  switch (QuestionId) {
  case 0x1000:
    mSecurityData = &mAdminPassword;
    switch (mState) {
    case BROWSER_STATE_VALIDATE_PASSWORD:
      Status = ValidatePassword (Value->string);
      if (Status == EFI_SUCCESS) {
        mState = BROWSER_STATE_SET_PASSWORD;
      } else {
        return EFI_NOT_READY;
      }
      break;
    ...
    case BROWSER_STATE_SET_PASSWORD:
      ...
      mState = BROWSER_STATE_VALIDATE_PASSWORD;
      break;
