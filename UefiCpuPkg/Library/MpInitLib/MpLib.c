  ...
  )
{
  ...
  //*** AMI PORTING BEGIN ***//
  // Bug fix. Make sure APs can retrieve PeiServices pointer.
  // According to PI specification PeiServices table pointer is stored right before ITD base
  // (see GetPeiServicesTablePointer in MdePkg\Library\PeiServicesTablePointerLibIdt\PeiServicesTablePointer.c).
  // BSP and AP use different IDT.
  // Original code was not placing PeiServices table pointer in memory before AP IDT.
  // As a result, any attempt to use functions from PeiServicesTablePointerLib or PeiServicesLib on AP
  // caused CPU exception.
  BufferSize += sizeof(UINTN); // Reserve space for PEI Services Table pointer
  //*** AMI PORTING END *****//  
  ...
  //*** AMI PORTING BEGIN ***//
  // Bug fix. Make sure APs can retrieve PeiServices pointer.
  // See comments above in this function for details.
  ...
  // Initialize PeiServices pointer. Copy the address from BSP.
  ...
  )
{
  ...
  UINTN                  CallerNumber;
  CPU_INFO_IN_HOB        *CpuInfoInHob;
  // APTIOV_SERVER_OVERRIDE_START : To match UefiCpuPkg with Intel RC 
  UINTN                  OriginalProcessorNumber;
  // APTIOV_SERVER_OVERRIDE_END : To match UefiCpuPkg with Intel RC 
  CpuMpData = GetCpuMpData ();
  CpuInfoInHob = (CPU_INFO_IN_HOB *) (UINTN) CpuMpData->CpuInfoInHob;
