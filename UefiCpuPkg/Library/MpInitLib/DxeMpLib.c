  ...
  )
{
  ...
  //
// AMI_OVERRIDE_START >>> EIP505669 - Improvement for AMI CSM enabled.
#if 0
  StartAddress = 0x88000;
#endif
  //  Set to 0xa0000 to follow our CSM setting and avoid discretely memory used.
  StartAddress = 0xA0000;
// AMI_OVERRIDE_END <<< EIP505669 - Improvement for AMI CSM enabled.
...
// AMI_OVERRIDE_START >>> Prevent this event run second time when Legacy to UEFI boot is used.
  if (mMpInitExitBootServicesEvent != NULL)
      gBS->CloseEvent (mMpInitExitBootServicesEvent);
  if (mLegacyBootEvent != NULL)
