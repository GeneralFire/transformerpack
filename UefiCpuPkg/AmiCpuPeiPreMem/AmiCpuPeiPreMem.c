  ...
  )
{
    ...
    if (((EFI_FIRMWARE_VOLUME_HEADER*)Fv)->Signature != EFI_FVH_SIGNATURE) return EFI_NOT_FOUND;
    ...
    while(TRUE) {
      ...
      FfsSize = (UINT32)((EFI_FFS_FILE_HEADER*)pFfs)->Size[0] + \
                (((UINT32)((EFI_FFS_FILE_HEADER*)pFfs)->Size[1]) << 8) + \
                (((UINT32)((EFI_FFS_FILE_HEADER*)pFfs)->Size[2]) << 16);
      ...
      if (CompareGuid(&((EFI_FFS_FILE_HEADER*)pFfs)->Name, &MicrocodeFfsGuid)) {
        ...
        if ((UINTN)Fv == FV_MICROCODE_UPDATE_BASE) {
            ...
            //Find Microcode
            pUc = (CPU_MICROCODE_HEADER*)(pFfs + sizeof(EFI_FFS_FILE_HEADER));
