        ...
        for (Index = 0; Index < Count; Index++) {
            ...
            if (Flags & DMIEDIT_DELETE_STRUC) {
                ...
                if (Status == EFI_SUCCESS) {
                    ...
                    switch (Type) {
                        case 00:
                                if (ValidateString((char*)DmiData, DmiDataSize)) {
                        ...
                        case 01:
                                ...
                                else if (ValidateString((char*)DmiData, DmiDataSize)) {
    ...
    if (IdTblPtr->VendorId != 0xff) {
        ...
        while (Strcmp(ManufacturerTable[Index].Manufacturer, "Undefined")) {
            if (i == ManufacturerTable[Index].NumContCode && Data8 == ManufacturerTable[Index].ModuleManufacturer) {
                  Status = ReplaceString(StructBuffer, 3, (UINT8*)ManufacturerTable[Index].Manufacturer);
                  break;
            }
...
)
{
    ...
    if (FindStructureType(&Buffer, &StructPtr, 0, 1)) {
        ((SMBIOS_BIOS_INFO*)StructPtr)->BiosRomSize = (UINT8)(BIOS_SIZE >> 16);
        return EFI_SUCCESS;
    }
