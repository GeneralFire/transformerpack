...
GetFlashDataLocation (VOID)
{
    ...
    for (i = 0; i < FV_BB_BLOCKS; i++) {
        ...
        AmiFlashRead(gFlashBlockPtr, Buffer, FLASH_BLOCK_SIZE);
        ...
        for (index = 0; index < FLASH_BLOCK_SIZE; index += 4) {
            ...
            if (*BufferPtr == 0x5f415342) {     // "_ASB"
                ...
                if ((gDmiEditTableOffset + gDmiEditTableSize) > FLASH_BLOCK_SIZE) {
                    ...
                    AmiFlashRead(gFlashBlockPtr + FLASH_BLOCK_SIZE, &Buffer[FLASH_BLOCK_SIZE], FLASH_BLOCK_SIZE);
                }
...
)
{
    ...
    AmiFlashBlockWriteEnable(FlashBlockAddress);

    AmiFlashEraseBlock(FlashBlockAddress);

    if (AmiFlashProgram(FlashBlockAddress, Data, FLASH_BLOCK_SIZE)) {
        Status = EFI_SUCCESS;
    }
    else {
    ...
    AmiFlashBlockWriteDisable(FlashBlockAddress);
