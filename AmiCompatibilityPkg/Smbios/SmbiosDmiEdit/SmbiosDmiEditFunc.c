    ...
    if (AmiValidateMemoryBuffer((VOID*)p, sizeof(GET_SMBIOS_INFO)) == EFI_SUCCESS) {
        ...
        if (AmiValidateMemoryBuffer((VOID*)(UINTN)p->DmiStorageBase32BitAddr, 4) == EFI_SUCCESS) {
            if (Smb2xTablePhysAddress != 0) {
                ...
                *(UINT32*)(UINTN)p->DmiStorageBase32BitAddr = (UINT32)((UINTN)SmbiosTable);
            }
        }
        else {
            return DMI_BAD_PARAMETER;
...
#else
{
    ...
    Status = StoreNvramData(&Data->StructureData, (UINTN)TableInfo.Size, Index);
}
#endif                                          // SMBIOS_DMIEDIT_DATA_LOC
...
#else
{
    ...
    Status = StoreNvramData(NvramData, (UINTN)TableInfo.Size, Index);
}
#endif                                          // SMBIOS_DMIEDIT_DATA_LOC
...
#else
{
                    ...
                    EfiStatus = PrepareDmiArrayDmiVar(TableInfo.Type, TableInfo.Handle, TableInfo.Offset, TableInfo.Flags);
					...
					// or store new record if total record count in DmiArray was successfully
					// updated
                    if (EfiStatus == EFI_BUFFER_TOO_SMALL || EfiStatus == EFI_SUCCESS) {
