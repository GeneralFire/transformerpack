...
//#include <Include/Guid/PchRcVariable.h>  //  PCH Setup variables are moved to PCH_SETUP
...
//    if (PchStepping () > LbgA0) {
      ...
      EfiUsbPolicyProtocol->AmiUsbHwSmiHcTable.HcContext = AllocatePool (sizeof(AMI_USB_HW_SMI_HC_CONTEXT*));
 ...
 //   }
    ...
    if ( gDisableAllUsbControllers ) {
        TotalStruc = sizeof(SkipMassTable)/sizeof (USB_SKIP_LIST);
        gUsbProtocol->UsbCopySkipTable(SkipMassTable, &TotalStruc, TRUE);
    ...
    if(PchRcConfiguration.PchUsbPerPortCtl == 1){	   
        ...
        if ( IndexPort != 0 ) {
            *TotalStruc = IndexPort +1;
            gUsbProtocol->UsbCopySkipTable(SkipOemMassTable, TotalStruc, TRUE);
        }
