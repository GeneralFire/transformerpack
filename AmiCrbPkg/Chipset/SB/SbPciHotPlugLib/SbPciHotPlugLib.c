...
*/  
  { 0, 0x1, 0xFF, 0x02, "BR1A", 1 }, // S0, 1A
  { 0, 0x1, 0xFF, 0x03, "BR1B", 2 }, // S0, 1B
  { 0, 0x1, 0xFF, 0x04, "BR1C", 3 }, // S0, 1C
  { 0, 0x1, 0xFF, 0x05, "BR1D", 4 }, // S0, 1D
  
  { 0, 0x2, 0xFF, 0x02, "BR2A", 5 }, // S0, 2A
  { 0, 0x2, 0xFF, 0x03, "BR2B", 6 }, // S0, 2B
  { 0, 0x2, 0xFF, 0x04, "BR2C", 7 }, // S0, 2C
  { 0, 0x2, 0xFF, 0x05, "BR2D", 8 }, // S0, 2D
  
  { 0, 0x3, 0xFF, 0x02, "BR3A", 9 },  // S0, 3A
  { 0, 0x3, 0xFF, 0x03, "BR3B", 10 }, // S0, 3B
  { 0, 0x3, 0xFF, 0x04, "BR3C", 11 }, // S0, 3C
  { 0, 0x3, 0xFF, 0x05, "BR3D", 12 }, // S0, 3D

  { 0, 0x4, 0xFF, 0x02, "BR4A", 13 },
  { 0, 0x4, 0xFF, 0x03, "BR4B", 14 },
  { 0, 0x4, 0xFF, 0x04, "BR4C", 15 },
  { 0, 0x4, 0xFF, 0x05, "BR4D", 16 },
  
  { 0, 0x5, 0xFF, 0x02, "BR5A", 17 },
  { 0, 0x5, 0xFF, 0x03, "BR5B", 18 },
  { 0, 0x5, 0xFF, 0x04, "BR5C", 19 },
  { 0, 0x5, 0xFF, 0x05, "BR5D", 20 },
  ...
  { 1, 0x0, 0xFF, 0x04, "PC06", 21 },
  { 1, 0x0, 0xFF, 0x05, "PC06", 21 },

  { 1, 0x1, 0xFF, 0x02, "QR1A", 22 },
  { 1, 0x1, 0xFF, 0x03, "QR1B", 23 },
  { 1, 0x1, 0xFF, 0x04, "QR1C", 24 },
  { 1, 0x1, 0xFF, 0x05, "QR1D", 25 },
  
  { 1, 0x2, 0xFF, 0x02, "QR2A", 26 },
  { 1, 0x2, 0xFF, 0x03, "QR2B", 27 },
  { 1, 0x2, 0xFF, 0x04, "QR2C", 28 },
  { 1, 0x2, 0xFF, 0x05, "QR2D", 29 },
  
  { 1, 0x3, 0xFF, 0x02, "QR3A", 30 },
  { 1, 0x3, 0xFF, 0x03, "QR3B", 31 },
  { 1, 0x3, 0xFF, 0x04, "QR3C", 32 },
  { 1, 0x3, 0xFF, 0x05, "QR3D", 33 },

  { 1, 0x4, 0xFF, 0x02, "QR4A", 34 },
  { 1, 0x4, 0xFF, 0x03, "QR4B", 35 },
  { 1, 0x4, 0xFF, 0x04, "QR4C", 36 },
  { 1, 0x4, 0xFF, 0x05, "QR4D", 37 },
  
  { 1, 0x5, 0xFF, 0x02, "QR5A", 38 },
  { 1, 0x5, 0xFF, 0x03, "QR5B", 39 },
  { 1, 0x5, 0xFF, 0x04, "QR5C", 40 },
  { 1, 0x5, 0xFF, 0x05, "QR5D", 41 },
  ...
  { 2, 0x0, 0xFF, 0x04, "PC12", 42 },
  { 2, 0x0, 0xFF, 0x05, "PC12", 42 },

  { 2, 0x1, 0xFF, 0x02, "RR1A", 43 },
  { 2, 0x1, 0xFF, 0x03, "RR1B", 44 },
  { 2, 0x1, 0xFF, 0x04, "RR1C", 45 },
  { 2, 0x1, 0xFF, 0x05, "RR1D", 46 },
  
  { 2, 0x2, 0xFF, 0x02, "RR2A", 47 },
  { 2, 0x2, 0xFF, 0x03, "RR2B", 48 },
  { 2, 0x2, 0xFF, 0x04, "RR2C", 49 },
  { 2, 0x2, 0xFF, 0x05, "RR2D", 50 },
  
  { 2, 0x3, 0xFF, 0x02, "RR3A", 51 },
  { 2, 0x3, 0xFF, 0x03, "RR3B", 52 },
  { 2, 0x3, 0xFF, 0x04, "RR3C", 53 },
  { 2, 0x3, 0xFF, 0x05, "RR3D", 54 },

  { 2, 0x4, 0xFF, 0x02, "RR4A", 55 },
  { 2, 0x4, 0xFF, 0x03, "RR4B", 56 },
  { 2, 0x4, 0xFF, 0x04, "RR4C", 57 },
  { 2, 0x4, 0xFF, 0x05, "RR4D", 58 },
  
  { 2, 0x5, 0xFF, 0x02, "RR5A", 59 },
  { 2, 0x5, 0xFF, 0x03, "RR5B", 60 },
  { 2, 0x5, 0xFF, 0x04, "RR5C", 61 },
  { 2, 0x5, 0xFF, 0x05, "RR5D", 62 },
  ...
  { 3, 0x0, 0xFF, 0x04, "PC18", 63 },
  { 3, 0x0, 0xFF, 0x05, "PC18", 63 },

  { 3, 0x1, 0xFF, 0x02, "SR1A", 64 },
  { 3, 0x1, 0xFF, 0x03, "SR1B", 65 },
  { 3, 0x1, 0xFF, 0x04, "SR1C", 66 },
  { 3, 0x1, 0xFF, 0x05, "SR1D", 67 },
  
  { 3, 0x2, 0xFF, 0x02, "SR2A", 68 },
  { 3, 0x2, 0xFF, 0x03, "SR2B", 69 },
  { 3, 0x2, 0xFF, 0x04, "SR2C", 70 },
  { 3, 0x2, 0xFF, 0x05, "SR2D", 71 },
  
  { 3, 0x3, 0xFF, 0x02, "SR3A", 72 },
  { 3, 0x3, 0xFF, 0x03, "SR3B", 73 },
  { 3, 0x3, 0xFF, 0x04, "SR3C", 74 },
  { 3, 0x3, 0xFF, 0x05, "SR3D", 75 },

  { 3, 0x4, 0xFF, 0x02, "SR4A", 76 },
  { 3, 0x4, 0xFF, 0x03, "SR4B", 77 },
  { 3, 0x4, 0xFF, 0x04, "SR4C", 78 },
  { 3, 0x4, 0xFF, 0x05, "SR4D", 79 },

  { 3, 0x5, 0xFF, 0x02, "SR5A", 80 },
  { 3, 0x5, 0xFF, 0x03, "SR5B", 81 },
  { 3, 0x5, 0xFF, 0x04, "SR5C", 82 },
  { 3, 0x5, 0xFF, 0x05, "SR5D", 83 },
};

UINT8 IioPcieIdxTblLen = sizeof (IioPcieIdxTable) / sizeof(IIO_PCIE_IDX_TABLE);

//ONLY ONE INSTANCE ALOUD IN THE SYSTEM!!!
//static EFI_GUID                   gEfiPchRcVariableGuid = PCH_SETUP;
...
SOCKET_IIO_HP_RESOURCE            gIioHpResourceConfig;
PCH_SETUP              gPchConfiguration;
UINTN                             PchRcVariableSize = sizeof (PCH_SETUP);
UINTN                             IioRcVariableSize = sizeof (SOCKET_IIO_CONFIGURATION);
UINTN                             gIioHpResourceConfigSize = sizeof (SOCKET_IIO_HP_RESOURCE);
// Intel Thunderbolt Support
...
#if defined (PCI_SETUP_USE_APTIO_4_STYLE) && PCI_SETUP_USE_APTIO_4_STYLE == 0
PCI_COMMON_SETUP_DATA             gPciSetupData;
UINTN                             gPciSetupDataSize = sizeof(PCI_COMMON_SETUP_DATA);
#endif
UINT8                             gAbove4gDecode;
BOOLEAN                           gTableUpdated = FALSE;
...
BOOLEAN HpcCheckHpCompatibleHost(PCI_HOST_BRG_DATA *HostBrg) {
    if( (HostBrg->HpcData != NULL) && (HostBrg->HpcData->HpcLocCount > 0) ) {
        DEBUG((EFI_D_INFO,"HostBrg->HpcData != NULL & HpcLocCount is %d\n", HostBrg->HpcData->HpcLocCount));
        return TRUE;
    }
...
//-----------------------------
    ...
    //----------------------------
        ...
        ASSERT_EFI_ERROR(Status);
        if(EFI_ERROR(Status) && (cnf == NULL)) return Status;
...
#if defined(PCI_SETUP_USE_APTIO_4_STYLE) && PCI_SETUP_USE_APTIO_4_STYLE == 0
    Status = pRS->GetVariable ( L"PCI_COMMON", \
                                &gPciSetupGuid, \
                                NULL, \
                                &gPciSetupDataSize, \
                                &gPciSetupData );
    ASSERT_EFI_ERROR (Status);
    gAbove4gDecode = gPciSetupData.Above4gDecode;
#else
    gAbove4gDecode = gSetupData.Above4gDecode;
#endif
        ...
        if ((parent->Bus == 0 && pcidp.Device == 0x1B) || \
            ...
            for (idx = 0; idx < IioPcieIdxTblLen; idx++) {
              if ((parent->Bus == IioPcieIdxTable[idx].Bus) && (pcidp.Device == IioPcieIdxTable[idx].Device)) {
                ...
                locdata->HpSetup.ArrayField.BusPadd      = (UINT64)gIioHpResourceConfig.IioExtraBusRsvd[NbPcieIndex];
                locdata->HpSetup.ArrayField.IoPadd       = (UINT64)gIioHpResourceConfig.IioIoRsvd[NbPcieIndex] * 0x400;
                locdata->HpSetup.ArrayField.Mmio32Padd   = (UINT64)gIioHpResourceConfig.IioMemRsvd[NbPcieIndex] * 0x100000;
                if (gAbove4gDecode)
                    locdata->HpSetup.ArrayField.Mmio64PfPadd = (UINT64)gIioHpResourceConfig.Iio64PFMemRsvd[NbPcieIndex] * 0x100000;
                else
                    locdata->HpSetup.ArrayField.Mmio32PfPadd = (UINT64)gIioHpResourceConfig.IioPFMemRsvd[NbPcieIndex] * 0x100000;
                break;
              }
            }
        }
...
//-------------------------
    ...
    Status = pRS->GetVariable ( \
		    SOCKET_IIO_HP_RESOURCE_NAME, \
		    &gEfiSocketIioVariableGuid, \
		    NULL, \
		    &gIioHpResourceConfigSize, \
		    &gIioHpResourceConfig);
    ASSERT_EFI_ERROR(Status);
        ...
        if ((sdltmpParent->Bus == 0 && sdltmp->Device == 0x1B) ||
          ...
          for (idx = 0; idx < IioPcieIdxTblLen; idx++) {
            if ((sdltmpParent->Bus == IioPcieIdxTable[idx].Bus) && (sdltmp->Device == IioPcieIdxTable[idx].Device)) {
              NbPcieIndex = IioPcieIdxTable[idx].Index;
              DEBUG((DEBUG_VERBOSE,"Make sure Pci device is NB PCIE root port: %a of CPU socket: %d Setup Index: %d\n", sdltmp->AslName, IioPcieIdxTable[idx].Socket, NbPcieIndex));
...
//------------------------
    ...
    *HpcList=hpcl;
    DEBUG((DEBUG_VERBOSE, __FUNCTION__" return %r\n", Status));
...
//------------------------
    ...
    DEBUG((DEBUG_VERBOSE, __FUNCTION__" Entry !!!\n"));
...
// PCH / IIO Resource reservation for Hot-Plug
    ...
    DEBUG((DEBUG_VERBOSE,"Hotplug location: %x -> B:%x|D:%x|F:%x\n", HpcPciAddress, Bus, Dev, Fun));
                  ...
                  for (PortIndex = 0; PortIndex < NUMBER_PORTS_PER_SOCKET; PortIndex++) {
                    if ((Bus == gIioSystemProtocol->IioGlobalData->IioVar.IioVData.SocketPortBusNumber[Socket][PortIndex]) &&
                      ...
                      switch(i) {
                        case ptMmio32:
                        case ptMmio64:
                          HpcFillDescriptor(qw, (HP_PADD_RES_TYPE)i, locdata->HpSetup.ARRAY[i], gIioHpResourceConfig.IioMemRsvdalig[PcieIndex]);
                          break;
                        case ptMmio32pf:
                          HpcFillDescriptor(qw, (HP_PADD_RES_TYPE)i, locdata->HpSetup.ARRAY[i], gIioHpResourceConfig.IioPFMemRsvdalig[PcieIndex]);
                          break;
                        case ptMmio64pf:
                          HpcFillDescriptor(qw, (HP_PADD_RES_TYPE)i, locdata->HpSetup.ARRAY[i], gIioHpResourceConfig.Iio64PFMemRsvdalig[PcieIndex]);
                          break;
                        default:
                          HpcFillDescriptor(qw, (HP_PADD_RES_TYPE)i, locdata->HpSetup.ARRAY[i], 1);
                      }
...
#endif            
            DEBUG((DEBUG_VERBOSE,"Resource type: %x. Resource size: %lx\n", i, locdata->HpSetup.ARRAY[i]));
            qw++;
        }
    }
...
//---------------------------
    ...
    DEBUG((DEBUG_VERBOSE,"PciHP: "__FUNCTION__"()... "));
