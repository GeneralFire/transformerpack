    ...
    )
{
  ...
  MOutdwm(MmioBase, 0x1e600000,0xAEED1A03);                                                         // add at V1.1
  MOutdwm(MmioBase, 0x1e600084,0x00010000);                                                         // add at V1.1
  MOutdwm(MmioBase, 0x1e600088,0x00000000);                                                         // add at V1.1
  //MOutdwm(mmiobase, 0x1e6e2000,0x1688A8A8);                                                         // add at V1.1
  /************************************************************************************************/
  
  // Patch for fast reset, ycchen@091117
  MOutdwm(MmioBase, 0x1e6e2000,0x1688A8A8); 
  Data = MIndwm(MmioBase, 0x1e6e2070);

  if (Data & 0x08000000)                            // check fast reset
  {       
    MOutdwm(MmioBase, 0x1E78501c,0xffffffff);                 
    MOutdwm(MmioBase, 0x1E785004,0x00000010);
    MOutdwm(MmioBase, 0x1E785008,0x00004755);
    MOutdwm(MmioBase, 0x1E78500c,0x00000013);
    uDelay(PeiServices, 1);  
  }  
  do {
    MOutdwm(MmioBase, 0x1e6e2000,0x1688A8A8);     
    Data = MIndwm(MmioBase, 0x1e6e2000);
  } while (Data != 1);  
  MOutdwm(MmioBase, 0x1e6e207c,0x08000000);         // clear fast reset
...
)
{
    ...
    // Reset USB port
    MOutdwm(MmioBase, 0x1E6E2090,0x20000000);                                                         // add at V1.2
    MOutdwm(MmioBase, 0x1E6E2094,0x00004000);                                                         // add at V1.2
    if(MIndwm(MmioBase, 0x1E6E2070) & 0x00800000){                                                    // add at V1.2
        MOutdwm(MmioBase, 0x1E6E207C,0x00800000);                                                     // add at V1.2
        uDelay(PeiServices, 100);                                                                     // add at V1.2
        MOutdwm(MmioBase, 0x1E6E2070,0x00800000);                                                     // add at V1.2
    }                                                                                                 // add at V1.2
    // Modify eSPI reset pin
    Data = MIndwm(MmioBase, 0x1E6E2070);                                                              // add at V1.3
    if(Data & 0x02000000){                                                                            // add at V1.3
        MOutdwm(MmioBase, 0x1E6E207C,0x00004000);                                                     // add at V1.3
    }
...
START_INIT:
    ...
    // Version Number
    Data = MIndwm(MmioBase, 0x1E6E0004);                                                              // add at V1.3
    MOutdwm(MmioBase, 0x1E6E0004, Data | 0x08300000);                                                 // add at V1.3
    MOutdwm(MmioBase, 0x1E6E0088, 0x20161229);
    return(1);
}
...
)
{
    ...
    // VGA Only
    if ((Reg & 0xC0) == 0) {
        PatchAhbAst2500 (MmioBase32, PeiServices);
        MOutdwm(MmioBase, 0x1e6e2000, 0x1688a8a8);
        do {
        ...
        MOutdwm(MmioBase, 0x1E6E2040, Temp | 0x40);
