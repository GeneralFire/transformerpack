...
//APTIOV OVERRIDE ends - To resolve build error for openssl-1.1.0
...
int PEM_get_EVP_CIPHER_INFO(char *header, EVP_CIPHER_INFO *cipher)
{
    ...
    header += int_strspn(header, " \t");	//APTIOV OVERRIDE - To resolve build error for openssl-1.1.0
    ...
    if (strncmp(header, ENCRYPTED, sizeof(ENCRYPTED)-1) != 0 ||
    	int_strspn(header+sizeof(ENCRYPTED)-1, " \t\r\n") == 0) {	//APTIOV OVERRIDE - To resolve build error for openssl-1.1.0
        PEMerr(PEM_F_PEM_GET_EVP_CIPHER_INFO, PEM_R_NOT_ENCRYPTED);
        return 0;
    }
    header += sizeof(ENCRYPTED)-1;
    header += int_strspn(header, " \t\r");	//APTIOV OVERRIDE - To resolve build error for openssl-1.1.0
    if (*header++ != '\n') {
     ...
     */
    dekinfostart = header;
    header += int_strcspn(header, " \t,");	//APTIOV OVERRIDE - To resolve build error for openssl-1.1.0
    c = *header;
    *header = '\0';
