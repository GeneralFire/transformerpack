...
//*                                                                     *
//***********************************************************************
/** @file RsdIScsiBmcParser.c
    This file parses the data read from BMC 
...
{
    RSD_ISCSI_BLOCK_DATA    *Current, *Prev = NULL;
...
)
{
    ...
    UINT8                   *TempInitRespData =(UINT8*) MdrInitResp + RSD_ISCSI_RESPONSE_DATA_OFFSET;
...
//    DisplayIpAddress(InitData.NicMacAddress, RSD_NIC_SIZE);
    ...
    // Increment the current offset to the end of the string
    gRsdIScsiCurOffset += Pos; 
    gRsdIScsiDataLeft -= Pos;
...
)
{
    ...
    UINT8               CurrentOffset = 0, Pos = 0;
    UINT8               *TempTarRespData = (UINT8*)MdrTarResp + RSD_ISCSI_RESPONSE_DATA_OFFSET;
...
//    DisplayIpAddress(TargetData.TargetIpAddress, (TargetData.IpAddressType == 1 ? 4 : 16));
    ...
    for(Pos = 2; Pos <= 5; Pos++) {
     ...
     // Get the size from the end of formatted header to the end of table
    for(Pos=0; ; Pos++)
    {
        if( (TempTarRespData[Pos] == 0x00) && (TempTarRespData[Pos+1] == 0x00) )
        {
            gRsdIScsiCurOffset += Pos;
            gRsdIScsiDataLeft -= Pos;
            break;
...
)
{
    ...
    UINT8                   BlockSize = 0;
    UINT8                   *TempAttemptRespData = (UINT8*)MdrAttemptResp + RSD_ISCSI_RESPONSE_DATA_OFFSET;
