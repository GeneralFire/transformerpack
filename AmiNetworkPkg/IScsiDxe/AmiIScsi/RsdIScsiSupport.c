...
//*                                                                     *
//***********************************************************************
/** @file RsdIScsiSupport.c
    Provides the Rsd related functions to 
    read different blocks of iSCSI MDR region 
...
#define RSD_ISCSI_RESP_DATA_INIT    0x06
...
ISCSI_ATTEMPT_CONFIG_NVDATA gRsdIScsiAttemptConfigData = {0};
#else
...
EFI_STATUS
EFIAPI
RsdIScsiConvertBmcDataToAttemptConfigNvData()
{
    EFI_STATUS                  Status; 
    ...
    for (Index = 0; Index < AttemptConfigOrderSize / sizeof (UINT8); Index++) {
        ...
        if (AttemptConfigData == NULL || (((AttemptConfigData->AttemptType == REDFISH_ATTEMPT)||(AttemptConfigData->AttemptType == RSD_ATTEMPT))&& (AttemptConfigData->SessionConfigData.Enabled)) ||
            ((AttemptConfigData->AttemptType == SETUP_ATTEMPT)&& (AttemptConfigData->Actived == ISCSI_ACTIVE_ENABLED)) ){
            continue;
        }
        break;
    }
    gRsdIScsiAttemptConfigData.AttemptConfigIndex = AttemptConfigData->AttemptConfigIndex;
    ...
    AttemptOrderTmp[TotalNumber - 1] = gRsdIScsiAttemptConfigData.AttemptConfigIndex;
    ...
    UnicodeSPrint (
                 ...
                 (UINTN) ISCSI_NAME_IFR_MAX_SIZE,
                 L"Attempt %d",
                 (UINTN) gRsdIScsiAttemptConfigData.AttemptConfigIndex
                 );
    UnicodeStrToAsciiStrS (mPrivate->PortString, gRsdIScsiAttemptConfigData.AttemptName, ATTEMPT_NAME_SIZE);
    Status = gRT->SetVariable (
                               mPrivate->PortString,
                               &gEfiIScsiInitiatorNameProtocolGuid,
...
#if (NET_PKG_AMI_PORTING_ENABLE == 1)     
    // RSD: list of Mac port strings
    Status = gRT->SetVariable(L"RsdMacName",
                             &gEfiRsdMACNamesListVarDataGuid,
                             EFI_VARIABLE_BOOTSERVICE_ACCESS,
                             (StrLen(MacString16) + 1)* sizeof(CHAR16), 
                             MacString16); 
    DEBUG((EFI_D_ERROR,"RSD_ISCSI_2_2:In %a, SetVariable Status(RsdMacName) :: %r MacString16  :: %s \n", __FUNCTION__, Status, MacString16));
#endif    
    //
    // Rsd Attempt order
    //
...
)
{
    ...
    if(EFI_ERROR(Status)) {
        DEBUG((EFI_D_ERROR,"RSD_ISCSI_2_2:In %a, locating IPMI_TRANSPORT_PROTOCOL_GUID failed. Status :: %r \n", __FUNCTION__, Status));
        return Status;
    }
#if(AMI_RSD_PKG_VERSION <= 6)    
...
)
{
    ...
    UINT8               RespSize = 0;
    RSD_MDR_READ_RES    *RsdIScsiVersionResp = NULL;
    ...
    if(LengthToRead < (RSD_ISCSI_MAX_READ_CAPACITY - RSD_ISCSI_RESP_DATA_INIT)) {
        gRsdIScsiReadReq.DataLength = (UINT8)LengthToRead;
        RsdIScsiVersionResp = AllocateZeroPool(gRsdIScsiReadReq.DataLength + RSD_ISCSI_RESP_DATA_INIT);
        if(NULL == RsdIScsiVersionResp){
            DEBUG ((EFI_D_ERROR, "RSD_ISCSI_2_2:In %a, AllocateZeroPool#1 failed. Status = %r \n", __FUNCTION__ ,Status));
            return EFI_OUT_OF_RESOURCES;
        }
    }
    ...
    DEBUG ((DEBUG_INFO, "RSD_ISCSI_2_2:In %a, MdrReadRegion#1 Status = %r \n", __FUNCTION__, Status));
    ...
    if(EFI_BUFFER_TOO_SMALL != Status &&
            EFI_ERROR(Status)) {
        DEBUG ((EFI_D_ERROR, "RSD_ISCSI_2_2:In %a, MdrReadRegion#1 failed. Status = %r \n", __FUNCTION__ , Status));
    ...
    if(EFI_BUFFER_TOO_SMALL == Status)
    {
        ...
        if(NULL == RsdIScsiVersionResp){
            DEBUG ((EFI_D_ERROR, "RSD_ISCSI_2_2:In %a, AllocateZeroPool#2 failed. Status = %r \n", __FUNCTION__));
            return EFI_OUT_OF_RESOURCES;
        }

        Status = MdrReadRegion(gIpmiTransport, &gRsdIScsiReadReq, RsdIScsiVersionResp, &RespSize);
        DEBUG ((DEBUG_INFO, "RSD_ISCSI_2_2:In %a, MdrReadRegion#2 Status = %r \n", __FUNCTION__, Status));
    }
...
)
{
    ...
    if(LengthToRead < (RSD_ISCSI_MAX_READ_CAPACITY - RSD_ISCSI_RESP_DATA_INIT)) {
        gRsdIScsiReadReq.DataLength = (UINT8)LengthToRead;
        RsdIScsiInitResp = AllocateZeroPool(gRsdIScsiReadReq.DataLength + RSD_ISCSI_RESP_DATA_INIT);
    } else {
        ReadMultiples = TRUE;
        gRsdIScsiReadReq.DataLength = RSD_ISCSI_MAX_READ_CAPACITY - RSD_ISCSI_RESP_DATA_INIT;
        RsdIScsiInitResp = AllocateZeroPool(RSD_ISCSI_MAX_READ_CAPACITY);
    }

    if(NULL == RsdIScsiInitResp) {
        DEBUG ((EFI_D_ERROR, "RSD_ISCSI_2_2:In %a, AllocateZeroPool#1 failed.\n", __FUNCTION__));
        return EFI_OUT_OF_RESOURCES;
    }
    ...
    if(EFI_BUFFER_TOO_SMALL == Status)
    {
        ...
        Status = MdrReadRegion(gIpmiTransport, &gRsdIScsiReadReq, RsdIScsiInitResp, &RespSize);
        if(EFI_ERROR(Status)) {
            DEBUG ((EFI_D_ERROR, "RSD_ISCSI_2_2:In %a, MdrReadRegion#2 failed. Status = %r \n", __FUNCTION__, Status));
            if(RsdIScsiInitResp)
                FreePool(RsdIScsiInitResp);
            return Status;
        }
    ...
    {
        gRsdIScsiReadReq.DataLength =(UINT8)( LengthToRead - RSD_ISCSI_MAX_READ_CAPACITY);
        RsdIScsiInitResp = ReallocatePool (RSD_ISCSI_MAX_READ_CAPACITY,LengthToRead,RsdIScsiInitResp);
        if(NULL == RsdIScsiInitResp) {
            DEBUG ((EFI_D_ERROR, "RSD_ISCSI_2_2:In %a, ReallocatePool#1 failed. Status = %r \n", __FUNCTION__, Status));
            return EFI_OUT_OF_RESOURCES;
        }
        ...
        if(EFI_BUFFER_TOO_SMALL == Status)
        {
            ...
            RsdIScsiInitResp = ReallocatePool (RSD_ISCSI_MAX_READ_CAPACITY,LengthToRead,RsdIScsiInitResp);
            if(NULL == RsdIScsiInitResp) {
                DEBUG ((EFI_D_ERROR, "RSD_ISCSI_2_2:In %a, ReallocatePool#2 failed. Status = %r \n", __FUNCTION__, Status));
                return EFI_OUT_OF_RESOURCES;
            }
        
            Status = MdrReadRegion(gIpmiTransport, &gRsdIScsiReadReq, RsdIScsiInitResp + RSD_ISCSI_MAX_READ_CAPACITY, &RespSize);
            if(EFI_ERROR(Status)) {
                DEBUG ((EFI_D_ERROR, "RSD_ISCSI_2_2:In %a, MdrReadRegion failed. Status = %r \n", __FUNCTION__, Status));
                if(RsdIScsiInitResp)
                    FreePool(RsdIScsiInitResp);
                return Status;
            }
    ...
    DEBUG ((DEBUG_INFO, "RSD_ISCSI_2_2:In %a, RsdIScsiParseInitiatorResponse Status = %r \n", __FUNCTION__, Status));
...
)
{
    ...
    if(LengthToRead < (RSD_ISCSI_MAX_READ_CAPACITY - RSD_ISCSI_RESP_DATA_INIT)) {
        gRsdIScsiReadReq.DataLength = (UINT8)LengthToRead;
        RsdIScsiTargetResp = AllocateZeroPool(gRsdIScsiReadReq.DataLength + RSD_ISCSI_RESP_DATA_INIT);
    } else {
        ReadMultiples = TRUE;
        gRsdIScsiReadReq.DataLength = RSD_ISCSI_MAX_READ_CAPACITY - RSD_ISCSI_RESP_DATA_INIT;
    ...
    if(EFI_BUFFER_TOO_SMALL != Status &&
            EFI_ERROR(Status)) {
        DEBUG ((DEBUG_INFO, "RSD_ISCSI_2_2:In %a, MdrReadRegion#1 failed. Status = %r \n", __FUNCTION__, Status));
        return Status;
    }
    ...
    if(ReadMultiples)
    {
        ...
        if(EFI_BUFFER_TOO_SMALL != Status &&
                EFI_ERROR(Status)) {
            DEBUG ((EFI_D_ERROR, "RSD_ISCSI_2_2:In %a, MdrReadRegion#3 failed. Status = %r \n", __FUNCTION__, Status));
            return Status;
        }
    ...
    DEBUG ((DEBUG_INFO, "RSD_ISCSI_2_2:In %a, RsdIScsiParseTargetResponse Status = %r \n", __FUNCTION__, Status));
...
)
{
    EFI_STATUS          Status = EFI_INVALID_PARAMETER;
    UINT8               RespSize = 0;
    RSD_MDR_READ_RES    *RsdIScsiBootResp = NULL;

    if(LengthToRead < (RSD_ISCSI_MAX_READ_CAPACITY - RSD_ISCSI_RESP_DATA_INIT)) {
        gRsdIScsiReadReq.DataLength = (UINT8)LengthToRead;
        RsdIScsiBootResp = AllocateZeroPool(gRsdIScsiReadReq.DataLength + RSD_ISCSI_RESP_DATA_INIT);
    ...
    if(EFI_BUFFER_TOO_SMALL == Status)
    {
        ...
        Status = MdrReadRegion(gIpmiTransport, &gRsdIScsiReadReq, RsdIScsiBootResp, &RespSize);
        if(EFI_ERROR(Status)) {
            DEBUG ((EFI_D_ERROR, "RSD_ISCSI_2_2:In %a, MdrReadRegion#2 Status = %r \n", __FUNCTION__, Status));
            return Status;
        }
    }
    ...
    DEBUG ((DEBUG_INFO, "RSD_ISCSI_2_2:In %a, RsdIScsiParseIScsiAttemptResponse Status = %r \n", __FUNCTION__, Status));
    ...
    while(gRsdIScsiDataLeft > 0) {
        ...
        if(!EFI_ERROR(Status)) {
            ...
            case VersionBlock:
                ...
                DEBUG((DEBUG_INFO,"RSD_ISCSI_2_2:In %a, RsdIScsiReadVersionBlock Status :: %r \n", __FUNCTION__, Status));
                break;
            ...
            case InitiatorBlock:
                ...
                // Read Initiator data.
                //
                Status = RsdIScsiReadInitiatorBlock(gRsdIScsiDataLeft, gRsdIScsiCurOffset);
                DEBUG((DEBUG_INFO,"RSD_ISCSI_2_2:In %a, RsdIScsiReadInitiatorBlock Status :: %r \n", __FUNCTION__, Status));
                break;
            case TargetBlock:
                //
                // Read Target data.
                //
                Status = RsdIScsiReadTargetBlock(gRsdIScsiDataLeft, gRsdIScsiCurOffset);
                DEBUG((DEBUG_INFO,"RSD_ISCSI_2_2:In %a, RsdIScsiReadTargetBlock Status :: %r \n", __FUNCTION__, Status));
                break;
            ...
            case IScsiAttemptBlock:
                ...
                DEBUG((DEBUG_INFO,"RSD_ISCSI_2_2:In %a, RsdIScsiReadIScsiAttemptBlock Status :: %r \n", __FUNCTION__, Status));
                break;
            ...
            if(!EFI_ERROR(Status)) {
			//+2 for the 2 NULL characters at the end of table
                gRsdIScsiCurOffset += (BlockSize+2);
                gRsdIScsiDataLeft -= (BlockSize+2);
            }
			else
			{
				break;				//To avoid looping when rsd data is not parsed properly. 
			}
            if(NULL != MdrReadResp &&
                    0 == gRsdIScsiDataLeft) {
                FreePool(MdrReadResp);
...
)
{
    ...
    IScsiMacAddrToStr ((EFI_MAC_ADDRESS *)InitData->NicMacAddress,NET_ETHER_ADDR_LEN,0, MacStringStr);
	//To check attempt data has valid MAC or not.
    if(!CheckMacAvailability(MacStringStr))
    {
        return EFI_NOT_FOUND;
    }
    ...
    if(EFI_ERROR(Status)) {
        DEBUG((EFI_D_ERROR,"\nRSD_ISCSI_2_2:In %a, BMC's Initiator name is invalid.\n", __FUNCTION__));
        //
    ...
    gRsdIScsiAttemptConfigData.AttemptType = RSD_ATTEMPT;  // To differntiate Attempts
    // Using variable services save the BMC parsed data 
    Status = RsdIScsiConvertBmcDataToAttemptConfigNvData();
...
#else
    Status = GetMdrRegionStatus (gIpmiTransport, &RsdMdrStatReq, RsdMdrStatResp);
#endif    

    DEBUG((DEBUG_INFO,"RSD_ISCSI_2_2:In %a, GetSmbiosMdrRegionStatus Status :: %r \n", __FUNCTION__, Status));
    if(EFI_ERROR(Status)) {
        DEBUG((EFI_D_ERROR,"RSD_ISCSI_2_2:In %a, GetSmbiosMdrRegionStatus failed. Status :: %r \n", __FUNCTION__, Status));
        return Status;
    }
...
#if(AMI_RSD_PKG_VERSION <= 4)
    (Unlocked != (RsdMdrStatResp->LockStatus))) {
#else
    (RsdMdrUnlocked != (RsdMdrStatResp->LockStatus))) {
#endif 
        //
        // If above conditions are not met, then the region is being used by some other and should not be disturbed.
        //
        DEBUG((EFI_D_ERROR,"RSD_ISCSI_2_2:In %a, Region is being used by someone else.\n", __FUNCTION__));
...
*/
    ...
    DEBUG((DEBUG_INFO,"RSD_ISCSI_2_2:In %a, GetMdrLockHandle for connection establishment Status :: %r, LockHandle :: 0x%X \n", __FUNCTION__, Status, LockHandle));
    if(EFI_ERROR(Status)) {
        DEBUG((EFI_D_ERROR,"RSD_ISCSI_2_2:In %a, GetMdrLockHandle failed. Status :: %r \n", __FUNCTION__, Status));
        return Status;
    }
    ...
    if(!EFI_ERROR(Status) &&
        ...
        DEBUG((DEBUG_INFO,"RSD_ISCSI_2_2:In %a, RsdIScsiReadUsedSize Status :: %r \n", __FUNCTION__, Status));
    }
...
RsdIScsiInitializeBmcInformation()
{
    ...
    if (!gRsdIScsiIpmiFound)
    {
        ...
        if(EFI_ERROR(Status)) {
            DEBUG((EFI_D_ERROR,"\nRSD_ISCSI_2_2:In %a, RsdIScsiInitializeReadRequest failed. Status = %r\n", __FUNCTION__, Status));
            return Status;
        }
    }

    DEBUG((DEBUG_INFO,"\nRSD_ISCSI_2_2:In %a, RsdIScsiInitializeReadRequest Status = %r\n", __FUNCTION__, Status));
    
    Status = RsdIScsiReadBMCAttemptData();
    if(EFI_ERROR(Status))
    {
        DEBUG((EFI_D_ERROR,"\nRSD_ISCSI_2_2:In %a, RsdIScsiReadBMCAttemptData failed. Status :: %r \n", __FUNCTION__, Status));
        return Status;
    }
    DEBUG((DEBUG_INFO,"\nRSD_ISCSI_2_2:In %a, RsdIScsiReadBMCAttemptData Status :: %r \n", __FUNCTION__, Status));
    //
    ...
    while(CurrentAttempt < gRsdIScsiAttemptCount) {
      ...
      DEBUG((DEBUG_INFO,"\nRSD_ISCSI_2_2:In %a, RsdIScsiFrameAttemptConfigData Status :: %r", __FUNCTION__, Status));
