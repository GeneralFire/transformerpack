...
#if (AMI_ISCSI_SUPPORT)
AMI_NETWORK_BLOCKED  BlockedISsciDxeNetworkList[] = {
        ISCSI_BLOCKED_NETWORK_DEVICE_LIST
        { 0xFFFF, 0xFFFF } 
};
#endif
...
#if (AMI_ISCSI_SUPPORT)
// To support Blocked Network device list for the ISCSI support
  if(!EFI_ERROR(Status)) {	
      // Check if the Network device is present in the IScsi Blocked List.
      // If present, don't install the IScsi support for the Network device.
      if(IsAmiNetworkDeviceBlocked(ControllerHandle, (AMI_NETWORK_BLOCKED *)BlockedISsciDxeNetworkList)) {
          return EFI_UNSUPPORTED;
      }
  }
#endif
  ...
  )
{
  EFI_STATUS        Status;
#if(AMI_ISCSI_SUPPORT)
  Status = AmiIScsiStart (This->DriverBindingHandle, ControllerHandle, IP_VERSION_4);
#else
...
#if(AMI_ISCSI_SUPPORT)
  Status = AmiIScsiStart (This->DriverBindingHandle, ControllerHandle, IP_VERSION_6);
#else
  ...
  )
{
  ...
  //
#if(AMI_ISCSI_SUPPORT)
  Status = AmiIScsiConfigFormInit (gIScsiIp4DriverBinding.DriverBindingHandle);
#else
  ...
  if (Status == EFI_NOT_FOUND) {
    ...
    Status = RedfishIScsiInit(ImageHandle);
#endif  
