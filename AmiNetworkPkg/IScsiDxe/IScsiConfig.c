...
#if (AMI_ISCSI_SUPPORT == 1)
  //Filling ChapType even when AuthenticationType is None
  ...
  )
{
  ...
  if (OffSet < ATTEMPT_BOOTENABLE_VAR_OFFSET) {
    ...
    if ((OffSet >= ATTEMPT_ISID_VAR_OFFSET) && (OffSet < ATTEMPT_INITIATOR_IP_ADDRESS_VAR_OFFSET)) {
      ...
      AmiIScsiParseIsIdFromString (IfrNvData->Keyword[Index].ISCSIIsId, Attempt->SessionConfigData.IsId);
#else
...
#if (AMI_ISCSI_SUPPORT)
    FORMID_LOCAL_ISCSI_FORM,         // Form ID
#else
...
#if (AMI_ISCSI_SUPPORT)
    AttemptConfigData->AttemptType = SETUP_ATTEMPT;
#endif
  ...
  )
{
  ...
  if (AttemptConfigOrder != NULL) {
    ...
    NET_LIST_FOR_EACH (Entry, &mPrivate->AttemptConfigs) {
      AttemptConfigData = NET_LIST_USER_STRUCT (Entry, ISCSI_ATTEMPT_CONFIG_NVDATA, Link);
#if (AMI_ISCSI_SUPPORT == 1)
  if(AttemptConfigData->AttemptType == SETUP_ATTEMPT || AttemptConfigData->AttemptType == REDFISH_ATTEMPT) {
      // Deleting the Attempt from DynamicOrderedList 
