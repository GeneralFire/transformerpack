...
/**
  Function to decide whether the validate the server certificate or not.

  @param[in]  HttpInstance       The HTTP instance private data.

  @retval TRUE                   If certification validation for the server need to be skipped.
  @retval FALSE                  If certification validation for the server need not to be skipped.
**/
#if (NET_PKG_AMI_PORTING_ENABLE == 1)       //AMI PORTING
BOOLEAN SkipCertificationValidation (IN HTTP_PROTOCOL      *HttpInstance)
{
  UINT8     SkipCertificate = 0;
  SKIP_HTTPS_CERT_VALIDATION_IPS  SkipValidationIps [] = {
                                      SKIP_HTTPS_CERT_VALIDATION_IPS_LIST
                                      {0, {0,0,0,0}}
                              };

  UINTN         SkipValidationIpsCount = sizeof (SkipValidationIps)/ sizeof (SkipValidationIps [0]);
  UINTN         iIndex = 0;

  SkipCertificate = PcdGetEx8 (&gEfiNetworkPkgTokenSpaceGuid, PcdSkipHttpsCertValidation);

  if (1 == SkipCertificate) {
      return TRUE;
  }
  else {
    for (iIndex = 0; iIndex < SkipValidationIpsCount - 1; iIndex ++) {
        if ( (SkipValidationIps [iIndex].Ipv4Port == HttpInstance->RemotePort) &&
                (!CompareMem (&SkipValidationIps [iIndex].Ipv4Addr, &HttpInstance->RemoteAddr, sizeof (EFI_IPv4_ADDRESS))) ) {
            return TRUE;
        }
    }
  }
...
#if (NET_PKG_AMI_PORTING_ENABLE == 1)
      if (TRUE == SkipCertificationValidation (HttpInstance)) {
          DEBUG ((DEBUG_INFO, "NWS: No certificate found\n"));
          return EFI_SUCCESS;
      }
#endif
    return Status;
...
#if (NET_PKG_AMI_PORTING_ENABLE == 1)  
if ((FALSE == SkipCertificationValidation (HttpInstance)) && (FALSE == PcdGetEx8 (&gEfiNetworkPkgTokenSpaceGuid,PcdSkipHostNameValidation))) {
      DEBUG ((DEBUG_INFO, "NWS: Certificate Valid. Proceed Verifying Host\n"));
#endif	  
	  Status = HttpInstance->Tls->SetSessionData (
									HttpInstance->Tls,
									EfiTlsVerifyHost,
